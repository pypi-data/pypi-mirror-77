...
class Px:
    ...
    def injectRequest(class_, context, req, tool):
        '''Inject all p_req(uest) values into the PX p_context'''
        # Inject every (key, value) pair found in the request into the context,
        # with automatic type conversion for values.
        for name, value in req.items():
            setattr(context, name, class_.convertValue(value, tool))
        # When some keys are found in the request, create some related,
