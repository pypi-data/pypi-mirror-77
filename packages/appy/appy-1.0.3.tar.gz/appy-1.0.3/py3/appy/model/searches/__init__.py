...
class UiSearch:
    ...
    pxResult = Px('''
     ...
     <div id=":mode.hook">
      ...
      <!-- Title -->
      <div if="not popup" class="pageTitle topSpaceS">
       ...
       <x if="uiSearch.search.add and guard.mayInstantiate(class_)"
          var2="asButton=True;
      ...
      <!-- (Bottom) navigation -->
      <div if="mode.batch" align=":dright">:mode.batch.pxNavigate</div>
    ...
    def parseFilters(self, req):
        '''Parses the filters from the p_req(uest) or return None if not
           found.'''
        r = req.filters
        if r is None: return {}
        return sutils.getDictFrom(r)
...
class Search:
    ...
    def __init__(self, name=None, group=None, sortBy='title', sortOrder='asc',
                 ...
                 actions=None, rowAlign='top', **fields):
    ...
    def run(self, handler, batch=True, start=0, ids=False, secure=True,
        ...
        if refObject:
            back = refField.back
            if back: params['cid'] = '%d_%s' % (refObject.iid, back.name)
    ...
    def get(class_, name, tool, modelClass, ctx, ui=True):
        ...
        if name == 'customSearch':
            ...
            if field.type == 'Ref':
                initiator = initiators.RefInitiator(o, field, fieldName, mode)
                r = field.getSelect(o)
    ...
    liveResults = Px('''
     <div var="search=uiSearch.search;
               filters=uiSearch.parseFilters(req);
    ...
    live = Px('''
     ...
     css='''.lsSelected { background-color: #d9d7d9 }
            .lsRef { display: flex }
            .lsSearch { display: flex; margin-top: 2px }
