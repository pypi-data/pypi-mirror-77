...
class Ref(Field):
    ...
    class Layouts(Layouts):
        ...
        l = Layouts(edit=Layout('lrv-f', width=None), view=Layout('l-f'))
        # Wide layout for a Ref with add=True: it must be 100% wide
        a = Layouts(edit=Layout('lrv-f'), view=Layout('lrv-f'))
     ...
     <x if="mayAdd and not inPickList">
      <form class=":inMenu and 'addFormMenu' or 'addForm'"
       ...
       <input type=":'button' if field.addConfirm or (create == 'noForm') \
        ...
        value=":label" style=":url('add.svg', bg='18px 18px')" title=":addLabel"
     ...
     <x if="changeOrder and (len(objects) &gt; 1) and \
      ...
      <img src=":url('asc.svg')" class="clickable iconS"
           ...
           onclick=":'askConfirm(%s,%s,%s)' % (q('script'), q(js,False), \
                                               q(sortConfirm))"/>
      <img src=":url('desc.svg')" class="clickable iconS"
           var="js='askBunchSortRef(%s, %s, %s, %s)' % \
    ...
    # Edit widget, for Refs with link == True
    pxEditSelect = Px('''
     <select var2="objects=field.getPossibleValues(o);
    ...
    pxFilterSelect = Px('''
     <select var="name=field.name;
      ...
      <option for="tied in objects" value=":tied.iid"
       var2="title=field.getReferenceLabel(o, tied, unlimited=True, \
                                           usage='filter')"
       selected=":(name in mode.filters) and (mode.filters[name] == tied.iid)"
       title=":title">:Px.truncateValue(title, charsWidth)</option>
     </select>''')
    ...
    def __init__(self, class_=None, attribute=None, validator=None,
      ...
      queryNbCols=1, navigable=False, changeOrder=True, numbered=False,
      checkboxes=True,checkboxesDefault=False, sdefault='', scolspan=1,
      swidth=None, sheight=None, sselect=None, persist=True, render='list',
      ...
      noValueLabel='choose_a_value', addLabel='object_add', filterable=True,
      supTitle=None, subTitle=None, separator=None, rowAlign='top'):
        ...
        # ----------------------------------------------------------------------
        # If "link"  | On the edit page, 
        # is ...     | the user will choose objects...
        # ----------------------------------------------------------------------
        #   True     | from a static dropdown menu;
        # ----------------------------------------------------------------------
        #  "list"    | from a list of objects which is similar to those rendered
        #            | in pxViewList;
        # ----------------------------------------------------------------------
        #  "popup"   | from a popup window. In this case, parameter "select" can
        #            | hold a Search instance or a method;
        # ----------------------------------------------------------------------
        # "dropdown" | by typing some chars in an input text field: in a
        #            | dropdown, the first matching objects will be shown and
        #            | will be selectable. The set of possible objects shown in
        #            | the dropdown must be defined by a Search instance in
        #            | attribute "select".
        # ----------------------------------------------------------------------
        # "popupRef" | from a popup window, that will display objects tied via a
        #            | Ref field. In this case, parameter "select" must be a
        #            | method that returns a tuple (obj, name), "obj" being the
        #            | source object of the Ref field whose name is in "name".
        # ----------------------------------------------------------------------
        self.link = link
    ...
    def initPx(self, o, req, c):
        ...
        r.target = c.ui.LinkTarget(self.class_, r.backHook)
    ...
    def getSelect(self, o, forSearch=False):
        ...
        r = method if isinstance(method, Search) else method(o)
    ...
    def getPopupLink(self, o, popupMode, name):
        ...
        if self.link in ('popup', 'dropdown'):
            ...
            r = '%s/pxField?name=%s&pageLayouts=w-b&popup=True&maxPerPage=%d' \
