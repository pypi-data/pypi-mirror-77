...
class Field:
    ...
    def getStoredValue(self, o, name=None, fromRequest=False):
        ...
        if self.isInner():
            ...
            if rowId == '-1': r = None # It is a template row
                ...
                if r:
                    if o.getField(outerName).outerType == PersistentList:
    ...
    def getSearchParams(self, req, layout):
        '''Encodes in a string all the params in the request that are required
        ...
        return ':'.join([(req[key] or '') for key in Field.searchParams])
    ...
    def getSearchResults(self, tool, searchParams):
        ...
        params = O()
        i = 0
        for part in searchParams.split(':', len(Field.searchParams) - 1):
            setattr(params, Field.searchParams[i], part)
            i += 1
        # Retrieve a Ref object when relevant
        Search = tool.Search
        refObject, refField = Search.getRefInfo(tool)
        # Get the Search instance
        class_ = tool.model.classes.get(params.className)
        search = Search.get(params.search, tool, class_, None, ui=False)
        # Executing the search may take some time and potentially slow down
        # the system: log this action.
        tool.log(SEARCH_TRIGGER % (self.type, class_.name, self.name,
                                   search.name))
        ...
        r = search.run(tool.H(), batch=False,
          ...
          filters=sutils.getDictFrom(params.filters),
