...
class FileInfo:
    ...
    def writeResponse(self, handler, path=None, disposition='attachment',
        ...
        Static.write(handler, path, None, response=handler.resp, fileInfo=self,
                     disposition=disposition, enableCache=cache)

    def writeFile(self, name, value, folder):
    ...
    def dump(self, o, filePath=None, format=None):
        ...
        if format:
            ...
            out, err = o.tool.convert(filePath, format)
...
class File(Field):
    ...
    def __init__(self, validator=None, multiplicity=(0,1), default=None,
      ...
      thumbnail=None):
        ...
        # here, set the foolowing attribute to True. Attribute p_cache can also
    ...
    def store(self, o, value):
        ...
        #    | content.
        #  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        #  b | a string. In this case, the string represents the path of a file
        #    | on disk;
        #  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        #  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        #  e | a FileInfo instance, that must be "not-in-DB", ie, with an
        ...
        if value:
            ...
            if isinstance(value, Object) or isinstance(value, FileInfo) or \
               isinstance(value, UnmarshalledFile):
                # Cases (a) and (e)
                info.writeFile(self.name, value, folder)
            ...
            if self.resize and self.width: info.resize(folder, self.width, o)
