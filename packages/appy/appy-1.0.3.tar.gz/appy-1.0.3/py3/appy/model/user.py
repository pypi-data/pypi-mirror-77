...
class User(Base):
    ...
    encrypted = Computed(show=showEncrypted,
    ...
    pxUserLink = Px('''
     <a if="config.ui.userLink" href=":user.url">
      <img src=":url('user.svg')" class="icon"/>
      <span class="headerText">:user.getFirstName()</span>
     </a>
        ...
        if place == 'sso':
            ...
            if (user is None) or user.isAnon() or (user.state=='inactive') or \
               ...
               (not ctx and config.authContext and \
                config.authContext.isMandatory(tool) and (place != 'basic')):
