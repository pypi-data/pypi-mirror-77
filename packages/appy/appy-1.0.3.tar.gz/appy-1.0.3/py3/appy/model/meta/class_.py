...
class Class(Meta):
    ...
    pxAdd = Px('''
     <form var="create=class_.getCreateVia(tool)"
           if="create" class="addForm" name=":'%s_add' % className"
           var2="target=ui.LinkTarget(class_.python); label=_('object_add');"
      ...
      <input var="suffix='Small' if asButton else 'Portlet';
                  size='18px 18px' if asButton else '24px 24px'"
             class=":'button%s button' % suffix" style=":url('add.svg',bg=size)"
             value=":label" title=":label" type="submit"
