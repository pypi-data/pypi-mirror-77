...
class GenomeGraph:
	...
	def read_graph(self, file=None, names_list='all', generate_freq=False):
		...
		for line in f_in:
			...
			start_coord = (string[4], string[5])

			end_coord = (string[6], string[7])
	...
	def _save_bed(self, data, outdir, contig):

		f_wc = open(outdir + '/window_complexity.bed', 'a+')
		for gene in data[0]:
			gene_coords = self.genes_info[contig][self.genes_decode[gene]]
			f_wc.write(
				'{contig}	{start}	{end}	{name}\n'.format(
					contig=contig,
					start = gene_coords[0],
					end=gene_coords[1],
					name='val={}'.format(data[0][gene])
				)
			)

		f_wc = open(outdir + '/prob_window_complexity.bed', 'a+')
		for gene in data[0]:
			gene_coords = self.genes_info[contig][self.genes_decode[gene]]
			f_wc.write(
				'{contig}	{start}	{end}	{name}\n'.format(
	...
	def _save_data(self, data, outdir, contig):
		...
		for gene in data[0]:
			...
			gene_coords = self.genes_info[contig][self.genes_decode[gene]]
			f_wc.write(str(gene_coords[0]) + ':' + str(gene_coords[1]) + '\t' + self.genes_decode[gene] + '\t' + str(data[0][gene]) + '\n')
			f_mc.write(str(gene_coords[0]) + ':' + str(gene_coords[1]) + '\t' + self.genes_decode[gene] + '\n')

		for gene in data[1]:
			gene_coords = self.genes_info[contig][self.genes_decode[gene]]
			f_io.write(str(gene_coords[0]) + ':' + str(gene_coords[1]) + '\t' + self.genes_decode[gene] + '\t' + str(data[1][gene]) + '\n')
		...
		for gene in data[3]:
			gene_coords = self.genes_info[contig][self.genes_decode[gene]]
			f_wc.write(str(gene_coords[0]) + ':' + str(gene_coords[1]) + '\t' + self.genes_decode[gene] + '\t' + str(data[3][gene]) + '\n')
			f_mc.write(str(gene_coords[0]) + ':' + str(gene_coords[1]) + '\t' + self.genes_decode[gene] + '\n')

		for gene in data[4]:
			gene_coords = self.genes_info[contig][self.genes_decode[gene]]
			f_io.write(str(gene_coords[0]) + ':' + str(gene_coords[1]) + '\t' + self.genes_decode[gene] + '\t' + str(data[4][gene]) + '\n')
	...
	def compute_complexity(self, outdir, reference, window=20, iterations=500, min_depth=0, max_depth=-1, save_db=None):
		...
		for contig in self.list_graph[reference]:
			...
			self._save_bed([complexity_table, io_table, all_bridges, 
						prob_complexity_table, prob_io_table, prob_all_bridges], outdir, contig)
