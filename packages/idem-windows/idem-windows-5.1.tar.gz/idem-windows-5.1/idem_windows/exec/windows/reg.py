...
def __init__(hub):
    ...
    hub.exec.windows.reg.VTYPE_REVERSE = {
        v: k for k, v in hub.exec.windows.reg.VTYPE.items()
    }

    hub.exec.windows.reg.OPTTYPE = {
    ...
    hub.exec.windows.reg.SUBKEY_SLASH_CHECK = {
    ...
    hub.exec.windows.reg.REGISTRY_32 = {
...
def key_exists(hub, hive: str, key: str, use_32bit_registry: bool = False) -> bool:
    ...
    """
    try:
        hkey = hub.exec.windows.reg.HKEYS[hive]
    except KeyError:
        raise KeyError(f"Invalid Hive: {hive}")
    access_mask = hub.exec.windows.reg.REGISTRY_32[use_32bit_registry]
    ...
    try:
        ...
        for i in range(win32api.RegQueryInfoKey(handle)[1]):
            ...
            value = {
                ...
                "key": key,
                "vname": vname,
                "vtype": hub.exec.windows.reg.VTYPE_REVERSE[vtype],
                "success": True,
            }
    ...
    try:
        ...
        try:
            ...
            if vdata or vdata in [0, ""]:
                # Only convert text types to unicode
                ret["vtype"] = hub.exec.windows.reg.VTYPE_REVERSE[vtype]
                ret["vdata"] = vdata
            else:
...
def set_value(
    ...
    try:
        ...
        raise KeyError(f"Invalid Hive: {hive}")
    vtype_value = hub.exec.windows.reg.VTYPE[vtype]
    access_mask = (
        hub.exec.windows.reg.REGISTRY_32[use_32bit_registry] | win32con.KEY_ALL_ACCESS
    )

    vdata = cast_vdata(vdata=vdata, vtype=vtype)

    if volatile:
        create_options = hub.exec.windows.reg.OPTTYPE["REG_OPTION_VOLATILE"]
    else:
        create_options = hub.exec.windows.reg.OPTTYPE["REG_OPTION_NON_VOLATILE"]
...
def delete_key_recursive(
    ...
    if not hub.exec.windows.reg.key_exists(hive, key, use_32bit_registry):
        return {}

    if (len(key) > 1) and (
        key.count("\\", 1) < hub.exec.windows.reg.SUBKEY_SLASH_CHECK[hkey]
    ):
        hub.log.error(
