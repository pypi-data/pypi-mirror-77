    ...
    def connect(self):
        ...
        if self.get_status() != self.connected:
            if return_code == self.barcode_kit_unavailable:
                raise ValueError("Barcode kit unavailable")
            elif return_code == self.invalid_response:
                raise ConnectionError("Received invalid response")
            ...
            # Handle return_code
            if return_code == self.failed:
                raise ConnectionError(
                    "Could not connect. Is the server running? Check your connection parameters."
                )
    ...
    def pass_read(
        ...
        if return_code != self.success:
            if return_code == self.not_ready:
                tries = 0
                while return_code != self.success:
                    return_code = super().pass_read(read)
                    tries += 1
                    if tries >= self.pass_attempts:
                        # Exceeded attempts, should eval False
                        break
                    time.sleep(self.throttle)
                else:
                    # Should only get here if return_code == success
                    return True
                return False
