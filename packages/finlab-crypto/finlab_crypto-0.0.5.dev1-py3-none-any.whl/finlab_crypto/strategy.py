...
def Strategy(**default_parameters):
  ...
  class Strategy:
    ...
    def plot_combination(self, portfolio):
      ...
      fig, axes = plt.subplots(2, 2, figsize=(15, 10), sharey=False, sharex=False, constrained_layout=True)
      fig.suptitle('Backtest Results')

      def heat_map(portfolio, name1, name2, item_name, ax):
        if name1 != name2:
          sns.heatmap(getattr(portfolio, item_name).reset_index().pivot(name1, name2)[0], cmap='magma_r', ax=ax)
        else:
      ...
      with sns.axes_style("whitegrid"):
        ...
        ax = (portfolio.cumulative_returns[best_10] * 10).plot(ax=axes[0][0])
        ax.set(xlabel='time', ylabel='cumulative return (%)')

        axes[0][1].title.set_text('Perfornace (return)')
        heat_map(portfolio, n1, n2, 'final_equity', axes[0][1])

        axes[1][1].title.set_text('Perfornace (sharpe ratio)')
        heat_map(portfolio, n1, n2, 'sharpe_ratio', axes[1][1])
        #sns.heatmap(item.to_frame().reset_index().pivot(n1, n2)[0], cmap='magma_r', ax=axes[1][1])
      ...
      fig, axes = plt.subplots(1, len(items), figsize=(15, 3), sharey=False, sharex=False, constrained_layout=True)
      ...
      for i, item in enumerate(items):
        ...
        for name in index_names:
          results[name] = getattr(portfolio, item).groupby(name).mean()
      ...
      fig, axes = plt.subplots(1, 3, figsize=(15, 5), sharey=False, sharex=False, constrained_layout=True)
