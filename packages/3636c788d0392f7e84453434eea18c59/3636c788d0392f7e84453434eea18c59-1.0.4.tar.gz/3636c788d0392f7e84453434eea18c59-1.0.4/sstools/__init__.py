...
# Breadth first seach algorithm to see the blocekd dates
def vista_blocked_dates(property_id,ci,co):
  rg = (datetime.datetime.strptime(co, "%Y-%m-%d") - datetime.datetime.strptime(ci, "%Y-%m-%d")).days    #Range

  steps = 0

  DTE = [(ci,co,rg)]                  # This list will contain all the date ranges to be checked - there will be additions and subtractions to this list
  ...
  response_obj = {
      "blocked_dates_and_price" : blocked,
      "meta_data": {
