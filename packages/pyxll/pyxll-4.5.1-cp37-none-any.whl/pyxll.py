...
class BoundFormatter:
    """Style bound to a return type"""

    def __init__(self, formatter, datatype, datatype_ndim, datatype_kwargs, transpose):
        if isinstance(formatter, dict):
            formatter = Formatter(**formatter)
        self.__formatter = formatter
        self.__datatype = datatype
        self.__datatype_ndim = datatype_ndim
        self.__datatype_kwargs = datatype_kwargs
        self.__transpose = transpose

    def clear(self, cell):
        return self.__formatter.clear(cell)

    def apply(self, cell, value):
        return self.__formatter.apply(cell,
                                      value,
                                      datatype=self.__datatype,
                                      datatype_ndim=self.__datatype_ndim,
                                      datatype_kwargs=self.__datatype_kwargs,
                                      transpose=self.__transpose)
    ...
    def __init__(self,
        ...
        if rows:
            ...
            for formatter in rows:
                formatter = _to_formatter(formatter)
                self.__row_formatters.append(formatter)

        self.__header_formatter = None
        if header:
            self.__header_formatter = _to_formatter(header)
        ...
        if self.__column_formatters and num_rows:
            for col, formatters in list(self.__column_formatters.items()):
                ...
                if len(formatters) == 1:
                    ...
                    for i, row in enumerate(df_array[header_rows:num_rows]):
