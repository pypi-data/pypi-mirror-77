...
class TimeSliceAPI(API):
    ...
    def get_time_slice_by_lines(
        ...
        if horizon:
            flat_horizon = [index for iline_horizon in horizon for index in iline_horizon]
            request = LineTimeSliceQueryRequest(file=file, rectangle=rectangle, horizon=Horizon(z_values=flat_horizon))
        else:
