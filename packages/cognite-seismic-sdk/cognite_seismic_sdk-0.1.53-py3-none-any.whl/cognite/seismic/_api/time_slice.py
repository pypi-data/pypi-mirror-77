...
from cognite.seismic.protos.query_service_messages_pb2 import Horizon
...
class TimeSliceAPI(API):
    ...
    def get_time_slice_by_lines(
        ...
        Args:
            ...
            horizon (List[int], optional): The horizon to use as the depth indices across the returned traces
        Returns:
        ...
        if horizon:
            request = LineTimeSliceQueryRequest(file=file, rectangle=rectangle, horizon=Horizon(z_values=horizon))
        else:
            request = LineTimeSliceQueryRequest(file=file, rectangle=rectangle, constant=z)

        def get_stream():
            return self.query.GetTimeSliceByLines(request, metadata=self.metadata)
