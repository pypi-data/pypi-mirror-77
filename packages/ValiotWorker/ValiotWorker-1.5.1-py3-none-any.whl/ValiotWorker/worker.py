...
class ValiotWorker():
  ...
  def reportListenedQueuesExhaustive(self):
    ...
    if not listened:
      self.log(LogLevel.SUCCESS, f'{self.worker_code} not listening to any Queue')
  ...
  def runJob(self, job):
    ...
    if not job_dict:
      self.log(LogLevel.ERROR, f"{fn_name} job not found (it might exist in another branch / parallel universe), skipping...")
  ...
  def abortStaleJobs(self):
    ...
    while hasNextPage:
      ...
      data, errors = self.gql.query(
        ...
        variables={'worker': workerRegex, 'cursor': cursor, 'limit': limit}
      )
    ...
    if abortedCount and pages >= 2:
      self.log(LogLevel.ERROR, f'Deleted a total of {abortedCount} stale jobs')
  ...
  def updateWorkerQueues(self):
    ...
    for name, queue in self.queues.items():
      ...
      if not workerQueue:
        _, errors = self.gql.mutate(mutations.create_queue_worker, { 'worker': self.worker_code, 'queue': name, 'enabled': queueEnabled })
        if errors:
        ...
        _, errors = self.gql.mutate(mutations.update_queue_worker, { 'id': workerQueue['id'], 'enabled': queueEnabled })
        if errors:
          log_lvl = LogLevel.ERROR
    ...
    for i, job_lock in enumerate(self.lockedJobs):
      if not job_lock['process'].is_alive() or force:
        if self.unlockQueue(job_lock['job']['queue']['name'], job_lock['lock']):
          _log(LogLevel.SUCCESS, f'{job_lock["job"]["queue"]["name"]} UNLOCKED')
          unlocked.append(i)
        else:
          _log(LogLevel.ERROR, f'error unlocking {job_lock["job"]["queue"]["name"]}')
    for u in unlocked:
  ...
  def registerJobUpdatesListener(self):
    ...
    unsub = self.gql.subscribe(query=subscriptions.JOB_UPDATED, callback=onJobUpdated)
    if (unsub):
      self.log( LogLevel.SUCCESS, f'REGISTERED LISTENER FOR JOB UPDATES' )
    else:
      self.log( LogLevel.ERROR, f'ERROR REGISTERING LISTENER FOR JOB UPDATES' )
  ...
  def registerContinuousJobs(self):
    ...
    for shedeuleQueue in shedeuleQueues:
      ...
      if queue_name:
        ...
        queueLocks, errors = self.gql.query_one(queries.get_queueLocks, variables={
                                               "workerCode": self.worker_code, "queueName": queue_name})                            
        if errors:
          self.log(LogLevel.ERROR, f'Error get_queueLocks:\n{errors}')
          continue
        if queueLocks:
          self.log(LogLevel.WARNING, f'{queue_name} queue (CONTINUOUS type) has queueLocks')
          variables = {
            'id': __.get(queueLocks, 'id', None),
            'active': False,
            'endAt': getUtcDate(datetime.now()).strftime("%Y-%m-%dT%H:%M:%SZ")
            }
          _, errors = self.gql.mutate(mutations.update_lock, variables)
          if errors:
            self.log(LogLevel.ERROR, f'Error update_lock:\n{errors}')
  ...
  def monitorContinuousJobs(self):
    ...
    for continuousJob in self.continuousJobs:
      if not continuousJob in running_jobs_names and self.queues[continuousJob]['enabled']:
        self.log( LogLevel.WARNING, f'Continuous Job {continuousJob} is not running' )
  ...
  def eventLoop(self, interval=1.0):
    try:
      currentQueues, errors = self.gql.query(queries.all_queues_w_listeners)
      if (errors):
  ...
  def run(self, interval=1.0):
    ...
    if (self.jobConfigMode == JobConfigMode.SYNC):
      ...
      self.log(LogLevel.INFO, 'No config sync performed. If required, set jobConfigMode to SYNC')
