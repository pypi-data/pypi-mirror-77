...
class Board(BoardBase):
    ...
    def get_init_data(self):
        ...
        self.config.target_resources_file_path = '/var/aos/resources_config.cfg'
    ...
    def _set_target_resources(self):
        if self.config.target_resources is not None:
            ...
            cmd_copy = "cat /var/aos/resources_config.cfg > RESOURCES_CONFIG"
            cmd_paste = "ssh root@192.168.0.3  \"echo ${RESOURCES_CONFIG} > /var/aos/resources_config.cfg > \""
            self.connector.execute(cmd_copy)
            self.connector.execute(cmd_paste)
