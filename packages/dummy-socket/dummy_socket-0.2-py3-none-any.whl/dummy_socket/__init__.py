...
import math
import ssl
from dummy_socket.exceptions import *
from dummy_socket.ip_address import *
...
def serve(port, socket1, key，advanced_encryption=True, ip=getIP()):
    host = ip
...
def serve_forever(port, socket1, key, advanced_encryption=True, ip=getIP()):
    host = ip
    if advanced_encryption:
        mySocket = ssl.wrap_socket(socket1, ssl_version=ssl.PROTOCOL_TLS, ciphers="ADH-AES256-SHA")
    else:
