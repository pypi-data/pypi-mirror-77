...
def leaderboard_record_add(eventid,client,phase,user,score,timechar,score_sort,metric,team='',email=''):
    insert_query= """INSERT INTO mdl_hack_score (eventid,client, phase,user, score,timechar,score_sort,metric,team,email)
    VALUES ('{}','{}','{}','{}',{},'{}','{}','{}','{}','{}')""".format(eventid,client, phase,user, score,timechar,score_sort,metric,team,email)
    try:
        ...
        mydb.commit()
        mydb.close()
        return(0,"Rows Inserted :"+str(mycursor.rowcount))
...
def get_leaderboard(eventid,client,phase,return_format): 
    leaderboard_query = "select eventid,client,phase,user,round(max(score),3) as max_score, round(min(score),3) as min_score, max(email) as email,max(team) as team, max(score_sort) as score_sort, max(metric) as metric  from mdl_hack_score where eventid = '{}' and client = '{}' and phase = '{}' and exclude is Null group by eventid, client, user, phase".format(eventid,client,phase)
    try:
        mydb = mysql.connector.connect(host="www.fnmathlogic.com",user="mlportal_hack", password="m@thhackathon", database="mlportal_mdln1")
        mycursor = mydb.cursor()
        mycursor.execute(leaderboard_query)
        df=pd.DataFrame(mycursor.fetchall(),columns=["eventid","client","phase","user","max_score","min_score","email","team","score_sort","metric"])
        mydb.close()
    except mysql.connector.Error as err:
        print("Something went wrong: {}".format(err))
        return "1","Something went wrong: {}".format(err)
...
def evaluate_result(indf,eventid,client,phase,metric,act_file,pred_cols):
    ...
    read_file = s3.get_object(Bucket='hackathon.key',Key=act_file)
    act = pd.read_csv(read_file['Body'],sep=',')
    if set(indf.columns) == set(pred_cols):
        indf = indf[pred_cols]
        cols = pred_cols
        cols.sort()
        cols.remove("id")
    else:
        print("Solution dataframe for "+ phase +" should contain only the following columns : ",pred_cols, "Exiting...")
        sys.exit(1)
    merged=pd.merge(act,indf,on="id",how="inner")
    if merged.shape[0] == act.shape[0]:
        import getpass
        from datetime import datetime
        user = getpass.getuser()
        if metric =='LogLoss':
            from sklearn.metrics import log_loss
            score=log_loss(merged['y_true'],merged[[*cols]].values)
            score_sort=1
        if metric =='RMSE':
            from sklearn.metrics import mean_squared_error
            score=mean_squared_error(merged['y_true'],merged['y_pred'],squared=False)
            score_sort=1
        if metric =='BinaryCrossEntropy':
            from tensorflow.keras.losses import BinaryCrossentropy
            act_cols = [x for x in act.columns if x not in ('id')]
            act_cols.sort()
            score = BinaryCrossentropy()(merged[act_cols].values, np.array(merged[cols],dtype=np.float)).numpy()
            score_sort=1
        #from mathlogic import leaderboard_record_add,get_leaderboard
        timechar=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        leaderboard_record_add(eventid=eventid,client=client,phase=phase,user=user,score=score,timechar=timechar,score_sort=score_sort,metric=metric)
        print(user,score)
        get_leaderboard(eventid=eventid,client=client,phase=phase,return_format="display_html")
    else:
        print("Given Dataframe does not contain all the required predictions")
