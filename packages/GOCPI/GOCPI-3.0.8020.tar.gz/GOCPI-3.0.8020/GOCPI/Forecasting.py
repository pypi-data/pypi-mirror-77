...
class Forecasting:
    ...
    def calculate_cagr_forecasts(self, cagr_dictionary, base_year_dictionary,
        ...
        Returns:
            (float, array): Fuel demand forecasts for the foreseeable future
        """
        # Initialises the size of the array
        forecast = np.ones((len(region), len(fuel), len(year)))
        # Set the first forecast as the base year
        for i in range(1, len(region), 1):
            for j in range(1, len(fuel), 1):
                forecast[i, j, 0] = base_year_dictionary[fuel[j]]

        # Calculates the forecasting
        for i in range(1, len(region), 1):
            for j in range(1, len(fuel), 1):
                for k in range(1, len(year), 1):
                    forecast[i, j,
                             k] = forecast[i, j,
                                           k - 1] * cagr_dictionary[fuel[j]]
