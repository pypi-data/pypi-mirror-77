...
DEFAULT_FLAGS = [
    ...
    click.option("-c", "--copy", type=click.IntRange(1, 3),
                 help="Copies the answer. 1 for exact_ans and 2 for approx_ans and 3 for a list of [exact_ans, "
                      "approx_ans].")
]
...
def eval(expression, vars_sub, **kwargs):
    ...
    if len(vars_sub) % 2 != 0:
        print(
            "Variable substitution must consist of an even number of arguments, see 'eval --help' for more information")
        return
...
def web(website_index, list):
    ...
    import json
    with open("caspy3/data/websites.json", "r", encoding="utf8") as json_f:
