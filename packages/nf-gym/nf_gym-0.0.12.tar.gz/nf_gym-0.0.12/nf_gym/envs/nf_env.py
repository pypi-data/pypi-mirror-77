...
class NFEnv(gym.Env):
    ...
    def __init__(self):
        self.step_counter = 0
        if os.path.exists("/Users/dj/stuff/bot/urdf/bot4.urdf"):
            p.connect(p.GUI)
        else:
            p.connect(p.DIRECT)
        p.setAdditionalSearchPath(pybullet_data.getDataPath())
        self.action_space = spaces.Box(np.array([-1]*4), np.array([1]*4))
        self.observation_space = spaces.Box(np.array([-1]*18), np.array([1]*18)) # XXX: range
    ...
    def step(self, action):
        ...
        for i in range(4):
            vel=action[i]*10
            joint_no=JOINTS[i]
            p.setJointMotorControl2(self.bot_id, joint_no, controlMode=mode, targetVelocity=vel, force=MAX_TORQUE)
