...
def importData(sql_query: str, db_cred: dict, df_meta = None, 
    ...
    Arguments:
        - sql_query         - SQL-query to run
        - db_cred           - Connection parameters from config file
        - df_meta           - DataFrame with schema information from importMetaData function
        - post_rename       - Dictionary for renaming dictionaries after dtype conversion
        - convert_dtypes    - Flag if data types should be converted using the 
                              data base schema for the used table
        - remove_tz         - Flag if timezone info should be removed from datetime columns
    Returns:
        - df                - DataFrame with loaded data
    """
    ...
    try:
        ...
        if (df_meta is not None) & (convert_dtypes):
            ...
            for col,data_dtype in df.dtypes.iteritems():
                ...
                assert col in df_meta_subset.column_name.unique(), f"Column {col} not present in schema"
                #assert df_meta_subset.data_type.nunique() == 1, f"Column {col} has multiple dtypes in schema"  Uncomment once Alex has replied to email
    ...
    # Post Dtype renaming
    for col in post_rename:
        assert col in df.columns, f"Column {col} in renameing dict not present in DataFrame"
    df = df.rename(columns=post_rename) 
    
    # Final check of dtypes
    if convert_dtypes:
        incorr_dtypes = [col for col in df.columns if df[col].dtype == np.dtype('O')]
        assert (len(incorr_dtypes) == 0), f"Column(s) {','.join(incorr_dtypes)} are dtype Object"
    
    return df

def importMetaData(db_cred: dict,like_str: str = "maximoworker_"):
    """Function for importing data base schema for every table with a tablename 
    starting with like_str 
    ...
    try:
        ...
        if df_meta.duplicated().sum() > 0:
            raise Exception("Data types not consistent in schema")
