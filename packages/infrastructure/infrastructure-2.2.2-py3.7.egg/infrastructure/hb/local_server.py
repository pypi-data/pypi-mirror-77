...
# @Last Modified time: 2020-08-18 20:57:32
...
class LocalServer(object):
	def __init__(self,alreadyRecord,JavaCoverageReportUrl="",coverageLog="",
		...
		self.mdestfile = JAVA_COV["mdestfile"].format(service_name=self.alreadyRecord.service_name)
		self.mergeReportDir = JAVA_COV["mergeReportDir"].format(service_name=self.alreadyRecord.service_name,
			recordID=self.alreadyRecord.id)
	...
	def cpFile(self):
		...
		for jcIndex in eval(self.alreadyRecord.sn_id):
			command = "cp {jacocoIt} {mergeFile}/{index}.exec".format(jacocoIt=self.jacocoIt.format(
				service_name=self.alreadyRecord.service_name,index=jcIndex),mergeFile=self.mergeFile,
				index=jcIndex)
	...
	def jacocoReport(self):
		if self.isMerge:
			...
			jacocoDumpFile = "{mdestfile}/all.exec".format(self.mdestfile)
		else:
			reportDir = self.reportDir
			jacocoDumpFile =  self.jacocoIt.format(service_name=self.alreadyRecord.service_name,
	...
	def parseReport(self):
		...
		if os.path.exists(index_path):
			...
			if "uat" in hostname:
				...
				env = "uat"
			report_html_url = ['https://{}-rfautotest.hellobike.cn/report/{}/{}/{}/index.html'.format(
