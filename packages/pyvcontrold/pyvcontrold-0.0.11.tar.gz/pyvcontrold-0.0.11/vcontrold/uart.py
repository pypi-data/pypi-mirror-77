...
class ViessmannProtocol(asyncio.Protocol):
    ...
    def data_received(self, data: bytes):
        ...
        while len(self._buf) > 0:
            if self._buf[0] == self._ACK:
                ...
                try:
                    addr = pkt.address
                    f = self._futures[addr]["result_future"]
                    sent = self._futures[addr]["sent_pkt"]
                    if f.done() or \
                       f.cancelled():
                       continue
                    ...
                    if pkt.answers(sent):
                        ...
                        logger.error("%s does not answer %s", pkt.show(dump=True), ssent.show(dump=True))
                        f.cancel()
    ...
    async def send_recv(self, pkt, timeout=3):
        addr = pkt.address
        if addr in self._futures:
            raise Exception("There is already a future for address %x" % (addr))
        else:
            self._futures[addr] = {
                "sent_pkt": pkt,
                "result_future": self._loop.create_future()
            }
        await self.send(pkt, timeout)
        try:
            answer = await asyncio.wait_for(self._futures[addr]["result_future"], timeout)
        except Exception as e:
            pass
        finally:
            del self._futures[addr]
        return answer
    ...
    async def writedata(self, address, data):
        equest = VS2Header() / VS2Data(type="request", command="writedata", address=address, data_len=len(data), data=data)
