# -*- coding: utf-8 -*-
import time,hashlib,json,re,os,platform,sys,shutil,requests,importlib,traceback,pip,gzip,tarfile,zipfile,random
...
class response:
    def tpl(path=None,status='200 ok',header={"Content-Type":"text/html; charset=utf-8"},**context):
        "返回模板内容"
        ...
        return Template(Temppath,config=config,**context),status,header
    def json(res=[],status='200 ok',header={"Content-Type":"application/json; charset=utf-8","Access-Control-Allow-Origin":"*"}):
        "响应json内容"
        return json_encode(res),status,header
    def pic(body):
        """输出图片
        ...
        return body,'200 ok',{"Content-Type":ContentType,"Cache-Control":"public, max-age=60"}
