...
static PyObject* Fprofile_StartTrace(PyObject* obj, PyObject* args, PyObject* kwargs) {
    ...
    if (existingProfile == NULL) {
        ...
        // XXX: We need to wait for the server to spawn.  There's probably a
        // better way.
        usleep(100000);
        ...
        mpack_start_map(&initWriter, 5);
        {
            ...
            mpack_write_cstr(&initWriter, "0.0.1");
...
static void Fprofile_ResetThreadState(void) {
    ...
    moduleConfiguration.pid = getpid();
    ThreadState* newState = Fprofile_CreateThreadState();
    ...
    // socket.
    // This shouldn't happen (we should catch exit cases and force a flush
    // then), but it can't hurt to be safe.
    //
    ...
    mpack_writer_flush_message(&newState->writer);
}
