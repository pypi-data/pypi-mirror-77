...
class Engine(Model):
    ...
    def _run(self):
        ...
        with nominode_ctx:
            ...
            nominode_handler = NominodeLogHandler()
            root_logger.addHandler(nominode_handler)
            try:
                ...
                self._current_action = None
                root_logger.removeHandler(nominode_handler)
                nominode_handler.shutdown()

    def _finalize_kwargs(self, model_params, params):
        kwargs = {}
        all_keys = set(model_params.keys()) | set(params.keys())
        union_params = {key: (model_params.get(key), params.get(key)) for key in all_keys}
        for keyword, (param, val) in union_params.items():
            if not param:
                ...
                try:
                    ...
                    logger.debug(f"\tValidating {keyword}:'{val}' with {param.type}")
                    param.validate(val)
    ...
    def __call__action__(self, func, *args, **kwargs):
        ...
        with mock:
            ...
            try:
                ...
                self._current_action = action
                if action.as_kwargs:
                    result = action.func(**final_kwargs)
                else:
                    result = action.func(final_kwargs)
            finally:
