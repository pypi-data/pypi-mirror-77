...
class InstallPluginCommand(install):
    def run(self):
        install.run(self)
        try:
            check_call(['pulumi', 'plugin', 'install', 'resource', 'terraform', '2.6.0-alpha.1597708883+be851dbd'])
            ...
            if error.errno == errno.ENOENT:
                ...
                `pulumi plugin install resource terraform 2.6.0-alpha.1597708883+be851dbd`
                """)
