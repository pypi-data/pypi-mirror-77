...
from Qt import QtGui, QtCore, QtWidgets, QtSvg, QtCompat
...
def geticon(gui, ico):
    return QtGui.QIcon(os.path.join(gui.mgvDirectory, 'icons', ico+'.svg'))
...
class MgvHudItem(QtWidgets.QGraphicsItem):
    ...
    def refreshData(self):
        ...
        for x in dicog:
            dico[x] = dicog[x]
        dicog['MGVSETTINGS'] = os.getenv('MGVSETTINGS')
...
class MgvGraphView(QtWidgets.QGraphicsView):
    ...
    def __init__(self, gui, graph):
        ...
        h = self.gui.root.splitter.geometry().height()
        self.gui.root.splitter.setSizes([h * .7, h * .3])
        self.splitterState = self.gui.root.splitter.saveState()
        self.splitterSizes = [h * .7, h * .3]

        self.scene = QtWidgets.QGraphicsScene()
        self.scene.setSceneRect(0, 0, 40000 * self.globalScale, 40000 * self.globalScale)
        self.scene.setBackgroundBrush(QtGui.QColor(self.gui.colorDic['edit']))
        ...
        while self.gui.root.tabWidget.count() > 1:
            self.gui.root.tabWidget.removeTab(1)
        self.gui.root.tabWidget.addTab(self.actionWindow, 'Actions')
    ...
    def showActions(self):
        if self.gui.root.splitter.sizes()[1] == 0:
            self.gui.root.splitter.setSizes(self.actionWindow.parent.splitterSizes)
            self.splitterState = self.gui.root.splitter.saveState()
        self.gui.root.tabWidget.setCurrentIndex(1)
        self.refreshHud()
...
class BinCategory(QtWidgets.QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        painter.setBrush(QtGui.QColor(self.view.gui.colorDic['dark']))
        painter.drawRect(self.x-5, self.y, self.width+10, 25)

        pen = QtGui.QPen()
        pen.setWidth(.25)
        pen.setColor(QtGui.QColor(QtGui.QColor(self.view.gui.colorDic['darker'])))
        painter.setPen(pen)
        painter.drawLine(self.x + self.width + 4.5, self.y+1, self.x + self.width + 4.5, self.y + 2000)

        pen = QtGui.QPen()
        pen.setWidth(1.5)
        pen.setColor(QtGui.QColor(self.view.gui.colorDic['text']))
        painter.setPen(pen)
...
class BinNode(QtWidgets.QGraphicsItem):
    ...
    def __init__(self, view, typ, x, y):
    ...
    def paint(self, painter, option, widget):
        ...
        pen.setWidth(1.5)
        if self.over:
            pen.setColor(self.view.gui.colorDic['text'])
        ...
        painter.setPen(QtCore.Qt.NoPen)
        painter.drawRect(self.x, self.y, self.width, 25)
        painter.setPen(pen)
        ...
        for i, key in enumerate(sorted(dico)):
            ...
            for t in sorted(dico[key], key=lambda sort_key: sort_key.getName()):
                if type_filter.lower() in t.getName().lower():
                    ...
                    maxy = max(maxy, y)
                    y += 30
        self.geowidth = 12 + len(dico)*100 + (len(dico)-1) * 9
        self.geoheight = maxy + 25
        self.resizeEvent(None)

    def resizeEvent(self, event):
        super(IconView, self).resizeEvent(event)
        geow = self.geometry().width()
        geoh = self.geometry().height()
        if self.verticalScrollBar().isVisible():
            geow -= self.verticalScrollBar().width()
        if self.horizontalScrollBar().isVisible():
            geoh -= self.horizontalScrollBar().height()
        self.scene.setSceneRect(0, 0, max(self.geowidth, geow), max(self.geoheight, geoh))
...
class MgvPlugItem(QtWidgets.QGraphicsItem):
    """Plug UI object."""
    obj = 'plug'

    def __init__(self, graphview):
        super(MgvPlugItem, self).__init__()
        self.graphview = graphview
        self.r = 6
        self.setZValue(3)
    ...
    def boundingRect(self):
        return QtCore.QRectF(-self.r, -self.r, self.r*2, self.r*2)

    def shape(self):
        chemin = QtGui.QPainterPath()
        chemin.addEllipse(-self.r*.5 * self.graphview.globalScale,
                          -self.r*.5 * self.graphview.globalScale,
                          self.r * self.graphview.globalScale,
                          self.r * self.graphview.globalScale)
        return chemin
    ...
    def paint(self, painter, option, widget):
        ...
        painter.setPen(pen)
        painter.setBrush(QtGui.QColor('#AAAA33'))
        painter.drawEllipse(-self.r*.5 * self.graphview.globalScale,
                            -self.r*.5 * self.graphview.globalScale,
                            self.r * self.graphview.globalScale,
                            self.r * self.graphview.globalScale)
...
class MgvLinkItem(QtWidgets.QGraphicsItem):
    ...
    def __init__(self, link, graphview):
        ...
        self.plugIn = MgvPlugItem(self.graphview)
        self.plugOut = MgvPlugItem(self.graphview)
    ...
    def shape(self):
        y = self.getY()
        path = QtGui.QPainterPath()
        fromX = self.link.linkfrom.item.pos().x() + self.link.linkfrom.item.width*.5
        toX = self.link.linkto.item.pos().x() - self.link.linkto.item.width*.5
        fromY = self.link.linkfrom.item.pos().y() + self.thickness * self.graphview.globalScale
        toY = self.link.linkto.item.pos().y() + self.thickness * self.graphview.globalScale

        path.moveTo(fromX, fromY)
        path.cubicTo(fromX*.5 + toX*.5, fromY, fromX*.5 + toX*.5, toY + y, toX, toY + y)
        path.lineTo(toX, self.link.linkto.item.pos().y() - self.thickness * self.graphview.globalScale + y)
        path.cubicTo(fromX*.5 + toX*.5,
    ...
    def paint(self, painter, option, widget):
        ...
        fromX = self.link.linkfrom.item.pos().x() + self.link.linkfrom.item.width * .5
        toX = self.link.linkto.item.pos().x() - self.link.linkto.item.width * .5
        fromY = self.link.linkfrom.item.pos().y()
        toY = self.link.linkto.item.pos().y()
        self.plugIn.setPos(fromX, self.link.linkfrom.item.pos().y())
        self.plugOut.setPos(toX, self.link.linkto.item.pos().y() + y)
        pen = QtGui.QPen()
    ...
    def delete(self):
        outputlinks = self.link.linkto.outputLinks
        inputlinks = self.link.linkto.inputLinks
        self.link.item = None
        self.link = None
        ...
        for x in inputlinks:
            if not isString(x):
                if x.item:
                    x.item.prepareGeometryChange()
                    x.item.update()
        for x in outputlinks:
            if not isString(x):
                if x.item:
                    x.item.prepareGeometryChange()
                    x.item.update()
...
class MgvNodeItem(QtWidgets.QGraphicsItem):
    ...
    def __init__(self, node, graphview):
        ...
        self.counter = 0
        data = os.path.join(self.graphview.gui.mgvDirectory, 'icons', 'lock.svg')
        with open(data) as fid:
            self.lockIco = fid.read()
    ...
    def chooseShape(self):
        ...
        if self.pshape == 'Image':
            ...
            if self.node.type and len(self.node.type.getImage()):
                self.image = QtGui.QImage()
                ByteArr = QtCore.QByteArray.fromBase64(self.node.type.getImage().encode('utf-8'))
                self.image.loadFromData(ByteArr)
    ...
    def paint(self, painter, option, widget):
        self.setZValue(1 if self.selected else 0)

        # USER HALO
        if self.user.name == self.graphview.graph.user:
        ...
        painter.setPen(pen)
        type_color = self.node.type.getColor() if self.node.type else '#FF0000'
        ...
        if self.image is not None:
            if self.pshape == 'Rectangle':
                ...
                img = self.image.scaledToHeight(height)
                width = img.width()
                painter.drawImage(-self.width/2 + width/3, -height/2, img, sw=width, sh=height)
            elif self.pshape == 'Circle':
                height = self.height * .5
                img = self.image.scaledToHeight(height)
                width = img.width()
                painter.drawImage(-width/2, -height/2, img, sw=width, sh=height)
        ...
        if self.user.name not in ['free', self.graphview.graph.user]:
            size = 30
            pos = [self.width*.5, -self.height*.5]
            if self.pshape == 'Image':
                pos = self.node.type.getShapeVector()[-5]
                pos = [pos[0]*self.graphview.globalScale, pos[1]*self.graphview.globalScale]
            elif self.pshape == 'Circle':
                pos = [self.width * .5 * .707, -self.height * .5 * .707]
            data = self.lockIco.replace(u'#A1A1A1', self.user.color.name())
            data = QtCore.QByteArray(data.encode())
            QtSvg.QSvgRenderer(data).render(painter, QtCore.QRectF(pos[0]-size*.5*self.graphview.globalScale, pos[1]-size*.5*self.graphview.globalScale, size*self.graphview.globalScale, size*self.graphview.globalScale))
    ...
    def setItemPos(self, x, y):
        ...
        for link in self.node.inputLinks:
            if not isString(link):
                if link.item:
                    ...
                    link.item.plugIn.prepareGeometryChange()
                    link.item.plugOut.prepareGeometryChange()
        ...
        for link in self.node.inputLinks:
            if not isString(link):
                if link.item:
                    link.item.update()
                    link.item.plugIn.update()
                    link.item.plugOut.update()
        for link in self.node.outputLinks:
            if not isString(link):
                if link.item:
                    link.item.update()
...
class MgvTypeEditor(QtWidgets.QMainWindow):
    ...
    def __init__(self, gui, typ):
        ...
        self.setWindowIcon(geticon(self.gui, 'mgvIcon'))
        self.type = typ._dup()
        self.realtype = typ
        ...
        self.root.deleteIconButton.setIcon(geticon(gui, 'x'))
        self.root.deleteIconButton.setIconSize(QtCore.QSize(24, 24))
        self.root.deleteVersionPushButton.setIcon(geticon(gui, 'trash'))
        self.root.deleteVersionPushButton.setIconSize(QtCore.QSize(16, 16))
        self.root.addParamButton.setIcon(geticon(gui, 'plus'))
        self.root.addActionButton.setIcon(geticon(gui, 'plus'))
        self.root.delParamButton.setIcon(geticon(gui, 'x'))
        self.root.delActionButton.setIcon(geticon(gui, 'x'))
        self.root.upParamButton.setIcon(geticon(gui, 'arrow-up'))
        self.root.upActionButton.setIcon(geticon(gui, 'arrow-up'))
        self.root.downParamButton.setIcon(geticon(gui, 'arrow-down'))
        self.root.downActionButton.setIcon(geticon(gui, 'arrow-down'))
        ...
        self.root.deleteIconButton.clicked.connect(self.deleteIcon)
        ...
        self.root.linksComboBox.setItemIcon(0, geticon(gui, 'plus'))
    ...
    def addOutput(self, outFile=None, first=True):
        ...
        w.deleteButton.setIcon(geticon(self.gui, 'trash'))
        w.deleteButton.setStyleSheet('background: transparent; color: #FF0000')
        ...
        w.deleteButton.clicked.connect(lambda dummy=0, widget=w, out=outFile: self.deleteOutput(widget, out))
...
""".replace('url(:', 'url(' + self.gui.mgvDirectory))
    ...
    def loadVersion(self):
        ...
        liste = [x.getName() for x in self.currentVersion.getParameters()]
        self.root.paramListWidget.addItems(liste)
        self.auto = False
        if self.currentParam:
            for i, x in enumerate(liste):
                if x == self.currentParam.getName():
                    self.root.paramListWidget.setCurrentItem(self.root.paramListWidget.item(i))
                    break
            else:
                self.currentParam = None
    ...
    def checkCurrentColor(self):
        ...
        if self.currentVersion is not self.type.versionActive:
            ...
            self.root.deleteVersionPushButton.setIcon(geticon(self.gui, 'trash'))
        else:
    ...
    def addLink(self, i):
        if i > 0:
            ...
            toto.clicked.connect(lambda dummy=0, t=typ, w=toto: self.removeLink(t, w))
    ...
    def currentParamDefaultChanged(self):
        ...
        if self.currentParam:
            ...
            try:
                ...
                self.root.paramDefaultTextEdit.text.setStyleSheet(self.root.paramDefaultTextEdit.text.style)
            except ValueError:
                self.root.paramDefaultTextEdit.text.setStyleSheet('background-color:#BB3333')
    ...
    def deleteIcon(self):
        self.type.image = ''
        self.root.shapeButton.setIcon(QtGui.QIcon())
        self.type.shapeVector = []
...
class MgvTask(object):
    ...
    def __init__(self, actionWindow, node_version, action, widget, item, item_button, start_button, close_button):
    ...
    def start(self):
        ...
        dico['MGVACTION'] = self.action.name
        if self.node_version:
...
Execute(graph)""" % script
        ...
        tmpfile.close()
        with codecs.open(tmpfile.name, 'w', encoding='utf-8') as fid:
            fid.write(script)
...
class MgvActionWindow(QtWidgets.QWidget):
    ...
    def __init__(self, parent):
        ...
        vvv = QtWidgets.QHBoxLayout()
        vvv.setContentsMargins(0, 0, 0, 0)
        vvv.addWidget(self.ui)
        self.setLayout(vvv)
        self.ui.listWidget.currentItemChanged.connect(self.rowChanged)
        self.ui.clearButton.clicked.connect(self.clear)
        self.dico = {}
        self.tasks = []
        self.ui.listWidget.setStyleSheet('background: %s' % self.parent.gui.colorDic['dark'])
        self.ui.clearButton.setStyleSheet('background: %s' % self.parent.gui.colorDic['darker'])
        self.dummyText = QtWidgets.QTextEdit()
        self.dummyText.setStyleSheet('QTextEdit{background-color: %s}' % self.parent.gui.colorDic['darker'])
        self.dummyText.setReadOnly(True)
        self.ui.layoutH.insertWidget(0, self.dummyText)
        self.ui.layoutH.setStretch(0, 1)
    ...
    def kill(self, task):
        ...
        if task.process and task.process.poll() is None:
            while task.process.poll() is None and c < 10:
                try:
                    if not sys.platform.startswith('win'):
                        os.killpg(os.getpgid(task.process.pid), signal.SIGKILL)
                    else:
                        os.popen('TASKKILL /PID %s /F /T 2>nul 1>nul' % task.process.pid)
                except Exception:
                    pass
                c += 1
            task.errThread.signal.emit('killed.', 1)
        else:
            self.tasks.remove(task)
            task.widget.deleteLater()
            self.ui.listWidget.takeItem(self.ui.listWidget.row(task.item))
    ...
    def colorItems(self):
        ...
        for task in self.tasks:
            color = self.parent.gui.colorDic['text'] if task is self.dico[i.id] else self.parent.gui.colorDic['grey']
            if task.process is not None:
                if task.stopped or (task.process and task.process.poll() is not None):
                    # fini
                    task.item_button.setStyleSheet('text-align: left; background: %s; color: %s' % (self.parent.gui.colorDic['selectDark'], color))
                    task.close_button.setIcon(geticon(self.parent.gui, 'x'))
                    task.close_button.setIconSize(QtCore.QSize(24, 24))
                else:
                    # en cours
                    task.start_button.hide()
                    task.item_button.setStyleSheet('text-align: left; background: %s; color: %s' % (self.parent.gui.colorDic['selection'], color))
                    task.close_button.setIcon(geticon(self.parent.gui, 'skull'))
                    task.close_button.setIconSize(QtCore.QSize(16, 16))
            else:
                # pas fait
                task.item_button.setStyleSheet('text-align: left; background: %s; color: %s' % (self.parent.gui.colorDic['edit'], color))
                task.close_button.setIcon(geticon(self.parent.gui, 'x'))
                task.close_button.setIconSize(QtCore.QSize(24, 24))
    ...
    def exe(self, node_version, action):
        ...
        w.setStyleSheet('QTextEdit{background-color: %s}' % self.parent.gui.colorDic['darker'])
        w.setReadOnly(True)
        self.ui.layoutH.insertWidget(0, w)
        self.ui.layoutH.setStretch(0, 1)
        ...
        item_button = QtWidgets.QPushButton(action_title + ' - ' + action.getName())
        item = QtWidgets.QListWidgetItem(action_title + ' - ' + action.getName())
        item.id = random.random()
        item.setToolTip(time.strftime('%H:%M', time.gmtime()))
        item_button.clicked.connect(lambda dummy=0, x=item: self.selectRow(item))
        h = QtWidgets.QHBoxLayout(item_button)
        h.setContentsMargins(0, 0, 0, 0)
        h.addStretch()
        close_button = QtWidgets.QPushButton()
        start_button = QtWidgets.QPushButton()
        start_button.setMaximumWidth(16)
        close_button.setMaximumWidth(16)
        start_button.setIcon(geticon(self.parent.gui, 'play'))
        close_button.setIcon(geticon(self.parent.gui, 'x'))
        start_button.setStyleSheet('text-align: center')
        close_button.setStyleSheet('text-align: center')
        h.addWidget(start_button)
        h.addWidget(close_button)

        task = MgvTask(self, node_version, action, w, item, item_button, start_button, close_button)
        start_button.clicked.connect(lambda dummy=0, x=task: self.now(x))
        close_button.clicked.connect(lambda dummy=0, x=task: self.kill(x))

        self.tasks.append(task)

        self.dico[item.id] = task
        self.ui.listWidget.insertItem(0, item)
        self.ui.listWidget.setItemWidget(item, item_button)
...
class MgvListener(QtCore.QThread):
    ...
    def interrupt(self, port):
        self.active = False
        try:
            socket.socket(socket.AF_INET, socket.SOCK_STREAM).connect((self.ip, port))
            self.s.close()
            sys.exit(0)
        except socket.error:
            pass
...
class MangroveGUI(object):
    ...
    def __init__(self, app):
        ...
        self.window.setWindowIcon(geticon(self, 'mgvIcon'))
        ...
        colorPath = os.path.join(self.mgvDirectory, 'UI', 'color.json')
        with open(colorPath) as fid:
            root = json.load(fid)
        self.colorDic = root
        for item in root:
            self.style = self.style.replace('$%s' % item, root[item])
        self.root.setStyleSheet(self.style)
        self.notify('Welcome to Mangrove !')

        self.root.tabWidget.addTab(self.icons, 'Nodes')
        ...
        self.root.splitter.splitterMoved.connect(self.splitterMoved)

        self.root.tabWidget.tabBar().setExpanding(True)
        tabStyle = '''
QTabBar::tab{                       background-color: transparent; color: $grey; border: 0px; padding: 10 0 10 0; }
QTabBar::tab:selected{              background-color: $edit; color: $text;}
QTabBar::tab:hover{                 background-color: $selection; color: $text;}'''
        for item in self.colorDic:
            tabStyle = tabStyle.replace('$%s' % item, self.colorDic[item])
        self.root.tabWidget.setStyleSheet(tabStyle)

        self.root.splitter.hide()
        #self.window.showMaximized()
        #self.root.splitter.setSizes([100, 300])
        ...
        label.setIcon(geticon(self, 'mgvIconG'))
        label.setIconSize(QtCore.QSize(250, 250))
        self.root.centralWidget().layout().insertWidget(0, self.root.splashTab)
        self.root.secondWidget.hide()
    ...
    def splitterMoved(self):
        if self.currentGraphView:
            i = self.root.splitter.sizes()
            if i[1] > 0:
                self.currentGraphView.splitterSizes = i

    def toggleIcons(self):
        if self.root.splitter.sizes()[1] > 0:
            self.root.splitter.setSizes([1, 0])
        else:
            if self.currentGraphView:
                self.root.splitter.setSizes(self.currentGraphView.splitterSizes)
    ...
    def networkReceive(self, text, client):
        ...
        if len(params) > 0:
            ...
            if text.startswith('OPEN'):
                if ':' in graphfullpath:
                    graphfullpath =graphfullpath.split(':')
                self.openGraph(graphfullpath)
            else:
                nodename = params[1]
    ...
    def tabChanged(self, i):
        if self.currentGraphView:
            self.currentGraphView.splitterState = self.root.splitter.saveState()
        ...
        oldIndex = self.root.tabWidget.currentIndex()
        while self.root.tabWidget.count() > 1:
            self.root.tabWidget.removeTab(1)
        if self.currentGraphView:
            self.root.splitter.restoreState(self.currentGraphView.splitterState)
            self.root.tabWidget.addTab(self.currentGraphView.actionWindow, 'Actions')
            self.root.tabWidget.setCurrentIndex(oldIndex)
            ...
            if not self.currentGraphView.stopRequest:
                ...
                if tocopy and nodepath and os.path.exists(nodepath):
                    if flatten:
                        ...
                        version = '_v' + str(node['oldVersionActive']).zfill(pad)
                    else:
                        version = 'all'
    ...
    def readLocalSettings(self):
        ...
        if localSettingsFile is not None:
            root = None
    ...
    def openGraph(self, path=None):
        ...
        self.root.splitter.show()
        self.root.secondWidget.show()
        ...
        if not len(graphview.graph.nodes) and path[-1] != '*template*':
            ...
            if len(_templates):
                if not len(graphview.graph.template_uuid):
                        ...
                        if show_icons:
                            try:
                                ...
                                if len(t.icon):
                                    ...
                                    b.setIcon(geticon, 'slash')
                                b.setIconSize(QtCore.QSize(70, 70))
                            except:
                                pass
                        b.clicked.connect(lambda dummy=0, ind=i: d.done(ind + 1))
                    out = d.exec_()
                    if out:
                    ...
                    for t in _templates:
                        if t.uuid == graphview.graph.template_uuid:
    ...
    def setEnv(self):
        if not (None in [x.node for x in self.currentGraphView.editors]):
            ...
            self.root.addDockWidget(QtCore.Qt.LeftDockWidgetArea, new)
...
"""
    ...
    @staticmethod
    def help():
        url = 'https://opensource.theyard-vfx.com/mangrove/documentation/'
        webbrowser.open(url)
...
class MgvWelcome(object):
    ...
    def __init__(self, gui):
        ...
        self.root.setWindowIcon(geticon(self.gui, 'mgvIcon'))
...
class MgvEnvEditor(QtWidgets.QDockWidget):
    ...
    def __init__(self, gui, graphview):
        ...
        addButton.setIcon(geticon(self.gui, 'chevron-down'))
...
class MgvRename(object):
    ...
    def __init__(self, editor):
        ...
        self.root.setWindowIcon(geticon(self.editor.gui, 'mgvIcon'))
...
class MgvNodeEditor(QtWidgets.QDockWidget):
    ...
    def __init__(self, gui, graphview, node):
        ...
        self.versionButton = QtWidgets.QPushButton()
        self.versionButton.setLayoutDirection(QtCore.Qt.RightToLeft)

        h = QtWidgets.QHBoxLayout()
        h.setContentsMargins(0, 0, 0, 0)
        h.addWidget(self.title)
        h.addWidget(self.versionButton)
        h.setStretch(0, 1)
        h.setSpacing(0)
        titleWidget.setLayout(h)
        self.setTitleBarWidget(titleWidget)
        mainUIPath = os.path.join(self.gui.mgvDirectory, 'UI', 'editor.ui')
        self.title.setStyleSheet('padding: 0 0 0 6')
        self.setAllowedAreas(QtCore.Qt.LeftDockWidgetArea)
        self.setFeatures(QtWidgets.QDockWidget.AllDockWidgetFeatures)
        if node and node.type:
            self.versionButton.setIcon(geticon(self.gui, 'chevron-down'))
            self.versionButton.setIconSize(QtCore.QSize(20, 20))
            self.root = QtCompat.loadUi(mainUIPath)
            self.setWidget(self.root)
            ...
            self.root.advancedButton.setStyleSheet('text-align: left; background: %s' % self.gui.colorDic['darker'])
            toto = QtWidgets.QHBoxLayout(self.root.advancedButton)
            self.advancedLabel = QtWidgets.QLabel()
            self.advancedLabel.setPixmap(geticon(self.gui, 'chevron-up').pixmap(24, 24))
            toto.addWidget(self.advancedLabel, alignment=QtCore.Qt.AlignRight)
            toto.setContentsMargins(0, 0, 0, 0)

            self.root.nameWidget.setStyleSheet('QWidget{background: %s}' % self.gui.colorDic['selectDark'])

            self.setStyleButton(self.root.newVersionButton, 'plus', 28)
            self.setStyleButton(self.root.delVersionButton, 'trash', 28)
            self.setStyleButton(self.root.metaButton, 'info', 28)
            self.setStyleButton(self.root.openButton, 'refresh', 28)
            self.setStyleButton(self.root.dataButton, 'info')
            self.setStyleButton(self.root.renameButton, 'edit')

            self.root.nameLabel.setStyleSheet('font: 12px; padding: 0 0 0 6')

            ico = 'lock'
            if self.node.versionActive.isLocked():
                ico = 'unlock'
            self.setStyleButton(self.root.lockButton, ico, 28)
            if hasattr(node.type, 'name'):
                opp = opposite(node.type.getColor())
                titleWidget.setStyleSheet('QWidget{ background-color: %s; color: %s}' % (
                    node.type.getColor(), opp))

                self.versionButton.setStyleSheet('color: %s' % opp)
                if opp == '#CCCCCC':
                    self.versionButton.setIcon(geticon(self.gui, 'chevron-down'))
                else:
                    self.versionButton.setIcon(geticon(self.gui, 'chevron-down-dark'))
                self.title.setText(node.type.getName().upper())
            else:
                self.title.setText(node.type.upper())
            ...
            self.versionButton.clicked.connect(self.versionPopup)
            ...
            self.read()
            self.root.commentTextEdit.setMinimumHeight(70)
            self.root.splitter.setSizes([1, 10000])

            self.setStyleSheet('QWidget#mainWidget{background-color: %s}' % self.gui.colorDic['dark'])
        else:
            self.setMinimumWidth(440)
            self.versionButton.hide()
        self.graphview.refreshHud()

    def setStyleButton(self, button, ico, size=None):
        button.setStyleSheet('''
        QPushButton{background: transparent}
        QPushButton::hover{background: %s}
        QPushButton::pressed{background: %s}
        ''' % (self.gui.colorDic['selection'], self.gui.colorDic['selectLight']))
        icon = QtGui.QIcon()
        if size is not None:
            p1 = geticon(self.gui, ico).pixmap(QtCore.QSize(size, size))
            p2 = geticon(self.gui, ico + '-grey').pixmap(QtCore.QSize(size, size))
            icon.addPixmap(p1, QtGui.QIcon.Normal)
            icon.addPixmap(p2, QtGui.QIcon.Disabled)
        else:
            icon.addPixmap(QtGui.QPixmap(self.gui.mgvDirectory + '/icons/' + ico + '.svg'), QtGui.QIcon.Normal)
            icon.addPixmap(QtGui.QPixmap(self.gui.mgvDirectory + '/icons/' + ico + '-grey.svg'), QtGui.QIcon.Disabled)
        button.setIcon(icon)
        if size is not None:
    ...
    def checkLock(self):
        if self.node.versionActive.isLocked():
            self.setStyleButton(self.root.lockButton, 'unlock')
        else:
            self.setStyleButton(self.root.lockButton, 'lock')
    ...
    def advancedChanged(self):
        ...
        if self.advanced:
            self.advancedLabel.setPixmap(geticon(self.gui, 'chevron-down').pixmap(24, 24))
            self.root.advancedWidget.show()
        else:
            self.advancedLabel.setPixmap(geticon(self.gui, 'chevron-up.svg').pixmap(24, 24))
            self.root.advancedWidget.hide()
        self.checkEditable()
    ...
    def read(self):
        if self.root:
            ...
            self.versionButton.setText('v' + str(self.node.versionActive.id).zfill(pad))
    ...
    def checkEditable(self):
        if self.node and self.node.type:
            ...
            if active:
                ...
                self.versionButton.setStyleSheet('color: #777777')
                self.versionButton.setIcon(geticon(self.gui, 'chevron-down-grey'))
    ...
    def readValues(self):
        self.versionButton.setToolTip('Last execution: %s by %s' % (
            ...
            for param in self.node.type.getParameters(self.node.versionActive.typeForceVersion):
                if param.visibility:
                    ...
                    if param.type == 'bool':
                        ...
                        edit.setStyleSheet('QCheckBox{padding: 0 -9 0 0} QCheckBox::indicator{ width: 24px; height: 24px}')
                        if param.advanced:
                            self.root.advancedLayout.addLayout(layout)
                        else:
                            self.root.paramsLayout.addLayout(layout)
                    ...
                    if param.type == 'file':
                        ...
                        button.setIcon(geticon(self.gui, 'folder'))
                        ...
                        edit.editingFinished.connect(lambda w=edit, p=param.getName(),
                                                     lab=label: self.stringChanged(w, p, lab))
                        button.clicked.connect(lambda dummy=0, p=edit.text(), w=edit: self.fileRequest(p, w))
                    if param.type == 'enum':
                        layout = QtWidgets.QHBoxLayout()
                        layout.setContentsMargins(1, 1, 1, 1)
                        label.setMinimumWidth(100)
    ...
    def versionPopup(self):
        ...
        menu.setMinimumWidth(self.root.geometry().width())
        action = menu.exec_(self.versionButton.mapToGlobal(QtCore.QPoint(0, 0)))
        for ac, v in zip(actions, self.node.versions):
            if ac == action:
                self.auto = True
                self.node.setVersion(v)
                self.root.commentTextEdit.setPlainText(v.comment)
                self.versionButton.setText('v' + str(v.id).zfill(pad))
                self.readValues()
                self.readVars()
                self.checkLock()
                self.node._ungotvar()
    ...
    def readVars(self):
        ...
        addButton = QtWidgets.QPushButton()
        addButton.setStyleSheet('QPushButton::!hover{background: #172525}')
        addButton.setIcon(geticon(self.gui, 'vars'))
        addButton.setMaximumHeight(18)
        addButton.clicked.connect(self.newVar)
        self.root.varsLayout.addWidget(addButton)
...
class MgvVariableWidget(object):
    ...
    def __init__(self, gui, ver, layout, variable):
        ...
        self.widget.delButton.setIcon(geticon(gui, 'x'))
...
class MgvBatch(object):
    ...
    def __init__(self, gui, mode):
        ...
        self.root = QtCompat.loadUi(mainUIPath)
        self.root.scriptAddButton.setIcon(geticon(gui, 'plus'))
        self.root.scriptDelButton.setIcon(geticon(gui, 'trash'))
        self.root.scriptRenameButton.setIcon(geticon(gui, 'edit'))
        self.root.scriptAddButton.setIconSize(QtCore.QSize(24, 24))
        self.root.scriptDelButton.setIconSize(QtCore.QSize(24, 24))
        self.root.scriptRenameButton.setIconSize(QtCore.QSize(24, 24))
        self.root.scriptAddButton.setStyleSheet('QPushButton::!hover{background: transparent}')
        self.root.scriptDelButton.setStyleSheet('QPushButton::!hover{background: transparent}')
        self.root.scriptRenameButton.setStyleSheet('QPushButton::!hover{background: transparent}')
        self.root.setWindowIcon(geticon(self.gui, 'mgvIcon'))
        self.root.setStyleSheet(self.gui.style)
    ...
    def templateChanged(self):
        ...
        for x in result:
            ...
            template_uuid = self.pattern.getTemplate(template_name=new_template).uuid
            mgvWrapper.setNodeAttr(graph_infos, template_uuid=template_uuid)
    ...
    def selectGraphs(self):
        ...
        if len(result):
            for i, x in enumerate(result):
                template_uuid = mgvWrapper.getGraphInfo(self.pattern, x)['template_uuid']
                template = self.pattern.getTemplate(template_uuid=template_uuid)
                if template is not None:
                    template_names.append(self.pattern.getTemplate(template_uuid=template_uuid).name)
                else:
        ...
        for var in variables:
            if len(variables[var]['value']) == len(result) or not self.root.allVarCheckBox.isChecked():
                ...
                d.setIcon(geticon(self.gui, "trash"))
                d.setIconSize(QtCore.QSize(24, 24))
                d.setFlat(True)
                d.setMaximumWidth(24)
                d.setStyleSheet("""QPushButton{background: transparent}QPushButton::hover{background: #663333}""")
                d.clicked.connect(lambda dummy=0, x=var: self.delVar(x))
                h.addWidget(c)
                h.addWidget(e1)
                h.addWidget(e2)
                h.addWidget(d)
...
""".replace('url(:', 'url(' + self.gui.mgvDirectory))
    ...
    def addVar(self):
        name, ok = QtWidgets.QInputDialog.getText(self.root, 'New name', 'Var name :')
        if ok:
            result = [x.text().split(':') for x in self.root.resultListWidget.selectedItems()]
            for x in result:
...
class MgvBatchThread(QtCore.QThread):
    ...
    def run(self):
        self.textedit.append('<font color=green># START #</font>')
...
""" % (self.mgvpath, self.project_name, self.pattern_name, ':'.join(x)) + self.script + "\nExecute(current_graph)"
        ...
        self.textedit.append('<font color=green># FINISHED #</font>')
...
class MgvCreateGraph(object):
    ...
    def __init__(self, gui, manageGui):
        ...
        self.root.setWindowIcon(geticon(self.gui, 'mgvIcon'))
        self.root.fileButton.setIcon(geticon(self.gui, "folder"))
        self.root.fileButton.setIconSize(QtCore.QSize(16, 16))
        self.root.addKeyButton.setIcon(geticon(self.gui, "plus"))
        self.root.addKeyButton.setIconSize(QtCore.QSize(16, 16))
        self.root.originalButton.setIcon(geticon(self.gui, "eye"))
            ...
            for y in range(start, stop+1):
                ...
                if not exists:
                    g = MgvGraph(pattern=self.pattern, name=self.pattern.convertGraphName(path),
                                 path=path, template_uuid=self.pattern.getTemplate(template_name=templateName).uuid,
                                 load=False)
                    g.create()
                    for p in properties:
    ...
    def create(self):
        ...
        if self.root.tabWidget.currentIndex() == 0:
            ...
            if not mgvWrapper.graphExists(self.pattern, graphpath):
                template = self.pattern.getTemplate(template_name=template_name)
                template_uuid = template.uuid if template is not None else ''
                g = MgvGraph(pattern=self.pattern, path=graphpath, name=graphname, template_uuid=template_uuid,
                             load=False)
                g.create()
                self.gui.createOpenMenu()
                self.manageGui.search()
            ...
            if go:
                ...
                for i, name in enumerate(namesok):
                    ...
                    MgvGraph(pattern=self.pattern, path=name.split(':'),
                             template_uuid=self.pattern.getTemplate(template_name=template_name).uuid,
    ...
    def addKey(self):
        ...
        closeButton.setIcon(geticon(self.gui, "trash"))
        ...
        button.clicked.connect(lambda dummy=0, b=button, f=field: self.colorClick(b, f))
        closeButton.clicked.connect(lambda dummy=0, l=layout, f=field: self.delField(l, f))
...
class MgvDownload(object):
    ...
    def __init__(self, gui):
        ...
        self.root.refreshPushButton.setIcon(geticon(self.gui, 'refresh'))
        self.root.refreshPushButton.setIconSize(QtCore.QSize(24, 24))
        self.root.refreshPushButton.setStyleSheet('QPushButton::!hover{background: transparent}')
...
class MgvTransfert(object):
    ...
    def __init__(self, gui):
        ...
        self.exportList = {}
        self.root.setWindowIcon(geticon(self.gui, 'mgvIcon'))
        self.root.addButton.setIcon(geticon(gui, 'arrow-right-circle'))
        self.root.removeButton.setIcon(geticon(gui, 'trash'))
        self.root.removeButton.setIconSize(QtCore.QSize(24, 24))
        self.root.fileAButton.setIcon(geticon(self.gui, 'folder'))
        self.root.fileBButton.setIcon(geticon(self.gui, 'folder'))
        self.root.addButton.setIconSize(QtCore.QSize(32, 32))
        self.root.fileAButton.setIconSize(QtCore.QSize(24, 24))
        self.root.fileBButton.setIconSize(QtCore.QSize(24, 24))
        self.root.setStyleSheet(self.gui.style)
...
class MgvProjectSettings(object):
    ...
    def __init__(self, gui, project):
        ...
        self.root.addPatternButton.setIcon(geticon(self.gui, 'plus'))
        self.root.addPatternButton.setIconSize(QtCore.QSize(24, 24))
        self.root.projectDelButton.setIcon(geticon(self.gui, 'trash'))
        self.root.projectDelButton.setIconSize(QtCore.QSize(24, 24))
        ...
        self.root.patternsTabWidget.setStyleSheet('QTabWidget::pane{border: 2px solid %s; }' % self.gui.colorDic['edit'])
    ...
    def readProject(self):
        ...
        for pattern in sorted(self.project.patterns, key=lambda x: x.order):
            ...
            widget.patternButton.clicked.connect(lambda dummy=0, p=pattern, w=widget: self.patternSelect(p, w))
            widget.templateNameLineEdit.textChanged.connect(
                lambda v, p=pattern, w=widget: self.templateNameChanged(p, w))
            widget.templateIconButton.clicked.connect(lambda dummy=0, w=widget: self.templateIconSelect(w))
            ...
            widget.templateAddButton.clicked.connect(lambda dummy=0, p=pattern, w=widget: self.templateAdd(p, w))
            widget.templateDelButton.clicked.connect(lambda dummy=0, p=pattern, w=widget: self.templateDel(p, w))
...
class MgvLocalSettings(object):
    ...
    def __init__(self, gui):
        ...
        self.root.pathButton.setIcon(geticon(self.gui, 'folder'))
        self.root.pathButton.setIconSize(QtCore.QSize(24, 24))
        self.wrapperModule = ''
    ...
    def moduleChanged(self, s):
        ...
        if s == 'mgvWrapperNoServer':
            ...
            self.root.userWidget.hide()
            self.root.pwdWidget.hide()
            self.root.hostWidget.hide()
            ...
            self.root.hostWidget.show()
            self.root.userWidget.show()
            self.root.pwdWidget.show()
