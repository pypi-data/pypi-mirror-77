...
Functions:
    ...
    getNodeVars: Return a node vars dictionary.
...
def getNodeVars(project_name, graph_path, node_name):
    """ Return a node execution vars dict.

    Parameters:
        project_name (str): the project name.
        graph_path (str): the graph path i.e. "Shots:0001:0001"
        node_name (str): the node name..
    """
    proj = MgvProject.Project(name=project_name)
    if proj is None: return {}

    pattern = proj.getPattern(graph_path.split(':')[0])
    if pattern is None: return {}

    path = graph_path.split(':')[1:]
    graph = MgvGraph(pattern=pattern, path=path, name=pattern.convertGraphName(path))
    if graph is None: return {}

    node = graph.getNode(node_name)
...
class MgvGraph(object):
    ...
    def __init__(self, pattern=None, path=None, uuid=None, template_uuid='', name='', template_name='', load=True):
        self.uuid = uuid
        self.pattern = pattern
        self.template_uuid = template_uuid
        if len(template_name):
            for tem in pattern.templates:
                if tem.name == template_name:
                    self.template_uuid = tem.uuid
    ...
    def _fillWithGraphTemplate(self, template=None):
        print('_fillWithGraphTemplate is deprecated, use fillWithGraphTemplate instead.')
        self.fillWithGraphTemplate(template)

    def fillWithGraphTemplate(self, template=None):
    ...
    def createNode(self, type='', name=''):
        ...
        Parameters:
            ...
            type (MgvType or str): the type of the node.
        Return:
            MgvNode: New node object.
        """
        node = MgvNode(graph=self, type=type, name=name)
        node.user = 'free'
        node.create()
        return node
    ...
    def refresh(self, graphview=None):
        ...
        for node in list(self.nodes):
            if node.getUser() != self.user:
                ...
                if up is None or up > node.updated:
    ...
    def getFromJson(cls, element, pattern):
        ...
        newGraph = cls(uuid=element['uuid'], pattern=pattern, path=[str(x) for x in element['path']],
                       template_uuid=element['template_uuid'],
...
class MgvNode(object):
    ...
    def setLinkTo(self, node):
        ...
        if link.linkto.type.uuid in link.linkfrom.type.getLinkWith():
            if link.linkfrom.versionActive.id != link.linkto.versionActive.id:
                link.linkto.setVersion(link.linkfrom.versionActive.id)
    ...
    def getLinkedGroup(self, checknodes=None):
        ...
        for i in self.getInputNodes():
            if i.type and self.type and self.type.uuid in i.type.linkWith:
