...
class MultiEditor(QtWidgets.QWidget):
    ...
    def setPlainText(self, txt):
        self.text.setPlainText(u'%s' % txt)
...
class CompletionTextEdit(QtWidgets.QPlainTextEdit):
    ...
    def findPrev(self, str):
        self.find(str, QtGui.QTextDocument.FindBackward)
    ...
    def keyPressEvent(self, event):
        ...
        if len(self.textCursor().selectedText()):
            ...
            if event.key() == QtCore.Qt.Key_Backspace:
                curs = self.textCursor()
                spos = curs.positionInBlock()
                count = 0
                while curs.block().text()[spos-count-1:spos-count] == ' ':
                    count += 1
                if spos-count == 0:
                    count = 4 if count % 4 == 0 else count % 4
