...
class JWT(AuthenticationClient):
    ...
    REQUIRED_FIELDS = 'client_secret org_id api_key technical_account_id technical_account_email'.split(' ')
    ...
    def check_config(config: dict) -> bool:
        if not all([k.lower() in config for k in JWT.REQUIRED_FIELDS]):
            missing = ", ".join(filter(lambda f: f.lower() not in config, JWT.REQUIRED_FIELDS))
            raise ConfigInsufficientInformationError(
                'The following required fields are missing in the config file: "{missing}". Please make sure that '
                'you use the JSON export file from Adobe IO in the format it provides.'.format(missing=missing)
            )
        return True
    ...
    def create_config(data: typing.Union[str, dict, typing.TextIO]) -> dict:
        ...
        dict_data = {k: v for k, v in dict_data.items() if k in JWT.REQUIRED_FIELDS}
        return dict_data
    ...
    def refresh_token(self):
        ...
        # set the expiration date in ms. this will be used for checks, if the token needs to be refreshed
    ...
    def authenticate(self, session: Session) -> Session:
        ...
        if not self.access_token or not self.expires_in or self.expires_in >= (now_in_ms() + 5000):
            self.refresh_token()
