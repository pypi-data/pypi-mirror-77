...
class MyWhatify:
        ...
        with allure.step('login with user: ' + str(email)):
            ...
            if imp_email:
                with allure.step('getting user ID from email'):
                    user_id = cls.get_user_id(imp_email)
    ...
    def get_user_id(cls, email: str = None):
        ...
        with allure.step('get user ID for current user or for user: ' + str(email)):
            ...
            if email:
                user = my_toolkit.UserManagement.get_user_list(username=email)
                if user:
    ...
    def get_account_id(cls, email: str = None):
        ...
        """
        account_id = -1
        with allure.step('get account ID for current user or for user: ' + str(email)):
            logging.info('get account ID for current user or for user: ' + str(email))
            if email:
                account = my_toolkit.UserManagement.get_account_list(name=email)
                if account:
    ...
    #############################
    @classmethod
    def get_api_post(cls, url, user_token=None):
        return APIRequestor().post(url=url, api_key=user_token)
