...
from toolkit_w.internal.vars_global import Estimator, Pipeline, InterpretabilityLevel, ValidationStrategy, SplittingStrategy, \
    TargetMetric, CVStrategy, ProblemType
from toolkit_w.internal.errors import APIError, InvalidRequestError
...
class Task(APIResource):
    ...
    def list(cls, search_term: str = None, page: int = None, page_size: int = None, sort: Dict = None,
        ...
        Returns:
            WhatifyResponse: Tasks are represented as nested dictionaries under `hits`.
        """
        return cls._list(search_term, page, page_size, sort, filter_, api_key)

    @classmethod
    def get_task_id(cls, task_name: str, api_key: str = None) -> WhatifyResponse:
    ...
    def get(cls, id: int, api_key: str = None) -> WhatifyResponse:
        ...
        Returns:
            WhatifyResponse: Information about the Task.
        """
        return cls._get(id, api_key)

    @classmethod
    def get_by_name(cls, name: str, api_key: str = None) -> WhatifyResponse:
        """
        ...
        if resp and 'total' in resp and resp['total'] > 0:
            ds = resp['hits'][0]
            return WhatifyResponse(data=ds)
        else:
    ...
    def create(cls, name: str, dataset_id: int, estimators: List[Estimator] = None, target_metric: TargetMetric = None,
               ...
               skip_if_exists: bool = False, api_key: str = None) -> WhatifyResponse:
        """
        ...
        Returns:
            WhatifyResponse: Task ID, if successful and wait=False or Task if successful and wait=True;
            raises FireflyError otherwise.
        """
        if horizon is not None:
            toolkit_w.logger.warning("Parameter `horizon` is DEPRECATED. Please use `forecast_horizon` and `model_life_time`.")

        existing_ds = cls.list(filter_={'name': [name]}, api_key=api_key)
        if existing_ds and existing_ds['total'] > 0:
            if skip_if_exists:
                return WhatifyResponse(data=existing_ds['hits'][0])
            else:
                raise InvalidRequestError("Task with that name already exists")

        try:
            dataset = toolkit_w.Dataset.get(id=dataset_id, api_key=api_key)
    ...
    @classmethod
    def refit(cls, id: int, datasource_id: int, wait: bool = False, api_key: str = None) -> WhatifyResponse:
        """
        ...
        Returns:
            WhatifyResponse: Ensemble ID, if successful and wait=False or Ensemble if successful and wait=True;
            raises FireflyError otherwise.
        """
        data = {
        ...
        if wait:
            utils.wait_for_finite_state(toolkit_w.Ensemble.get, new_ens_id, api_key=api_key)
            response = toolkit_w.Ensemble.get(new_ens_id, api_key=api_key)
        else:
            response = WhatifyResponse(data={'id': new_ens_id}, headers=response.headers,
    ...
    def edit_notes(cls, id: int, notes: str, api_key: str = None) -> WhatifyResponse:
        ...
        Returns:
            WhatifyResponse: `task_id` value if successful, raises FireflyError otherwise.
        """
        requestor = APIRequestor()
    ...
    @classmethod
    def get_task_progress(cls, id: int, api_key: str = None) -> WhatifyResponse:
        """
        ...
        Returns:
            WhatifyResponse:  List of all the Task's Ensembles' scores.
        """
        requestor = APIRequestor()
    ...
    @classmethod
    def get_task_result(cls, id: int, api_key: str = None) -> WhatifyResponse:
        """
        ...
        Returns:
            WhatifyResponse: Task's full results.
        """
        requestor = APIRequestor()
    ...
    def add_additional_time_to_completed_task(cls, id: int, new_time_budget: int,
                                              api_key: str = None) -> WhatifyResponse:
        """
    ...
    @classmethod
    def rerun_task(cls, task_id: int, api_key: str = None) -> WhatifyResponse:
        """
    ...
    @classmethod
    def pause_task(cls, task_id: int, api_key: str = None) -> WhatifyResponse:
        """
    ...
    @classmethod
    def cancel_task(cls, task_id: int, api_key: str = None) -> WhatifyResponse:
        """
    ...
    @classmethod
    def resume_task(cls, task_id: int, api_key: str = None) -> WhatifyResponse:
        """
    ...
    def _get_config_defaults(cls, dataset_id, problem_type, inter_level, api_key=None):
        ...
        estimators = toolkit_w.Dataset.get_available_estimators(id=dataset_id, inter_level=inter_level)
        pipeline = toolkit_w.Dataset.get_available_pipeline(id=dataset_id, inter_level=inter_level)
