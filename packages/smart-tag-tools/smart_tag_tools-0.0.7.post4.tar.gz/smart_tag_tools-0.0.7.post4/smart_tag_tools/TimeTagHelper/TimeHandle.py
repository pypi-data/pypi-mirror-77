...
def handle(time_list, base_time=arrow.now()):
    """
    时间处理
    ...
    for thred in thred_list:
        result = thred.get_result().get("timestamp", "")
        result_list.append(date_format(result))
    return result_list

# time_list = '25秒,32号,1号,1分27秒,23秒早,27号,7号,13号,17号,18号,三秒两秒,20号,2016,4号,9号'.split(',')
# print(handle(time_list))
