...
class Image(FireStoreBase):
    ...
    ext_fields = ['image_number','image_url','name','description','blobkey','storage_bucket']
    ...
    def getAllImages(cls):
        ...
        files = col.list_documents()
        for imgRef in files:
            img = Image.getInstance(imgRef)
            files[img.image_number] = {'filename': img.name,'web_path':img.image_url}
        return files
