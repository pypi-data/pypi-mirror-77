...
@LastEditTime: 2020-08-17 16:22:42
...
from models.db_models.throw.throw_goods_model import *
...
from libs.customize.seven import *
...
class InitThrowGoodsCallBackHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        act_info_model.update_table("is_throw=1", "id=%s", params=act_id)

        if close_goods_id != "":
            throw_goods_model.update_table("is_throw=0", "act_id=%s and goods_id in (" + close_goods_id + ")", params=act_id)
...
class SaveThrowGoodsStatusHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if update_goods_id > 0:
            throw_goods_model.update_table("is_throw=abs(is_throw-1),is_sync=0,throw_date=%s", "act_id=%s and goods_id=%s", [self.get_now_datetime(), act_id, update_goods_id])

        return self.reponse_json_success()


class ThrowGoodsListHandler(SevenBaseHandler):
    """
    @description: 投放商品列表
    @param {type} 
    @return {type} 
...
class AsyncThrowGoodsHandler(SevenBaseHandler):
    """
    ...
    def test_async(self):
        result={'miniapp_distribution_items_bind_response': {'model_list': {'distribution_order_bind_target_entity_open_result_dto': [{'bind_result_list': {'distribution_order_bind_base_dto': [{'fail_msg': '商品不存在', 'success': False, 'target_entity_id': '55'}, {'fail_msg': '商品不存在', 'success': False, 'target_entity_id': '2'}, {'fail_msg': '2020 : 实体绑定关系不存在', 'success': False, 'target_entity_id': '613878861009'}, {'fail_msg': '2020 : 实体绑定关系不存在', 'success': False, 'target_entity_id': '620482163358'}, {'fail_msg': '商品不属于当前商家', 'success': False, 'target_entity_id': '620003498461'}, {'success': True, 'target_entity_id': '620480991439'}, {'success': True, 'target_entity_id': '616454842353'}, {'fail_msg': '商品不属于当前商家', 'success': False, 'target_entity_id': '573998736785'}, {'fail_msg': '2020 : 实体绑定关系不存在', 'success': False, 'target_entity_id': '619094193197'}, {'fail_msg': '商品不存在', 'success': False, 'target_entity_id': '10'}]}, 'scene_name': '详情悬浮', 'url': 'https://m.duanqu.com/?_ariver_appid=3000000006168005&query=actid%3D133'}]}, 'request_id': '4kc53bm4dfjg'}}
        return result