...
@LastEditTime: 2020-08-13 14:24:19
@LastEditors: HuangJingCan
@Description: 用户处理
"""
from handlers.seven_base import *
from handlers.top_base import *
...
from models.db_models.machine.machine_value_model import *
...
from models.db_models.coin.coin_order_model import *
...
class LoginHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        user_info = user_info_model.get_entity("open_id=%s and act_id=%s and app_id=%s", params=[open_id, act_id, app_id])
        ...
        if not user_info:
            is_add = True
        if is_add:
            user_info = self.get_default_user(act_info)
    ...
    def get_default_user(self, act_info):
        """
        @description: 默认用户信息
        @param {type} 
        @return: 
        ...
        user_info.act_id = act_info.id
        user_info.user_nick = ""
        user_info.avatar = ""
        user_info.is_auth = 0
        user_info.app_id = self.get_taobao_param().source_app_id
        user_info.is_new = 1
...
class UserPrizeListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        prize_roster_field = "id,app_id,act_id,open_id,machine_id,machine_name,machine_price,prize_id,prize_name,prize_price,prize_pic,prize_detail,tag_id,award_id,award_name,user_nick,prize_code,order_status,is_sku,prize_order_id,prize_order_no,sku_id,properties_name,goods_id,goods_code,goods_code_list,order_no,frequency_source,create_date"
        page_list, total = prize_roster_model.get_dict_page_list(prize_roster_field, page_index, page_size, condition, "", "create_date desc", [open_id, act_id])
...
class RosterNoticeHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        prize_roster_model = PrizeRosterModel()
        prize_roster_field = "id,app_id,act_id,open_id,machine_id,machine_name,machine_price,prize_id,prize_name,prize_price,prize_pic,prize_detail,tag_id,award_id,award_name,user_nick,prize_code,order_status,is_sku,prize_order_id,prize_order_no,sku_id,properties_name,goods_id,goods_code,goods_code_list,order_no,frequency_source,create_date"
        prize_roster_list_dict = prize_roster_model.get_dict_list("act_id=%s", order_by="create_date desc", field=prize_roster_field, limit="20", params=act_id)
...
class PrizeOrderHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if prize_order_list_dict:
            ...
            prize_roster_field = "id,app_id,act_id,open_id,machine_id,machine_name,machine_price,prize_id,prize_name,prize_price,prize_pic,prize_detail,tag_id,award_id,award_name,user_nick,prize_code,order_status,is_sku,prize_order_id,prize_order_no,sku_id,properties_name,goods_id,goods_code,goods_code_list,order_no,frequency_source,create_date"
            prize_roster_list_dict = prize_roster_model.get_dict_list("prize_order_id in (" + prize_order_ids + ")", field=prize_roster_field)
