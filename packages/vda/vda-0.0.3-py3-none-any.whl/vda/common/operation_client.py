...
class OperationClient:
    ...
    def create_backend_nvmf(
        ...
        if nvmf_conf is not None \
           ...
           and nvmf_conf["hosts"][0]["name"] != frontend_nqn_name:
            params = {
    ...
    def create_exp_primary_nvmf(
            ...
            initiator_nqn, standby_nqn_list, cn_listener_conf):
        ...
        if nvmf_conf is None:
            ...
            for standby_nqn in standby_nqn_list:
                self._create_nvmf_host(exp_nqn_name, standby_nqn)
        else:
            if len(nvmf_conf["namespaces"] == 0):
                self._create_nvmf_ns(exp_nqn_name, snap_full_name)
            if len(nvmf_conf["listen_addresses"]) == 0:
            ...
            for standby_nqn in standby_nqn_list:
                host_nqn_set_2.add(standby_nqn)
    ...
    def create_exp_standby_nvmf(
            self, exp_nqn_name, standby_bdev_name,
            initiator_nqn, cn_listener_conf):
        nvmf_conf = self.nvmf_dict.get(exp_nqn_name)
        if nvmf_conf is not None \
           and len(nvmf_conf["namespaces"]) > 0 \
           and nvmf_conf["namespaces"][0]["name"] != standby_bdev_name:
            params = {
                "nqn": exp_nqn_name,
            }
            self.spdk_client.call_and_check("nvmf_delete_subsystem", params)
        ...
        if nvmf_conf is None:
            ...
            self._create_nvmf_subsystem(exp_nqn_name)
            self._create_nvmf_ns(exp_nqn_name, standby_bdev_name)
            self._create_nvmf_listener(
                nqn=exp_nqn_name,
                trtype=cn_listener_conf["trtype"],
                adrfam=cn_listener_conf["adrfam"],
    ...
    def create_standby_nvme(
            self, standby_nvme_name, exp_nqn_name,
            standby_nqn_name, primary_listener_conf_str):
        primary_listener_conf = json.loads(primary_listener_conf_str)

        params = {
            "name": standby_nvme_name,
        }
        ret = self.spdk_client.call("bdev_nvme_get_controllers", params)
        if "result" in ret:
            return
        ...
        params = {
            ...
            "subnqn": exp_nqn_name,
            "hostnqn": standby_nqn_name,
        }
        self.spdk_client.call_and_check("bdev_nvme_attach_controller", params)

    def delete_standby_nvme(self, standby_nvme_name):
        params = {
            "name": standby_nvme_name,
        }
        self.spdk_client.call("bdev_nvme_detach_controller", params)

    def get_standby_nvme_list(self, standby_nvme_prefix):
        ret = self.spdk_client.call_and_check("bdev_nvme_get_controllers")
        standby_nvme_list = []
        for nvme_conf in ret["result"]:
            name = nvme_conf["name"]
            if name.startswith(standby_nvme_prefix):
                standby_nvme_list.append(name)
        return standby_nvme_list
