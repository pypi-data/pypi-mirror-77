...
from tensorflow.keras.layers import AveragePooling2D, Conv2D, Activation, BatchNormalization
...
class PspNetConfig(object):
    ...
    def __init__(self, height, width, down_sample_factor = 8):
        self.height = height
        self.width = width
        self.down_sample_factor = down_sample_factor
        min_size = down_sample_factor*6
        is_wrong_shape = (height % min_size!=0 or width%min_size!=0 or height < min_size or width < min_size)
        assert down_sample_factor in [4,8,16], 'Supported down_sample_factor -> 4, 8, 16'
        assert is_wrong_shape == False, 'height and width should be multiple of down_sample_factor*6'
        self.input_shape = (self.height, self.width, 3)

def log_PspNet_config_params(config):

    assert isinstance(config, PspNetConfig), 'config not supported {}'.format(config)
...
#     x = resize_image(x, strides)
    x = Upsample(K.int_shape(x)[1]*strides[0], K.int_shape(x)[2]*strides[1])(x)
    return x
class PspNetPredictor(object):
    ...
    def load_image(self, image_path):
            img_array= np.array(tf.keras.preprocessing.image.load_img(path=image_path))
    ...
    def predict(self, image):
        ...
        image = cv2.resize(im, (self.width, self.height))  #.astype(keras.backend.floatx())
