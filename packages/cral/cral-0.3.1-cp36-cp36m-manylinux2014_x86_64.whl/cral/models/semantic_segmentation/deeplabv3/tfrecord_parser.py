...
import numpy as np
from .utils import Deeplabv3Config
...
def decode_pad_msk(mask_string, pad_height, pad_width):
  ...
  mask = tf.numpy_function(_pad, [mask, pad_height, pad_width, 1], Tout=tf.uint8)
  #mask.set_shape([None, None, 3])
  return mask


class DeepLabv3Generator(object):
  ...
  def _parse_function(self, serialized):
      ...
      image_batch = tf.numpy_function(self.normalize_func, [image_batch], Tout=tf.keras.backend.floatx())
      ...
      image_batch = tf.image.resize(image_batch,(self.config.height, self.config.width))
      image_batch = tf.cast(image_batch, tf.keras.backend.floatx())

      mask_batch = tf.image.resize(mask_batch,(self.config.height, self.config.width), method='nearest')
