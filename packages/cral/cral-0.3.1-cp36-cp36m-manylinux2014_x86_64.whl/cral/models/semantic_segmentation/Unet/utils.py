...
class UNetConfig(object):
    ...
    def __init__(self, height, width):
        ...
        assert height%32 == 0 and width%32 == 0, 'Height and width both should be a multiple of 32'
        self.height = height
        self.width = width
        self.input_shape = (self.height, self.width, 3)

def log_UNet_config_params(config):

    assert isinstance(config, UNetConfig), 'config not supported {}'.format(config)
