...
class RetinanetGenerator(DetectionBase):
    ...
    def process_bboxes_labels(self, image_array, bboxes, labels):
        ...
        # augment here
        if self.aug != None:
          image_array,bboxes,labels = self.aug.apply(image_array,bboxes,labels)
    ...
    def yield_image_regression_classification(self, xmin_batch, ymin_batch, xmax_batch, ymax_batch, label_batch, image_batch):
        ...
        for index in range(self.batch_size):
            ...
            augmented_image, gt_regression, gt_classification = tf.numpy_function(self.process_bboxes_labels, [image_array, bboxes, labels], Tout=[tf.uint8, tf.keras.backend.floatx(), tf.keras.backend.floatx()])

            regression_batch.append(gt_regression)
            classification_batch.append(gt_classification)
            image_batch_aug.append(augmented_image)

        return tf.convert_to_tensor(image_batch_aug), tf.convert_to_tensor(regression_batch), tf.convert_to_tensor(classification_batch)
