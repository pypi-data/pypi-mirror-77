...
def load_model(model_uri, ctx):
    ...
    >>> model = cral.tracking.gluon.load_model("runs:/" + gluon_random_data_run.info.run_id + "/model")
...
def save_model(gluon_model, path, mlflow_model=Model(), conda_env=None):
    ...
    :param conda_env: Either a dictionary representation of a Conda environment or
                      ...
                      :func:`cral.tracking.gluon.get_default_conda_env()` environment is added to
    ...
    >>> cral.tracking.gluon.save_model(net, gluon_model_path)
    """
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.gluon", env=conda_env_subpath)
...
def log_model(gluon_model, artifact_path, conda_env=None):
    ...
    >>>   est.fit(train_data=train_data, epochs=100, val_data=validation_data)
    >>>   cral.tracking.gluon.log_model(net, "model")
    """
    Model.log(artifact_path=artifact_path, flavor=cral.tracking.gluon, gluon_model=gluon_model,
              conda_env=conda_env)
    ...
    class __MLflowGluonCallback(EpochEnd, TrainEnd, TrainBegin):
        ...
        def epoch_end(self, estimator, *args, **kwargs):
            ...
            for metric in estimator.val_metrics:
                metric_name, metric_val = metric.get()
                logs[metric_name] = metric_val
            try_mlflow_log(cral.tracking.log_metrics, logs, step=self.current_epoch)
            self.current_epoch += 1

        def train_begin(self, estimator, *args, **kwargs):
            try_mlflow_log(cral.tracking.log_param, "num_layers", len(estimator.net))
            if estimator.max_epoch is not None:
                try_mlflow_log(cral.tracking.log_param, "epochs", estimator.max_epoch)
            if estimator.max_batch is not None:
                try_mlflow_log(cral.tracking.log_param, "batches", estimator.max_batch)
            try_mlflow_log(cral.tracking.log_param, "optimizer_name",
                           type(estimator.trainer.optimizer).__name__)
            if hasattr(estimator.trainer.optimizer, "lr"):
                try_mlflow_log(cral.tracking.log_param, "learning_rate",
                               estimator.trainer.optimizer.lr)
            if hasattr(estimator.trainer.optimizer, "epsilon"):
                try_mlflow_log(cral.tracking.log_param, "epsilon",
