"""
The ``cral.tracking.tensorflow`` module provides an API for logging and loading TensorFlow models.
...
import cral.tracking
import tensorflow
import cral.tracking.keras
...
from cral.tracking.models import Model
from cral.tracking.protos.databricks_pb2 import DIRECTORY_NOT_EMPTY
from cral.tracking.tracking.artifact_utils import _download_artifact_from_uri
from cral.tracking.utils import keyword_only, experimental
from cral.tracking.utils.environment import _mlflow_conda_env
from cral.tracking.utils.file_utils import _copy_file_or_tree
from cral.tracking.utils.model_utils import _get_flavor_configuration
from cral.tracking.utils.autologging_utils import try_mlflow_log, log_fn_args_as_params
from cral.tracking.entities import Metric
...
def log_model(tf_saved_model_dir, tf_meta_graph_tags, tf_signature_def_key, artifact_path,
    ...
    """
    return Model.log(artifact_path=artifact_path, flavor=cral.tracking.tensorflow,
...
def save_model(tf_saved_model_dir, tf_meta_graph_tags, tf_signature_def_key, path,
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.tensorflow", env=conda_env_subpath)
...
def load_model(model_uri, tf_sess=None):
    ...
    *With TensorFlow version <2.0.0, this method must be called within a TensorFlow graph context.*
    ...
    :param model_uri: The location, in URI format, of the MLflow model. For example:
                      ...
                      `Referencing Artifacts <https://www.cral.tracking.org/docs/latest/concepts.html#
    ...
    >>> import cral.tracking.tensorflow
    >>> import tensorflow as tf
    >>> tf_graph = tf.Graph()
    >>> tf_sess = tf.Session(graph=tf_graph)
    >>> with tf_graph.as_default():
    >>>     signature_definition = cral.tracking.tensorflow.load_model(model_uri="model_uri",
    >>>                            tf_sess=tf_sess)
...
class __MLflowTfKerasCallback(Callback):
    ...
    def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
        ...
        if hasattr(opt, 'optimizer'):
            opt = opt.optimizer
            try_mlflow_log(cral.tracking.log_param, 'optimizer_name', type(opt).__name__)
        if hasattr(opt, '_lr'):
            lr = opt._lr if type(opt._lr) is float else tensorflow.keras.backend.eval(opt._lr)
            try_mlflow_log(cral.tracking.log_param, 'learning_rate', lr)
        if hasattr(opt, '_epsilon'):
            epsilon = opt._epsilon if type(opt._epsilon) is float \
                else tensorflow.keras.backend.eval(opt._epsilon)
            try_mlflow_log(cral.tracking.log_param, 'epsilon', epsilon)

        sum_list = []
        self.model.summary(print_fn=sum_list.append)
        summary = '\n'.join(sum_list)
        try_mlflow_log(cral.tracking.set_tag, 'model_summary', summary)
        ...
        try:
            ...
            with open(summary_file, 'w') as f:
                f.write(summary)
            try_mlflow_log(cral.tracking.log_artifact, local_path=summary_file)
        finally:
    ...
    def on_train_end(self, logs=None):  # pylint: disable=unused-argument
        try_mlflow_log(cral.tracking.keras.log_model, self.model, artifact_path='model')
...
class __MLflowTfKeras2Callback(Callback):
    ...
    def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
        ...
        for attribute in config:
            try_mlflow_log(cral.tracking.log_param, "opt_" + attribute, config[attribute])
    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch-1) % _LOG_EVERY_N_STEPS == 0:
            try_mlflow_log(cral.tracking.log_metrics, logs, step=epoch)
...
def _log_artifacts_with_warning(**kwargs):
    try_mlflow_log(cral.tracking.log_artifacts, **kwargs)
...
def _log_event(event):
    ...
    """
    if not cral.tracking.active_run():
        try_mlflow_log(cral.tracking.start_run)
        global _AUTOLOG_RUN_ID
        _AUTOLOG_RUN_ID = cral.tracking.active_run().info.run_id
            ...
            if v.HasField('simple_value'):
                if (event.step-1) % _LOG_EVERY_N_STEPS == 0:
                    _thread_pool.submit(_add_to_queue, key=v.tag,
                                        ...
                                        run_id=cral.tracking.active_run().info.run_id)
...
def autolog(every_n_iter=100):
    ...
    def _manage_active_run():
        if not cral.tracking.active_run():
            try_mlflow_log(cral.tracking.start_run)
            global _AUTOLOG_RUN_ID
            if cral.tracking.active_run() is not None:  # defensive check in case `cral.tracking.start_run` fails
                _AUTOLOG_RUN_ID = cral.tracking.active_run().info.run_id
        yield cral.tracking.active_run()
        if cral.tracking.active_run() is not None and cral.tracking.active_run().info.run_id == _AUTOLOG_RUN_ID:
            try_mlflow_log(cral.tracking.end_run)
    ...
    def train(self, *args, **kwargs):
        with _manage_active_run():
            ...
            if len(args) >= 3:
                try_mlflow_log(cral.tracking.log_param, 'steps', args[2])
                if len(args) >= 4:
                    try_mlflow_log(cral.tracking.log_param, 'max_steps', args[3])
            if 'steps' in kwargs:
                try_mlflow_log(cral.tracking.log_param, 'steps', kwargs['steps'])
            if 'max_steps' in kwargs:
                try_mlflow_log(cral.tracking.log_param, 'max_steps', kwargs['max_steps'])
    ...
    def export_saved_model(self, *args, **kwargs):
        ...
        if not cral.tracking.active_run():
            global _AUTOLOG_RUN_ID
            if _AUTOLOG_RUN_ID:
                try_mlflow_log(cral.tracking.start_run, _AUTOLOG_RUN_ID)
            else:
                try_mlflow_log(cral.tracking.start_run)
                auto_end = True
    ...
    def _log_early_stop_callback_params(callback):
        if callback:
            try:
                ...
                try_mlflow_log(cral.tracking.log_params, earlystopping_params)
    ...
    def _log_early_stop_callback_metrics(callback, history):
        if callback:
            ...
            try_mlflow_log(cral.tracking.log_metric, 'stopped_epoch', stopped_epoch)
            ...
            if stopped_epoch != 0 and restore_best_weights:
                ...
                try_mlflow_log(cral.tracking.log_metric, 'restored_epoch', restored_epoch)
                ...
                if metric_key is not None:
                    ...
                    try_mlflow_log(cral.tracking.log_metrics, restored_metrics, step=last_epoch)
