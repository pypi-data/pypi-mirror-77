...
from cral.tracking.tracking.context.abstract_context import RunContextProvider

_JAVA_PACKAGE = "org.cral.tracking.spark.autologging"
    ...
    if _get_current_listener() is None:
        ...
        try:
            ...
            raise MlflowException("Exception while attempting to initialize JVM-side state for "
                                  ...
                                  "in http://cral.tracking.org/docs/latest/tracking.html#"
                                  "automatic-logging-from-spark-experimental. Exception:\n%s"
                                  % e)

        # Register context provider for Spark autologging
        from cral.tracking.tracking.context.registry import _run_context_provider_registry
...
class PythonSubscriber(object):
    ...
    class implements a Java interface (org.cral.tracking.spark.autologging.MlflowAutologEventSubscriber,
    ...
    def _notify(self, path, version, data_format):
        ...
        active_run = cral.tracking.active_run()
        if active_run:
