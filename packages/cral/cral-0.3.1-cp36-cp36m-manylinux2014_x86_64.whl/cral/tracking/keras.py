"""
The ``cral.tracking.keras`` module provides an API for logging and loading Keras models. This module
...
from cral.tracking.utils.annotations import experimental
...
def save_model(keras_model, path, conda_env=None, mlflow_model=Model(), custom_objects=None,
    ...
    :param custom_objects: A Keras ``custom_objects`` dictionary mapping names (strings) to
                           ...
                           when the model is loaded with :py:func:`cral.tracking.keras.load_model` and
                           :py:func:`cral.tracking.pyfunc.load_model`.
    ...
    >>> cral.tracking.keras.save_model(keras_model, keras_model_path)
    """
    if keras_module is None:
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.keras",
...
def log_model(keras_model, artifact_path, conda_env=None, custom_objects=None, keras_module=None,
    ...
    :param conda_env: Either a dictionary representation of a Conda environment or
                      ...
                      :func:`cral.tracking.keras.get_default_conda_env()` environment is added to
    ...
    >>> with cral.tracking.start_run() as run:
    >>>   cral.tracking.keras.log_model(keras_model, "models")
    """
    Model.log(artifact_path=artifact_path, flavor=cral.tracking.keras,
...
def _save_custom_objects(path, custom_objects):
    ...
    :param custom_objects: Keras ``custom_objects`` is a dictionary mapping
                           ...
                           loaded with :py:func:`cral.tracking.keras.load_model` and
                           :py:func:`cral.tracking.pyfunc.load_model`.
    """
    import cloudpickle
...
def load_model(model_uri, **kwargs):
    ...
    >>> keras_model = cral.tracking.keras.load_model("runs:/96771d893a5e46159d9f3b49bf9013e2" + "/models")
...
def autolog():
    ...
    class __MLflowKerasCallback(keras.callbacks.Callback):
        ...
        def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
            try_mlflow_log(cral.tracking.log_param, 'num_layers', len(self.model.layers))
            try_mlflow_log(cral.tracking.log_param, 'optimizer_name', type(self.model.optimizer).__name__)
