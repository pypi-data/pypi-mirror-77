"""
The ``cral.tracking.pytorch`` module provides an API for logging and loading PyTorch models. This module
...
import cral.tracking.pyfunc.utils as pyfunc_utils
...
from cral.tracking.protos.databricks_pb2 import RESOURCE_DOES_NOT_EXIST
from cral.tracking.pytorch import pickle_module as mlflow_pytorch_pickle_module
...
def get_default_conda_env():
    ...
    return _mlflow_conda_env(
        ...
        additional_pip_deps=[
            ...
            # and `log_model()`: `cral.tracking.pytorch.pickle_module`.
...
def log_model(pytorch_model, artifact_path, conda_env=None, code_paths=None,
    ...
    >>>   cral.tracking.log_param("epochs", 500)
    >>>   cral.tracking.pytorch.log_model(model, "models")
    """
    pickle_module = pickle_module or mlflow_pytorch_pickle_module
    Model.log(artifact_path=artifact_path, flavor=cral.tracking.pytorch, pytorch_model=pytorch_model,
...
def save_model(pytorch_model, path, conda_env=None, mlflow_model=Model(), code_paths=None,
    ...
    >>> with cral.tracking.start_run() as run:
    >>>   cral.tracking.log_param("epochs", 500)
    >>>   cral.tracking.pytorch.save_model(pytorch_model, pytorch_model_path)
    """
    import torch
    pickle_module = pickle_module or mlflow_pytorch_pickle_module
    ...
    # supplying the MLmodel configuration to `cral.tracking.pytorch._load_pyfunc`
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.pytorch", data=model_data_subpath,
...
def load_model(model_uri, **kwargs):
    ...
    >>> pytorch_model = cral.tracking.pytorch.load_model("runs:/" + run_id + "/" + model_path_dir)
