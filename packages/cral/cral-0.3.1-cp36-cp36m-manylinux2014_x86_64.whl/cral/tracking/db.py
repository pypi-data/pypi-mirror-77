...
def upgrade(url):
    ...
    **always take a backup of your database before running migrations**. The migrations README,
    ...
    """
    if cral.tracking.store.db.utils._is_initialized_before_mlflow_1(url):
        cral.tracking.store.db.utils._upgrade_db_initialized_before_mlflow_1(url)
    cral.tracking.store.db.utils._upgrade_db(url)
