"""
The ``cral.tracking.azureml`` module provides an API for deploying MLflow models to Azure
Machine Learning.
"""
...
from cral.tracking.utils import PYTHON_VERSION, experimental, get_unique_resource_id
from cral.tracking.utils.file_utils import TempDir, _copy_file_or_tree, _copy_project
from cral.tracking.version import VERSION as mlflow_version
...
def build_image(model_uri, workspace, image_name=None, model_name=None,
    ...
    >>> azure_image, azure_model = cral.tracking.azureml.build_image(
    >>>                                 model_uri="<model_uri>",
    ...
    """
    # The Azure ML SDK is only compatible with Python 3. However, the `cral.tracking.azureml` module should
...
from cral.tracking.pyfunc import load_pyfunc
from cral.tracking.pyfunc.scoring_server import parse_json_input, _get_jsonable_obj
