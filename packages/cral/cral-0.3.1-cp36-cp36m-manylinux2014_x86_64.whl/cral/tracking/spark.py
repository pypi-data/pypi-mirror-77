"""
The ``cral.tracking.spark`` module provides an API for logging and loading Spark MLlib models. This module
...
from cral.tracking import pyfunc, mleap
from cral.tracking.exceptions import MlflowException
from cral.tracking.models import Model
from cral.tracking.protos.databricks_pb2 import INVALID_PARAMETER_VALUE
...
from cral.tracking.utils.file_utils import TempDir
from cral.tracking.utils.uri import is_local_uri, append_to_uri_path
from cral.tracking.utils.model_utils import _get_flavor_configuration_from_uri
...
def log_model(spark_model, artifact_path, conda_env=None, dfs_tmpdir=None,
    ...
    >>> cral.tracking.spark.log_model(model, "spark-model")
    """
    ...
    run_id = cral.tracking.tracking.fluent._get_or_start_run().info.run_id
    run_root_artifact_uri = cral.tracking.get_artifact_uri()
    ...
    if is_local_uri(run_root_artifact_uri):
        return Model.log(artifact_path=artifact_path, flavor=cral.tracking.spark, spark_model=spark_model,
    ...
    with TempDir() as tmp:
        ...
        cral.tracking.tracking.fluent.log_artifacts(tmp_model_metadata_dir, artifact_path)
        if registered_model_name is not None:
            cral.tracking.register_model("runs:/%s/%s" % (run_id, artifact_path), registered_model_name)
...
def _save_model_metadata(dst_dir, spark_model, mlflow_model, sample_input, conda_env):
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.spark", data=_SPARK_MODEL_PATH_SUB,
...
def load_model(model_uri, dfs_tmpdir=None):
    ...
    >>> from cral.tracking import spark
    >>> model = cral.tracking.spark.load_model("spark-model")
...
def autolog():
    ...
    `mlflow-spark JAR
    <http://cral.tracking.org/docs/latest/tracking.html#automatic-logging-from-spark-experimental>`_
    ...
    >>> import cral.tracking.spark
    >>> from pyspark.sql import SparkSession
    >>> # Create and persist some dummy data
    >>> spark = SparkSession.builder\
    >>>   .config("spark.jars.packages", "org.cral.tracking.mlflow-spark").getOrCreate()
    ...
    """
    from cral.tracking import _spark_autologging
