"""
The ``cral.tracking.lightgbm`` module provides an API for logging and loading LightGBM models.
...
def save_model(lgb_model, path, conda_env=None, mlflow_model=Model()):
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.lightgbm",
...
def log_model(lgb_model, artifact_path, conda_env=None, registered_model_name=None, **kwargs):
    ...
    """
    Model.log(artifact_path=artifact_path, flavor=cral.tracking.lightgbm,
...
def load_model(model_uri):
    ...
    :param model_uri: The location, in URI format, of the MLflow model. For example:
                      ...
                      `Referencing Artifacts <https://www.cral.tracking.org/docs/latest/tracking.html#
    ...
    def train(*args, **kwargs):
        ...
        def log_feature_importance_plot(features, importance, importance_type):
            ...
            try:
                ...
                try_mlflow_log(cral.tracking.log_artifact, filepath)
            finally:
        ...
        # logging booster params separately via cral.tracking.log_params to extract key/value pairs
        # and make it easier to compare them across runs.
        params = args[0] if len(args) > 0 else kwargs['params']
        try_mlflow_log(cral.tracking.log_params, params)
        ...
        for idx, metrics in enumerate(eval_results):
            try_mlflow_log(cral.tracking.log_metrics, metrics, step=idx)
        ...
        if early_stopping:
            ...
            try_mlflow_log(cral.tracking.log_metric, 'stopped_iteration', len(eval_results))
            # best_iteration is set even if training does not stop early.
            try_mlflow_log(cral.tracking.log_metric, 'best_iteration', model.best_iteration)
            # iteration starts from 1 in LightGBM.
            try_mlflow_log(cral.tracking.log_metrics, eval_results[model.best_iteration - 1],
                           step=extra_step)
