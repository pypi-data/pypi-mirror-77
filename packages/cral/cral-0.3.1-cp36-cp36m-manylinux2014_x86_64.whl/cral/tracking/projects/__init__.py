"""
The ``cral.tracking.projects`` module provides an API for running MLflow projects locally or remotely.
"""
...
import cral.tracking.projects.databricks
#import cral.tracking.tracking as tracking
from cral.tracking import tracking
#import cral.tracking.tracking.fluent as fluent
from cral.tracking.tracking import fluent
from cral.tracking.entities import RunStatus, SourceType
from cral.tracking.exceptions import ExecutionException, MlflowException
from cral.tracking.projects import _project_spec
from cral.tracking.projects.submitted_run import LocalSubmittedRun, SubmittedRun
from cral.tracking.projects.utils import (
...
from cral.tracking.tracking.context.default_context import _get_user
from cral.tracking.tracking.context.git_context import _get_git_commit
...
from cral.tracking.utils import databricks_utils, file_utils, process
from cral.tracking.utils.file_utils import path_to_local_sqlite_uri, path_to_local_file_uri
...
from cral.tracking.utils.uri import get_db_profile_from_uri, is_databricks_uri
...
def _run(uri, experiment_id, entry_point="main", version=None, parameters=None,
    ...
    if backend == "databricks":
        tracking.MlflowClient().set_tag(active_run.info.run_id, MLFLOW_PROJECT_BACKEND,
                                        "databricks")
        from cral.tracking.projects.databricks import run_databricks
        ...
        from cral.tracking.projects import kubernetes as kb
...
def run(uri, entry_point="main", version=None, parameters=None,
    ...
    :raises: :py:class:`cral.tracking.exceptions.ExecutionException` If a run launched in blocking mode
             is unsuccessful.
    ...
    :param backend_config: A dictionary, or a path to a JSON file (must end in '.json'), which will
                           ...
                           at https://www.cral.tracking.org/docs/latest/projects.html.
    ...
    :param run_id: Note: this argument is used internally by the MLflow project APIs and should
                   ...
                   creating a new run.
    :return: :py:class:`cral.tracking.projects.SubmittedRun` exposing information (e.g. run ID)
    ...
    if backend == "databricks":
        cral.tracking.projects.databricks.before_run_validations(cral.tracking.get_tracking_uri(), backend_config)
...
def _fetch_zip_repo(uri):
    ...
    # ```cral.tracking.data.download_uri()`` when the API supports the same set of available stores as
...
def _get_conda_bin_executable(executable_name):
    ...
    ``cral.tracking.projects.MLFLOW_CONDA_HOME`` environment variable. If
    ``cral.tracking.projects.MLFLOW_CONDA_HOME`` is unspecified, this method simply returns the passed-in
    executable name.
    """
