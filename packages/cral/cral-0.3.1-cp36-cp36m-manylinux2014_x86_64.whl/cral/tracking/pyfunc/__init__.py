...
"""
The ``cral.tracking.pyfunc`` module defines a generic :ref:`filesystem format <pyfunc-filesystem-format>`
...
The ``cral.tracking.pyfunc`` module also defines utilities for creating custom ``pyfunc`` models
...
- loader_module [required]:
         ...
         e.g. ``cral.tracking.sklearn``, it will be imported using ``importlib.import_module``.
...
::
  ...
  python_function:
    code: code
    data: data/model.pkl
    loader_module: cral.tracking.sklearn
    env: mlflow_env.yml
    main: sklearn_iris
...
queries, such as preprocessing and postprocessing routines. Therefore, ``cral.tracking.pyfunc``
...
2. Interpreting pre-existing data as an MLflow model.
   ...
   implementation in cral.tracking.keras <https://github.com/mlflow/mlflow/blob/
...
import cral.tracking.pyfunc.model
import cral.tracking.pyfunc.utils
from cral.tracking.models import Model
from cral.tracking.pyfunc.model import PythonModel, PythonModelContext, get_default_conda_env
from cral.tracking.tracking.artifact_utils import _download_artifact_from_uri
from cral.tracking.utils import PYTHON_VERSION, deprecated, get_major_minor_py_version
from cral.tracking.utils.file_utils import TempDir, _copy_file_or_tree
...
from cral.tracking.protos.databricks_pb2 import INVALID_PARAMETER_VALUE, RESOURCE_ALREADY_EXISTS
...
def load_model(model_uri, suppress_warnings=True):
    ...
    if CODE in conf and conf[CODE]:
        ...
        cral.tracking.pyfunc.utils._add_code_to_system_path(code_path=code_path)
...
@deprecated("cral.tracking.pyfunc.load_model", 1.0)
...
def spark_udf(spark, model_uri, result_type="double"):
    ...
    >>> predict = cral.tracking.pyfunc.spark_udf(spark, "/my/local/model")
    ...
    :param model_uri: The location, in URI format, of the MLflow model with the
                      :py:mod:`cral.tracking.pyfunc` flavor. For example:
    ...
    # functionality.
    from cral.tracking.pyfunc.spark_model_cache import SparkModelCache
...
def save_model(path, loader_module=None, data_path=None, code_path=None, conda_env=None,
    ...
    :param mlflow_model: :py:mod:`cral.tracking.models.Model` configuration to which to add the
    ...
    :param artifacts: A dictionary containing ``<name, artifact_uri>`` entries. Remote artifact URIs
                      ...
                      in :func:`PythonModel.load_context() <cral.tracking.pyfunc.PythonModel.load_context>`
                      and :func:`PythonModel.predict() <cral.tracking.pyfunc.PythonModel.predict>`.
    ...
    if first_argument_set_specified:
        ...
        return cral.tracking.pyfunc.model._save_model_with_class_artifacts_params(
...
def _save_model_with_loader_module_and_data_path(path, loader_module, data_path=None,
    ...
    cral.tracking.pyfunc.add_to_model(
