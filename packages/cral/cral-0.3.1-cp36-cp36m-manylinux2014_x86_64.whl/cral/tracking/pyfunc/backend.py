...
import subprocess
import posixpath
from cral.tracking.models import FlavorBackend
from cral.tracking.models.docker_utils import _build_image, DISABLE_ENV_CREATION
from cral.tracking.pyfunc import ENV
from cral.tracking.pyfunc import scoring_server

from cral.tracking.projects import _get_or_create_conda_env, _get_conda_bin_executable, \
                            _get_conda_command
from cral.tracking.tracking.artifact_utils import _download_artifact_from_uri
from cral.tracking.utils.file_utils import path_to_local_file_uri
from cral.tracking.version import VERSION
...
class PyFuncBackend(FlavorBackend):
    ...
    def predict(self, model_uri, input_path, output_path, content_type, json_format, ):
        ...
        if not self._no_conda and ENV in self._config:
            conda_env_path = os.path.join(local_path, self._config[ENV])
            command = ('python -c "from cral.tracking.pyfunc.scoring_server import _predict; _predict('
                       'model_uri={model_uri}, '
    ...
    def serve(self, model_uri, port, host):
        ...
        if os.name != "nt":
            command = ("gunicorn --timeout=60 -b {host}:{port} -w {nworkers} ${{GUNICORN_CMD_ARGS}}"
                       " -- cral.tracking.pyfunc.scoring_server.wsgi:app").format(
                host=host,
                port=port,
                nworkers=self._nworkers)
        else:
            command = ("waitress-serve --host={host} --port={port} "
                       "--ident=mlflow cral.tracking.pyfunc.scoring_server.wsgi:app").format(
                host=host,
                port=port)
    ...
    def build_image(self, model_uri, image_name, install_mlflow=False, mlflow_home=None):
        ...
        def copy_model_into_container(dockerfile_context_dir):
            ...
            return """
                ...
                RUN python -c \
                'from cral.tracking.models.container import _install_pyfunc_deps;\
        ...
        pyfunc_entrypoint = (
            'ENTRYPOINT ["python", "-c", "from cral.tracking.models import container as C; C._serve()"]'
        )
        _build_image(
