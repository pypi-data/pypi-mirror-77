"""
The ``cral.tracking.pyfunc.model`` module defines logic for saving and loading custom "python_function"
...
import cral.tracking.pyfunc
import cral.tracking.utils
...
def get_default_conda_env():
    ...
    :return: The default Conda environment for MLflow Models produced by calls to
             :func:`save_model() <cral.tracking.pyfunc.save_model>`
             and :func:`log_model() <cral.tracking.pyfunc.log_model>` when a user-defined subclass of
...
class PythonModelContext(object):
    ...
    :func:`save_model() <cral.tracking.pyfunc.save_model>` and
    :func:`log_model() <cral.tracking.pyfunc.log_model>` persistence methods, using the contents specified
...
def _save_model_with_class_artifacts_params(path, python_model, artifacts=None, conda_env=None,
    ...
    :param mlflow_model: The model configuration to which to add the ``cral.tracking.pyfunc`` flavor.
    """
    ...
    cral.tracking.pyfunc.add_to_model(model=mlflow_model, loader_module=__name__, code=saved_code_subpath,
...
def _load_pyfunc(model_path):
    pyfunc_config = _get_flavor_configuration(
            model_path=model_path, flavor_name=cral.tracking.pyfunc.FLAVOR_NAME)
    ...
    if python_model_cloudpickle_version is None:
        cral.tracking.pyfunc._logger.warning(
