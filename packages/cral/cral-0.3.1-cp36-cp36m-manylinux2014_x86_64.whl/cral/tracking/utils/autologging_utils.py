...
def try_mlflow_log(fn, *args, **kwargs):
    ...
    try:
        ...
        warnings.warn("Logging to Refine server failed: " + str(e), stacklevel=2)
...
def log_params_decorator(fn):
    ...
    def log_fn_args_as_params(*args, **kwargs):  # pylint: disable=W0102
        ...
        try_mlflow_log(cral.tracking.log_params, _args_as_dict)
...
def log_fn_args_as_params(fn, args, kwargs, unlogged=[]):  # pylint: disable=W0102
    ...
    if all_default_values is not None and len(all_default_values) > 0:
        ...
        for name in [name for name in defaults.keys() if name in unlogged]:
            del defaults[name]
        try_mlflow_log(cral.tracking.log_params, defaults)
    ...
    if len(args_dict.keys()) > 0:
        try_mlflow_log(cral.tracking.log_params, args_dict)

    # Logging the kwargs passed by the user
    for param_name in kwargs:
        if param_name not in unlogged:
            try_mlflow_log(cral.tracking.log_param, param_name, kwargs[param_name])
