...
from cral import __version__
from cral.tracking.protos import databricks_pb2
from cral.tracking.utils.proto_json_utils import parse_dict
from cral.tracking.utils.string_utils import strip_suffix
from cral.tracking.exceptions import MlflowException, RestException, MlflowliteException

from cral.tracking.lite_extensions.client_utils import get_host_uri, get_token
#from cral.tracking.tracking.fluent import _get_experiment_id
...
def http_request(endpoint, retries=3, retry_interval=3,
                 max_rate_limit_interval=60, **kwargs):
    """
    ...
    :param host_creds: A :py:class:`cral.tracking.rest_utils.MlflowHostCreds` object containing
    ...
    if 'params' in kwargs:
        kwargs['params'].update({'token': token, 'experiment_id': experiment_id})
    elif 'json' in kwargs:
        kwargs['json'].update({'token': token, 'experiment_id': experiment_id})
    else:
        raise MlflowliteException("no 'json' or 'params' field found to send in request")
