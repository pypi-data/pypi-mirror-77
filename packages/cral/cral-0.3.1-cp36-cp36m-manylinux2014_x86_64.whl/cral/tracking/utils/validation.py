...
from cral.tracking.protos.databricks_pb2 import INVALID_PARAMETER_VALUE, RESOURCE_DOES_NOT_EXIST
from cral.tracking.store.db.db_types import DATABASE_ENGINES
from cral.tracking.utils.string_utils import is_string_type

from cral.tracking.lite_extensions.server_only.file_utils import _ROOT_DIR
...
def _validate_artifact(key, path):
    """
    Check that an artifact with the specified key & valid and raise an exception if it isn't.
    """
...
def _validate_artifact_name(name):
    """
    Check that `name` is a valid artifact name and raise an exception if it isn't.
    # TODO: Warn the user if artifact with same `name` already exists in the current run.
    """
    pass

def _validate_artifact_path(path):
    """
    Check that `path` is a valid artifact local path and raise an exception if is isn't.
    """
    pass
...
def _validate_run_id_exp_id(exp_id, run_id):
    """Check that `run_id` is a valid run ID and raise an exception if it isn't."""
    if _RUN_ID_REGEX.match(run_id) is None:
        raise MlflowException("Invalid run ID: '%s'" % run_id, error_code=INVALID_PARAMETER_VALUE)

    if _EXPERIMENT_ID_REGEX.match(exp_id) is None:
        raise MlflowException("Invalid exp ID: '%s'" % run_id, error_code=INVALID_PARAMETER_VALUE)

    if not os.path.isdir(os.path.join(_ROOT_DIR, exp_id, run_id)):
        raise MlflowException("RunId: {} doesnot exist under expID: {}".format(run_id, exp_id), error_code=RESOURCE_DOES_NOT_EXIST)
