...
from cral.tracking.entities import Experiment, Run, RunInfo, Metric, ViewType
from cral.tracking.exceptions import MlflowException
from cral.tracking.protos import databricks_pb2
from cral.tracking.protos.service_lite_pb2 import CreateExperiment, MlflowService, GetExperiment, \
    ...
    LogArtifact
from cral.tracking.store.tracking.abstract_store import AbstractStore
from cral.tracking.utils.proto_json_utils import message_to_json
from cral.tracking.utils.rest_utils import call_endpoint, extract_api_info_for_service

from cral.tracking.protos.service_lite_pb2 import RunTag
...
class RestStore(AbstractStore):
    ...
    :param get_host_creds: Method to be invoked prior to every REST request to get the
      :py:class:`cral.tracking.rest_utils.MlflowHostCreds` for the request. Note that this
    ...
    def get_experiment(self, experiment_id):
        ...
        :return: A single :py:class:`cral.tracking.entities.Experiment` object if it exists,
    ...
    def create_run(self, experiment_id, start_time, tags):
        ...
        tag_protos = [RunTag(key=tagkey, value=tagvalue) for tagkey, tagvalue in tags.items()]
    ...
    def log_artifact(self, run_id, experiment_id, artifact):
        ...
        req_body = message_to_json(
            LogArtifact(run_id=run_id, experiment_id=experiment_id, key=artifact.key)
        )
        response_proto = self._call_endpoint(LogArtifact, req_body)
        if response_proto is None:
            _logger.warning(f"Artifact log HTTP status code: {response_proto.status_code}")
        ...
        upload_thread = threading.Thread(target=artifact_uploader,
            kwargs={'artifact': artifact, 'response_proto': response_proto},
        )
    ...
    def set_terminated(self, run_id, status, end_time):
        #pass
        self.update_run_info(run_id=run_id, run_status=status, end_time=end_time)
