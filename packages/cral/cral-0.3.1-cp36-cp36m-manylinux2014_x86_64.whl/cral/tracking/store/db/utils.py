...
from cral.tracking.store.tracking.dbmodels.initial_models import Base as InitialBase
from cral.tracking.protos.databricks_pb2 import INTERNAL_ERROR
...
def _upgrade_db_initialized_before_mlflow_1(url):
    ...
    in cral.tracking.store.dbmodels.initial_models (with a small number of special-case databases
    initialized pre-1.0 and migrated to have the same schema as cral.tracking.store.dbmodels.initial_models
    via this method).
