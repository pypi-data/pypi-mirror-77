"""
The ``cral.tracking.xgboost`` module provides an API for logging and loading XGBoost models.
...
def save_model(xgb_model, path, conda_env=None, mlflow_model=Model()):
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.xgboost",
...
def log_model(xgb_model, artifact_path, conda_env=None, registered_model_name=None, **kwargs):
    ...
    """
    Model.log(artifact_path=artifact_path, flavor=cral.tracking.xgboost,
...
def autolog(importance_types=['weight']):  # pylint: disable=W0102
    ...
    def train(*args, **kwargs):
        ...
        if early_stopping:
            ...
            try_mlflow_log(cral.tracking.log_metric, 'stopped_iteration', len(eval_results) - 1)
            try_mlflow_log(cral.tracking.log_metric, 'best_iteration', model.best_iteration)
            try_mlflow_log(cral.tracking.log_metrics, eval_results[model.best_iteration],
                           step=extra_step)
