"""
The ``cral.tracking.models`` module provides an API for saving machine learning models in
"flavors" that can be understood by different downstream tools.

The built-in flavors are:

- :py:mod:`cral.tracking.pyfunc`
- :py:mod:`cral.tracking.h2o`
- :py:mod:`cral.tracking.keras`
- :py:mod:`cral.tracking.pytorch`
- :py:mod:`cral.tracking.sklearn`
- :py:mod:`cral.tracking.spark`
- :py:mod:`cral.tracking.tensorflow`
...
class Model(object):
    ...
    def log(cls, artifact_path, flavor, registered_model_name=None, **kwargs):
        ...
        with TempDir() as tmp:
            ...
            cral.tracking.tracking.fluent.log_artifacts(local_path, artifact_path)
            try:
                cral.tracking.tracking.fluent._record_logged_model(mlflow_model)
                ...
                _logger.warning(
                    ...
                    "1.7.0 or above.", cral.tracking.get_artifact_uri())
            if registered_model_name is not None:
                run_id = cral.tracking.tracking.fluent.active_run().info.run_id
                cral.tracking.register_model("runs:/%s/%s" % (run_id, artifact_path),
