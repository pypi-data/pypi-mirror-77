...
from cral.tracking.models.docker_utils import DISABLE_ENV_CREATION
...
def _serve_pyfunc(model):
    ...
    nginx_conf = resource_filename(cral.tracking.models.__name__, "container/scoring_server/nginx.conf")
    ...
    os.system("pip -V")
    os.system("python -V")
    os.system('python -c"from cral.tracking.version import VERSION as V; print(V)"')
    cmd = "gunicorn -w {cpu_count} ".format(cpu_count=cpu_count) + \
          "${GUNICORN_CMD_ARGS} cral.tracking.models.container.scoring_server.wsgi:app"
...
def _serve_mleap():
    serve_cmd = ["java", "-cp", "\"/opt/java/jars/*\"", "org.cral.tracking.sagemaker.ScoringServer",
