"""
The ``cral.tracking.sagemaker`` module provides an API for deploying MLflow models to Amazon SageMaker.
"""
...
from cral.tracking.protos.databricks_pb2 import RESOURCE_DOES_NOT_EXIST, INVALID_PARAMETER_VALUE
from cral.tracking.tracking.artifact_utils import _download_artifact_from_uri
from cral.tracking.utils import get_unique_resource_id
from cral.tracking.utils.file_utils import TempDir
from cral.tracking.utils.logging_utils import eprint
from cral.tracking.models.container import SUPPORTED_FLAVORS as SUPPORTED_DEPLOYMENT_FLAVORS
from cral.tracking.models.container import DEPLOYMENT_CONFIG_KEY_FLAVOR_NAME
...
def push_image_to_ecr(image=DEFAULT_IMAGE_NAME):
    ...
    fullname = _full_template.format(account=account, region=region, image=image,
                                     version=cral.tracking.version.VERSION)
...
def deploy(app_name, model_uri, execution_role_arn=None, bucket=None,
    ...
    :param mode: The mode in which to deploy the application. Must be one of the following:
                 ...
                 ``cral.tracking.sagemaker.DEPLOYMENT_MODE_ADD``
    ...
    :param archive: If ``True``, any pre-existing SageMaker application resources that become
                    ...
                    ``cral.tracking.sagemaker.DEPLOYMENT_MODE_REPLACE`` mode) are preserved.
    ...
    :param vpc_config: A dictionary specifying the VPC configuration to use when creating the
                       ...
                       >>> import cral.tracking.sagemaker as mfs
                       >>> vpc_config = {
                       ...                  'SecurityGroupIds': [
                       ...                      'sg-123456abc',
                       ...                  ],
    ...
    :param flavor: The name of the flavor of the model to use for deployment. Must be either
                   ``None`` or one of cral.tracking.sagemaker.SUPPORTED_DEPLOYMENT_FLAVORS. If ``None``,
...
def _get_default_image_url(region_name):
    ...
    return (repository_conf["repositoryUri"] + ":{version}").format(version=cral.tracking.version.VERSION)
...
def _update_sagemaker_endpoint(endpoint_name, model_name, model_uri, image_url, model_s3_path,
    ...
    :param vpc_config: A dictionary specifying the VPC configuration to use when creating the
                       new SageMaker model associated with this SageMaker endpoint.
    :param mode: either cral.tracking.sagemaker.DEPLOYMENT_MODE_ADD or
                 cral.tracking.sagemaker.DEPLOYMENT_MODE_REPLACE.
    :param role: SageMaker execution ARN role.
    :param sage_client: A boto3 client for SageMaker.
    :param s3_client: A boto3 client for S3.
