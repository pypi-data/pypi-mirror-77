...
import cral.tracking.sagemaker
from cral.tracking.sagemaker import DEFAULT_IMAGE_NAME as IMAGE
from cral.tracking.utils import cli_args
import cral.tracking.models.docker_utils
...
@click.option("--mode", default=cral.tracking.sagemaker.DEPLOYMENT_MODE_CREATE,
              help="The mode in which to deploy the application."
                   " Must be one of the following: {mds}".format(
                     mds=", ".join(cral.tracking.sagemaker.DEPLOYMENT_MODES)))
@click.option("--archive", "-ar", is_flag=True,
              help=("If specified, any SageMaker resources that become inactive (i.e as the"
                    ...
                    " `--async`.".format(
                      mode_replace=cral.tracking.sagemaker.DEPLOYMENT_MODE_REPLACE)))
@click.option("--instance-type", "-t", default=cral.tracking.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_TYPE,
              help="The type of SageMaker ML instance on which to deploy the model. For a list of"
                   " supported instance types, see"
                   " https://aws.amazon.com/sagemaker/pricing/instance-types/.")
@click.option("--instance-count", "-c", default=cral.tracking.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_COUNT,
...
@click.option("--flavor", "-f", default=None,
              help=("The name of the flavor to use for deployment. Must be one of the following:"
                    ...
                    " from the model's available flavors.".format(
                      supported_flavors=cral.tracking.sagemaker.SUPPORTED_DEPLOYMENT_FLAVORS)))
...
def deploy(app_name, model_uri, execution_role_arn, bucket, image_url, region_name, mode, archive,
    ...
    https://www.cral.tracking.org/docs/latest/models.html#sagemaker-deployment.
    """
    if vpc_config is not None:
        with open(vpc_config, "r") as f:
            vpc_config = json.load(f)

    cral.tracking.sagemaker.deploy(app_name=app_name, model_uri=model_uri,
...
def delete(app_name, region_name, archive, asynchronous, timeout):
    ...
    elapses.
    """
    cral.tracking.sagemaker.delete(
...
def run_local(model_uri, port, image, flavor):
    ...
    """
    cral.tracking.sagemaker.run_local(model_uri=model_uri, port=port, image=image, flavor=flavor)
...
def build_and_push_container(build, push, container, mlflow_home):
    ...
    if build:
        ...
        ENTRYPOINT ["python", "-c", "import sys; from cral.tracking.models import container as C; \
        C._init(sys.argv[1])"]
        """
        ...
        def setup_container(_):
            return "\n".join([
                ...
                'RUN python -c "from cral.tracking.models.container import _install_pyfunc_deps;'
                '_install_pyfunc_deps(None, False)"'
            ])

        cral.tracking.models.docker_utils._build_image(
            container,
    ...
    if push:
        cral.tracking.sagemaker.push_image_to_ecr(container)
