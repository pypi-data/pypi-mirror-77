...
This is a lower level API than the :py:mod:`cral.tracking.tracking.fluent` module, and is
exposed in the :py:mod:`cral.tracking.tracking` module.
"""
...
from cral.tracking.utils.validation import _validate_param_name, _validate_tag_name, _validate_run_id, \
    _validate_experiment_artifact_location, _validate_experiment_name, _validate_metric, \
    _validate_artifact
from cral.tracking.entities import Param, Metric, RunStatus, RunTag, ViewType, ExperimentTag, \
    Artifact
from cral.tracking.store.artifact.artifact_repository_registry import get_artifact_repository
from cral.tracking.utils.mlflow_tags import MLFLOW_USER
...
class TrackingServiceClient(object):
    ...
    #Remove from client because this should be server only, add to a separate class TrackingServiceServer
    ...
    def log_artifact(self, run_id, key, path):
        """
        Log an artifact against the run ID.
        """
        _validate_artifact(key)
        artifact = Artifact(key, path)
        self.store.log_artifact(run_id, artifact)
    ...
    def _record_logged_model(self, run_id, mlflow_model):
        if not isinstance(mlflow_model, Model):
            raise TypeError("Argument 'mlflow_model' should be of type cral.tracking.models.Model but was "
