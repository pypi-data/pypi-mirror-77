...
from cral.tracking.tracking.context.default_context import DefaultRunContext
from cral.tracking.tracking.context.git_context import GitRunContext
from cral.tracking.tracking.context.databricks_notebook_context import DatabricksNotebookRunContext
from cral.tracking.tracking.context.databricks_job_context import DatabricksJobRunContext
...
class RunContextProviderRegistry(object):
    ...
    entrypoints `cral.tracking.run_context_provider` group can be automatically registered through the
    ...
    def register_entrypoints(self):
        ...
        for entrypoint in entrypoints.get_group_all("cral.tracking.run_context_provider"):
            try:
...
def resolve_tags(tags=None):
    ...
    :py:class:`cral.tracking.tracking.context.RunContextProvider`.
