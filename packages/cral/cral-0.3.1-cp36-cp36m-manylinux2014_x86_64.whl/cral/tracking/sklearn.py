"""
The ``cral.tracking.sklearn`` module provides an API for logging and loading scikit-learn models. This
...
from cral.tracking.protos.databricks_pb2 import INVALID_PARAMETER_VALUE, INTERNAL_ERROR
from cral.tracking.protos.databricks_pb2 import RESOURCE_ALREADY_EXISTS
...
def save_model(sk_model, path, conda_env=None, mlflow_model=Model(),
    ...
    :param mlflow_model: :py:mod:`cral.tracking.models.Model` this flavor is being added to.
    :param serialization_format: The format in which to serialize the model. This should be one of
                                 the formats listed in
                                 ``cral.tracking.sklearn.SUPPORTED_SERIALIZATION_FORMATS``. The Cloudpickle
                                 format, ``cral.tracking.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE``,
                                 provides better cross-system compatibility by identifying and
                                 packaging code dependencies with the serialized model.

    >>> import cral.tracking.sklearn
    ...
    >>> sk_path_dir_1 = ...
    >>> cral.tracking.sklearn.save_model(
    >>>         sk_model, sk_path_dir_1,
    >>>         serialization_format=cral.tracking.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE)
    >>>
    >>> #Save the model in pickle format
    >>> #set path to location for persistence
    >>> sk_path_dir_2 = ...
    >>> cral.tracking.sklearn.save_model(sk_model, sk_path_dir_2,
    >>>                           serialization_format=cral.tracking.sklearn.SERIALIZATION_FORMAT_PICKLE)
    """
    import sklearn
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="cral.tracking.sklearn", data=model_data_subpath,
...
def log_model(sk_model, artifact_path, conda_env=None,
    ...
    >>> #log model params
    >>> cral.tracking.log_param("criterion", sk_model.criterion)
    >>> cral.tracking.log_param("splitter", sk_model.splitter)
    >>> #log model
    >>> cral.tracking.sklearn.log_model(sk_model, "sk_models")
    """
    return Model.log(artifact_path=artifact_path,
                     flavor=cral.tracking.sklearn,
                     sk_model=sk_model,
                     conda_env=conda_env,
                     serialization_format=serialization_format,
...
def _save_model(sk_model, output_path, serialization_format):
    ...
    :param serialization_format: The format in which to serialize the model. This should be one of
                                 the following: ``cral.tracking.sklearn.SERIALIZATION_FORMAT_PICKLE`` or
                                 ``cral.tracking.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE``.
    """
...
def load_model(model_uri):
    ...
    >>> import cral.tracking.sklearn
    >>> sk_model = cral.tracking.sklearn.load_model("runs:/96771d893a5e46159d9f3b49bf9013e2/"sk_models")
