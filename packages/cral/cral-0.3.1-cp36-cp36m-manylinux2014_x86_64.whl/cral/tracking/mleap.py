"""
The ``cral.tracking.mleap`` module provides an API for saving Spark MLLib models using the
...
from cral.tracking.utils import keyword_only
...
def log_model(spark_model, sample_input, artifact_path, registered_model_name=None):
    ...
    >>> #log parameters
    >>> cral.tracking.log_param("max_iter", 10)
    >>> cral.tracking.log_param("reg_param", 0.001)
    >>> #log the Spark MLlib model in MLeap format
    >>> cral.tracking.mleap.log_model(spark_model=model, sample_input=test_df,
    >>>                        artifact_path="mleap-model")
    """
    return Model.log(artifact_path=artifact_path, flavor=cral.tracking.mleap,
...
def save_model(spark_model, sample_input, path, mlflow_model=Model()):
    ...
    :param mlflow_model: :py:mod:`cral.tracking.models.Model` to which this flavor is being added.
    """
