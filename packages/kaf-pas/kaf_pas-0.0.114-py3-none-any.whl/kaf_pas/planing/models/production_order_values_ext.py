...
from isc_common import Stack, Wrapper, delAttr
...
class Production_order_values_ext:
            ...
            with managed_progress(
                ...
                with transaction.atomic():
                    ...
                    for record in records:
                        production_order = Production_order.objects.get(id=record.get('id'))
                        ...
                        qty = production_order.value_sum

                        if production_order.status.id == settings.OPERS_TYPES_STACK.PRODUCTION_TASK_STATUSES.get(new).id and production_order.cnt_opers == 1:
                            ...
                            setAttr(record, 'childs', [model_to_dict(production_order_opers) for production_order_opers in Production_order_opers.objects.filter(parent_id=production_order.id)])
                            setAttr(record, 'value', production_order.value_sum)
                            setAttr(record, 'edizm__name', edizm.name)
                            setAttr(record, 'edizm_id', edizm.id)
                            setAttr(record, 'value_start', qty)

                            delAttr(record, 'launch_id')
                            delAttr(record, 'launch__code')
                            delAttr(record, 'launch__name')
    ...
    def rec_items_releases(self, launches_values, user):
        ...
        for launch_id in launches_values.launches_ids:
            ...
            for launch_release in launch_releases:
                launch = launch_release.launch if launch_release.launch.parent != None else launch_release.launch.child_launches[0]
    ...
    def get_color(self, color, operation, item=None, launch=None):
        ...
        if operation != None and operation.production_operation_is_grouped:
            color = None
            for buffer_item in Buffers.objects.filter(item=item, launch=launch):
                color = buffer_item.color
                break
            return color
        elif operation != None and operation.production_operation_is_absorption:
            color = None
            for buffer_item in Buffers.objects.filter(item=item, launch=launch):
                color = buffer_item.color
                break
    ...
    def make_launch_release(
        ...
        if not this_tech_operation.production_operation_is_launched or data.launch == None:
    ...
    def makeAll(self, data, user, lock=True):
        ...
        try:
            ...
            for operation in operations:
                ...
                if not operation.production_operation_is_grouped and operation.production_operation_is_launched and first_step:
                    color = data.color
                    if data.color != None:
                ...
                if operation.production_operation_is_grouped:
                    ...
                    if prev_tech_operation != None and prev_tech_operation.production_operation_is_grouped:
                        # Ищем главную группирующую

                        for grouped_operation in prev_tech_operation.plus_grouped_operations:
                            # Открываем блок сгруппированных

                            grouping_operations = map(lambda x: x.child, Operation_refs.objects.filter(parent=grouped_operation.operation, props=Operation_refs.props.product_making_block))

                            for grouping_operation in grouping_operations:
                                operation_value = Operation_value.objects.get(operation=grouping_operation)

                                operation_item = Operation_item.objects.get(operation=operation_value.operation)
                                ...
                                self.make_launch_release(
                                    ...
                                    production_order=production_order,
                                    min_value_made=operation_value.value,
    ...
    def delete_operation(self, operation, check=True):
        ...
        if check:
            ...
            value_made = ToDecimal(Production_order_opers.objects.filter(id=parent.id).aggregate(Sum('value_made')).get('value_made__sum'))

            value = ToDecimal(Production_order_values.objects.filter(id=operation.id, opertype__code__in=[MADE_OPRS_PLS_TSK]).aggregate(Sum('value')).get('value__sum'))
            ...
            value_made_next = 0
            for detail_order_opers_oper in detail_order_opers_operation_next:
                value_made_next += detail_order_opers_oper.value_made

            value_enabled = value_made - value_made_next
            if value_enabled < value:
                raise Exception(f'Сумму: {DecimalToStr(value)} удалить не могу т.к. она уже была использована в последующей операции. Максимально возможная сумма к удалению {DecimalToStr(value_enabled)}')
