...
from isc_common.common import red
...
class Launches_ext:
    ...
    def rec_operation(self, operations_item, launch, get_full_path):
        ...
        if created:
            ...
            for operation_resources in operation_resources_query:
                ...
                if operation_resources.resource_fin == None:
                ...
                if operations_item.operation.is_transportation:
                    if operation_resources.resource_fin == None and operation_resources.location_fin == None:
        ...
        if cnt == 0:
            ...
            if section != 'Документация':
                ...
                for operations_item in operations_item_query:
                    try:
                        if (operations_item.ed_izm != None and operations_item.qty == None) or \
                                (operations_item.ed_izm == None and operations_item.qty != None):
    ...
    def rec_launch(self, mode, date, description, key, user, qty=None, item=None, parentlaunch=None, demand=None):
        ...
        with managed_progress(
                id=f'demand_{demand.id}_{user.id}' if demand != None else f'demand_{item.id}_{user.id}',
                qty=cntAll,
                user=user.id,
                message=message,
                title='Выполнено',
            ...
            with transaction.atomic():
                ...
                qty = demand.qty if demand != None else qty
                ...
                launch = Launches.objects.create(
                    ...
                    code=child_code,
                    date=date,
                    demand=demand if demand != None else None,
                    item=item if item != None else demand.precent_item.item,
                    description=description,
                    qty=qty,
                    status=settings.PROD_OPERS_STACK.FORMIROVANIE
                )
                ...
                for item_ref in item_ref_query:
                    ...
                    _count_per_one = StrToNumber(item_line.SPC_CLM_COUNT.value_str) if count != None and item_line.section != 'Документация' else 0
                    _count = StrToNumber(count.value_str) if count != None and item_line.section != 'Документация' else 0
                    ...
                    # start_time = time.clock()
                    logger.debug('//////////////////////////')
                    logger.debug(f'parent: {item_ref.parent.item_name if item_ref.parent else None}, child: {item_ref.child.item_name}, qrt: {_count}, qty_per_one: {_count_per_one}')
                    logger.debug('//////////////////////////')
                    ...
                    if item_ref.parent:
                        if first_step == False and demand != None:
    ...
    def make_launch(self, data, mode='add'):
        ...
        user = data.get('user')
        id = data.get('id')
        parentlaunch = Launches.objects.get(id=id) if id != None else None
