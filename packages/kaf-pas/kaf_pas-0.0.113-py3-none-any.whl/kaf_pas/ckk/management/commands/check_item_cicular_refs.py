...
def check_level(
    ...
    logger.debug(f'in to level: {level}\n{item.id if item != None else None}\n')

    if table == 'production_operations':
        query = Operations.objects.filter(parent_id=item.id if item != None else None)
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            if table == 'Item_refs':
                ...
                cycle_ref = check_level(parent_array=parent_array, item=Operations.objects.get(id=item_id) if item_id != None else None, level=1, table=table)
