    ...
    def get_range_rows_4_Production_values(self, request, function=None, distinct_field_names=None, remove_fields=None):
        ...
        if len(request.get_criteria()) > 0:
            ...
            if isinstance(advancedCriteria.criteria[0].value, dict):
                ...
                for buffers_item in Buffers.objects.filter(last_operation=record_0.childs[production_operation_num - 2].production_operation):
            ...
            if isinstance(data, dict):
                ...
                if isinstance(operations, list):
                    for operation in operations:
                        ...
                        for buffers_item in Buffers.objects.filter(last_operation=record_0.childs[record_0.production_operation_num - 2].production_operation):
                            value_odd = buffers_item.value
