                    ...
                    with connection.cursor() as cursor:
                                    ...
                                    if cnt1 > 0:
                                        for launch_operations_item in Launch_operations_item.objects.filter(item_id=child_id, launch_id=launch_id).order_by('num'):
                                            ...
                                            operation_value = Operation_value.objects.create(
                                                operation=outcome_operation,
                                                value=qty_per_one,
                                                edizm_id=edizm_id if edizm_id != None else edizm_shtuka.id,
                        ...
                        for row in rows:
                            ...
                            try:
                                ...
                                message = 'Товарная позиция не обнаружена среди товарных позиций, прошедших внутреннюю маршрутизацию. Запустите анализатор готовности,и проверьте какая-то сборочная Единица стоит в разделе документации.'
                                mess2 = ''
        ...
        for launch_id in list(set([a.launch.id for a in updated_launch_operations_items])):
            ...
            for updated_launch_operations_item in lst:
                ...
                if updated_launch_operations_item.num == 1:
                    if old_data != None and data.production_operation_num < old_data.production_operation_num:
                        ...
                        prev_num_operation = self.get_num_operation(
                            launch=updated_launch_operations_item.launch,
                            item=updated_launch_operations_item.item,
                            old_num=old_data.production_operation_num if old_data != None else updated_launch_operations_item.num
                        )
