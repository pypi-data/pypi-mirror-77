            ...
            with transaction.atomic():
                ...
                self.PRODUCTION_TASK_STATUSES = Status_operation_typesManager.make_statuses(
                    ...
                    status_map=[
                        ...
                        dict(code=restarted, name=name_restarted, color=green),
                        dict(code=transferred, name=name_transferred),
                        dict(code=stoped, name=name_stoped, color='#310062'),
                        dict(code=doing, name=name_doing),
                        dict(code=closed, name=name_closed),
                    ]
                )
                ...
                # self.RELEASE_TASK_PLS, _ = Operation_types.objects.update_or_create(code=RELEASE_TSK_PLS, defaults=dict(
                #     # props=Operation_types.props.plus | Operation_types.props.accounting,
                #     props=0,
                #     name='Выпуск изделий',
                #     editing=False,
                ...
                # ))
                #
                # self.RELEASE_TASK_PLS_STATUSES = Status_operation_typesManager.make_statuses(
                #     opertype=self.RELEASE_TASK_PLS,
                #     status_map=[
                #         dict(code=new, name=name_new),
                #     ]
                # )
                #
                # self.RELEASE_TASK_MNS, _ = Operation_types.objects.update_or_create(code=RELEASE_TSK_MNS, defaults=dict(
                #     # props=Operation_types.props.minus | Operation_types.props.accounting,
                #     props=0,
                ...
                # ))
                #
                # self.RELEASE_TASK_MNS_STATUSES = Status_operation_typesManager.make_statuses(
                #     opertype=self.RELEASE_TASK_MNS,
                #     status_map=[
                #         dict(code=new, name=name_new),
                #     ]
                # )
