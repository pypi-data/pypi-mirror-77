...
class Launch_itemsManager(Manager):
    @staticmethod
    def find_item(item, launch=None):

        items = ItemManager.find_item(item)
        if launch != None:
            try:
                return Launch_items.objects.filter(launch=launch, item__in=items).exclude(location=F('wlocation')).order_by('num')[0]
            except IndexError:
                return None
        else:
            try:
                return Launch_items.objects.filter(item__in=items).exclude(location=F('wlocation')).order_by('num')[0]
            except IndexError:
                return None
