...
from intentionet.bfe.proto.assertions.adhoc_pb2 import (
    ...
    AD_HOC_EXECUTION_STATUS_NOT_FOUND,
    AdHocAssertion,
    AdHocAssertionId,
...
from pybatfish.datamodel import HeaderConstraints, Interface, PathConstraints
...
class Asserts(BaseAsserts):
    ...
    def assert_that(
        ...
        snapshot = self._get_snapshot_or_raise(snapshot)
        return self._assert(
    ...
    def _wait_for_assertion_complete(
        ...
        while not done:
            ...
            if current_status == AD_HOC_EXECUTION_STATUS_EXECUTED:
                return
            if current_status == AD_HOC_EXECUTION_STATUS_NOT_FOUND:
    ...
    def _get_network_or_raise(self, network: Optional[str]) -> str:
        ...
        """
        net = network if network else self.session.network
        if not net:
    ...
    def _get_snapshot_or_raise(self, snapshot: Optional[str]) -> str:
        ...
        """
        snap = snapshot if snapshot else self.session.snapshot
        if not snap:
    ...
    def _assert(
        ...
        passed = status == ASSERTION_STATUS_PASS  # type: bool
...
class Session(BaseSession):
    ...
    def __init__(self, port=443, **kwargs):
        ...
        kwargs["port_v1"] = self._get_api_gw_port()
        kwargs["port_v2"] = self._get_api_gw_port()
        ...
        from intentionet.bfe.proto.api_gateway_pb2_grpc import (
            ApiGatewayStub,
            HealthStub,
        )
        import grpc
    ...
    def get_base_url(self) -> str:
        ...
        protocol = "https" if self.ssl else "http"
        base = "/apiv0a" + CoordConsts.SVC_CFG_WORK_MGR
        return "{0}://{1}:{2}{3}".format(protocol, self.host, self.port_v1, base)

    def get_base_url2(self) -> str:
        """Generate the base URL for V2 of the coordinator APIs."""
        protocol = "https" if self.ssl else "http"
        base = "/apiv0b" + CoordConsts.SVC_CFG_WORK_MGR2
        return "{0}://{1}:{2}{3}".format(protocol, self.host, self.port_v2, base)
    ...
    def _check_session_is_bfe(self) -> bool:
        """Check if the session points to a BfE backend."""
        return bool(self._get_bfe_version() is not None)

    def _get_bfe_version(self) -> Optional[str]:
        """Get the BfE backend version."""
        v = get_component_versions(self).get("Batfish-Extension-Pack")
        if v is not None:
            return str(v)
        return None
    ...
    def put_topology_aggregates(self, aggregates: TopologyAggregates) -> None:
        ...
        self.put_network_object(
            _NETWORK_OBJECT_TOPOLOGY_AGGREGATES, json.dumps(attr.asdict(aggregates))
        )
