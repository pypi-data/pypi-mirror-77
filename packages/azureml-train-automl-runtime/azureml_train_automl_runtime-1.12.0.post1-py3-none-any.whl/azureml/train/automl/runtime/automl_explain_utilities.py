...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import DatasetsFeatureCountMismatch, \
    FeatureUnsupportedForIncompatibleArguments, InvalidArgumentType, InvalidArgumentWithSupportedValues
...
from azureml.automl.core.shared.exceptions import OnnxConvertException, \
    ScenarioNotSupportedException, OptionalDependencyMissingException, ValidationException
...
def _automl_perform_best_run_explain_model(parent_run: AutoMLRun, dataset: DatasetBase,
    ...
    if automl_settings.model_explainability and parent_run.tags.get('model_explain_run') is not None:
        try:
            if parent_run._has_cached_output() or _should_query_for_best_run(parent_run, logger):
...
def _automl_auto_mode_explain_model(child_run: Any,
    ...
    if not _check_model_explain_packages():
        logger_str = "Some of the packages in azureml.interpret are missing."
