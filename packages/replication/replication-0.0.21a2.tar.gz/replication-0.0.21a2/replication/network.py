...
from .exception import StateError, NetworkFrameError
        ...
        if self._subscriber in sockets:
            try:
                datablock = ReplicatedDatablock.fetch(
                    self._subscriber,
                    self._factory)
            except NetworkFrameError:
                logging.error("Frame corrupted received, skipping.")
                continue
            ...
            if current_time-last_update_time > 1000:
                ...
                for client in self._clients_state.keys():
                    ...
                    if ping > self._timeout:
                        logging.error(
                            f"{client} has been disconnected.")
