...
class MultiAction(object):
    ...
    def wait_for_completed(self, timeout=8):
        ...
        while spent_time < timeout:
            ...
            # logger.debug("MultiAction: wait_for_all_completed spent time {0}s".format(spent_time))
...
class MultiModule(object):
    ...
    def execute_command(self, command_name, *input_args, **input_kw):
        ...
        for robot_id in self._robot_group._robots_id_in_group_list:
            # robot_item[0]: robot id, [1]: robot obj
            robot_module = self._robot_group.all_robots_dict[robot_id].get_module(self._module_name)
        ...
        for robot_id, thread_obj in thread_dict.items():
            thread_obj.start()
            '''result = getattr(robot_module, command_name)(*input_args)
            if not result:
                logger.warn(
                    "[MultiModule] Robot id {0}: {1} {2} error".format(
                        robot_id, robot_module, command_name))
            else:
                logger.info(
                    "[MultiModule] Robot id {0}: {1} {2} successfully".format(
                        robot_id, robot_module, command_name))
...
class MultiRmModule(MultiModule):
    ...
    def move(self, *args, **kw):
        ...
        :return:
        """
        # logger.info("Multi Module robot id {0} : move action".format())
...
class MultiDroneModule(MultiModule):
    ...
    def go(self, go_dict):
        ...
        :param args: go_dict: {robot_id: x, y, z, speed, mid=None, ... }
        :return:
        """
        action_dict = {}
    ...
    def set_led_blink(self, *args, **kw):
        ...
        :return: bool: 设置结果
        """
        return self.execute_command('set_ext_led_blink', *args, **kw)

    def set_bright(self, *args, **kw):
        """ 扩展模块点阵屏亮度
    ...
    def set_mled_graph(self, *args, **kw):
        ...
        :return: bool: 设置结果
        """
        return self.execute_command('set_ext_mled_graph', *args, **kw)
    ...
    def set_led(self, *args, **kw):
        ...
        :return:
        """
        return self.execute_command("set_ext_led", *args, **kw)
