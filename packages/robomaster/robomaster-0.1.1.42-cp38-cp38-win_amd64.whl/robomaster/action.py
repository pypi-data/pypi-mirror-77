...
class Action(metaclass=_AutoRegisterAction):
    ...
    # Action control protocol
    _action_proto_cls = None
    # Action push protocol
...
class TextAction(Action):
    ...
    def found_action(self, proto):
        if proto._action_state == 'ok' or proto._action_state == 'error' or proto._action_state == 'out of range' \
                or proto._action_state == "error No valid marker":
            return True
        else:
            return False
...
class ActionDispatcher(object):
    ...
    def _on_recv(cls, self, msg):
        ...
        # TODO add the handling of found_proto for text_action
        # modify action state when receive action result.
        if found_proto:
            if proto._retcode == 0:
                if proto._accept == 0:
        ...
        # update action state when receive action push msg.
        if found_action:
    ...
    def send_action(self, action, action_type=ACTION_NOW):
        """ 发送任务动作命令 """
        # TODO action_type ACTION_REQUEST, ACTION_NOW, ACTION_QUEUE
        ...
        if isinstance(action, TextAction):
            # TODO state: ACTION_IDLE -> ACTION_STARTED
            # Because Tello's firmware has no action-command confirmation,
            # it is assumed that all sent action-command are received.
            action._changeto_state(ACTION_STARTED)
