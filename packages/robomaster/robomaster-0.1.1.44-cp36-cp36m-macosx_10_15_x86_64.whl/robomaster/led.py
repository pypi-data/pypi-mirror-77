...
class TelloLed(object):
            ...
            if resp_msg:
                ...
                if proto:
                    if proto.resp == "led ok":
                        ...
                        logger.warning("Drone: set ext led faild, resp {0}".format(proto.resp))
    ...
    def set_led_breath(self, speed=1, r=0, g=255, b=0):
        """ 设置扩展模块led以指定的颜色与频率实现呼吸效果

        :param: freq: int:[1, 10], 扩展ked呼吸模式下的速度，共十档，随着数字增大速度变快
        :param r: int:[0, 255], 扩展led红色通道的强度
        :param g: int:[0, 255], 扩展led绿色通道的强度
        :param b: int:[0, 255], 扩展led蓝色通道的强度
        :return: bool: 扩展led模块控制结果
        """
        cmd = "EXT led br {0} {1} {2} {3}".format(speed, r, g, b)
        proto = protocol.TextProtoDrone()
        proto.text_cmd = cmd
        msg = protocol.TextMsg(proto)
        try:
            ...
            if resp_msg:
                ...
                if proto:
                    if proto.resp == "led ok":
                        ...
                        logger.warning("Drone: set ext led breath faild, resp {0}".format(proto.resp))
    ...
    def set_led_blink(self, speed=5, r1=0, g1=255, b1=0, r2=0, g2=255, b2=255):
        """ 设置扩展模块led以制定的两种颜色与频率实现闪烁效果

        :param: freq: int:[1, 10], 扩展ked闪烁模式下的速度， 共十档，随着数字增大速度变快
        ...
        :return: bool: 扩展led模块控制结果
        """
        cmd = "EXT led bl {0} {1} {2} {3} {4} {5} {6}".format(speed, r1, g1, b1, r2, g2, b2)
        proto = protocol.TextProtoDrone()
        proto.text_cmd = cmd
        msg = protocol.TextMsg(proto)
        try:
            ...
            if resp_msg:
                ...
                if proto:
                    if proto.resp == "led ok":
                        ...
                        logger.warning("Drone: set ext led blink faild, resp {0}".format(proto.resp))
            ...
            if resp_msg:
                ...
                if proto:
                    if proto.resp == "matrix ok":
                        ...
                        logger.warning("Drone: set ext led bright faild, resp {0}".format(proto.resp))
            logger.error("Drone: set ext led bright failed")
            return False
        except Exception as e:
            logger.error("Drone: set ext led bright, send_sync_msg exception {0}".format(str(e)))
            return False

    def set_mled_boot(self, matrix_str):
        """ 设置点阵屏的开机画面

        :param matrix_str: string: 长度最大为64，点阵屏显示图案的编码字符串，每个字符解读为二进制后对应位置的led点的状态，
        '0'为关闭该位置led，'r'为点亮红色，'b'为点亮蓝色，'p' 为点亮紫色，输入的长度不足64，后面对应的led点默认都是'0'熄灭状态
        :return: bool: 设置开机动画的结果
        """
        cmd = "EXT mled sg {0}".format(matrix_str)
        proto = protocol.TextProtoDrone()
        proto.text_cmd = cmd
        msg = protocol.TextMsg(proto)
        try:
            ...
            if resp_msg:
                ...
                if proto:
                    if proto.resp == "matrix ok":
                        ...
                        logger.warning("Drone: set ext led boot display faild, resp {0}".format(proto.resp))
            logger.error("Drone: set ext led boot display failed")
            return False
        except Exception as e:
            logger.error("Drone: set ext led boot display, send_sync_msg exception {0}".format(str(e)))
            return False
    ...
    def set_mled_char(self, color="r", display_char="0"):
        ...
        :param: display_char: char: [0~9, A~F]， 显示的字符
        :return: bool: 控制结果
        """
    ...
    def set_mled_graph(self, graph_str):
        """ 用户自定义扩展点阵屏显示图案

        :param graph_str: string: 长度最大为64，点阵屏显示图案的编码字符串，每个字符解读为二进制后对应位置的led点的状态，
        '0'为关闭该位置led，'r'为点亮红色，'b'为点亮蓝色，'p' 为点亮紫色，输入的长度不足64，后面对应的led点默认都是'0'熄灭状态
        :return:bool: 控制结果
        """
        cmd = "EXT mled g {0}".format(graph_str)
        proto = protocol.TextProtoDrone()
        proto.text_cmd = cmd
        msg = protocol.TextMsg(proto)
        try:
            ...
            if resp_msg:
                ...
                if proto:
                    if proto.resp == "matrix ok":
                        ...
                        logger.warning("Drone: set ext mled resp {0}".format(proto.resp))
            logger.warning("Drone: set mled resp failed.")
            return False
        except Exception as e:
            logger.warning("Drone: set ext mled, send_sync_msg exception {0}".format(str(e)))
            return False

    def set_mled_scroll(self, direction='l', color='r', speed=5, display_str="DJI"):
        """ 控制扩展点阵屏滚动显示字符串

        :param: direction: char: 点阵屏滚动方向，'l': 字符串向左移动，'r': 字符串向右移动，'u' 字符串向上移动，'d' 字符串向下移动
        :param: color: char: 点阵屏显示的颜色， 'r'红色，'b'蓝色，'p'紫色
        :param: speed: int:[0, 10], 点阵屏滚动的速度, 一共有十档， 随着数字增大速度变快
        :param: display_str: string:需要显示的字符串
        :return: 设置结果
        """
        cmd = "EXT mled {0} {1} {2} {3} ".format(direction, color, speed, display_str)
        proto = protocol.TextProtoDrone()
        proto.text_cmd = cmd
        msg = protocol.TextMsg(proto)
        try:
            ...
            if resp_msg:
                ...
                if proto:
                    if proto.resp == "matrix ok":
                        ...
                        logger.warning("Drone: set ext mled scroll fail, resp {0}".format(proto.resp))
