...
class ExportClass2SWGClass():
    ...
    def export(self,exclude_classes:list=None,include_classes:list=None):
        ...
        for cls in classes :
            # 已产生swagger class
            if cls.name in classes_swg or  '%smng'%cls.name in packages_swg :
                continue
            # 有排除表时，不能产生
            if exclude_classes and  cls.name.lower() in exclude_classes:
                continue
            # 有包含表时，只产生包含表的类别
            if include_classes and cls.name not in include_classes:
                continue
    ...
    def export_one_swgclass(self,bocls:str):
        ...
        if not package_cls_swg:
            ...
            with codecs.open(self.dest_umlfile, mode='w', encoding='utf-8') as file:
                file.write(json.dumps(self.dest_model, ensure_ascii=False))
            print('The class (%smng) has insert' % bocls)
        else:
            print('The class (%smng) has exist'%bocls)
    ...
    def add_operation(self,swgpackage_name:str,opname:str,method:str='get'):
        ...
        if method not in ['get','put','post','delete']:
            print("add_operation() error, the method %s not in ['get','put','post','delete']"%method)
        # 获取swg中该类的package
        package_cls_swg = self.__find_element(self.swg_package['ownedElements'], 'name',swgpackage_name)
        if not package_cls_swg:
            print('the swagger package(%s) is not exist'%swgpackage_name)
            return
        ...
        if op:
            print('the swagger package(%s) has a opname（%s） UMLSignal class，请修改opname再试'%(swgpackage_name,opname))
            return
        # 从模板中拷贝 物件
        import uuid
        ...
        with codecs.open(self.dest_umlfile, mode='w', encoding='utf-8') as file:
            file.write(json.dumps(self.dest_model, ensure_ascii=False))
        print("add_operation(%s,%s,%s) success" % (swgpackage_name,opname,method))
