...
@pytest.fixture
def msg_all_set():
    msg = pb2.OneOfTimestamp()
    msg.value2.seconds = 12324346
    msg.value2.nanos = 333222
...
@pytest.fixture
def msg_unsets():
    return mimic_protobuf_wire_transfer(pb2.OneOfTimestamp())
...
def model_class_optional():
    ...
    class ModelOptional(Model, protobuf_message=pb2.OneOfTimestamp):

        inner = types.OneOfType(variants_spec={
            'value1': StringType(),
            'value2': types.TimestampType(),
        })
...
def model_class_required():
    ...
    class ModelRequired(Model, protobuf_message=pb2.OneOfTimestamp):
...
def model_class_none_not_dumped():
    ...
    class ModelNoneNotDumped(Model, protobuf_message=pb2.OneOfTimestamp):
        inner = types.OneOfType(variants_spec={
            'value1': StringType(),
            'value2': types.TimestampType(),
        })
...
def model_class_field_renamed():
    ...
    class ModelFieldRenamed(Model, protobuf_message=pb2.OneOfTimestamp):
        custom_inner = types.OneOfType(
            variants_spec={
                'value1': StringType(),
                'custom_value2': types.TimestampType(
...
def model_class_field_renamed_required():
    ...
    class ModelFieldRenamedRequired(Model, protobuf_message=pb2.OneOfTimestamp):
        custom_inner = types.OneOfType(
            variants_spec={
                'value1': StringType(),
                'custom_value2': types.TimestampType(
...
def model_class_validated_factory():
    def _factory(validator_func=None, inner_validator_func=None):
        ...
        class ModelValidated(Model, protobuf_message=pb2.OneOfTimestamp):
            inner = types.OneOfType(
                variants_spec={
                    'value1': StringType(),
                    'value2': types.TimestampType(
...
def test_optional_all_set(model_class_optional, msg_all_set):
    ...
    assert model.inner.value == DUMMY_DATETIME
    ...
    assert data['inner']['value'] == DUMMY_DATETIME
...
def test_renamed_all_set(model_class_field_renamed, msg_all_set):
    ...
    assert model.custom_inner.value == DUMMY_DATETIME
    ...
    assert data['custom_inner']['value'] == DUMMY_DATETIME
...
def test_validated_all_set(model_class_validated_factory, msg_all_set):
    ...
    assert model.inner.variant == 'value2'
    assert model.inner.value == DUMMY_DATETIME
    validator_func.assert_called_once_with(OneOfVariant('value2', DUMMY_DATETIME))
...
def test_validated_validation_error__inner(model_class_validated_factory, msg_all_set):
    ...
    validator_func.assert_called_once_with(DUMMY_DATETIME)
    assert 'inner' in errors
...
def test_validated_validation_error_renamed(model_class_validated_renamed_factory, msg_all_set):
    ...
    validator_func.assert_called_once_with(OneOfVariant('custom_value2', DUMMY_DATETIME))
    assert 'custom_inner' in errors
