...
def msg_all_set():
    msg = pb2.OneOfNested()
    msg.value1.value = value_for_primitive('string_field')
...
@pytest.fixture
def msg_unsets():
    return mimic_protobuf_wire_transfer(pb2.OneOfNested())
...
def model_class_optional():
    ...
    class ModelOptional(Model, protobuf_message=pb2.OneOfNested):

        class InnerMsgModel(Model, protobuf_message=pb2.OneOfNested.Inner):
            value = StringType()
...
def model_class_required():
    ...
    class ModelRequired(Model, protobuf_message=pb2.OneOfNested):
        class InnerMsgModel(Model, protobuf_message=pb2.OneOfNested.Inner):
...
def model_class_none_not_dumped():
    ...
    class ModelNoneNotDumped(Model, protobuf_message=pb2.OneOfNested):
...
def model_class_field_renamed():
    ...
    class ModelFieldRenamed(Model, protobuf_message=pb2.OneOfNested):
...
def model_class_field_renamed_required():
    ...
    class ModelFieldRenamedRequired(Model, protobuf_message=pb2.OneOfNested):
...
def model_class_validated_factory():
    def _factory(validator_func=None, inner_validator_func=None):
        ...
        class ModelValidated(Model, protobuf_message=pb2.OneOfNested):
...
def test_optional_all_set(model_class_optional, msg_all_set):
    ...
    assert model.inner.variant == 'value1'
    assert model.inner.value.value == msg_all_set.value1.value
    ...
    assert 'value' in data['inner']['value']
    assert data['inner']['variant'] == 'value1'
    assert data['inner']['value']['value'] == msg_all_set.value1.value
...
def test_renamed_all_set(model_class_field_renamed, msg_all_set):
    ...
    # check model instance fields
    assert model.custom_inner.variant == 'custom_value1'
    assert model.custom_inner.value.custom_value == msg_all_set.value1.value
    ...
    assert 'custom_value' in data['custom_inner']['value']
    assert data['custom_inner']['variant'] == 'custom_value1'
    assert data['custom_inner']['value']['custom_value'] == msg_all_set.value1.value
...
def test_validated_all_set(model_class_validated_factory, msg_all_set):
    ...
    validator_func.assert_called_once_with(
        OneOfVariant(
            'value1',
            model_cls.InnerMsgModel({'value': msg_all_set.value1.value}),
        )
    )
