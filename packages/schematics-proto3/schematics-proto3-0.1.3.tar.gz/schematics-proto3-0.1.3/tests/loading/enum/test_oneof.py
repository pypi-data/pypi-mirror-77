...
@pytest.fixture
def msg_unsets():
    return mimic_protobuf_wire_transfer(pb2.OneOfEnum())
...
def test_optional_all_set(model_class_optional, msg_all_set):
    ...
    assert model.inner.value is TestEnum.SECOND
    ...
    assert data['inner']['value'] is TestEnum.SECOND.name
...
def test_renamed_all_set(model_class_field_renamed, msg_all_set):
    ...
    assert model.custom_inner.value is TestEnum.SECOND
    ...
    assert data['custom_inner']['value'] is TestEnum.SECOND.name
...
def test_validated_all_set(model_class_validated_factory, msg_all_set):
    ...
    assert model.inner.variant == 'value2'
    assert model.inner.value is TestEnum.SECOND
    validator_func.assert_called_once_with(OneOfVariant('value2', TestEnum.SECOND))
...
def test_validated_validation_error__inner(model_class_validated_factory, msg_all_set):
    ...
    validator_func.assert_called_once_with(TestEnum.SECOND)
    assert 'inner' in errors
...
def test_validated_validation_error_renamed(model_class_validated_renamed_factory, msg_all_set):
    ...
    validator_func.assert_called_once_with(OneOfVariant('custom_value2', TestEnum.SECOND))
    assert 'custom_inner' in errors
