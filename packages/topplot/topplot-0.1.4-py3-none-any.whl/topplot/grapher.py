...
class Grapher:
        ...
        *,
        ...
        if self.cores > 1 or force_single_core is not None:
            ...
            if display_cpu_markers:
                ...
                df.plot(
                    y=labels, color=colours, ax=ax_cpu,
                )
    ...
    def graph_overview(self, use_progress_window: bool = False):
        ...
        def generate_figman():
            return FigManager(
                figman_name, self, self.title(fig_title), self.window_size, 2, 2,
            )

        self.dispense_graph(
            fig_title, figman_name, setup, generate_figman, use_progress_window,
        )
    ...
    def figs_foo(self, func, all_figs: bool, name: str):
        if self.fig_manager:
    ...
    def doit(self, at_showtime, at_close):
        ...
        if len(self.config.which_graphs) > 0:
            prev_figure_opened_okay = False

            for c in self.config.which_graphs:
                if c.isnumeric():
                    prev_figure_opened_okay = True
                    i = int(c)
                    if i == 0:
                        self.graph_overview()
