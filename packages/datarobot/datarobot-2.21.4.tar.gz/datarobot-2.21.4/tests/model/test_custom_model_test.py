...
def test_create_non_blocking(
    unittest_endpoint, mocked_tests, make_tests_url, custom_model_id, tmpdir
):
...
@responses.activate
def test_get_test(mocked_test, make_tests_url):
...
def test_list_tests_multiple_pages(mocked_tests, make_tests_url, custom_model_id):
    ...
    tests = CustomModelTest.list(custom_model_id)
    assert len(tests) == len(mocked_tests["data"]) + len(
        mocked_tests_2nd["data"]
    )
    for test, mocked_test in zip(
        tests, mocked_tests["data"] + mocked_tests_2nd["data"]
    ):
        assert_test(test, mocked_test)
