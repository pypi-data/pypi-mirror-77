...
def run_flow(tasks, source, ingress, transformer, egress, bucket_size=1, max_buckets=1, log_level=INFO, log_dir=None):
    ...
    """
    tasks = source.read(tasks or list())
    info(f'Starting processing of [{len(tasks)}]')
    debug(f'Processing [{tasks}]')
    apply(
        _run_flow,
        tasks,
        args=(ingress, transformer, egress),
        kwargs={
            "log_level": log_level,
            "log_dir": log_dir,
        },
...
def _run_flow(tasks, ingress, transformer, egress, log_level=INFO, log_dir=None):
