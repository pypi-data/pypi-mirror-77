...
class GetLongDeferredExpense(object):#长期待摊费用
    def __init__(self):
        pass
    ...
    def get_data(self, data, username, identify): # 定义数据位置
        ...
        change = ChangeData()
        i = 0
        while i < 14:  # 最大到14行,利用while循环遍历找到起点和终点,但一定要从第二行开始,列数定义为第一列
            i += 1
        ...
        for i in range(start, end):
            if data.cell_value(i, 0):
                ...
                initial_value = change.Changing(data.cell_value(i, 1))
                add_value = change.Changing(data.cell_value(i, 2))
                minus_value = change.Changing(data.cell_value(i, 3))
                end_value = change.Changing(data.cell_value(i, 4))
                remark = change.Changing(data.cell_value(i, 5))
                dic = {
                    "ID": identify,
                    "username": username,
    ...
    def CheckError(self, df):
        ...
        errorlist = []
        if len(df) == 0:
            error = "长期待摊费用明细表未填写,请核查.如果该科目没有数据可忽略"
            errorlist.append(error)
        return df, errorlist


if __name__ == "__main__":
    d = GetLongDeferredExpense()