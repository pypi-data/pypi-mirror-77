...
def test_add_prices_to_shipments():
    ...
    prices_final_check_by_category_df = pd.read_csv(
        os.path.join(cwd, "prices_final_check_by_category.csv")
    )
    prices_billings_by_category_df = pd.read_csv(
        os.path.join(cwd, "prices_billings_by_category.csv")
    )
    prices_propositions_by_category_df = pd.read_csv(
        os.path.join(cwd, "prices_by_category.csv")
    )
    ...
    assert_frame_equal(
        ...
        result_should_be[columns_to_test]
        .sort_values("foresea_name")
