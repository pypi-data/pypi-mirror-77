...
from azureml.automl.core.shared.exceptions import (
    OutOfBoundDataException, ConfigException, DataException, ValidationException)
...
class TimeSeriesInferenceDataset(TimeSeriesDataset):
    ...
    def _check_required_prediction_horizon(X_df, horizon, time_column, target_column):
        ...
        if X_df[X_df[time_column] >= forecast_origin].shape[0] > horizon:
            raise DataException._with_error(AzureMLError.create(MaxHorizonExceeded, target="horizon"))
