define(['base/js/namespace', 'jquery'], function (Jupyter, $) {
  ...
  var add_bby_step_desc_cells = function (type) {
    insert_markdown_cell_below(`### 步骤标题 ${type}`);
...
- 代码练习提示`);
  ...
  var add_bby_string_step = function () {
    add_bby_step_desc_cells('(String)');
...
单元简介（目前不展示这部分，所以可以写的简单一点）`);
  ...
  var add_bby_verify_step = function () {
    add_bby_step_desc_cells('(Verify)');
...
if add(2) == 3 and add(-1) == 0: 
  ...
  var add_matplotlib_step = function () {
    add_bby_step_desc_cells('(Matplotlib)');
...
print("#matplotlib#")`);
  ...
  var add_turtle_step = function () {
    add_bby_step_desc_cells('(Turtle)');
...
`;
  ...
  var add_quiz_step = function () {
    insert_markdown_cell_below(`### 步骤标题 (Quiz)`);
...
- 总结视频中的知识点`);
  ...
  var add_file_upload_cell = function () {
    insert_code_cell_below(`import os
# basepath与惠楚确认
