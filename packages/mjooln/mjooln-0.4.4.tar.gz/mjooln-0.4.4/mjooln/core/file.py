...
class File(Path):
    ...
    def make_file_name(cls,
        ...
        :param stub: File stub, barring extensions
        ...
        new_name = cls.EXTENSION_SEPARATOR.join(new_names)
        if is_hidden:
            new_name = cls.HIDDEN_STARTSWITH + new_name
        return new_name
    ...
    def _read(self, mode='r', num_lines=None):
        with open(self, mode=mode) as f:
            if not num_lines:
                content = f.read()
            else:
