...
from utils import generate_sentences,generate_sentences_dw,convert_to
...
class Embed(object):
    def __init__(self,data_path,model='w2v',num_walks=100,walk_length=10):
        ...
        self.data ,self.le = convert_to(data_path,le)
        self.w2v_model = None
        self._annoy = None
        ...
        if self.model_type == 'w2v':
            ...
            self.graph, self.sentences = generate_sentences(self.data)
    ...
    def train(self,
        ...
        if self.model_type == 'w2v' or self.model_type=='deepwalk':
            ...
            for node in self.w2v_model.wv.vocab:
                # node = "'" + str(node) +"'"
                #print(node)
                #print(self.w2v_model.wv[node])
                self._annoy.add_item(int(node),self.w2v_model.wv[node])
                #self.
        ...
        if self.model_type == 'gat':
            model = AttentionWalkTrainer(graph_path=self.data,
                                        ...
                                        gamma=self.gamma )
            model.fit()
            self.emb = model.save_embedding()

            for id in self.emb.id:
                self._annoy.add_item(int(id),self.emb[self.emb.id==id].values.tolist()[0][1:])
    ...
    def get_embeddings(self,):
        if self.model_type == 'w2v' or self.model_type == 'deepwalk':
            for node in self.w2v_model.wv.vocab:
                #print(node)
                #print(self.le.inverse_transform([int(node)]))
                self._embeddings[self.le.inverse_transform([int(node)])[0]] = self.w2v_model.wv[node]

        if self.model_type == 'gat':

            for id in self.emb.id:
                self._embeddings[self.le.inverse_transform([int(id)])[0]]= self.emb[self.emb.id==id].values.tolist()[0][1:]
