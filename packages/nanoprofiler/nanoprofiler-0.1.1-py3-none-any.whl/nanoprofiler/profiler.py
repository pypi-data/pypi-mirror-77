...
class Profiler:
    ...
    def load_results(self, direc, prefix):
        ...
        csv_files = []
        for f in all_files:
            if f[-4:] == ".csv" and f[:len(prefix)] == prefix:
                filename = os.path.join(direc, f)
                key = f[len(prefix)+1:-4]
    ...
    def plot_top_time(self, filename=False, time="cumtime", keys=None, title="", n=10, size=(5, 4)):
        ...
        for key in keys:
            df = dfs[key].sort_values(time, ascending=False)
    ...
    def plot_function(self, functions=[], n=10, filename=False, time="cumtime", percent=False, 
                      title=False, log_scale=False, legend_outside=False, xlabel="Execution label", size=(5, 4)):
        ...
        if functions == []:
            df = dfs[keys[-1]].sort_values(time, ascending=False)
            functions = df.iloc[:n].function.tolist()
        # Initialize empty arrays to store results
        x = list(range(len(dfs)))
        y = [[] for i in x]
        # Iterate on all executions
        for i in x:
            # Load the dataframe sorting by the column defined by time
            df = dfs[keys[i]].sort_values(time, ascending=False)
            exectime = sum(df['tottime'])
            ...
            for j in range(len(functions)):
                ...
                if lines.function.count() >= 1:
                    ...
                    line.append(lines[time].iloc[0])
                    # If the plot is percent divides by the execution time
                    if percent:
                       line[-1] = line[-1]*100./exectime
