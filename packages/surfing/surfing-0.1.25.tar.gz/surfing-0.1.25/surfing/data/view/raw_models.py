...
class EmStockDividend(Base):
    ...
    REPORT_DATES = Column(DATE, primary_key=True)
    DATES = Column(DATE, primary_key=True) # 日期
    DIVCASHPSBFTAX = Column(DOUBLE(asdecimal=False))
    DIVSTOCKPS = Column(DOUBLE(asdecimal=False))
    DIVPROGRESS = Column(CHAR(16))
    DIVRECORDDATE = Column(DATE)
    DIVEXDATE = Column(DATE)
    DIVPRENOTICEDATE = Column(DATE)
    DIVIMPLANNCDATE = Column(DATE)
    DIVAGMANNCDATE = Column(DATE)
    DIVPLANANNCDATE = Column(DATE)
    DIVORNOT = Column(CHAR(1))
