...
class BasicDataPart1:
    ...
    def index_info_from_raw(self, index_list: Union[Tuple[str], List[str]], index_ids: List[str]):
        ...
        df['is_select'] = False
        df['tag_method'] = 'PE百分位'
        df['price_source'] = df['price_source'].transform(lambda x: x.name)
    ...
    def fund_nav(self, start_date, end_date):
        try:
            ...
            df = self._raw_data_api.get_em_fund_nav(start_date, end_date)
            df['fund_id'] = df.apply(
    ...
    def fund_size(self, start_date, end_date):
        try:
            ...
            if df is None:
                ...
                return False
            # 过滤掉非.OF的基金
            df = df[df.CODES.str.contains('.OF')]
    ...
    def fund_status_latest(self, end_date: str):
        try:
            df: Optional[pd.DataFrame] = self._raw_data_api.get_em_fund_status(end_date=end_date)
            if df is None:
                print('failed to get em fund scale for fund status latest')
                return False
            # 过滤掉非.OF的基金
            df = df[df.CODES.str.contains('.OF')]
            df = df.pivot(index='DATES', columns='CODES', values=['PURCHSTATUS', 'REDEMSTATUS', 'LPLIMIT', 'OTCLPLIMIT', 'TRADESTATUS']).fillna(method='ffill')
    ...
    def fund_rate(self, end_date):
        ...
        try:
            ...
            if sentry.empty:
                ...
                real_date = pd.to_datetime(str(end_date_dt.year)+sentry.array[-1], infer_datetime_format=True).date()
    ...
    def fund_size_hold_rate_history(self):
        ...
        fund_scale['fund_id'] = fund_scale.apply(
            lambda x: self._data_helper._get_fund_id_from_order_book_id(x['CODES'].split('.')[0], x['DATES']), axis=1)
        fund_holding_rate = fund_holding_rate.drop(['CODES', '_update_time'], axis=1).rename(columns={
            'DATES':'datetime',
        ...
        fund_scale = fund_scale.dropna(how='all').drop_duplicates(subset=['size','fund_id'],keep='first')
        fund_holding_rate = fund_holding_rate.dropna(how='all')
    ...
    def fund_hold_asset(self, start_date: str, end_date: str):
        # history, update only on report date
        em_hold_asset = self._raw_data_api.get_em_fund_hold_asset(start_date, end_date)
        ...
        self._data_helper._upload_basic(em_hold_asset, FundHoldAsset.__table__.name)

    def fund_hold_industry(self, end_date: str):
        #history, update only on report date
        em_hold_industry = self._raw_data_api.get_em_fund_hold_industry(end_date)
        ...
        self._data_helper._upload_basic(em_hold_industry, FundHoldIndustry.__table__.name)

    def fund_hold_stock(self, end_date: str):
        #history, update only on report date
        em_hold_stock = self._raw_data_api.get_em_fund_hold_stock(end_date)
        ...
        self._data_helper._upload_basic(em_hold_stock, FundHoldStock.__table__.name)

    def fund_hold_bond(self, end_date: str):
        #history, update only on report date
        em_hold_bond = self._raw_data_api.get_em_fund_hold_bond(end_date)
        ...
        self._data_helper._upload_basic(em_hold_bond, FundHoldBond.__table__.name)
