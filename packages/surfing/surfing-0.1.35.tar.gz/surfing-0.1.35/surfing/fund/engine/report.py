...
class ReportHelper:
    ...
    def plot_init(self, dm, taa_helper=None):
    ...
    def _calc_stat(self, df):
        ...
        w = copy.deepcopy(self.saa_weight) if self.saa_weight else AssetWeight(hs300=1.0).__dict__ # asset weight float
    ...
    def get_last_position(self):
        ...
        for i in pos_df.itertuples():
            _score_dict = self.fund_score_dm[dt][i.index_id]
    ...
    @staticmethod
    def fund_rank_recent_analysis(dm, fund_id):
        ...
        score_func = dm._score_manager.funcs[index_id]
        return HybridPainter.fund_rank_recent_analysis(fund_id,index_id,score_dict,fund_info,fund_indicator,fund_nav,index_price,score_func)
    ...
    @staticmethod
    def recent_week_return(pos_df,fund_result,weight_df,saa,mv_df,m,benchmark,end_date=None,begin_date=None,port_id=''):
        # 把买卖交易费平滑打入净值中
        ...
        desc_dic = m.dts.fund_info[['fund_id','desc_name']].set_index('fund_id').to_dict()['desc_name']
        index_dic = m.dts.fund_info[['fund_id','index_id']].set_index('fund_id').to_dict()['index_id']
