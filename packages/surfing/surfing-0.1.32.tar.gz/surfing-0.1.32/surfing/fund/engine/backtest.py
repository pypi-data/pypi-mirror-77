...
class FundEngine:
    ...
    def __init__(self, data_manager: FundDataManager, trader, taa_params:TAAParam=None, fa_params:FAParam=None, taa_param_details:dict=None, is_old_version=False, white_list:set=None, black_list:set=None, fund_score_funcs: Optional[Dict[str, Union[ScoreFunc, str]]] = None):
    ...
    def calc_trade(self, dt, cur_asset_position: AssetPosition, cur_fund_position: FundPosition=None, is_real_trade=False):
        if not self.is_fund_backtest:
            ...
            v_fund_position, fund_trade_list, trigger_reason = self._trader.calc_trade(
                ...
                self._prep_fund_nav, self._prep_fund_score, self._prep_asset_price,self._prep_fund_unit_nav, is_real_trade
            )
...
class BasicFundBacktestEngine(object):
    ...
    def calc_trade(self, dt, cur_asset_position: AssetPosition, cur_fund_position: FundPosition=None, is_real_trade=False):
        ...
        for fund_id, weight in self._target_fund_weight[dt]:
            # TODO: what if fund_id does not belong to any index! e.g. 000001!0
            if fund_id in self._dm.dts.fund_index_map:
                index_id = self._dm.dts.fund_index_map[fund_id]
            else:
                index_id = 'others'
...
def test1():
    import json
    import pickle
    with open('/shared/robo-advisor/temp/data_manager', 'rb') as fp:
        data_manager = pickle.load(fp)

    t = BasicFundTrader()

    params = json.loads(untagged_params_str)
    ...
    b = BasicFundBacktestEngine(data_manager=data_manager, trader=t, fund_weight_list=fund_weight_list)
    b.init()
