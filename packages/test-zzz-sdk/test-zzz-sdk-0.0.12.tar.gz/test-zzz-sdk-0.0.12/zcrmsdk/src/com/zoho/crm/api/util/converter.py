    ...
    class Converter(ABC):
        ...
        def value_checker(self, class_name, member_name, key_details, value, unique_values_map, instance_number):
                ...
                if isinstance(value, list) and Constants.STRUCTURE_NAME in key_details:
                    ...
                    for each_instance in value:
                        if not isinstance(each_instance, class_holder):
                            ...
                            expected_type = Constants.LIST_KEY + '[' + structure_name + ']'
                            given_type = each_instance.__class__.__module__
                            break
            ...
            if not check:
                ...
                details_jo[Constants.ACCEPTED_TYPE] = expected_type
                details_jo[Constants.GIVEN_TYPE] = given_type

                raise SDKException(Constants.TYPE_ERROR, None, details_jo, traceback.format_stack(limit=6))

            if Constants.VALUES in key_details:
                ...
                if value not in values_ja:
                    ...
                    raise SDKException(Constants.UNACCEPTED_VALUES_ERROR, None, details=details_jo, cause=traceback.format_stack(limit=6))
            ...
            if Constants.UNIQUE in key_details:
                ...
                if value in values_array:
                    ...
                    raise SDKException(Constants.UNIQUE_KEY_ERROR, None, details=details_jo, cause=traceback.format_stack(limit=6))
                else:
            ...
            if Constants.MIN_LENGTH in key_details or Constants.MAX_LENGTH in key_details:
                ...
                if Constants.MAX_LENGTH in key_details and count > key_details[Constants.MAX_LENGTH]:
                    ...
                    raise SDKException(Constants.MAXIMUM_LENGTH_ERROR, None, details=details_jo, cause=traceback.format_stack(limit=6))
                ...
                if Constants.MIN_LENGTH in key_details and count < key_details[Constants.MIN_LENGTH]:
                    ...
                    raise SDKException(Constants.MINIMUM_LENGTH_ERROR, None, details=details_jo, cause=traceback.format_stack(limit=6))
            ...
            if Constants.REGEX in key_details:
                if re.search(value, key_details[Constants.REGEX]) is None:
                    ...
                    raise SDKException(Constants.REGEX_MISMATCH_ERROR, None, details=details_jo, cause=traceback.format_stack(limit=6))
        ...
        def value_checker(self, class_name, member_name, key_details, value, unique_values_map, instance_number):
            ...
            if Constants.VALUES in key_details:
                ...
                if value not in values_ja:
                    ...
                    details_jo[Constants.INDEX] = instance_number
                    details_jo[Constants.ACCEPTED_VALUES] = values_ja
                    raise SDKException(Constants.UNACCEPTED_VALUES_ERROR, None, details=details_jo,
                                      cause=traceback.format_stack(limit=6))
            ...
            if Constants.UNIQUE in key_details:
                ...
                if value in values_array:
                    ...
                    raise SDKException(Constants.UNIQUE_KEY_ERROR, None, details=details_jo,
                                      cause=traceback.format_stack(limit=6))
                else:
                    unique_values_map[name].append(value)
            ...
            if Constants.MIN_LENGTH in key_details or Constants.MAX_LENGTH in key_details:
                ...
                if Constants.MAX_LENGTH in key_details and count > key_details[Constants.MAX_LENGTH]:
                    ...
                    raise SDKException(Constants.MAXIMUM_LENGTH_ERROR, None, details=details_jo,
                                      cause=traceback.format_stack(limit=6))
                ...
                if Constants.MIN_LENGTH in key_details and count < key_details[Constants.MIN_LENGTH]:
                    ...
                    raise SDKException(Constants.MINIMUM_LENGTH_ERROR, None, details=details_jo,
                                      cause=traceback.format_stack(limit=6))
            ...
            if Constants.REGEX in key_details:
                ...
                if re.search(value, key_details[Constants.REGEX]) is None:
                    ...
                    raise SDKException(Constants.REGEX_MISMATCH_ERROR, None, details=details_jo,
                                       cause=traceback.format_stack(limit=6))
