...
class FormDataConverter(Converter):
    ...
    def form_request(self, request_instance, pack, instance_number):
        ...
        for member_name, member_detail in class_detail.items():
            ...
            try:
                ...
                FormDataConverter.logger.info(Constants.EXCEPTION_IS_KEY_MODIFIED + e.__str__())
            ...
            if modification is not None and modification != 0 and self.value_checker(
                ...
                if data_type == Constants.LIST_NAMESPACE:
                    ...
                    request[key_name] = self.form_request(field_value, member_detail.get(Constants.STRUCTURE_NAME), 1)
    ...
    def append_to_request(self, request_base, request_object):
        ...
        for key_name, key_value in request_object.items():
            if isinstance(key_value, StreamWrapper):
                form_data_request_body[key_name] = key_value.get_stream()
            else:
                form_data_request_body[key_name] = key_value
