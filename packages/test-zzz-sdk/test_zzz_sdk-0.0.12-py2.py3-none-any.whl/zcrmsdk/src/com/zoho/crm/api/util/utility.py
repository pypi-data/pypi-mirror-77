...
class Utility(object):
    ...
    def get_fields(module_api_name):
        ...
        with Utility.lock:
            ...
            if not os.path.exists(resources_path):
                if Utility.search_json_details(module_api_name) is not None:
                    return
            ...
            if os.path.exists(record_field_details_path):
                ...
                if Initializer.get_initializer().auto_refresh_fields and not Utility.new_file and not Utility.get_modified_modules:
                    Utility.get_modified_modules = True
                    Utility.modify_fields(record_field_details_path, record_field_details_json[
                        Constants.FIELDS_LAST_MODIFIED_TIME] if Constants.FIELDS_LAST_MODIFIED_TIME in record_field_details_json else None)
                    Utility.get_modified_modules = False

                if module_api_name in Initializer.get_json(record_field_details_path):
                    return

                else:
                    Utility.fill_data_type()
                    record_field_details_json[module_api_name] = {}
                    ...
                    record_field_details_json = Initializer.get_json(record_field_details_path)
                    record_field_details_json[module_api_name] = field_details
                    Utility.write_to_file(file_path=record_field_details_path, file_contents=record_field_details_json)

            elif Initializer.get_initializer().auto_refresh_fields:
                ...
                for module in module_api_names:
                    if module not in record_field_details_json:
                        record_field_details_json[module] = {}
                        ...
                        record_field_details_json = Initializer.get_json(record_field_details_path)
                        record_field_details_json[module] = field_details
                        Utility.write_to_file(file_path=record_field_details_path,
                                              file_contents=record_field_details_json)
                ...
                if isinstance(data_object, RelatedLists.ResponseWrapper):
                    ...
                    for related_list in related_lists:
                        related_list_detail = {
                            ...
                            Constants.MODULE: related_list.get_module(),
                            Constants.NAME: related_list.get_name(),
                            Constants.HREF: related_list.get_href()
                        }
    ...
    def get_fields_details(module_api_name):
        ...
        fields_details = {}
        response = Field.FieldsOperations(module_api_name).get_fields(None)
        ...
        if response is not None:
            ...
            if isinstance(data_object, Field.ResponseWrapper):
                ...
                if module_api_name in Constants.INVENTORY_MODULES:
                    field_detail = {
                        ...
                        Constants.STRUCTURE_NAME: Constants.LINETAX
                    }
    ...
    def set_data_type(field_detail, field, module_api_name):
        ...
        if key_name.lower() == Constants.PRODUCT_DETAILS.lower() and module_api_name in Constants.INVENTORY_MODULES:
        ...
        elif key_name.lower() == Constants.PRICING_DETAILS.lower() and module_api_name.lower() == Constants.PRICE_BOOKS.lower():
            field_detail[Constants.NAME] = key_name
            field_detail[Constants.TYPE] = Constants.LIST_NAMESPACE
            ...
            return
                ...
                module_api_name.lower() == Constants.EVENTS.lower() or module_api_name.lower() == Constants.ACTIVITIES.lower()):
            field_detail[Constants.NAME] = key_name
            field_detail[Constants.TYPE] = Constants.LIST_NAMESPACE
        ...
        elif key_name.lower() == Constants.COMMENTS.lower() and module_api_name.lower() == Constants.SOLUTIONS.lower():
            field_detail[Constants.NAME] = key_name
            field_detail[Constants.TYPE] = Constants.LIST_NAMESPACE
        ...
        if field.get_pick_list_values() is not None and len(field.get_pick_list_values()) > 0:
            values = list(map(lambda x: x.get_actual_value(), field.get_pick_list_values()))
