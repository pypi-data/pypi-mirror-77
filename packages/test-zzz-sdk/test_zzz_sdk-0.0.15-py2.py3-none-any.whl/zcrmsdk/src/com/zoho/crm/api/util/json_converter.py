...
class JSONConverter(Converter):
    ...
    def form_request(self, request_instance, pack, instance_number):
        ...
        if isinstance(request_instance, Record):
            module_api_name = self.common_api_handler.module_api_name
            return_json = self.is_record_request(request_instance, class_detail)
            self.common_api_handler.module_api_name = module_api_name
            return return_json

        else:
            return self.is_not_record_request(request_instance, class_name, class_detail, instance_number)

    def is_not_record_request(self, request_instance, class_name, class_detail, instance_number):
        try:
        ...
        for member_name, member_detail in class_detail.items():
            ...
            try:
                ...
                JSONConverter.logger.info(Constants.EXCEPTION_IS_KEY_MODIFIED + e.__str__())
            ...
            if (modification is None or modification == 0) and Constants.REQUIRED in member_detail and bool(
                ...
                raise SDKException(Constants.MANDATORY_VALUE_MISSING_ERROR,
                                   Constants.MANDATORY_KEY_MISSING_ERROR + member_name)

            field_value = getattr(request_instance,
                                  self.construct_private_member(class_name=class_name, member_name=member_name))

            if modification is not None and not modification == 0 and self.value_checker(class_name=class_name,
    ...
    def is_record_request(self, record_instance, class_detail):
        try:
            from zcrmsdk.src.com.zoho.crm.api.initializer import Initializer
        except Exception:
            from ..initializer import Initializer
        ...
        if module_api_name is not None:
            self.common_api_handler.module_api_name = None
        ...
        for key_name in key_values.keys():
            key_detail = {}
            key_value = key_values.get(key_name)
            json_value = None
            member_name = self.build_name(key_name)
            modification = key_modified.get(key_name)

            if len(module_detail) > 0 and (member_name in module_detail or key_name in module_detail):
                if key_name in module_detail:
            ...
            if len(key_detail) > 0:
                ...
                is_required = Constants.REQUIRED in key_detail and bool(key_detail[Constants.REQUIRED])

                if (modification is None or modification == 0) and is_required:
                    raise SDKException(Constants.MANDATORY_VALUE_MISSING_ERROR, Constants.MANDATORY_KEY_MISSING_ERROR + member_name)

                elif (modification is None or modification == 0) and not is_required:
                    continue
            ...
            if modification is None or modification == 0:
                continue
    ...
    def set_data(self, member_detail, field_value):
        if field_value is not None:
            ...
            if data_type == Constants.LIST_NAMESPACE:
                ...
                return self.is_record_request(field_value, self.__get_module_detail_from_user_spec_json(
                    member_detail[Constants.MODULE]))

            elif Constants.STRUCTURE_NAME in member_detail:
                return self.form_request(field_value, member_detail[Constants.STRUCTURE_NAME], 0)
        ...
        if len(request_objects) > 0:
            if member_detail is None or (member_detail is not None and Constants.STRUCTURE_NAME not in member_detail):
                ...
                if pack == Constants.CHOICE_NAMESPACE:
                    ...
                    for request in request_objects:
                        json_array.append(self.is_record_request(request, self.__get_module_detail_from_user_spec_json(
                            member_detail[Constants.MODULE])))
        ...
        if total_points == 0:
            ...
            for member_name in class_detail:
                ...
                if key_name is not None and key_name in response_json and response_json.get(key_name) is not None:
                    ...
                    structure_name = member_detail[Constants.STRUCTURE_NAME] if Constants.STRUCTURE_NAME in member_detail else None
                    ...
                    if data_type == member_detail[Constants.TYPE]:
                        matches += 1
