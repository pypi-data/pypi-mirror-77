...
def test_crawl_url_already_visited_not_collected(test_session, capsys):
    links = set()
    ...
    assert "already visited" in captured.out

def test_crawl_url_out_of_domain_not_collected(test_session, capsys):
    links = set()
    url='http://www.example.com/xmpl2'
    assert crawl(test_session,url,links,'www.not_correct_domain.com') == None
    captured = capsys.readouterr()
    assert "out of domain" in captured.out
