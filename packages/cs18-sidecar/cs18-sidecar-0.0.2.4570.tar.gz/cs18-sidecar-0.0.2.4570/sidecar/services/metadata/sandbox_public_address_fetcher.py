...
class AwsSandboxPublicAddressFetcher(SandboxPublicAddressFetcher):
    ...
    def _get_sandbox_public_ip(self) -> str:
        self._logger.info(f'**** _get_sandbox_public_ip: get stack {self._session.alb_stack_name}')

        stack = self._session.get_cf_resource().Stack(self._session.alb_stack_name)
        ...
        if not alb_dns:  # Backward compatibility
            # Get ALB arn
