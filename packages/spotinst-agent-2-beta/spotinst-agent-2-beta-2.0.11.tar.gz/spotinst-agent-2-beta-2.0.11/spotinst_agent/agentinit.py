...
def init__configuration(config, cloud_provider, debug=False):
    ...
    # if the user supply custom host we take it from env variables
    # else take the default from config file(spotinst-agent.yml)
    host = os.getenv('CUSTOM_HOST', config['default_host'])
