...
class Numbers(object):
  ...
  def to(self, number=0, title=None, label=None, icon=None, color=None, tooltip='', htmlCode=None,
             options=None, helper=None, width=(150, 'px'), profile=None):
    """
    ...
    if options is not None:
      dflt_options.update(options)
    html_number = html.HtmlText.Numeric(self.context.rptObj, 0, title, label, icon, color, tooltip, htmlCode,
                                        dflt_options, helper, width, profile)
    html_number.style.css.font_factor(15)
    self.context.rptObj.body.onReady([
      self.context.rptObj.js.objects.number(0, varName="%s_counter" % html_number.htmlCode, setVar=True),
      self.context.rptObj.js.while_(self.context.rptObj.js.objects.number.get("%s_counter" % html_number.htmlCode) < number, [
          self.context.rptObj.js.objects.number(self.context.rptObj.js.objects.number.get("%s_counter" % html_number.htmlCode) + 1, varName="%s_counter" % html_number.htmlCode, setVar=True),
          html_number.build(self.context.rptObj.js.objects.number.get("%s_counter" % html_number.htmlCode)),
      ]).next("")
    ])
    return html_number
