...
"""
Register module for tpDcc-libs-python
"""
...
def register_class(cls_name, cls, is_unique=True, skip_store=False):
    ...
    if REGISTER_ATTR not in tpDcc.libs.python.__dict__:
        tpDcc.libs.python.__dict__[REGISTER_ATTR] = list()

    if not is_unique and cls_name in tpDcc.libs.python.__dict__:
        return

    tpDcc.libs.python.__dict__[cls_name] = cls
    if not skip_store:
        tpDcc.libs.python.__dict__[REGISTER_ATTR].append(cls_name)
