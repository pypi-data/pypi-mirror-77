...
class VisaSession(object):
	...
	def __init__(self, resource_name: str, settings: InstrumentSettings, direct_session=None):
		...
		if self.reusing_session:
			# Reuse the session
			assert isinstance(direct_session, visa.Resource) or isinstance(direct_session, SocketIo), f"Direct_session must be a VISA resource object. Actual type: '{type(direct_session)}', value: '{direct_session}'"
	...
	@classmethod
	def get_resource_manager(cls, visa_select: str) -> visa.ResourceManager:
		...
		if visa_select.lower() in ['@ni', 'ni', 'visa-ni', 'nivisa', 'ni-visa', 'nationalinstruments', 'nationalinstrumentsvisa']:
			return visa.ResourceManager('@ni')
		...
		if 'rohde&schwarz' in visa_select.lower() or 'rohdeschwarz' in visa_select.lower() or visa_select.lower() == 'rsvisa' or visa_select.lower() == 'rs' or visa_select.lower() == 'r&s':
			if operating_system == 'windows':
				...
				return visa.ResourceManager(visa_select)
	...
	def _get_visa_manufacturer(self) -> str:
		...
		try:
			return self._rm.visalib.get_attribute(self._rm.session, visa.constants.VI_ATTR_RSRC_MANF_NAME)[0]
		except TypeError:
	...
	def _last_status_more_data_available(self):
		...
		return self.last_status == visa.constants.StatusCode.success_max_count_read
	...
	def _parse_bin_data_header(self, exc_if_not_bin: bool) -> Tuple[ReadDataType, str, int]:
		...
		if exc_if_not_bin:
			...
			if self.last_status == visa.constants.StatusCode.success_max_count_read:
				self._flush_junk_data()
	...
	def _read_bin_block_known_len(self, stream: StreamWriter, length: int) -> None:
		...
		try:
			...
			left_to_read = length
			self.last_status = visa.constants.StatusCode.success
			with self._session.ignore_warning(visa.constants.StatusCode.success_max_count_read):
				chunk_ix = 0
