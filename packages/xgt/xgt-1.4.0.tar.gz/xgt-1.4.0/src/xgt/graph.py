...
class TableFrame(object):
  ...
  def load(self, paths, headerMode=HeaderMode.NONE, **kwargs):
    """
    ...
    **kwargs
    ...
    if len(server_paths) > 0:
      return self._ingest(server_paths, headerMode, **kwargs)
    if len(url_paths) > 0:
      return self._ingest(url_paths, headerMode, **kwargs)

  def save(self, path, offset=0, length=None, headers=False, **kwargs):
    """
    ...
    **kwargs
    ...
    if len(server_paths) > 0:
      return self._egest(server_paths[0], headers, **kwargs)
