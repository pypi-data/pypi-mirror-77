...
from .common import (_assert_noerrors, _validated_property_name,
                     _validated_schema, _validated_frame_name, _to_str,
...
class Connection(object):
  ...
  def _launch_job(self, query, **kwargs):
    ...
    if 'optlevel' not in kwargs:
      kwargs['optlevel'] = self.current_opt_level
  ...
  def max_user_memory_size(self):
    ...
    return response.pool_size / Connection.GIB
  ...
  def free_user_memory_size(self):
    ...
    return response.free_memory_size / Connection.GIB
  ...
  def get_table_frames(self,names=None, namespace=None):
    ...
    if namespace is not None:
      request.namespace_name = _validated_frame_name(namespace)
    ...
    for data in response.container:
      ...
      frames.append(TableFrame(self, data.name, schema, data.error_frame_name))
    return frames
  ...
  #------------------------- DDL Methods
  def _container_security_labels_helper(self, request, security_labels):
    for access in security_labels:
      ...
      for l in security_labels[access]:
        request.crud_label_map.access_labels[access_lower].label.append(l)
    return request

  def create_namespace(self, name, security_labels = None):
    """
    ...
    if security_labels is not None:
      request = self._container_security_labels_helper(request, security_labels)
    self._call(request, self._graph_svc.CreateNamespace)

  def create_table_frame(self, name, schema, security_labels = None):
    """
    ...
    frame = TableFrame(self, data.name, schema_returned, data.error_frame_name)
    return frame

  def create_vertex_frame(self, name, schema, key, security_labels = None):
    """
  ...
  def create_edge_frame(self, name, schema, source, target, source_key,
                        target_key, security_labels = None):
    """
  ...
  def drop_namespace(self, namespace, force_drop = False):
    """
  ...
  def get_frame_security_labels(self, frame):
    """
    ...
    try:
      access_labels = response.crud_label_map.access_labels
    except:
      return response
    label_map = dict()
  ...
  def set_optimization_level(self, optlevel = DEFAULT_OPT_LEVEL):
    ...
    -------
    XgtValueError
      If the level requested is not valid.
    """
  ...
  def run_job(self, query, **kwargs):
    ...
    **kwargs
      Keyword arguments which are used for job policies, where the key is the
      policy name and the value is the policy setting. Possible policies are:

      - record_history (bool)
        If true, records the history of the job.
        Defaults to true.
      - description (str)
        Optional description of the job.  If no description is given this will
        default to the query text.
      - timeout (int)
        Maximum number of seconds that the query should take before being
        automatically canceled.
        Defaults to no limit on the query time.
      - optlevel (int)
        ...
        Defaults to query optimization level 4.
  ...
  def schedule_job(self, query, **kwargs):
    ...
    **kwargs
    ...
    >>> job = conn.schedule_job(query, timeout=200)
    >>> print(job)
