...
def show_user(ctx, email, org_id):
    ...
    if _user:
        output_entry(ctx, json.loads(_user))
...
def add_user_role(ctx, email, application, role):
    ...
    if _user:
        ...
        output_entry(ctx, json.loads(users.get_user(ctx, _user["id"])))
...
def list_groups(ctx, organisation, org_id, type, **kwargs):
    ...
    users_groups = users.query(ctx, org_id, type="group", **kwargs)
...
def add_application(ctx, name, org_id, category):
    output_entry(ctx, json.loads(apps.add(ctx, name, org_id, category)))
...
def list_files(ctx, **kwargs):
    ...
    table = PrettyTable(
        ["id", "name", "tag", "label", "created", "last_accessed", "size"]
    )
    table.align = "l"
    for _file in _files:
...
def update_client(ctx, **kwargs):
    output_entry(ctx, issuers.update_client(ctx, **kwargs,))
