...
def cli_test(context, endpoint_url):
    ...
    if not endpoint_url:
        if os.environ.get('COTOBA_ENDPOINT_URL'):
            ...
            raise click.UsageError('endpoint-url is not set.')
...
def compare(
    ...
    try:
        with open(test_file, encoding='utf-8') as f:
            try:
                ...
                raise click.UsageError(f'{test_file} is invalid json file. {str(e)}')
            test_list = df.get('test')
    except OSError as e:
        raise click.UsageError(str(e))
    if test_list is None:
        raise click.UsageError('Test data is empty.')
    for test_elm in test_list:
        ...
        click.secho('response:' +
                    res.get('response'),
                    fg='bright_magenta')
        # TODO: check test_elm has proper keys.
        ...
        for key in compare_keys:
            if key not in expected:
                continue
            if res.get(key) in expected[key]:
                result = 'OK'
            else:
                result = 'NG'
            click.echo(f'compare_result[{key}]: {result}'.format(key, result))
...
def get_conversations_history(auth,
                              ...
                              history_size=None,
                              endpoint_url=None):
    debug_api_response = platform.debug_bot(auth,
                                            bot_id=bot_id,
                                            api_key=api_key,
                                            user_id=user_id,
                                            endpoint_url=endpoint_url)
