...
def process_block_header(epochs_ctx: EpochsContext, state: BeaconState, block: BeaconBlock) -> None:
    ...
    print("block.parent_root", block.parent_root)
    print("hash_tree_root(state.latest_block_header)", hash_tree_root(state.latest_block_header))
    ...
    print("process header state.latest_block_header, state root null", hash_tree_root(state.latest_block_header))
...
def process_slot(epochs_ctx: EpochsContext, state: BeaconState) -> None:
    ...
    if state.latest_block_header.state_root == Bytes32():
        ...
        print("  changing lbh state root, after change", hash_tree_root(state.latest_block_header))
    # Cache block root
