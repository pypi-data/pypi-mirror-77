...
data_base_dir: ../../../ # base dir for data upload conf eg, examples/data/breast_hetero_guest.csv -> $data_base_dir/examples/data/breast_hetero_guest.csv
parties:
  guest: [10000]
  host: [9999, 10000]
  arbiter: [9999]

services: # services of fate flow
  - parties: [9999, 10000]
    address: 127.0.0.1:9380

# open the ssh tunnel(s) if the remote service
...
#
#ssh_tunnel:
#  - ssh_address: <remote ip>:<remote port> # format: ip:port, ssh address to login
...
#    services: [127.0.0.1:9380] # each service has one local binding port
"""
    ...
    def __init__(self, config):
        ...
        for service_config in config["services"]:
            ...
            if service_config.get("ssh_tunnel", {}).get("enable", False):
                ...
                self.tunnel_id_to_tunnel[tunnel_id] = self.tunnel(tunnel_config["ssh_address"],
...
Config.load("testsuite_config.yaml")
