...
def test_mq_delay_produce():
    ...
    import json
    produce = RabbitProducer('rabbit_88', delay=1, dlx_queue='lk_relay', dlx_exchange='lk_relay_exchange', dlx_routing='lk_relay', queue_ttl=3 * 60000)
    for _k in range(1, 20):
        st = {'index': _k, 'time': now_time(), 'q_ttl': 3 * 60000, 'msg_ttl': 30000}
        if _k % 2 == 0:
            produce.send_safely(topic='lk_alg_delay_queue', message=json.dumps(st), msg_ttl=str(10000))
        else:
