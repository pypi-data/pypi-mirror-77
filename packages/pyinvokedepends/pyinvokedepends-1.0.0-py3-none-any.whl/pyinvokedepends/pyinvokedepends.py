...
def dependencies(*args, **kvargs):
    ...
    if len(args)>0:
        if not isinstance(args[0], tasks.Task) or len(args)!=1:
            raise ValueError("@dependencies only possible with kwargs")
        warnings.warn('@dependencies without arguments will do nothing', SyntaxWarning)
        kvargs['checks']=['.']
        kvargs['creates']=['.']
    ...
    if n in (2,0):
        if n==0:
            warnings.warn('@dependencies without "checks" and "creates" will do nothing', SyntaxWarning)
            kvargs['checks']=['.']
            kvargs['creates']=['.']
        return decorator
    raise ValueError('@dependencies needs both "checks" and "creates"')
