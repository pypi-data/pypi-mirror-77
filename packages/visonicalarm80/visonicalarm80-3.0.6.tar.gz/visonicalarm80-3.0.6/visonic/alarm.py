...
class System(object):
    ...
    __system_devices = []
    __scheduler = None
    __scheduler_thread = None
    __scheduler_event = None
    ...
    def __init__(self, hostname, app_id, user_code, user_email, user_password, panel_id, partition):
        ...
        self.__scheduler_thread = threading.Thread(target=self.__scheduler.run, daemon=True, name="state_schedule_thread")

    def _run_scheduled_func(self, timer_seconds, timer_function, token, func):
        timer_function(token, func)
        self.schedule(timer_seconds, timer_function, token, func)

    def schedule(self, timer_seconds, timer_function, token, func):
        self.__scheduler.enter(timer_seconds, 1, self._run_scheduled_func,(timer_seconds, timer_function, token, func))

        if not self.__scheduler_thread.isAlive():
    ...
    def disarm(self):
        ...
        res = self.__api.disarm(self.__api.partition)

        self.ProcessStatus(res['process_token'], "DISARM")

    def arm_home(self):
        """ Send Arm Home command to the alarm system. """
        res = self.__api.arm_home(self.__api.partition)
        
        self.ProcessStatus(res['process_token'], "HOME")

    def arm_away(self):
        """ Send Arm Away command to the alarm system. """
        res = self.__api.arm_away(self.__api.partition)
        
        self.ProcessStatus(res['process_token'], "AWAY")

    def ProcessStatus(self, token, function):
        logging.debug("Calling get_process_status with token " + token)
        res = self.__api.get_process_status(token)
        
        if res == 'handled':
            if function == 'AWAY':
                self.__system_state = 'EXITDELAYAWAY'
            elif function == 'HOME':
                self.__system_state = 'EXITDELAYHOME'
            elif function == 'DISARM':
                self.__system_state = 'ENTRYDELAY'

            time.sleep(1)
            self.ProcessStatus(token, function)
        elif res == 'succeeded':
            self.update_status()
            
            logging.debug("Succeded.. updating status" )
            return
        else:
            logging.debug("Stuck processstatus?")
            return
