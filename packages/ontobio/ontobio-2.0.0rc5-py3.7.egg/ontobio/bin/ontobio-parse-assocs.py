...
def main():
    ...
    logging.info("Welcome FOO!!")

    handle = args.resource

    # Ontology Factory
    logging.info("{}: Creating factory".format(time.time()))
    ofactory = OntologyFactory()
    logging.info("{}: Creating ont object from: {} {}".format(time.time(), handle, ofactory))
    ont = ofactory.create(handle)
    logging.info("{}: ont: {}".format(time.time(), ont))
