...
class Model(Connection):
    ...
    def is_data_ready(self) -> bool:
        ...
        if any([model['state'] == 'done' for model in r.json() if model['id'] == self.id]):
            return True
        if any([model['state'] == 'error' for model in r.json() if model['id'] == self.id]):
    ...
    def generate_training_data(self) -> None:
        ...
        for _ in tqdm(range(0, 10), total=10):
