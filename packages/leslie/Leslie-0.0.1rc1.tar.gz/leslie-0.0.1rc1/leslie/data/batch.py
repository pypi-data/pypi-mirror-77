...
from leslie.data.vocabulary import Vocabulary
from leslie.common.util import ensure_list
...
class Batch(Iterable):
    ...
    ) -> Dict[str, Union[tf.Tensor, Dict[str, tf.Tensor]]]:
        # This complex return type is actually predefined elsewhere as a DataArray,
        # but we can't use it because my py doesn't like it.
        """
        This method converts this ``Batch`` into a set of tf.Tensors that can be passed
    ...
    def print_statistics(self) -> None:
        ...
        print("\n10 Random instances: ")
        for i in list(np.random.randint(len(self.instances), size=10)):
            print(f"Instance {i}:")
