    ...
    async def gather_data(self, ipt, ids, *args, **kwargs):
        ...
        if not self.rethinkdb_origin.get(ids):
            ...
            try:
                ...
                rprint(f"{idex}: Conectando a rdb-> RC {rc}, {code_db}")
                await shield(wait_for(rc.async_connect(), timeout=120))
                await asyncio.sleep(60)
                rprint(f"{idex}: OK RC, {code_db}")
                ...
                bprint(f"Obteniendo indexes  {code_db}, {table_name}")
        ...
        """
        if not self.rethinkdb_origin_enu.get(ids):
            # las dos instancias a dbs rethinkdb
            # rc: origen
            # rec_enu: destino
            rprint(f"Creando instancia destino para {table_name}")
            if not rc_enu:
                rc_enu = self.add_sta_instance_destiny(ids, loop)
                await asyncio.sleep(3)
            try:
                bprint(f"RC ENU OK {rc_enu}")
