...
def clustalo(temp_fastaText):
    fp = tempfile.TemporaryFile(mode='w',delete=False)
    os.system("clustalo -i "+temp_fastaText+" -o "+fp.name+" --auto --outfmt clu --force")
...
def getCons(sequences):
    ...
    align_str = []
    if len(seq_list) > 1:
        align = clustalo(fastaText.name)
        align2 = []
        for i in align:
            align2.append(list(i.seq))
            align_str.append(str(i.seq))
        ...
        align_str.append(consensus)
    
    os.unlink(fastaText.name)
    return consensus, align_str
    
def getCons_text(sequences,detailing='dp'):
    consensus=getCons(sequences)
...
def removePattern(fastatxt,rex):
    fastatxt = list(fastatxt)
    for i in range(0,len(fastatxt)):
        for ii in rex:
            s = re.sub(ii,'',str(fastatxt[i].seq)) # find and remove id
            fastatxt[i].seq=Seq(s)
    return fastatxt
