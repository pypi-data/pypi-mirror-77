...
class ChannelClient(MLSClient):
    ...
    def __init__(
        ...
        ```
        channel_client = ChannelClient(env=MLSENV.STG, username="mls_account", password="mls_password")
        ```
        """
    ...
    def create_channel(self, name: str, experiments: List["Experiment"]) -> Channel:
        ...
        ```
        """
        assert type(experiments) == list, "`experiments`는 Experiemnt객체를 담은 list타입이어야 합니다."
