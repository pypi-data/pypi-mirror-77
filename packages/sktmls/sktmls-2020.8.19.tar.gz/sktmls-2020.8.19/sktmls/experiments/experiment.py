...
from sktmls.models import ManualModel, MLModel, MLModelClient
...
class Bucket:
    ...
    def generate_request_body(self) -> Dict:
        ...
        ml_model_id = self.prediction.ml_model
        model_type = self.prediction.request["type"]

        data["prediction_type"] = model_type
        if model_type == "online":
            data["online_model_id"] = ml_model_id
        elif model_type == "batch":
            data["batch_model_id"] = ml_model_id
...
class ExperimentClient(MLSClient):
    ...
    def __init__(
        ...
        ```
        experiment_client = ExperimentClient(env=MLSENV.STG, username="mls_account", password="mls_password")
        ```
        """
    ...
    def update_bucket_ratio(self, experiment: Experiment, bucket_ratio: Dict[Bucket, int]) -> List[Bucket]:
        ...
        response = self._request(
            method="PUT", url=f"{MLS_EXPERIMENTS_API_URL}/{experiment.id}/bucket_ratio", data=data,
        ).results
    ...
    def create_bucket(
        ...
        - model: (optional) (`sktmls.models.MLModel`) 버킷에 연결할 모델
    ...
    def list_buckets(self, experiment: Experiment, **kwargs) -> List[Bucket]:
        ...
        ml_model_client = MLModelClient(
            env=self._MLSClient__env, username=self._MLSClient__username, password=self._MLSClient__password
        )
    ...
    def update_bucket(
        ...
        - model: (optional) (`sktmls.models.MLModel`) 수정할 버킷에 연결할 모델
        - static_value: (optional) (dict) (고정값일 경우) Static Value
        ...
        my_bucket = experiment_client.get_bucket(
            name="my_bucket"
        )
