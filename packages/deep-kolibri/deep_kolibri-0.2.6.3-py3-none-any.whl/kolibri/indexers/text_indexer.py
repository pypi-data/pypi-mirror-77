...
class TextIndexer(BaseIndexer):
    ...
    def transform(self,
        ...
        if seq_length is None:
            seq_length_from = "max length_train of the samples"
            seq_length = max([len(i) for i in samples]) + 2
        if max_position is not None and max_position < seq_length:
            seq_length_from = "max embedding seq length_train"
            seq_length = max_position

        if seq_length_from and not self._showed_seq_len_warning:
            logger.warning(
                f'Sequence length_train is None, will use the {seq_length_from}, which is {seq_length}')
