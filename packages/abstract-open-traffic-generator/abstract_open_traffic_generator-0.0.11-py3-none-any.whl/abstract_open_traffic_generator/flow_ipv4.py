...
class Priority(object):
    ...
    - dscp (Union[Dscp, type(None)]): Differentiated services code point (DSCP) packet field. PHB (per-hop-behavior) value is 6 bits: >=0 PHB <=63. ECN (explicit-congestion-notification) value is 2 bits: >=0 ECN <=3
...
class Dscp(object):
    ...
    Differentiated services code point (DSCP) packet field. PHB (per
    hop
    behavior) value is 6 bits: >=0 PHB <=63. ECN (explicit
    congestion
    notification) value is 2 bits: >=0 ECN <=3
