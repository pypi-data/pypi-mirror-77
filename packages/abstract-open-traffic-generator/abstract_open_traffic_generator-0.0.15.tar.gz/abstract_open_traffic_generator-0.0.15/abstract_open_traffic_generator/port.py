...
class Port(object):
    ...
    - location (Union[str, None]): The location of a test port
        ...
        local interface
        virtual machine, docker container, kubernetes cluster The test port location format is implementation specific
        Use the /results API to determine what formats the implementation supports for the location property
    - link_state (Union[up, down, None]): The configured link state of the port
        Compare the actual state vs the configured state by using the /results API
    - capture_state (Union[started, stopped, None]): The configured capture state of the port
        Compare the actual state vs the configured state by using the /results API
    """
