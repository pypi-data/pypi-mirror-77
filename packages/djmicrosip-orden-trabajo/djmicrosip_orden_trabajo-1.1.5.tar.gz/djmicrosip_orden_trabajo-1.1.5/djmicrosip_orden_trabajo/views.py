...
def get_venta(request):
	...
	vrntas=ventas.order_by('-folio')
	ventas=ventas.values_list('id','descripcion','folio','cliente__id','cliente__nombre','vendedor__id','vendedor__nombre')
	print("pag ini")
...
@login_required(login_url='/login/')
def add_pedido(request,id,template_name='djmicrosip_orden_trabajo/add_pedido.html'):
	#mandamos llamar el articulo predeterminado en las preferencias
			...
			if formset.is_valid():
				...
				if form_crm.is_valid():
					...
					if pedido:
						...
						tipo_llamada=tipo_llamada,llamada=llamada,fecha_registro=fecha_registro,preprogramado=preprogramado)
