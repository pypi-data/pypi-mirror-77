...
class Clip(NewReferenceObject):
    ...
    def _process_ties(self, chords):
        ...
        for (i, slot) in enumerate(chords):
            ...
            if i == 0 and (slot and len(slot.notes) and not slot.notes[0].tie):
                previous_chord = slot

            is_tie = False
            if slot and len(slot.notes) and slot.notes[0].tie:
                is_tie = True
            ...
            if not is_tie:
                if previous_was_tie:
                    previous_chord.with_tied(previous_chord.notes[0].length * tied)
                tied = 0

            previous_was_tie = is_tie

            if slot and len(slot.notes) and not slot.notes[0].tie:
                previous_chord = slot
                ...
                for note in slot.notes:
                    ...
                    if note is not None and not note.tie and not note.muted:
                        ...
                        if not note.repeat:
                            ...
                            event1 = Event(type=NOTE_ON, note=note,time=int(note.start_time))
                            events.append(event1)
                        else:
                            delta = (note.end_time - note.start_time) / (note.repeat + 1)
                            for x in range(0, note.repeat):
                                n1 = note.copy()
                                n1.length = delta
                                n1.start_time = note.start_time + (x*delta)
                                n1.end_time = n1.start_time + delta
                                if x == note.repeat:
                                    n1.length = n1.length + tie_bonus
                                    n1.end_time = n1.end_time + tie_bonus
                                event2 = Event(type=NOTE_ON, note=n1, time=int(n1.start_time))
