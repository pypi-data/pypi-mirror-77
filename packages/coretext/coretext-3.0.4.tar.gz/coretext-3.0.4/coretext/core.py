...
def get_stoplists():
    ...
    stoplists_dir = os.path.join(
        os.path.dirname(sys.modules['coretext'].__file__), 'stoplists')
...
def get_stoplist(language):
    ...
    stoplist_contents = pkgutil.get_data('coretext',
        os.path.join('stoplists', language + '.txt')).decode('utf-8')
...
def decode_html(html_string, encoding=None, default_encoding=DEFAULT_ENCODING,
    ...
    for re_meta in (re_meta1, re_meta2, re_meta3, re_meta4, re_meta5):
        ...
        if m:
            meta_encoding = m.group(1).decode('ascii')
            try:
...
def coretext(html_text, stoplist=None, length_low=LENGTH_LOW_DEFAULT,
        length_high=LENGTH_HIGH_DEFAULT, stopwords_low=0,
        stopwords_high=0, max_link_density=MAX_LINK_DENSITY_DEFAULT,
        max_good_distance=MAX_GOOD_DISTANCE_DEFAULT,
        max_heading_distance=MAX_HEADING_DISTANCE_DEFAULT, no_headings=NO_HEADINGS_DEFAULT,
        encoding=None, default_encoding=DEFAULT_ENCODING,
        enc_errors=DEFAULT_ENC_ERRORS):

    stoplist = get_stoplist('Thai') if None == stoplist else stoplist

    return justext(html_text, stoplist, length_low,
        length_high, stopwords_low,
        stopwords_high, max_link_density,
        max_good_distance,
        max_heading_distance, no_headings,
...
def main():
    import getopt
    import io
    from coretext import __version__ as VERSION
