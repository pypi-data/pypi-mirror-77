...
def _compute_q_values(policy, model, obs, obs_space, action_space):
    # action_mask = tf.cast(obs[:, 0:300], tf.float32)
    action_mask = tf.cast(obs[:, 0:100], tf.float32)
    config = policy.config
    ...
    if config["dueling"]:
        ...
        if config["num_atoms"] > 1:
            ...
            # value = state_score + action_scores_centered + inf_mask

            value = state_score + action_scores_centered
            paddings = tf.ones_like(value) * (-2 ** 32 + 1)
            value = tf.where(tf.equal(mask, 1.0), value, paddings)
    else:
        value = action_scores
