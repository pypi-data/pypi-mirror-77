...
class Generator:
    def __init__(self, l1_mask, l2_mask, l3_mask, t0_mask, t1_mask, t2_mask, t3_mask, t4_mask, l0_ssr_mask, taohua_mask, new_item=None, is_training=True, user=None, item_cand=None, lr=0.001, temperature=1, item_id_map=None, train_sample='random', predict_sample='random'):
        # 输入特征
        ...
        self.new_item = tf.constant(new_item, dtype=tf.int64)
        ...
        with tf.variable_scope("decoder"):
            ...
            if item_id_map is not None:
                ...
                random_ssr = tf.random_uniform([1],0,len(new_item),dtype=tf.int64)
                random_ssr = tf.gather(self.new_item,random_ssr)
                # random_ssr = tf.py_func(np.random.choice, [self.new_item, 1], tf.int64)
                random_ssr = tf.tile(random_ssr[tf.newaxis, :], [batch_size, 9])

                mask = tf.convert_to_tensor([0, 0, 0, 0, 0, 0, 1, 0, 0], dtype=tf.int64)
                new_fragment = tf.cast(tf.less(tf.random_uniform([1], 0, 1, dtype=tf.float32), 0.05), tf.int64)
                mask = new_fragment * mask
                mask = tf.tile(mask[tf.newaxis, :], [batch_size, 1])
                mask = tf.cast(mask, dtype=tf.bool)

                self.custom_result_map = tf.where(mask, random_ssr, self.custom_result_map)
                ...
                # self.custom_result_map_random = tf.as_string(self.custom_result_map_random)
                # self.custom_result_map_random = tf.identity(self.custom_result_map_random, 'custom_result_map_random')
