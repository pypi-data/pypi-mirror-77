...
import os, sys
import signal
import subprocess # subprocess module depends on Python version
...
class FormatedPs(object):
    ...
    def __init__(self, format_list):
        ...
        _processes = subprocess.getoutput('ps -eo "' + ' '.join(format_list) + '"').split('\n')
...
def __searchForChildren(pid, processes=None):
    if processes == None: processes = [[int(x[0]), int(x[1]), x[2]] for x in FormatedPs(['p', 'P', 'a']).processes]
...
def printTree(topParentId=1, maxWidth=None):
    ...
    processes = [[int(x[0]), int(x[1]), x[2]] for x in FormatedPs(['p', 'P', 'a']).processes]
    processNames = {}
...
def callCommand(cmd, *args, **kwargs):
    ...
    if "report_failure" in list(kwargs.keys()) :
...
class RetryCommand(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        retval = self._command(*args, **kwargs)
