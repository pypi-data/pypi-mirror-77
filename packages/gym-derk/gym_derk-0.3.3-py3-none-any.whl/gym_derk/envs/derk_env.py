...
os.environ['PYPPETEER_CHROMIUM_REVISION'] = '798057' if platform == 'win32' else ('798580' if platform == 'linux' else '798027')
...
class DerkEnv(gym.Env):
  ...
  def __init__(self, mode: str=None, host: str=None,
      ...
      headless: bool=False, chrome_executable: str=None, browser: pyppeteer.browser.Browser=None,
      safe_reset: bool=None):
    self.mode = mode if mode is not None else os.environ.get('DERK_MODE', 'train')
    self.host = host if host is not None else os.environ.get('DERK_HOST', ('file://' + os.path.abspath(__file__ + '/../../app_build/index.html')))
    ...
    if (not self.dummy_mode):
      asyncio.get_event_loop().run_until_complete(self._async_init_game())
    else:
      self.n_senses = 32
  ...
  def step(self, action_n: List[List[float]]) -> Tuple[np.ndarray, np.ndarray, List[bool], List[Dict]]:
    """Run one timestep.
  ...
  async def _async_init_game(self):
  ...
  async def async_reset(self):
    ...
    observations = await self.page.evaluate('''(config) => window.derk.reset(config)''', config)
  ...
  async def async_step(self, action_n):
    ...
    res = await self.page.evaluate('''(actions) => window.derk.step(actions)''', base64_actions)
  ...
  def _decode_observations(self, observations):
    ...
    return obs.reshape((-1, self.n_agents, 4)).swapaxes(0, 1).reshape((self.n_agents, -1))
