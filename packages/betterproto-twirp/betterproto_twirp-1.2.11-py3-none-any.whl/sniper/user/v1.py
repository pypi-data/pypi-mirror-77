...
class TakeGiftPackageReq(betterproto.Message):
    # 相关联ID等(气泡ID, 运营弹窗ID, 口令弹窗ID)
    id: int = betterproto.int32_field(1)
    # 礼包相关联类型 1 首页运营弹窗 2 浮动气泡 3 口令弹窗
...
class UserStub(betterproto.ServiceStub):
    ...
    @allure.step("""获取验证码初始化信息(验证真人)""")
    def get_validation(self, request: EmptyRequest) -> GetValidationResp:
        """获取验证码初始化信息(验证真人)"""
        logging.getLogger(__name__).info(
            "start call " + "[get_validation]" + " api" + "=>" + """获取验证码初始化信息(验证真人)"""
        )
    ...
    @allure.step("""发送各类用户短消息""")
    def send_sms(self, request: SendSmsReq) -> SendSmsResp:
        """发送各类用户短消息"""
        logging.getLogger(__name__).info(
            "start call " + "[send_sms]" + " api" + "=>" + """发送各类用户短消息"""
        )
    ...
    @allure.step(
        """
        解除风控用户冻结 异常code 100011: "短信验证码错误", 100013: "短信验证码已过期，请重新发送", 100017:
        "短信验证错误次数过多,请重新发送",
        """
    )
