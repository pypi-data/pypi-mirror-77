...
class FissionInfo(betterproto.Message):
    ...
    # 已获得漫读券
    coupons: int = betterproto.int32_field(3)
    # 挂件时长
    pendant_duration: int = betterproto.int32_field(4)


@dataclass
class GetFissionRewardReq(betterproto.Message):
    # 邀请人 ID
    refid: int = betterproto.int64_field(1)
...
@dataclass
class GetFissionRankInfoResp(betterproto.Message):
    code: int = betterproto.int32_field(1)
    msg: str = betterproto.string_field(2)
...
class RankInfo(betterproto.Message):
    ...
    rank: int = betterproto.int32_field(1)
    # 前十名
    ranks: List["PersonRank"] = betterproto.message_field(2)


@dataclass
class PersonRank(betterproto.Message):
    # 用户名
    name: str = betterproto.string_field(1)
    # 邀请人
    invited_count: int = betterproto.int32_field(2)
    # 当前排名可获取奖品
    prize_name: str = betterproto.string_field(3)
    # UID
...
class ActivityStub(betterproto.ServiceStub):
    ...
    @allure.step("""获取裂变活动排名信息""")
    def get_fission_rank_info(self, request: EmptyRequest) -> GetFissionRankInfoResp:
        """获取裂变活动排名信息"""
        logging.getLogger(__name__).info(
            "start call " + "[get_fission_rank_info]" + " api" + "=>" + """获取裂变活动排名信息"""
        )
        response = GetFissionRankInfoResp()
        self._do_twirp_rpc(
            "/activity.v2.Activity/GetFissionRankInfo", request, response
        )
        logging.getLogger(__name__).info(
            "call " + "[get_fission_rank_info]" + " api success!"
        )
        return response
