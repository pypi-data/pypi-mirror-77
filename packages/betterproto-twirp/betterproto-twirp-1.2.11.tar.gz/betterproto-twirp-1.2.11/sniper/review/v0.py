...
@dataclass
class CleanReviewReq(betterproto.Message):
    id: int = betterproto.int64_field(1)
...
class ReviewStub(betterproto.ServiceStub):
    ...
    @allure.step("""清空举报内容""")
    def clean_review(self, request: CleanReviewReq) -> DefaultResponse:
        """清空举报内容"""
        logging.getLogger(__name__).info(
            "start call " + "[clean_review]" + " api" + "=>" + """清空举报内容"""
        )
        response = DefaultResponse()
        self._do_twirp_rpc("/review.v0.Review/CleanReview", request, response)
        logging.getLogger(__name__).info("call " + "[clean_review]" + " api success!")
        return response
