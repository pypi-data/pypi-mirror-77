...
class GetCommonBannerReq(betterproto.Message):
    # banner 类型  1-评论小黄条 5-漫画详情页 banner  6-月票榜   7-打赏页
    type: int = betterproto.int32_field(1)
    # 漫画 id
...
class ComicDetail(betterproto.Message):
    ...
    # 整本漫画折扣类型： 0、无折扣 1、折扣 2、限免 3、指定章节限免
...
@dataclass
class ImageClip(betterproto.Message):
    """我也不明白这些神奇字母的含义"""

    r: int = betterproto.int32_field(1)
    b: int = betterproto.int32_field(2)
    t: int = betterproto.int32_field(3)
    l: int = betterproto.int32_field(4)
    pic: int = betterproto.int32_field(5)


@dataclass
class ImageSize(betterproto.Message):
    # 图片宽度
    cx: int = betterproto.int32_field(1)
    # 图片高度
    cy: int = betterproto.int32_field(2)


@dataclass
class ImageIndex(betterproto.Message):
    """https://info.bilibili.co/pages/viewpage.action?pageId=13239497"""

    # 图片路径列表
    pics: List[str] = betterproto.string_field(1)
    # 图片尺寸列表
    sizes: List["ImageSize"] = betterproto.message_field(2)
    # 图片切片列表
    clips: List["ImageClip"] = betterproto.message_field(3)


@dataclass
class ImageInfo(betterproto.Message):
    # 图片路径
    image_path: str = betterproto.string_field(1)
    # 图片url
    image_url: str = betterproto.string_field(2)
    # 图片token
    image_token: str = betterproto.string_field(3)
...
class ImageIndexData(betterproto.Message):
    ...
    # index 文件内容，存放 index.data 解码后的 json 此字段目前只用于灰度测试。 - 如果 index 非
    # null，客户端可直接使用本字段内容。 - 如果 index 为 null，客户端需从 bfs 下载 index.data 文件并解码。
    index: "ImageIndex" = betterproto.message_field(2)
    ...
    # 预览图片 目前只返回第一张图的元素
    pre_view_images: List["ImageInfo"] = betterproto.message_field(5)
    # 当前话最后一张图片的路径
    last_image_path: str = betterproto.string_field(19)
    # 当前话最后一张图片的链接
    last_image_url: str = betterproto.string_field(20)
...
class LayoutData(betterproto.Message):
    ...
    # 8-高能单话 9-新人三图 10-更多推荐 11-登录条 12-福利 13-番推漫
    type: int = betterproto.int32_field(1)
    # 模块 id
    id: int = betterproto.int32_field(2)
    # 模块名
...
class GetInterestingStyles(betterproto.Message):
    styles: List["StyleInfo"] = betterproto.message_field(1)
...
class HomeHotReq(betterproto.Message):
    # 榜单类型，0、人气榜；1、免费榜 2、飙升榜 3、日漫榜 4、国漫榜 5、新作榜 6、韩漫榜 7、新版国漫榜
    type: int = betterproto.int32_field(1)
...
class ComicStub(betterproto.ServiceStub):
    ...
    def reward_comic(self, request: RewardComicReq) -> DefaultResponse:
