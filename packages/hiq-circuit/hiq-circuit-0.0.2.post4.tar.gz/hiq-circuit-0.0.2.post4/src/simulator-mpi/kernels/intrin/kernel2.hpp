...
inline void kernel_compute(V &psi, std::size_t I, std::size_t d0,
                           std::size_t d1, const __m256d m[],
                           const __m256d mt[])
{
     const __m256d v[4] = {load2(&psi[I]), load2(&psi[I + d0]),
...
inline void kernel_core(V &psi, unsigned id1, unsigned id0, M const &m,
                        std::size_t ctrlmask)
{
          ...
          for (std::ptrdiff_t i0 = 0; i0 < n; i0 += 2 * dsorted[0]) {
               for (std::size_t i1 = 0; i1 < dsorted[0]; i1 += 2 * dsorted[1]) {
                    for (std::size_t i2 = 0; i2 < dsorted[1]; ++i2) {
                         kernel_compute(psi, i0 + i1 + i2, d0, d1, mm, mmt);
                    }
               }
          }
     }
               ...
               for (std::size_t i1 = 0; i1 < dsorted[0]; i1 += 2 * dsorted[1]) {
                    for (std::size_t i2 = 0; i2 < dsorted[1]; ++i2) {
                         if (((i0 + i1 + i2) & ctrlmask) == ctrlmask)
                              kernel_compute(psi, i0 + i1 + i2, d0, d1, mm,
                                             mmt);
                    }
               }
          }
...
template <class V, class M,
          void K(V &, unsigned, unsigned, M const &, std::size_t)>
inline void kernelK(V &v, unsigned id1, unsigned id0, M const &m,
                    std::size_t ctrlmask)
{
     K(v, id1, id0, m, ctrlmask);
}
}  // namespace intrin
