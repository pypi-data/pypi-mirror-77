...
# 断言-字符串检验
def assert_proxy(url='',content='',conuts=1):
    with open('log.txt','r') as fp:
        requests_log = json.loads(fp.readlines()[0])
    counts_sj = 0
    ...
    for i in requests_log[url]:
        if content in i:
            counts_sj += 1
    if counts_sj > counts_sj :
        print(u"*** 成功找到url 但请求数量过多！实际：%s , 预期：%s ***"%(str(counts_sj),str(conuts)))
    elif counts_sj < counts_sj :
        print(u"*** 成功找到url 但请求数量过少！实际：%s , 预期：%s ***"%(str(counts_sj),str(conuts)))
    else:
        print(u"*** 成功找到url 且数量正常！实际：%s , 预期：%s ***"%(str(counts_sj),str(conuts)))
