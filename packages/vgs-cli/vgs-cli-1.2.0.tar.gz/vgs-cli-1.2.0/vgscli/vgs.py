...
from vgscli.access_logs import prepare_filter, fetch_logs, format_logs, wrap_records
...
def logs():
    ...
    # Return all logs available for a vault\t\t\t\t\t\t
    vgs logs access -V tntbnqdfizv

    # Show all logs in the last hour\t\t\t\t\t\t
    vgs logs access -V tntbnqdfizv --since=1h

    # Show all logs after a specific date\t\t\t\t\t\t
    vgs logs access -V tntbnqdfizv --since=2020-08-18T11:40:45

    # Display only the most recent 25 log records\t\t\t\t\t\t
    vgs logs access -V tntbnqdfizv --tail=25
    """
    pass
...
@logs.command('access')
@click.option('--output', '-o', help='Output format', type=click.Choice(['json', 'yaml']), default='yaml', show_default=True)
# @click.option('--follow', '-f', help='Specify to stream logs as they appear on the VGS dashboard.', is_flag=True, default=False)
@click.option('--since', help='Only show logs newer than a relative duration like 30s, 5m, or 3h or after a specific RFC 3339 date.', type=DateTimeDuration(formats=["%Y-%m-%dT%H:%M:%S"]))
@click.option('--tail', help='Number of log records to show. Defaults to all logs if unspecified.', default=-1, callback=validate_tail)
@click.option('--until', help='Only show logs older than a relative duration like 30s, 5m, or 3h or before a specific RFC 3339 date.', type=DateTimeDuration(formats=["%Y-%m-%dT%H:%M:%S"]))
