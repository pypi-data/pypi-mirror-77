...
from sqlalchemy_plus.helpers import columns
from sqlalchemy_plus.helpers import relationships
from sqlalchemy_plus.helpers import primarykey
...
class ModelDAO:
    ...
    def update(self, instance, payload, **kwargs):
        ...
        for mtm in set([i.key for i in rels]).intersection(set(payload.keys())):
            related_model = getattr(instance.__mapper__.relationships, mtm)
            pk = related_model.entity.entity.__mapper__.primary_key[0].name
            if not related_model.uselist:
                # then we want to define and set a single instance to the field
                entity = related_model.entity.entity
                lookup = payload.get(mtm).get(pk, None)
                if not lookup:
                    raise ValueError(f'Field [{pk}] not available in payload. Cannot make query from unknown')
                candidate = entity.query.get(lookup)
                setattr(instance, mtm, candidate)
                continue

            current = set([getattr(i, pk) for i in getattr(instance, mtm)])
            candidates = set(map(lambda item: item.get(pk, item), payload[mtm]))
            for addition in candidates.difference(current):
                association = getsession().query(getattr(instance.__mapper__.relationships, mtm).entity).get(
                    addition)
                getattr(instance, mtm).append(association)

            for removal in current.difference(candidates):
                association = getsession().query(getattr(instance.__mapper__.relationships, mtm).entity).get(removal)
