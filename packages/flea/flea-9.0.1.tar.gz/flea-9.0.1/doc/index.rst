...
.. testsetup:: *
    ...
    testapp.route_wsgi('/', Response(content=[form_html]).buffered())
    r = Agent(testapp).get('/')
...
For more complex requirements you can also use XPath, with :meth:`~flea.agent.Agent.find` or with dictionary-style access:
...
You can also call the :class:`~flea.agent.Agent` directly, passing either an XPath
...
:meth:`~flea.agent.Agent.fill` will raise an exception if you ask it to fill in a field that does
not exist in the form.
:meth:`~flea.agent.Agent.fill_sloppy` does not have this restriction,
...
:func:`flea.html.first`,
:func:`flea.html.last`,
:func:`flea.html.by_index` and
:func:`flea.html.random_choice` for filling options from a select box or
...
content-type, data)`` to :meth:`~flea.agent.Agent.fill`. The data part can either be a
string:
...
The :meth:`~flea.agent.Agent.fill` method, when called on a form element, is a useful shortcut to
...
:attr:`~flea.agent.Agent.request` is a
...
You can also query the response directly, via
:attr:`~flea.agent.Agent.response`.
This is a
...
The :attr:`~flea.agent.Agent.body` property contains the raw response
from the server:
...
:attr:`~flea.agent.Agent.body`
is decoded according to the content type supplied in the response.
For the raw response body, use
:attr:`~flea.agent.Agent.body`
instead.

The
:meth:`~flea.agent.Agent.striptags`
method returns only the text node descendants
of an HTML response.
...
be the literal HTML emitted by your application - use :attr:`~flea.agent.Agent.body` for that.

Accessing textual content of selected elements
````````````````````````````````````````````````

:meth:`~flea.agent.Agent.striptags` removes all HTML tags and normalizes whitespace to make string comparisons easier:
...
The :meth:`~flea.agent.Agent.showbrowser`
method opens a web browser and displays the content of the
most recently loaded request::

    >>> r.get('/').showbrowser()

The :meth:`~flea.agent.Agent.serve`
method starts a HTTP server running your WSGI application and
...
specify these when first initializing the :class:`~flea.agent.Agent` object::
...
browser exactly where the :class:`~flea.agent.Agent` object left off.
