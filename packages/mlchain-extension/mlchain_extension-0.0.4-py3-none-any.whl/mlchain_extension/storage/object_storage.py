...
class ObjectStorage(MLStorage):
    def __init__(self, bucket=None, client=None, url=None, access_key=None, secret_key=None, provider=None,ping = False):
        ...
        self.bucket = bucket or mlconfig.OBJECT_STORAGE_BUCKET
        self._client = client
        self.url = url or mlconfig.OBJECT_STORAGE_URL
        self.access_key = access_key or mlconfig.OBJECT_STORAGE_ACCESS_KEY
        self.secret_key = secret_key or mlconfig.OBJECT_STORAGE_SECRET_KEY
        self.provider = provider or mlconfig.OBJECT_STORAGE_PROVIDER or 's3'
        if ping:
            try:
    ...
    def client(self):
        if self._client is None:
            ...
            self._client = session.client(
                self.provider,
                endpoint_url=self.url,use_ssl=False)
        return self._client
