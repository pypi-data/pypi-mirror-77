...
class PreParation:
    ...
    def read_model(self):
        ...
        if self.model is not None:
            ...
            try:
                ...
                # chaindict = self.chainmaps(chaindict)
                # print(chaindict)
        ...
        for (key, value) in chaindict.items():
            ...
            for chainfile in value:
                ...
                try:
                    ...
                    modelmap = emda_methods.model2map(chainfile, dim, float(self.resolution), unit_cell)
                    emda_methods.write_mrc(modelmap, modelmapname, unit_cell)
    ...
    def modelstomaps(self):
        ...
        if not self.model:
            print('No model map as no model was given.')
        else:
            ...
            if self.method == 'sp':
                ...
                for model in self.model:
                    ...
                    if errlist:
                        try:
                            modelmapname = '{}{}_modelmap.map'.format(self.dir, model)
                            modelmap = emda_methods.model2map(self.dir + model, dim, float(self.resolution), unit_cell, lig=True)
                            emda_methods.write_mrc(modelmap, modelmapname, unit_cell)
                            modelmaps.append(modelmapname)
                            print('Model map produced without the ligand description.')
                        except:
                ...
                print("No model map is produced as this is either not a single particle entry or nothing specified for "
                      "-m(pleae use: -m sp).")
    ...
    def read_halfmaps(self):
        ...
        if self.emdid:
            ...
            if self.evenmap is not None and self.oddmap is not None:
                try:
                    oddname = self.oddmap[:-4]
                    evenname = self.evenmap[:-4]
                    rawmapname = '{}{}_{}_rawmap.map'.format(self.dir, oddname, evenname)
