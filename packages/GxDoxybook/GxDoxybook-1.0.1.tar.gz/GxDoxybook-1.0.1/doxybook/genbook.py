...
class GenMD():
    def __init__(self, start_path, bookpath, source_paths, xmlpath, \
            path_type = 'all', link_path = '', now_dir = '', rel_path = ''):
        self.bookpath = bookpath
        self.source_paths = source_paths
        self.xmlpath = xmlpath
        self.start_path = start_path #goxceed_path
        self.path_type = path_type
    ...
    def copymdfile(self, path):
        ...
        for mdfile in mdfiles:
            ...
            if mdfile == 'docs' and self.path_type == 'part':
                continue
            if os.path.isfile(os.path.join(path,mdfile)):
    ...
    def before_work(self):
        ...
        if self.path_type == 'all':
            ...
            for cfgdir in self.source_paths:
                path_msg = cfgdir.split('/doc')[1].strip('/')
                md_dict[path_msg] = {
                        'path':cfgdir,
                        }
    ...
    def GenMD(self, is_all):
        ...
        if self.path_type == 'all':
            ...
            cmd = "doxybook -i " + self.xmlpath + " -o " + self.bookpath + ' -s ' + \
                self.bookpath + '/SUMMARY.md -t gitbook -a ' + is_all + ' -n'
        os.system(cmd)
        #for cfgdir in self.source_paths:
...
def main():
    if len(sys.argv) == 1:
        print("")
        print("Please input the goxceed path!")
        print("")
        print(" genbook ../goxceed")
        print("")
        sys.exit(1)
    args = parse_options()
    if args.dir != None:
        now_dir = os.getcwd()
        goxceed_path = args.dir
        if not goxceed_path.startswith('/'):
            goxceed_path = os.path.join(now_dir, args.dir)
        ...
        path_type = 'all'
        if goxceed_path != root_path:
            path_type = 'part'

        rel_path = os.path.relpath(goxceed_path, root_path)
        ...
        print('doc_path : ' +  goxceed_path)
        print('rel_path : ' + rel_path)

        goxceed_path = os.path.abspath(goxceed_path)
        ...
        os.chdir(goxceed_path)
        genxml = GenXML(goxceed_path, outpath, path_type = path_type)
        cfgdirs = genxml.GetXml()
        #sys.exit()
        bookoutpath = os.path.join(outpath, 'book')
        print(bookoutpath)
        genmd = GenMD(goxceed_path, bookoutpath, cfgdirs, outpath+'/XML', \
                path_type, link_path, now_dir, rel_path)
        genmd.GenMD('yes')
        ...
        goxceed_path = args.input
        is_server = args.server
        allgenerator = args.allgenerator
        goxceed_path = os.path.abspath(goxceed_path)
        outpath = goxceed_path
        outpath = os.path.join(outpath, 'docs')
        os.chdir(goxceed_path)
        genxml = GenXML(goxceed_path, outpath, is_server)
...
#for dirpath,dirnames,filenames in os.walk(goxceed_path+'/gxtest/doc'):
#    print(dirpath, dirnames, filenames)
#print(os.listdir(goxceed_path))
#
#print(os.getcwd())
##os.chdir()
#print(goxceed_path)
