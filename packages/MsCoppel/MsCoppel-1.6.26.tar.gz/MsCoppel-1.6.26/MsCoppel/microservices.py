...
import _thread
import asyncio
from uuid import uuid4  # Requerido
    ...
    def hasProcessMessage(self, data):
        ...
        if os.environ.get('REDISVALIDATE', None) is None:
            ...
            if response_redis is None:
                self.redisclient.expire(shahex, (10 * 60))
                return False
            print('--------------------------')
            print(data)
            print('--------------------------')
    ...
    def initilizeTracer(self):
        config = Config(
            config={
                ...
                'logging': False if os.environ.get('JAEGER_LOGS', None) is None else True,
                'tags': {
