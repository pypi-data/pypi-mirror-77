...
def main():
	...
	parser.add_option("--n_iter",action="store",type='int', dest="n_iterations", default=10000, help="The maximum number of iterations for the optimization. Should be at least 250. default=%default" )
	...
	parser.add_option("-s","--dotSize",action="store",type='float', default=1.5, dest="dot_size",help="Size of dot or circle. To reduce the dot/circle size, use a value of less than 1; to increase the dot/circle size, use a value greater than 1. default=%default")
	parser.add_option("-t","--labelSize",action="store",type='float', default=0.6, dest="label_size",help="Size of the text labels. To reduce the text size, use a value of less than 1; to increase the text size, use a value greater than 1. default=%default")
	...
	if options.plot_alpha:
		...
		print ('plot(PC1, PC2, col = alpha(Colors, %f), pch=%d, cex=%f, main="t-SNE 2D map")' % (options.plot_alpha, pch[options.plot_char], options.dot_size), file=ROUT)
	else:
		print ('plot(PC1, PC2, col = Colors, pch=%d, cex=%f, main="t-SNE 2D map")' % (pch[options.plot_char], options.dot_size), file=ROUT)	
	if options.text_label:
		print ('text(PC1, PC2, labels=Sample_ID, col = Colors, cex=%f, pos=1)' % (options.label_size), file=ROUT)
