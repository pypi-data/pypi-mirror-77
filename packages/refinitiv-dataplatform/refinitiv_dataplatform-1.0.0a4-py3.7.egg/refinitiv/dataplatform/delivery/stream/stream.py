...
from .pause_resume_mixin import PauseResumeMixin
...
class Stream(abc.ABC, PauseResumeMixin):
    ...
    def _send(self, message):
        ...
        self._session.debug(f'Stream(id={self._stream_id}, name={self._name}, connection={self.connection}).send(message = {message})')
        self._session.send(self.connection, message)

    #######################################
    #  methods to open and close stream

    def open(self, with_updates=True):
        """ Open synchronously the data stream """
        self._loop.run_until_complete(self.open_async(with_updates=with_updates))
        return self._state
