...
class StreamingChain(object):
    ...
    async def _decode_async(self, with_updates=True):
        ...
        with self._chainRecordNameToItemStreamDictLock:
            for chainRecordName, itemStream in self._chainRecordNameToItemStreamDict.items():
        ...
        while thisChainRecordName:
            ...
            if self._chainRecordNameToItemStreamDict[thisChainRecordName].state != StreamState.Closed:
                ...
                self._session._loop.call_soon_threadsafe(self._is_complete_decoded.set_result, False)
        ...
        if not self._is_complete_decoded.done():
            self._session._loop.call_soon_threadsafe(self._is_complete_decoded.set_result, True)
    ...
    def _updateChainRecord(self, chainRecordName, update):
        ...
        for index, (oldConstituent, newConstituent) in chainRecordUpdateInfo.indexToOldAndNewConstituentTupleDict.items():
