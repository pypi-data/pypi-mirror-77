...
class OMMItemStream(OMMStream):
    ...
    def open(self, with_updates=True):
        ...
        """
        self._session.debug(f"Open synchronously OMMItemStream {self.stream_id} to {self._name}")
        state = super().open(with_updates=with_updates)
        return state
    ...
    def close(self):
        ...
        """
        if self._session:
            self._session.log(logging.DEBUG, f"Close OMMItemStream subscription {self.stream_id}")

        super(OMMItemStream, self).close()
        self._code = "Closed"
        self._message = ""
        return self._state
    ...
    async def open_async(self, with_updates=True):
        ...
        await super().open_async(with_updates=with_updates)
        return self._state
