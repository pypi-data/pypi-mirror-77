...
class ESG(object):
    ...
    async def _get_data_async(self, universe, data_type, start=None, end=None, closure=None):
        ...
        if not universe:
            raise ESGError(-1, f'Universe was not requested.')

        if not data_type:
            raise ESGError(-1, f'Data type was not requested.')
...
def _convert_esg_json_to_pandas(json_esg_data):
    ...
    if _data:
        ...
        esg_dataframe = pd.DataFrame(_numpy_array, columns=_fields).convert_dtypes()
    else:
