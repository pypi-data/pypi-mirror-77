...
class Chain(object):
    ...
    class Data(Endpoint.EndpointData):
        ...
        def _convert_raw_to_dataframe(raw):
            ...
            #   extact constituents from data
            constituents = data[Chain.Data._response_constitients_name]

            #   construct pandas dataframe from constituents
            if len(constituents):
                return DataFrame({ric: constituents}).convert_dtypes()
            else:
                return DataFrame([], columns=[ric])
    ...
    def _on_response_cb(self, endpoint, response):
        ...
        if self._on_response_cb_func:
            ...
            if response.is_success:
                #   success request, so parse the reponse into chain constituents data
                # warning IMRPOVE_ME :: THIS IS A WORKAROUND FOR CONVERT GENERAL ENDPOINT RESPONSE TO THIS SPECIFIC CHAIN RESPONSE DATA
    ...
    async def decode_async(*, session=None, on_response=None, universe, **kwargs):
        """ call synchronous a decode chain with givn universe parameter
    ...
    on_response: legacy, optional
        a callback legacy when response from decoding chain requested
        default: None
    universe: str
    ...
    -------
    object
        The response object from given chian parameters
        """
