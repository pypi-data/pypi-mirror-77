...
class Lookup(object):
    ...
    class LookupData(Endpoint.EndpointData):
        ...
        def _convertLookupResponseToDataframe(raw):
            ...
            matchesListOfDict = [matches[key] for key in list(matches.keys())]
            propertyNames = set([key for keys in [list(item.keys()) for item in matchesListOfDict] for key in keys])
            for matchName, matchValueDict in list(matches.items()):
            ...
            #   done, return 
            if matchDataframe:
                return DataFrame(matchDataframe).convert_dtypes()
            else:
                return DataFrame([])
