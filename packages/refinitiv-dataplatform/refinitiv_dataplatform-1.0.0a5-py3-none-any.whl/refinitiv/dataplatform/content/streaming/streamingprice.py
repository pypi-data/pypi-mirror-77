...
from refinitiv.dataplatform.delivery.stream import OMMItemStream, StreamState, OMMItemStreamCallback
from refinitiv.dataplatform.delivery.stream.stream_cache import StreamCache
from refinitiv.dataplatform.delivery.stream import PauseResumeMixin


class StreamingPrice(StreamCache, PauseResumeMixin):
    """
    ...
    def __init__(self,
        ...
        self._item_stream = OMMItemStream(session=self._session,
                                       name=self._name,
    ...
    def open(self, with_updates=True):
        ...
        """
        self._session.debug(f'Open synchronously StreamingSinglePrice {self.id} to {self._name}')
        state = self._item_stream.open(with_updates=with_updates)
        return state

    def _set_pause(self):
        # This will be set in self._item_stream.pause()
        pass

    def _set_resume(self):
        # This will be set in self._item_stream.resume()
        pass

    def _do_pause(self):
        self._item_stream.pause()

    def _do_resume(self):
        self._item_stream.resume()

    def close(self):
        """
        Close the data stream
        """
        self._session.debug(f'Stop StreamingSinglePrice subscription {self.id} to {self._name}')
        state = self._item_stream.close()
        return state
    ...
    def _decode_partial_update_field(self, key, value):
        """ this legacy is used to proces the partial update
    ...
    def _on_update(self, stream, update):
        for data in update:
            if data == "Fields":
                ...
                for key, value in list(update[data].items()):
