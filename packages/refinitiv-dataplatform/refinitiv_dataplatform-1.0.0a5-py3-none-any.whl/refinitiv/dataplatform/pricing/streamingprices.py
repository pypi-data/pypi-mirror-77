...
from refinitiv.dataplatform.delivery.stream import StreamState, PauseResumeMixin
from refinitiv.dataplatform.errors import StreamingPricesError


class StreamingPrices(PauseResumeMixin):
    """
    ...
    def open(self):
        ...
        """
        state = self._session._loop.run_until_complete(self.open_async())
        return state

    def _do_pause(self):
        results = [v.pause() for v in list(self._streaming_prices.values())]
        assert all([r == StreamState.Pause for r in results])

    def _do_resume(self):
        results = [v.resume() for v in list(self._streaming_prices.values())]
        assert all([r != StreamState.Pause for r in results])
    ...
    async def open_async(self):
        ...
        self._complete_event_nb = 0
        streaming_prices = iter(list(self._streaming_prices.values()))
        stream_opens_socket = next(streaming_prices)
        await stream_opens_socket.open_async()

        await asyncio.gather(*[i.open_async() for i in streaming_prices])

        self._session.log(1, f'StreamingPrices : start asynchronously streaming on {self.params.universe} done')

        #   all streaming price is opened, so set the state to be open
    ...
    def close(self):
        if self._state is not StreamState.Closed:
            self._session.log(1, f'StreamingPrices : close streaming on {self.params.universe}')
            self._state = StreamState.Closed
    ...
    def _on_complete(self, stream):
        ...
        if self._complete_event_nb == len(self.params.universe):
            # self._state = StreamState.Open
