...
class PlatformSession(Session):
    ...
    class RefreshTokenThread(Thread):
        ...
        def run(self):
            ...
            #   done
            self._session.debug('FINISHED :: RefreshTokenThread.run()')
        ...
        def _refresh_token(self):
            ...
            try:
                ...
                if _retry_at < self._session._token_expires_at:
                    self._session.warning(f"EDP Token Refresh failed : {str(error)}.\nRetry in {self._retry_token_request_in} seconds")
                    ...
                    self._session.warning(f"Refresh token is going to expire in {_time_to_expire} seconds")
        ...
        def _request_refresh_token(self, grant):
            ...
            self._session.debug("Send refresh token request to {}".format(self._session.authentication_token_endpoint_uri))
        ...
        def _check_token_response(self, response):
            ...
            self._session.log(logging.DEBUG, f"Received refresh token {_json_response['refresh_token']}")
            self._session.log(logging.DEBUG, f"\tExpire in {_json_response['expires_in']} seconds")
            ...
            self._session._token_expires_in_secs = _json_response["expires_in"]
            self._session._token_expires_at = time.time() + int(self._session._token_expires_in_secs)
            ...
            self._session.log(logging.DEBUG, f"Set refresh token delay to {self._delay} sec")
            return True
        ...
        def http_request(self, url: str, method=None, headers={},
            ...
            try:
                _request_response = self._http_session.send(_prepared_request, **kwargs)
                self._session.log(logging.DEBUG, f"HTTP request response : HTTP {_request_response.status_code} - {_request_response.text}")
                return _request_response
            except Exception as e:
                self._session.log(logging.ERROR, f"HTTP request failed: {e!r}")
    ...
    def __init__(
        ...
        super().__init__(
            ...
            deployed_platform_username=deployed_platform_username, dacs_position=dacs_position, dacs_application_id=dacs_application_id,
            **kwargs
        )
        ...
        # for Deployed platform connection
        self._deployed_platform_host = deployed_platform_host
        self._deployed_platform_connection_name = 'pricing' if deployed_platform_connection_name is None \
                                                    else deployed_platform_connection_name

        #   building the deployed platform connection dict
        self._deployed_platform_connection_dict = {}
        if self._deployed_platform_host is not None:
            self._deployed_platform_connection_dict = {
                            f'{self._deployed_platform_connection_name}.type':'ads-websocket',
                            f'{self._deployed_platform_connection_name}.websocket-url':self._deployed_platform_host,
    ...
    async def _create_and_start_stream_connection(self, stream_connection_name:str):
        ...
        StreamConnectionCls = None
        websocket_thread_name = None
        ...
        #   unknown streaming service, raise the exception
            raise PlatformSession(f'ERROR!!! cannot create the stream connection because "{stream_connection_name}" has a unknown streaming connection protocol "{stream_config.protocol}"')
    ...
    def get_omm_login_message_key_data(self):
        ...
        #   connect streaming to platform
			...
			# HACK TRADING TEAM #####################
            return { "Name": self._dacs_params.deployed_platform_username,
                     # "NameType": "AuthnToken",
                     "Elements": {
                         # "AuthenticationToken": self._access_token,
                         "ApplicationId": self._dacs_params.dacs_application_id,
    ...
    def close(self):
        ...
        if self._connection_type == self.PlatformConnectionType.RefinitivDataplatformConectionOnly \
            ...
            if self._refresh_token_thread:
                self.log(logging.DEBUG, "Stop refresh token thread...")
                self._refresh_token_thread.cancel()
                self._refresh_token_thread.join()
                self.log(logging.DEBUG, "Refresh token thread was stopped")
            return super().close()
    ...
    async def open_async(self):
        ...
        if self._connection_type == self.PlatformConnectionType.RefinitivDataplatformConectionOnly \
            ...
            try:
                ...
                self.warning(f"EDP Authentication failed. {str(e)}")
            ...
            self.debug('RDP connection state = {}.'.format(self._state))
        ...
        if self._connection_type == self.PlatformConnectionType.DeployedPlatformOnly \
            ...
            await self.wait_for_streaming(self._deployed_platform_connection_name)
    ...
    async def _request_access_token(self, grant):
        ...
        self.log(logging.DEBUG, "Send request token to {}"
                 .format(self.authentication_token_endpoint_uri))
        ...
        try:
            ...
            _response and self.log(logging.DEBUG, "Request token response: {}".format(_response.text))
            return _response
    ...
    def _init_refresh_token_thread(self):
        ...
        _delay = int(self._token_expires_in_secs)
    ...
    def _check_token_response(self, response):
        ...
        if response.status_code != requests.codes.ok:
            ...
            self._on_event(self, self._last_stream_connection_name, self._last_event_code, self._last_event_message)
            return False
        ...
        self._token_expires_in_secs = _json_response["expires_in"]
        return True
    ...
    async def http_request_async(self, url: str, method=None, headers=None,
        ...
        if self._connection_type == self.PlatformConnectionType.DeployedPlatformOnly:
            ...
            raise PlatformSessionError(-1,
                                       'Error!!! Platform session cannot connect to refinitiv dataplatform. Please check or provide the access right.')
