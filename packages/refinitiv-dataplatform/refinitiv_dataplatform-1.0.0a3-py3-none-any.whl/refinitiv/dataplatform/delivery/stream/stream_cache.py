...
class StreamCache:
    ...
    def __str__(self):
        if self.service:
            return str("|".join([self.service, self.name]) + "[" + ",".join([f"{f}:{v}" for f, v in list(self.items())]) + "]")
        else:
            return str("|".join(["Unknown service", self.name]) + "[" + ",".join([f"{f}:{v}" for f, v in list(self.items())]) + "]")
