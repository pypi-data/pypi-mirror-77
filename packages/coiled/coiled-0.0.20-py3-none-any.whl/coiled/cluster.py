...
class Cluster(distributed.deploy.Cluster):
    ...
    async def _start(self):
        ...
        if should_create:
            ...
            self.cluster_id = await self.cloud.create_cluster(
                account=self.account, configuration=self.configuration, name=self.name,  # type: ignore
            )
    ...
    async def _scale(self, n: int) -> None:
        await self.cloud.scale(account=self.account, cluster_id=self.cluster_id, n=n)  # type: ignore
