...
import coiled
from ..utils import ParseIdentifierError, ExperimentalFeatureWarning
...
async def test_worker_options_scheduler_options(cloud, software_env, cleanup):
    ...
    async with coiled.Cluster(
        ...
        async with Client(cluster, asynchronous=True) as client:
            ...
            result = await client.run(lambda dask_worker: dask_worker.lifetime == 61)
...
async def test_worker_class(cloud, software_env, cleanup):
    ...
    await cloud.create_cluster_configuration(
        name="my-config",
        software=software_env,
        worker_class="distributed.deploy.tests.test_local.MyWorker",
    )
    ...
    async with coiled.Cluster(
        ...
        async with Client(cluster, asynchronous=True) as client:
            ...
            result = await client.run(
                lambda dask_worker: type(dask_worker).__name__ == "MyWorker"
            )
            assert all(result.values())
...
async def test_scaling_limits(cloud, cleanup, cluster_configuration, sample_user):
    ...
    # There's a 10 % buffer thought
    try:
        await new_cluster.scale(5)
    except Exception:
        pytest.fail("Unexpected Error. We should be able to sacle up and down")

    with pytest.raises(Exception) as info:
        await new_cluster.scale(6)

    assert "limit" in str(info.value)
    assert str(sample_user.membership.limit) in str(info.value)
    # TODO: Not too sure about these two asserts. They seem too rigid
    assert str(6) in str(info.value)
...
async def test_connect_same_name(cloud, cluster_configuration, cleanup, capsys):
    ...
    async with coiled.Cluster(
        ...
        async with coiled.Cluster(
            name="foo-123", asynchronous=True, configuration=cluster_configuration,
        ) as cluster2:
...
async def test_jobs_experimental(sample_user, cloud, software_env, cleanup):
    with pytest.warns(ExperimentalFeatureWarning, match="create_job_configuration"):
        coiled.create_job_configuration(
            name="my-app", command=["python", "--version"], software=software_env,
        )
...
async def test_jobs_listable_clusters_not(
    ...
    await cloud.create_job_configuration(
        name="my-job", command=["python", "--version"], software=software_env,
    )
...
def test_jobs_limit(sample_user, cloud, software_env, cleanup):
    ...
    job_name = "my-app"
    coiled.create_job_configuration(
        name=job_name, software=software_env, command=["python", "--version"],
    )
