...
class Cloud:
    ...
    def __init__(
        ...
        with _cluster_creation_lock:
            ...
            self._default_region = region or dask.config.get(
                "coiled.aws.region", default="us-east-2"
            )
        self.session = None
        self.status = "init"
    ...
    async def _create_cluster(
        ...
        return await self._get_cluster_by_name(
            name=name, account=account, region=region
        )
    ...
    def get_cluster_by_name(
        self, name: str, account: str = None, region: str = None
    ) -> Optional[int]:
        return self._sync(self._get_cluster_by_name, name=name, account=account, region=region)  # type: ignore
    ...
    async def _get_cluster_by_name(
        ...
        response = await self.session.request(
            ...
            params={"exclude_statuses": "stopping,stopped", "region": region},
        )
    ...
    async def _create_software_environment(
        ...
        if conda_env_name is not None:
            conda["name"] = conda_env_name  # type: ignore
    ...
    async def _list_job_configurations(self, account=None):
        ...
        response = await self.session.request(
            "GET",
            self.server + f"/api/v1/{account}/processes/",
    ...
    async def _delete_job_configuration(self, name):
        account, name = self._normalize_name(name)
        response = await self.session.request(
            "DELETE", self.server + f"/api/v1/{account}/processes/{name}/",
        )
        if response.status >= 400:
            text = await response.text()
            raise Exception(text)
    ...
    async def _list_jobs(self, account=None):
        ...
        if response.status >= 400:
            ...
            results = (await response.json())["results"]
            results = {r["name"]: format_job_output(r) for r in results}
            return results
    ...
    async def _create_job_configuration(
        ...
        # Check if we already have a config
        response = await self.session.request(
            "GET", self.server + f"/api/v1/{account}/processes/{name}/"
        )
        if response.status == 404:
            data.update({"name": name})
            response = await self.session.request(
                "POST", self.server + f"/api/v1/{account}/processes/", json=data,
            )
        else:
            response = await self.session.request(
                "PATCH",
                self.server + f"/api/v1/{account}/processes/{name}/",
                json=data,
            )
    ...
    def start_job(
        self, configuration: str = None, account: str = None, log_output=sys.stdout,
    ):
        return self._sync(
            self._start_job,
            configuration=configuration,
    ...
    async def _start_job(
        self, configuration: str, account: str = None, log_output=sys.stdout,
    ):
        account = account or self.default_account
        self._verify_account(account)
        name = self.user + "-" + str(uuid.uuid4())[:10]
        data = {"name": name, "server": self.server, "job": configuration}
        ws_server = self.server.replace("http", "ws", 1)
    ...
    def stop_job(
        ...
        return self._sync(self._stop_job, name=name,)

    async def _stop_job(self, name):
        account, name = self._normalize_name(name)
        response = await self.session.request(
            "DELETE", self.server + f"/api/v1/{account}/jobs/{name}/",
        )
        if response.status >= 400:
            text = await response.text()
            raise Exception(text)
...
def create_software_environment(
    ...
    post_build
        ...
        have been installed.
    region
        Name of the AWS region to create the software environment in.
        If not provided, will default to us-east-2.
    log_output
        Stream to output logs to. Defaults to ``sys.stdout``.
    conda_env_name
        Name of conda environment to treat as "primary" -- all packages will be installed into
        this, and it will be activated before running Dask or application commands. Defaults to "base"
        (the conda default) if not provided
    """
    with Cloud() as cloud:
...
def create_cluster(
    ...
    region
        Name of the AWS region to create the cluster in
        If not provided, will default to us-east-2
...
def start_job(configuration: str, account: str = None, log_output=sys.stdout):
    with Cloud(account=account) as cloud:
        return cloud.start_job(
            configuration=configuration, account=account, log_output=log_output,
        )
