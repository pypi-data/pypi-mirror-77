...
from ..dataset import Data, Frame, Segment
from ..utility import TBRNInfo, TBRNType
from .gas import GAS
...
@click.option(
    "-c", "--config", "config_name", type=str, default="default", help="The environment to login."
)
@click.pass_context
def cli(ctx: Context, access_key: str, url: str, config_name: str) -> None:
...
def publish(obj: Dict[str, str], name: str) -> None:
    ...
    :param obj: a dictionary including config info
    :param name: the name of the dataset to be published, like "tb:KITTI"
    """
    info = TBRNInfo(tbrn=name)
    if info.type != TBRNType.DATASET:
        click.echo(f'"{name}" is not a dataset', err=True)
...
@click.option(
    ...
    "continues",
    is_flag=True,
    help="Whether to support the breakpoint resume function.",
)
@click.pass_obj
def cp(  # pylint: disable=invalid-name, too-many-arguments
    ...
    :param continues: whether to support the breakpoint resume function
    """
    ...
    if info.type in [TBRNType.SEGMENT, TBRNType.NORMAL_FILE]:
        ...
        if (
            ...
            and info.remote_path
            and not info.remote_path.endswith("/")
        ):
            dataset.get_or_create_segment(info.segment_name).upload_data(
                local_abspaths[0], info.remote_path
            )
            return

        segment = _get_segment_object(
            info.segment_name, local_abspaths, info.remote_path, is_recursive
        )
        dataset.upload_segment_object(segment, jobs=jobs, continues=continues)
        return
...
def _get_segment_object(
    segment_name: str, local_abspaths: List[str], remote_path: str, is_recursive: bool
) -> Segment:
    ...
    if isinstance(segment, SegmentClient):
        ...
        if not list_all_files:
            for index in range(len(frames)):
                click.echo(TBRNInfo(dataset_name, segment_name, index).get_tbrn())
        else:
            for index, frame in enumerate(frames):
                _echo_frame(dataset_name, segment_name, index, frame)


def _echo_frame(
    dataset_name: str, segment_name: str, frame_index: Union[str, int], frame: Frame
) -> None:
    ...
    :param frame: an object of class 'Frame'
    """
    for sensor_name, data in frame.items():
        click.echo(
            TBRNInfo(
                dataset_name, segment_name, frame_index, sensor_name, remote_path=data.fileuri
            ).get_tbrn()
        )
...
def _echo_data(dataset_name: str, segment_name: str, data_iter: Iterable[str]) -> None:
    ...
    for data in data_iter:
        click.echo(TBRNInfo(dataset_name, segment_name, remote_path=data).get_tbrn())


def _ls_dataset(gas: GAS, info: TBRNInfo, list_all_files: bool) -> None:
    dataset = gas._get_dataset(info.dataset_name)  # pylint: disable=protected-access
    segment_names = dataset.list_segments()
    if not list_all_files:
        for segment_name in segment_names:
            click.echo(TBRNInfo(info.dataset_name, segment_name).get_tbrn())
        return
...
def _ls_segment(gas: GAS, info: TBRNInfo, list_all_files: bool) -> None:
    dataset = gas._get_dataset(info.dataset_name)  # pylint: disable=protected-access
    _echo_segment(
...
def _ls_frame(gas: GAS, info: TBRNInfo, list_all_files: bool) -> None:
    ...
    if not list_all_files:
        for sensor_name in frame:
            click.echo(
                TBRNInfo(
                    info.dataset_name, info.segment_name, info.frame_index, sensor_name
                ).get_tbrn()
            )
    else:
        _echo_frame(info.dataset_name, info.segment_name, info.frame_index, frame)


def _ls_sensor(  # pylint: disable=unused-argument
    gas: GAS, info: TBRNInfo, list_all_files: bool
) -> None:
    dataset = gas.get_fusion_dataset(info.dataset_name)
    fusion_segment = dataset.get_segment(info.segment_name)
    frame = fusion_segment.list_frame_objects()[int(info.frame_index)]
    ...
    click.echo(
        TBRNInfo(
            ...
            info.frame_index,
            info.sensor_name,
            remote_path=frame[info.sensor_name].fileuri,
        ).get_tbrn()
    )


def _ls_fusion_file(  # pylint: disable=unused-argument
    gas: GAS, info: TBRNInfo, list_all_files: bool
) -> None:
    dataset = gas.get_fusion_dataset(info.dataset_name)
    fusion_segment = dataset.get_segment(info.segment_name)
    ...
    if frame[info.sensor_name].fileuri != info.remote_path:
        click.echo(f'No such file: "{info.remote_path}"!', err=True)
        sys.exit(1)

    click.echo(info.get_tbrn())
...
def _ls_normal_file(  # pylint: disable=unused-argument
    ...
    _echo_data(
        info.dataset_name, info.segment_name, _filter_data(segment.list_data(), info.remote_path)
    )
...
def ls(  # pylint: disable=invalid-name
    ...
    if not tbrn:
        for dataset_name in gas.list_datasets():
            click.echo(TBRNInfo(dataset_name).get_tbrn())
        return
...
def config(obj: Dict[str, str], access_key: str, url: str) -> None:
    ...
    if not access_key.startswith(_ACCESSKEY_PREFIX):
        click.echo(f'Error: AccessKey should start with "{_ACCESSKEY_PREFIX}"', err=True)
        sys.exit(1)
