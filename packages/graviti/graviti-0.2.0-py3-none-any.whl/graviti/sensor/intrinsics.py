...
import math
from typing import Dict, Generator, Iterator, List, Mapping, Optional, Sequence, Tuple
...
class CameraMatrix:
    ...
    def as_matrix(self) -> List[List[float]]:
        """Returns camera matrix in a 3x3 list.

        :return: A 3x3 list representing the camera matrix
        """
        return [
...
class DistortionCoefficients(Mapping[str, float]):
    ...
    def _calculate_radial_distortion(self, r2: float, is_fisheye: bool = False) -> float:
        ...
        for i, (_, value) in enumerate(self._distortion_generator("k", self._data)):
            radial_distortion += value * factor2 ** (i + 1)
...
class CameraIntrinsics:
    ...
    :param loads: A dictionary containig 'camera_matrix' and 'distortion_coefficient'
    ...
    def __init__(
        ...
        **kwargs: float,
        ...
        if loads:
            self._camera_matrix = CameraMatrix(loads=loads["camera_matrix"])
            if "distortion_coefficient" not in loads:
                self._distortion_coefficients = None
            else:
                self._distortion_coefficients = DistortionCoefficients(
                    loads=loads["distortion_coefficient"]
                )
            return

        self._camera_matrix = CameraMatrix(camera_matrix, loads=None, **kwargs)
        if "k1" not in kwargs or "p1" not in kwargs:
            self._distortion_coefficients = None
            return
    ...
    def dumps(self) -> Dict[str, Dict[str, float]]:
        ...
        """
        data = {"camera_matrix": self._camera_matrix.dumps()}
        if self._distortion_coefficients:
            data["distortion_coefficients"] = self._distortion_coefficients.dumps()
    ...
    def set_camera_matrix(
        ...
        **kwargs: float,
        ...
        :param loads: A dict containing camera matrix info
        :param kwargs: Keyword arguments containing fx, fy, cx, cy, skew(optional)
        """
        self._camera_matrix = CameraMatrix(matrix=matrix, loads=loads, **kwargs)
    ...
    def set_distortion_coefficient(
        ...
        :param loads: A dict containing camera distortion coefficients
        :param kwargs: Keyword arguments containing p1, p2, ..., k1, k2, ...
        """
        self._distortion_coefficients = DistortionCoefficients(loads=loads, **kwargs)
