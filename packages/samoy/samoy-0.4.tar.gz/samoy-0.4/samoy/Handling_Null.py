...
def dropnull(dataframe,method=None,axis=None):
    ...
    1. res = drop_null(df,"rc_all_null",1) --> this will drop the columns whose 
                                                         entire value is null
       res = drop_null(df,"rc_all_null",0) --> this will drop the rows whose 
                                                         entire value is null
    2. res = drop_null(df,"rc_any_null",1) --> this will drop the columns whose 
                                                         atleast one value is null
       res = drop_null(df,"rc_any_null",0) --> this will drop the rows whose 
                                                         atleast one value is null
...
def dropnull_th(dataframe,percentage):
    ...
    res = drop_null_th(df,20) --> res is the dataframe where you will be stroing the 
...
def swapnull(dataframe,method="mean",num_val=0,char_val="unknown"):
    ...
    1. for method = "custom"
    res=impute_null(df,"custom",1,"not known")
    res
    2. for method = "mean"
    res=impute_null(df)
    res
    3. for method = "median"
    res=impute_null(df,"median")
    res
...
def swapnull_subset(dataframe,columns,method="mean",num_val=0):
    ...
    1. for method = "custom"
    res=impute_null(df,['a','b'],"custom",1,"not known")
    df - name of dataframe where you want to perform imputation
    ['a','b'] - name of columns in dataframe df
    res
    2. for method = "mean"
    res=impute_null(df,['a','b'])
    res
    3. for method = "median"
    res=impute_null(df,['a','b'],"median")
    res
