...
class Tool:
    ...
    def get_html(self, url, headers={}, proxy={}, cookies={}, data={}, timeout=5, debug=False):

        for i in range(5):
            try:
                res = self.session.get(url, headers=headers, timeout=timeout, proxies=proxy, cookies=cookies, data=data)
                return res
    ...
    def post_html(self, url, headers={}, data={}, json={}, proxy={}, cookies={}, timeout=5, debug=False):
        ...
        for i in range(5):
            try:
                res = self.session.post(url, data=data, json=json, headers=headers, timeout=timeout, proxies=proxy, cookies=cookies)
                return res
            except Exception as e:
                if debug:
                    logging.info(e)
    ...
    def dup_insert(self, table: str, datas: dict or list) -> list:
        ...
        def analysis_dict(self, item, type="install"):
            if type == "install":
                key = item.keys()
                value = item.values()
