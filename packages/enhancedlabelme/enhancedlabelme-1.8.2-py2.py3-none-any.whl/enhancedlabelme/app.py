...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        autoPoint = action(
            ...
            slot=self.autoSeg,
            icon='edit',
            shortcut=shortcuts['auto_segmentation'],
            tip='Auto Segmentation',
            enabled=True,
        )
        ...
        self.actions = utils.struct(
            ...
            addPointToEdge=addPointToEdge, removePoint=removePoint, autoPoint=autoPoint,
    ...
    def autoSeg(self):
        ...
        def getRect(pt1):
            ...
            x3, y3 = pt1[1][0], pt1[1][1]
            x2, y2 = x3, y1
            x4, y4 = x1, y3
            return ([x1, y1], [x2, y2], [x3, y3], [x4, y4])
        item = self.currentItem()
        shapes = format_shape(item.shape())
        rect_xy = getRect(list(shapes['points'])) # 꼭짓점 좌표 (jsohn points 형식)
        imgData = self.imageData # image Data
        arrData = utils.img_data_to_arr(self.imageData) # ImageData -> Array
        imageFile = self.filename # 현재 이미지 파일 path (png, jpeg)
        print(rect_xy)
        # 이 좌표로 이미지에서 크롭 하시면 될 것 같습니다. 
