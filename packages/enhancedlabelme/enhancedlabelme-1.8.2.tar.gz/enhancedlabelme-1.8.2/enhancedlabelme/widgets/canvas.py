...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if osp.isfile(fn) and self.firstLoad == False:
            with open(fn, 'r', encoding='utf-8') as f:
                ...
                if lines[0] == str(QtCore.Qt.SolidLine):
                    self.penStyle = QtCore.Qt.SolidLine
                elif lines[0] == str(QtCore.Qt.DashLine):
                    self.penStyle = QtCore.Qt.DashLine
                elif lines[0] == str(QtCore.Qt.DotLine):
                    self.penStyle = QtCore.Qt.DotLine
                
                self.penThickness = float(lines[1])
                
                if lines[2] == str(QtCore.Qt.black):
                    self.penColor = QtCore.Qt.black
                elif lines[2] == str(QtCore.Qt.white):
                    self.penColor = QtCore.Qt.white
                elif lines[2] == str(QtCore.Qt.gray):
                    self.penColor = QtCore.Qt.gray
                elif lines[2] == str(QtCore.Qt.gray):
                    self.penColor = QtCore.Qt.red
                elif lines[2] == str(QtCore.Qt.yellow):
                    self.penColor = QtCore.Qt.yellow
                elif lines[2] == str(QtCore.Qt.green):
                    self.penColor = QtCore.Qt.green
                elif lines[2] == str(QtCore.Qt.blue):
                    self.penColor = QtCore.Qt.blue
                elif lines[2] == str(QtCore.Qt.cyan):
                    self.penColor = QtCore.Qt.cyan
                elif lines[2] == str(QtCore.Qt.magenta):
                    self.penColor = QtCore.Qt.magenta
                    
                self.firstLoad = True
        elif self.firstLoad == False:
            self.penStyle = QtCore.Qt.DashLine
            self.penThickness = 1.0
            self.penColor = QtCore.Qt.black
            self.firstLoad = True
        ...
        try:
            ...
            try:
                if self.cross == True and self.pixmap and self.dialog == False:
                    ...
                    p.drawLine(self.posc.x(), self.pixmap.height(), self.posc.x(), 0)
                    p.drawLine(0, self.posc.y(), self.pixmap.width(), self.posc.y())
                    self.update()
            except Exception:
                pass
    ...
    def closeEnough(self, p1, p2):
        ...
        return enhancedlabelme.utils.distance(p1 - p2) < (self.epsilon / self.scale)
    ...
    def intersectingEdges(self, point1, point2, points):
        ...
        for i in range(4):
            ...
            if 0 <= ua <= 1 and 0 <= ub <= 1:
                ...
                d = enhancedlabelme.utils.distance(m - QtCore.QPoint(x2, y2))
                yield d, i, (x, y)
    ...
    def keyboard_con(self):
        self.keyboard_button.press(self.keyboard_key.ctrl)
        self.keyboard_button.press("2")
        self.keyboard_button.release(self.keyboard_key.ctrl)
        self.keyboard_button.release("2")