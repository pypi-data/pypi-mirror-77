        ...
        for node in node_zero_degree:
            if self._rule.is_method(node.id):
                ...
                if num_classnames == 1:
                    ...
                    dynamic_init_obj = self.__init_instance(self._class_mapping, classname, self[param_id])
                    self[node.id] = dynamic_init_obj
                    
                    if self.debug:
                        _logger.info("Assigned %s @ %s | dynamic_init_obj: %s" % (node.id, str(self[node.id]), str(dynamic_init_obj)))
...
#             break
    ...
    def __init_instance(self, class_mapping, class_str, param_dict):
        ...
        if isinstance(init_params, dict):
            ...
            if self.debug:
                _logger.debug("Init class %s : %s => %s" % (str(obj_class), str(init_params), str(obj)))
