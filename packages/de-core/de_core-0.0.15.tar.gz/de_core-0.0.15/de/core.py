...
__version__ = '0.0.15'       # ALSO CHANGE IN setup.py
...
""" pre-compiled regular expression for to detect and bump the namespace patch number of version string """
...
def namespace_env_vars(namespace_name: str, pkg_root_path: str = "", old_nev: Optional[NevType] = None) -> NevType:
    ...
    portion_type, portion_name, _ = portion_data(portion_root_path)
    ...
    if portion_type:    # additional package info exclusively by portions (modules or sub-packages)
        ...
        for p_name in portions_package_names:
            ...
            _pt, _pn, extra_modules = portion_data(os.path.join(setup_path, '..', p_name, namespace_name))
...
def portion_data(portion_root_path: str) -> Tuple[str, str, tuple]:
