...
class Runner(Executor):
    ...
    def __call__(self, args, rerun=False, **compute_kwargs):
        ...
        """
        max_cmds = sum(list(map(len, list(self.prj._samples_by_interface.values()))))
        ...
        for piface, conductor in list(submission_conductors.items()):
        ...
        for sample, failures in list(failures.items()):
            for f in failures:
        ...
        if samples_by_reason:
            _LOGGER.info("\n{} unique reasons for submission failure: {}".format(
                len(samples_by_reason), ", ".join(list(samples_by_reason.keys()))))
            full_fail_msgs = [_create_failure_message(reason, samples)
                              for reason, samples in list(samples_by_reason.items())]
            _LOGGER.info("\nSummary of failures:\n{}".
                         format("\n".join(full_fail_msgs)))
...
def _create_stats_summary(project, counter):
    ...
    for sample in project_samples:
        ...
        columns.extend(list(sample_stats.keys()))
