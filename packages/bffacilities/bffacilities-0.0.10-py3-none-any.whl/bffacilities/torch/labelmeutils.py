...
class LabelmeModifier(object):
    ...
    def _resize(self, name, size, jsoncontent):
        ...
        # if self.debug:
            # newImage.show(f"Resized Image {name}")
    ...
    def _rotate(self, name, jsoncontent, degree, center, boarderValue, scale=1.0):
        ...
        rotated = cv.warpAffine(image, M, (iw, ih), borderValue=boarderValue)
        # if self.debug:
    ...
    def rotate(self, start=-10, end=10, num=20, center=None, boarderValue = (0, 0, 0)):
        """
        Arguments:
            start(float): start_degree for rotation
            end(float): end_degree for rotation
            interval(float): interval_degree for rotation
        ...
        for name in tqdm(self.names):
            ...
            # print("JsonContent", jsoncontent)
            rc = 0
            for deg in np.linspace(start, end, num):
                jsoncontent = copy.deepcopy(orijsoncontent) 
                newImage = self._rotate(name, jsoncontent, deg, center, boarderValue)
                newName = f"{name}-r{rc:04}"
    ...
    def _getAnno(self, label_file, img, image_id):
        ...
        for shape in label_file.shapes:
            ...
            if shape_type == "point": 
                ...
                if matched[1] not in keypoints:
                    keypoints[matched[1]] = []
                keypoints[matched[1]].append((matched[2], label, points[0]))
                continue
        ...
        for instance, mask in masks.items():
            ...
            self.data["annotations"].append(anno)
