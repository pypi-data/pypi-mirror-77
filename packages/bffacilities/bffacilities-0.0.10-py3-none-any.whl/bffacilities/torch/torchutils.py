...
class PlotHelper():
    ...
    def plotArray(self, im, cmap='gray', **kwargs):
        # print(im.shape)
        fig = plt.figure(figsize=(12, 8), dpi= 100, facecolor='w', edgecolor='w', clear=True)
...
class PredictionViewer(MaskContourSaver):
    ...
    def showKeypoint(self, im, radius=5, kp_fill=(128, 200, 128), **kwargs):
        """
        Arugments:
            im(PIL.Image.Image)
        """
        if self.keypoints is not None:
            ...
            for kps in keypoints:
                # print(k, fill)
                for k in kps:
                    fill = tuple(x for x in colormap[num_color-i-1])
...
def getSGDOptimizer(model, lr=0.005, weight_decay=0.00002, step_size=6):
