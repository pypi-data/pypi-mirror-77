...
class HourGlassModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=0,\
        ...
        kernel_size = (2, 2)
        padding = 0
        self.down_1 = block(in_channels, mid_channels, pool_kernel_size=kernel_size, pool_padding=padding, stride=2, batch=True)
        # 256 x 16 x 16  # 32 x 32
        self.down_2 = block(mid_channels, mid_channels, pool_kernel_size=kernel_size, pool_padding=padding, stride=2, batch=True)
        
        # 256 x 8 x 8    # 16 x 16     
        self.down_3 = block(mid_channels, mid_channels, pool_kernel_size=kernel_size, pool_padding=padding, stride=2, batch=True)
        # 256 x 4 x 4    # 8 x 8
        self.down_4 = block(mid_channels, mid_channels*2, pool_kernel_size=kernel_size, pool_padding=padding, stride=2, batch=True)
        # 256 x 2 x 2
        ###############################
        # 256 x 2 x2
        block = _HourGlassResUpUnit if residual else _HourGlassUpUnit
        self.up_4 = block(mid_channels*2, mid_channels)
        # 256 x 4 x 4
        self.up_3 = block(mid_channels, mid_channels)
        # 256 x 8 x 8
        self.up_2 = block(mid_channels, mid_channels)
        # 256 x 16 x 16
        self.up_1 = block(mid_channels, out_channels)
        # 256 x 32 x 32
