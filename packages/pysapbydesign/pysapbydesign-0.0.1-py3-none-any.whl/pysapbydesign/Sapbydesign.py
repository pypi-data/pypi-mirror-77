...
class Sapbydesign:
    ...
    def get_report(self, _object_key, sap_para_fiscyearper_start=None,
        ...
        for ccomp in _object.get('filter').get('CCOMPANY_UUID'):
            ccompany_uuid = ccomp
            for setofbks in _object.get('filter').get('CCOMPANY_UUID').get(ccomp).get('PARA_SETOFBKS'):
                para_setofbks = setofbks
                log_info('CCOMPANY_UUID: %s - PARA_SETOFBKS: %s' % (ccompany_uuid, setofbks))
                self._get_specific_report(_object_key, para_setofbks, ccompany_uuid, sap_para_fiscyearper_start,
                                          sap_para_fiscyearper_end, cleaning)

    def _get_specific_report(self, _object_key, para_setofbks, ccompany_uuid, sap_para_fiscyearper_start=None,
                             sap_para_fiscyearper_end=None, cleaning=True):
        _object = self.objects[_object_key]
        query_filter = ["PARA_SETOFBKS eq '%s'" % para_setofbks, "CCOMPANY_UUID eq '%s'" % ccompany_uuid]
