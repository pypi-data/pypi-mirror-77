...
from typing import Sequence, Optional, List, Dict, cast

from pyexlatex.logic.format.and_join import join_with_commas_and_and_output_list

from derobertis_cv.models.skill import SkillModel, first_word_untouched_rest_capitalized, \
...
PLOTTING_SKILL = SkillModel('plotting', 4, parents=(PROGRAMMING_SKILL, PRESENTATION_SKILL))
...
_SKILLS: List[SkillModel] = [
    ...
    SkillModel('empirical research', 5, parents=(RESEARCH_SKILL, STATISTICS_SKILL)),
    ...
    SkillModel('pandas', 5, parents=DS_FRAMEWORK_SKILLS, flexible_case=False),
    SkillModel('NumPy', 4, parents=DS_FRAMEWORK_SKILLS, flexible_case=False),
    SkillModel('SciPy', 3, parents=DS_FRAMEWORK_SKILLS, flexible_case=False),
    SkillModel('SymPy', 4, parents=DS_FRAMEWORK_SKILLS, flexible_case=False),
    SkillModel('scikit-learn', 3, parents=DS_FRAMEWORK_SKILLS + (MACHINE_LEARNING_SKILL,), flexible_case=False),
    SkillModel('Jupyter', 4, parents=DS_FRAMEWORK_SKILLS, flexible_case=False),
...
def get_skills(exclude_skills: Optional[Sequence[str]] = None,
    ...
    if not exclude_skill_children:
        ...
        child_excluded_skills = set()
        for skill_name in exclude_skills:
            skill = SKILLS[skill_name]
            child_excluded_skills.update(skill.get_nested_children())
    ...
    if order is not None:
        use_skills.sort(
            key=lambda skill: order.index(skill.to_lower_case_str()) if skill.to_lower_case_str() in order else 100000  # type: ignore
        )
