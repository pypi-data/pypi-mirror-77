...
class Ellipsoids(Policy):
    ...
    def __init__(self, num_states=None, num_pre_change_states=None, ellipsoids=None):
        if ellipsoids is not None:
            ...
            self._free_params = 0.5 * np.ones(size=param_size)
    ...
    def __repr__(self):
        return "Ellipsoids".format(
            self._free_params, self.ellipsoids
        )
    ...
    def _reparameterise(ellipsoid_coefficients):
        ...
        """
        return logit(ellipsoid_coefficients)
...
class _StoppingProblem(ModuleFactory, FilterMixin):
    ...
    def optimise(
        ...
        for step in range(steps):
            ...
            logging.debug(
                " ******* Started optimisation step {}/{} ********".format(step, steps)
            )
            ...
            logging.debug("Param set 1 under test:\n {}".format(params1))
            logging.debug("Param set 2 under test:\n {}".format(params2))
