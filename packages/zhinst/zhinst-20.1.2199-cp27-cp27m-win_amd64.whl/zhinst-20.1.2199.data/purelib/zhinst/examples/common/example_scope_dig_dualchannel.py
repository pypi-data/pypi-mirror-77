...
def run_example(device_id, do_plot=False, scope_length=2**12, scope_trigholdoff=0.050, module_averager_weight=1,
    ...
    if do_plot:
        ...
        def plot_scope_records(plot_axes, scope_records, scope_input_channel, scope_time=0):
            """
            ...
            for index, record in enumerate(scope_records):
                ...
                if not record[0]['channelmath'][scope_input_channel] & 2:
                    ...
                    ax.plot(1e6*t, wave, color=colors[scope_input_channel][index])
                    ...
                    ax.semilogy(f/1e6, wave, color=colors[scope_input_channel][index])
            ax.grid(True)
        ...
        plot_scope_records(ax1, data_no_trig[wave_nodepath], 0)
        plot_scope_records(ax1, data_no_trig[wave_nodepath], 1)
        ax1.set_title('{} Scope records from 2 channels ({}, triggering disabled)'.format(
            len(data_no_trig[wave_nodepath]), device))

        # Plot the scope data with triggering enabled.
        plot_scope_records(ax2, data_with_trig[wave_nodepath], 0)
        plot_scope_records(ax2, data_with_trig[wave_nodepath], 1)
        ax2.axvline(0.0, linewidth=2, linestyle='--', color='k', label="Trigger time")
        ax2.set_title('{} Scope records from 2 channels ({}, triggering enabled)'.format(
        ...
        plot_scope_records(ax, data_fft[wave_nodepath], 0, scope_time)
        plot_scope_records(ax, data_fft[wave_nodepath], 1, scope_time)
        ax.set_title('FFT of {} scope records from 2 channels ({})'.format(len(data_fft[wave_nodepath]), device))
        ax.set_xlabel('f [MHz]')
        fig2.show()
