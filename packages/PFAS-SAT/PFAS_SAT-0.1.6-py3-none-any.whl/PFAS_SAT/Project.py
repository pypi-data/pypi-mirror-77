...
import pandas as pd
import numpy as np
from PFAS_SAT.SubProcesses import split
import graphviz
...
class Project():
    ...
    def setup_network(self):
        ...
        Project.calc(product, 'Start', self.ProcessModels, self.FlowParams, self.Inventory)

    @staticmethod
    def calc(product, source, processmodel, FlowParams, Inventory):
        for prdct in product:
            if FlowParams[prdct]:
                flows = split(InputFlow=product[prdct], **FlowParams[prdct])
                for prcs, flw in flows.items():
                    if sum(flw.PFAS.values) > 500:
                        Inventory.add(prdct, source, prcs, flw)
                        processmodel[prcs]['Model'].calc(flw)
                        product_2 = processmodel[prcs]['Model'].products()
                        Project.calc(product_2, prcs, processmodel, FlowParams, Inventory)
            else:
                # Change the warning to exception when all the process models are done
                warnings.warn("There is no process to manage {}".format(prdct))
    ...
    def plot_sankey(self, view=True, filename=None):
        ...
        edge_color_dict = {'CompFeed': (202, 255, 112),  # darkolivegreen1  #CAFF70
                           'ComRes': (118, 238, 0),  # chartreuse2	#76EE00
