...
def pv(T,tc,pc,acentric,p_0=1,method='pr',alfa_function='alfa_peng_robinson',pvt=False):
    ...
    pressures=[]
    for p in pres:
        x=solve_eos(T,p,tc,pc,acentric,method,alfa_function,diagram=True)
...
def pt(tc,pc,acentric,p_0=1,t_0=298,method='pr',alfa_function='alfa_peng_robinson',pvt=False):
    ...
    t_first = solve_VLE(300,pressures[0],tc,pc,acentric,method,alfa_function,solving_for='temperature')
    temperatures=[t_first,]
    for i in range(1,len(pressures)):
        t = solve_VLE(temperatures[i-1],pressures[i],tc,pc,acentric,method,alfa_function,solving_for='temperature')
...
def pvt(tc,pc,acentric,t_0=298,p_0=1,method='pr',alfa_function='alfa_peng_robinson'):
    ...
    i=1
    for T in temp:
        pressures,volumes = pv(T,tc,pc,acentric,p_0=1,method,alfa_function,pvt=True)
