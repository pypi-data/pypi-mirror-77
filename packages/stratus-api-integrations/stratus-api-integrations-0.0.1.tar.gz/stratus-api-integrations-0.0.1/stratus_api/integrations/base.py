...
def get_schema_values_recursive(schema, value_name):
    example = dict()
    if schema.get('properties'):
        for k, v in schema.get('properties').items():
            value = get_schema_values_recursive(schema=v, value_name=value_name)
...
def validate_settings(settings, settings_type, platform_name=None):
    import jsonschema
    from stratus_api.core.exceptions import ApiError
...
def create_entity(entity_type, name, platform_name, settings, **kwargs):
