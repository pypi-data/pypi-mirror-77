...
def eks_init(
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".yaml", mode="w+") as f:
            ambassador_yaml = generate_main_ambassador_yaml(
                "AWS", load_balancer_visiblity == "public"
            )
        ...
        with tempfile.NamedTemporaryFile(suffix=".yaml", mode="w+") as f:
            ambassador_yaml = generate_ambassador_host_yaml(
                cluster["serving_cluster_domain"], load_balancer_visiblity == "public"
            )
            f.write(ambassador_yaml)
            f.flush()
            subprocess.check_call(
    ...
    cluster_utils.add_prometheus_stack(kubectl_env=kubectl_env)
