...
@click.command(
    ...
    short_help="Sets the cloud machine instance permissions for the cluster (Currently AWS ONLY)",
    hidden=True,
)
@click.pass_context
@cluster_utils.pass_cluster
...
def set_instance_permissions(
    ...
    For AWS this requires an IAM Role and an IAM Instance Profile that match. More details here:
    https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use_switch-role-ec2_instance-profiles.html

    For GCP this requires an IAM Service Account that has granted the `serviceAccountUser` role to the
    Spell service account `193976455398-compute@developer.gserviceaccount.com`. More details here:
    ...
    if cluster_type == "AWS":
        ...
        #     ctx.exit(1)
        raise ExitException(
            "Sorry, but GCP is not yet supported for this feature.".format(cluster_type)
        )
    else:
...
@click.command(
    ...
    short_help="Unsets the cloud machine instance permissions for the cluster (Currently AWS ONLY)",
    hidden=True,
)
@click.pass_context
@cluster_utils.pass_cluster
...
def serving_group_scale(
    ...
    if cluster["cloud_provider"] == "AWS":
        eks_scale_nodegroup(aws_session, cluster, serving_group_name, min_nodes, max_nodes)
    elif cluster["cloud_provider"] == "GCP":
