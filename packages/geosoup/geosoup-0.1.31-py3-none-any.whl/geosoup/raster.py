            ...
            if any(list(type(elem) != int for elem in overviews)):
                ...
                for elem in overviews:
                    try:
                        ...
                        warnings.warn('Conversion error: {} -for- {}'.format(e.args[1], elem))
        ...
        for i, b in enumerate(band_order):
            ...
            for i_try in range(n_tries):
                try:
                    ...
                    read_success = False
                    Opt.cprint(e)
                    warnings.warn('Retrying failed read... attempt {}'.format(i_try + 1))
                    time.sleep(random.random() + 1.0)

            if not read_success:
                warnings.warn('Retrying failed with {} attempts...using no-data value'.format(str(n_tries)))
