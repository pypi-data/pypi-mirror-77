...
from .deid_field import DeIdField, DeIdRegexSubField
...
class FileProfile:
    ...
    def to_config(self):
        ...
        if self.datetime_format != self.__class__.datetime_format:
            result["datetime-format"] = self.datetime_format

        if self.date_format != self.__class__.date_format:
        ...
        for fieldname, field in self.field_map.items():
            if isinstance(field, DeIdRegexSubField):
                for member in field.list_members:
                    ...
                    if invalid_fields:
                        errors.append(
                            f"regex-sub field {fieldname} regex "
