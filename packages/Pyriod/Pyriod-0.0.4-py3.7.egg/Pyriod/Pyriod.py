...
class Pyriod(object):
   ...
   """     
    def _display_original_lc(self):
        ...
        ymin = np.min([np.min(self.lc_orig.flux[self.include]),np.min(self.lc_model_sampled.flux[self.include])])
        ymax = np.max([np.max(self.lc_orig.flux[self.include]),np.max(self.lc_model_sampled.flux[self.include])])
        self.lcax.set_ylim(ymin-0.05*(ymax-ymin),ymax+0.05*(ymax-ymin))
        #fold if requested
        if self._fold.value:
            xdata=self.lc_orig.time*self._fold_on.value*self.freq_conversion % 1.
            self.lcplot_data.set_offsets(np.dstack((xdata,self.lc_orig.flux))[0])
            self.lcplot_model.set_alpha(0)
            self.lcax.set_xlim(0,1)
        else:
            self.lcplot_data.set_offsets(np.dstack((self.lc_orig.time,self.lc_orig.flux))[0])
    ...
    def _display_residuals_lc(self):
        ...
        if self._fold.value:
            xdata=self.lc_orig.time*self._fold_on.value*self.freq_conversion % 1.
            self.lcplot_data.set_offsets(np.dstack((xdata,self.lc_resid.flux))[0])
            self.lcplot_model.set_alpha(0)
            self.lcax.set_xlim(0,1)
        else:
            self.lcplot_data.set_offsets(np.dstack((self.lc_orig.time,self.lc_resid.flux))[0])
            self.lcplot_model.set_alpha(1)
            self.lcax.set_xlim(np.min(self.lc_orig.time),np.max(self.lc_orig.time))
        self.selector.update(self.lcplot_data)
        self.lcfig.canvas.draw()
  """      
    def _display_lc(self,residuals=False):
        ...
        if residuals:
            lc = self.lc_resid
            self.lcplot_model.set_ydata(np.zeros(len(lc.flux)))
        else:
            self.lcplot_model.set_ydata(lc.flux)
        #rescale y to better match data
    ...
    def TimeSeries(self):
        ...
        options.set_title(0, 'options')
        return VBox([self.lcfig.canvas,self._reset_mask,options])
