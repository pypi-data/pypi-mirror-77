# AUTOGENERATED! DO NOT EDIT! File to edit: Structure IO.ipynb (unless otherwise specified).

__all__ = ['save_mp_API', 'load_mp_data', 'get_crystal', 'get_poscar', 'trace_kpath', 'get_kmesh', 'intersect_3p_p_3v',
...
def get_poscar(formula ,api_key=None,mp_id=None,max_sites = None):
    ...
    for cr in crys:
        ...
        poscars.append(vp.Dic2Dot(net_out))
    return poscars


# Cell
def trace_kpath(hsk_list=[],n=5,weight= None ,ibzkpt = None,labels=[],outfile=None):
    """
    ...
    - **Parameters**
        ...
        - n        ; int, number of points between and including two hsk_points.
    ...
    """
    import numpy as np
    if np.ndim(hsk_list)==2:
        hsk_list = [hsk_list] # Make overall 3 dimensions to include breaks in path
    xs,ys,zs, inds,joinat = [],[],[],[],[]
    for j,a in enumerate(hsk_list):
        for i in range(len(a)-1):
            inds.append(n*(i+j))
            if j !=0:
                joinat.append(n*(i+j))
            xs.append(np.linspace(a[i][0],a[i+1][0],n))
            ys.append(np.linspace(a[i][1],a[i+1][1],n))
            zs.append(np.linspace(a[i][2],a[i+1][2],n))
    xs,ys,zs = np.reshape(xs,(-1)),np.reshape(ys,(-1)),np.reshape(zs,(-1))
    if weight == None:
        weight = 1/len(xs)
    out_str = ["{0:>16.10f}{1:>16.10f}{2:>16.10f}{3:>8}".format(x,y,z,weight) for x,y,z in zip(xs,ys,zs)]
    out_str = '\n'.join(out_str)
    N = np.size(xs)
    if ibzkpt != None:
        import os
...
def get_bz(poscar = None,loop = True,digits=8):
    ...
    mid_all = np.concatenate([[[0,0,0]],mid_faces,mid_edges,verts])
    mid_basis_all = np.array([np.linalg.solve(basis.T,v) for v in mid_all])
    symprec = 1e-10 # precison on purpose to handle very small values around zero.
    x_inds = np.where(mid_basis_all[:,0]>=-symprec)
    y_inds = np.where(mid_basis_all[:,1]>=-symprec)
    z_inds = np.where(mid_basis_all[:,2]>=-symprec)
    xyz_inds = np.intersect1d(np.intersect1d(x_inds,y_inds),z_inds)
    mid_basis_p = np.take(mid_basis_all,xyz_inds,axis=0).squeeze()
    mid_all_p = np.take(mid_all,xyz_inds,axis=0).squeeze()

    # Round off results
    mid_all_p    = np.round(mid_all_p,digits)
    mid_basis_p  = np.abs(np.round(mid_basis_p,digits)) # Only this absolute to remove negative zeros.
...
# Cell
def plot_bz(poscar = None,fill = True,color = 'rgba(168,204,216,0.4)',background = 'rgb(255,255,255)'):
    """
    - Plots interactive figure showing axes,BZ surface, special points and basis, each of which could be hidden or shown.
    - **Parameters**
        - pocar      : POSCAR or 3 basis vectors' list forming POSCAR. Auto picks in working directory.
    ...
    # Special Points
    i=0
    for txt,ma in bz.specials.items():
        i = i+1
        fig.add_trace(go.Scatter3d(x=[ma[0]], y=[ma[1]],z=[ma[2]],
            hovertext="P{}".format(txt),name="K<sub>{}</sub>".format(i),line_color='blue'))
