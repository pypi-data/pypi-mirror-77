...
def read_asxml(path=None):
    ...
    if(not path):
        return print('vasprun.xml not found.')
    else:
        tree = ET.parse(path)
...
def export_vasprun(path=None,skipk=None,elim=[],joinPathAt=[],shift_kpath=0):
    ...
    dim_dic=vp.Dic2Dot({'kpoints':'(NKPTS,3)','kpath':'(NKPTS,1)','bands':'(NKPTS,NBANDS)',\
...
def load_export(path= './vasprun.xml',
    ...
    structure         = vp.get_structure(vp.read_asxml(file_name))

    # Load Data
    bands= np.loadtxt('Bands.txt')
    pro_bands= np.loadtxt('Projection.txt')
    pro_dos = np.loadtxt('pDOS.txt')
    dos= np.loadtxt('tDOS.txt')
    with open('pDOS.txt','r') as f:
        fields = f.readline().split('[')[1].split(']')[0].split()
    ...
    # Work now!
    sys_info = vp.Dic2Dot({'SYSTEM': SYSTEM,'NION': NION,'TypeION': TypeION,'ElemName': ElemName,
               'ElemIndex': ElemIndex,'ISPIN': ISPIN})
    dim_info = vp.Dic2Dot({'kpoints': '(NKPTS,3)','kpath': '(NKPTS,1)','bands': '(NKPTS,NBANDS)',
               'dos': '(grid_size,3)','pro_dos': '(NION,grid_size,en+pro_fields)',
               'pro_bands': '(NION,NKPTS,NBANDS,pro_fields)'})
    poscar = structure # Already Dic2Dot Object
