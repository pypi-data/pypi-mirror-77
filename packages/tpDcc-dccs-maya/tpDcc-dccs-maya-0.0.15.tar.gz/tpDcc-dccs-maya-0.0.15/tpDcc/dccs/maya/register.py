...
def register_class(cls_name, cls, is_unique=True, skip_store=False):
    """
    This function registers given class into tpDcc-dccs-maya module
    ...
    if REGISTER_ATTR not in tpDcc.dccs.maya.__dict__:
        tpDcc.dccs.maya.__dict__[REGISTER_ATTR] = list()

    if not is_unique and cls_name in tpDcc.dccs.maya.__dict__:
        return

    tpDcc.dccs.maya.__dict__[cls_name] = cls
    if not skip_store:
        tpDcc.dccs.maya.__dict__[REGISTER_ATTR].append(cls_name)
