...
class ValidFormatJob:
    ...
    def repartition(self, df: DataFrame, cluster_spec: ClusterSpec, num_partitions: str) -> Union[DataFrame, InvalidJob]:

        size = df.size.compute()
        workers = cluster_spec.num_workers() or self.partitions
    ...
    def run(self, cluster_spec: ClusterSpec) -> Union[ExecutedJob, InvalidJob]:
        df = self.df()
        if isinstance(df, DataFrame):
            def write_partitioned(df: PDataFrame, partition_columns: List[str]):
                ddf = dd.from_pandas(df, npartitions=cluster_spec.num_workers() or self.partitions)
                labels = {}
