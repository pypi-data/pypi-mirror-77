...
class DSRequest(RPCRequest):
    ...
    def get_data_array(self, excluded_keys=['grid']):
        ...
        if isinstance(self.json, dict):
            ...
            # res = (True, [dict((key, value) for (key, value) in operation.items() if value != None and not str(key).startswith('_') and key not in excluded_keys) for operation in operations])
        return res
    ...
    def get_oldValues(self, excluded_keys=['grid']):
        ...
        if isinstance(self.json, dict):
            ...
            # res = dict((key, value) for (key, value) in res.items() if value != None and not str(key).startswith('_') and key not in excluded_keys)
        return res
