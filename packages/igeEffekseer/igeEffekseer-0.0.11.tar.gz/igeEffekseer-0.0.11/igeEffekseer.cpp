...
PyObject* effekseer_new(PyTypeObject* type, PyObject* args, PyObject* kw)
{
	...
	int culling_enable = false;
	if (PyArg_ParseTuple(args, "|i", &culling_enable))
	{
		self = (effekseer_obj*)type->tp_alloc(type, 0);
		self->effekseer = new pyxieEffekseer();
		self->effekseer->init(culling_enable);
	}
...
{
	static char* kwlist[] = { "proj", "view", "dt", "clear", "update", "render", "culling", "culling_size", NULL };

	float dt = 0.0f;
	float culling_size = 1000.0;
	int clear = false;
	int update = true;
	int render = true;
	int culling = false;
	PyObject* mat_proj;
	PyObject* mat_view;
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOf|iiiif", kwlist , &mat_proj, &mat_view, &dt, &clear, &update, &render, &culling, &culling_size))
		return NULL;
...
{
	static char* kwlist[] = { "name", "loop", "position", "rotation", "scale", NULL };
	...
	char* name;
	int loop = 0;
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|iOOO", kwlist, &name, &loop, &position, &rotation, &scale)) return NULL;
	...
	int handle = self->effekseer->play(name , loop, *((Vector3D*)pos), *((Vector3D*)rot), *((Vector3D*)scl));
