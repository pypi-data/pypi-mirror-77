...
pyxieEffekseer::pyxieEffekseer()
	...
	, projection_near(1.0)
	, projection_far(1000)
	, camera_eye(Vector3D(0.0, 0.0, -20.0))
	, camera_at(Vector3D(0.0, 0.0, 0.0))
	, camera_up(Vector3D(0.0, 1.0, 0.0))
	, viewport(Vector2D(-1.0, -1.0))
	, viewsize(Vector2D(480.0, 640.0))
{
}
...
void pyxieEffekseer::update(float dt)
{
	if (projection_ortho)
	{
		renderer->SetProjectionMatrix(Matrix44().OrthographicRH(viewsize.X, viewsize.Y, projection_near, projection_far));
		renderer->SetCameraMatrix(Matrix44().LookAtRH(camera_eye, camera_at, camera_up));
	}
	else
	{
		renderer->SetProjectionMatrix(Matrix44().PerspectiveFovRH(projection_fov / 180.0f * 3.14f, projection_aspect, projection_near, projection_far));
		renderer->SetCameraMatrix(Matrix44().LookAtRH(camera_eye, camera_at, camera_up));
	}
