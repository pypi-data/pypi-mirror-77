...
@click.option("-pc", "--priority_class", default = "best-effort", required=True, help="cloudml resource type")
@click.option("-ts", "--test_set", default = None, required=True, help="local test txt, if not set, will use the test.txt in datasets")
@click.argument("job_name", callback=warn_with_no_job)
def predict(job_name, gpu_type, gpu_memory, gpu_number, priority_class, test_set):
    ...
    if test_set:
        # check the file exists in local and copy to target folder
        if not os.path.exists(test_set):
            click.echo(error("not found {} in local disk".format(test_set)))
            return
        
        # copy the local file to target
        target_test_name = "test_{}.txt".format(hash(test_set))
    ...
    for cloud_ml_task in tqdm(cloud_ml_tasks, desc="Submiting"):
        ...
        if target_test_name:
            upload_file_to_fds(default_config.fds_bucket, test_set, os.path.join(data_uri, target_test_name))
        ...
        with create_temp_dir() as trainer_uri:
            ...
            if target_test_name:
                bert_job.test_filename(target_test_name)

            bert_job.submit()

            cloudml_task = Cloud_ml_task(cloudml_name, Cloud_ml_task_type.TRAIN, model_name)

            if target_test_name:
                cloudml_task.test_file_name = target_test_name 
                cloudml_task.local_test_path = test_set      

            predict_cloud_ml_tasks.append(cloudml_task)
