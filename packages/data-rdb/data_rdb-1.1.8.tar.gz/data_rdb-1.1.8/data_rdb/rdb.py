...
class Rethink_DBS(GeneralData):
        ...
        try:
            if self.default_db:
                ...
                self.session = await self.r.connect(db=dbname,
                                                    ...
                                                    port=dbport,
                                                    **kwargs)
                self.logger.info("Connected to RethinkDB with database %s" %
                                 self.default_db)
            else:
            ...
            msg = f'''PD_TOA_-1 + {conn_error}, ConnectionClosed rc_enu connection....'''
            self.logger.exception(kwargs["origin_exception"])
            rc = None
            raise conn_error
    ...
    def close(self):
        self.session.close(noreply_wait=True)
        super().close()
    ...
    async def create_table(self, table_name: str, dbname: str = None):
        ...
        try:
            ...
            self.logger.exception(
                "Error crítico al crear tabla %s, exception" % (table_name, e))
            raise e
    ...
    async def create_index(self,
        ...
        try:
            ...
            self.logger.exception("Index %s can't be created %s, exception" %
                                  (index, table_name, e))
            raise e
    ...
    async def save_data(self, table_name, data, options=None):
        ...
        try:
            if table_name in self.tables[self.defaultdb]:
                ...
                return await self.r.db(
                    self.defaultdb).table(table_name).insert(
                        data, **options).run(self.session, **options)
        except Exception as ex:
    ...
    async def get_data(self, table_name, index_name, value):
        return await self.r.db(dbname).table(table_name).get_all(
            ...
            if table_name in self.tables.get(self.default_db, {}):
                if index:
                    ...
                    if not type(filter_exp) == list:
                        ...
                        upper = filter_exp[1]
                    self.logger.info(
                        "Obtaining data from %s to %s on table %s, with filters %s using index %s"
                        % (lower, upper, table_name, filter_opt, index))
                    return await self.r.db(dbname).table(table_name).between(
                    ...
                    return await self.r.db(dbname).table(table_name).filter(
                        filter_exp).order_by(order_by).run(
                            self.session, **options)
            else:
                return []
        except Exception as ec:
            self.logger.exception("Error on the data extraction, error %s" %
                                  ec)
