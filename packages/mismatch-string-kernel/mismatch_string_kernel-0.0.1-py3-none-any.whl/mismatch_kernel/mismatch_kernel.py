...
class MismatchKernel:
    ...
    def get_kernel(self, x1, x2):
        ...
        if x1 not in self.MISMATCH_VECTORS:
            x1_norm, mv = self.mismatch_tree.vectorize(x1)
            self.MISMATCH_VECTORS[x1_norm] = mv
        if x2 not in self.MISMATCH_VECTORS:
            x2_norm, mv = self.mismatch_tree.vectorize(x2)
            self.MISMATCH_VECTORS[x2_norm] = mv

        v1 = self.MISMATCH_VECTORS[x1]
        v2 = self.MISMATCH_VECTORS[x2]

        # kernel = np.dot(v1, v2)  # only valid if mismatch vectors are standard format and not DOK
        # normalized_kernel = kernel / (np.sqrt(np.dot(v1, v1)) * np.sqrt(np.dot(v2, v2))) if kernel != 0 else 0
        ...
        self.KERNEL_MATRIX[x1][x2] = normalized_kernel
