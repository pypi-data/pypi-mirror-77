...
@LastEditTime: 2020-08-24 17:50:05
...
class WorkWechatHelper(object):
    ...
    def _get_msg_params(self, notice_content, notice_object, notice_object_type, notice_content_type="text"):
        """
    ...
    def get_web_auth_link(self, redirect_uri, state=None):
        ...
        try:
            res = requests.get(url=self.oauth_url, params=params).json()
            if int(res["result"]) == 1:
                return res["data"]["auth_url"]
            print(f"get请求url:{self.oauth_url}异常,{res['desc']}")
            return None
        except:
            print(f"get请求url:{self.oauth_url},params:{params}异常")
            return None
    ...
    def send_msg_by_webhook(self, notice_content, notice_object, notice_content_type="text"):
        ...
        try:
            res = requests.post(url=self.msg_url, data=params, headers={
                                'Content-Type': 'application/x-www-form-urlencoded'}).json()
            if int(res["result"]) == 1:
                return True
            print(f"post请求url:{self.msg_url}异常,{res['desc']}")
            return None
        except:
            print(f"post请求url:{self.msg_url},params:{params}异常")
            return None
    ...
    def send_msg_by_template(self, notice_content, notice_object, notice_content_type="text"):
        ...
        params = self._get_msg_params(notice_content=notice_content, notice_content_type=notice_content_type,
