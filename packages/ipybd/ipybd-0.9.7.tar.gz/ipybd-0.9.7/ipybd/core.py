...
class FormatTable:
    ...
    def table_mapping(self, mapping:dict):
        """按照给定的 dict 对 dataframe 各列进行列拆分、合并、修改列名
            ...
            mapping: dataframe 需要重塑的各列映射关系，相应写法如下：
                     ...
                     其中 raw 为原列名，new 为新列名，sp 为列合并或分割使用的分隔符
        """
    ...
    def merge_columns(self, columns, separators, new_header):
        """ 按指定模式合并多列
            ...
            separators: 合并后，各列值之间的分隔符，如果各列之间分隔符不同，可按序组成
                        元组传递
                        如果传递的是字符 'd', 'r', 'o', 'l', 'a' 则分别表示将各列合
                        并成字典、行列，json object 对象、json array 对象。
            new_header: 合并列的新列名
        """
        if separators == 'r':
            ...
            mergers = self.__merge2pairs(columns, new_header, typ='dict')
    ...
    def __unmapped_columns2std(self, unmapped_fields:list):
        ...
        if elements[-2][-1].strip() == "=" and elements[-1][-1].strip() == "":
            ...
            if elements[0][0] in ['d', 'l', 'r', 'o', 'a']:
                ...
                separators = tuple(sep[2].strip() if set(sep[2]) != {" "} else " " for sep in elements[0:-2])
            if separators == ():
                # 如果合并的列名只有一个，为一对一映射
        ...
        # 单个列名分拆为多个标准列名
            ...
            # 分隔符是空格的，保留一个空格，非空格的分隔符去除两侧空格
            splitters = tuple(spl[2].strip() if set(spl[1]) != {" "} else " " for spl in elements[1:-1])
...
class RestructureTable(FormatTable):
    def __init__(self, io, fcol = False):
        ...
        #self.new_columns from std_table_objects
        self.rebuild_table()
    ...
    def __re_range_columns(self):
        std_columns = [column for column in self.new_columns.__members__.keys() if column in self.df.columns]
        other_columns = [column for column in self.df.columns if column not in std_columns]
    ...
    def rebuild_columns(self):
        ...
        for field in self.new_columns:
            ...
            if isinstance(field.value[0], (str, tuple, list)):
                ...
                column_name = field.name[1:] if field.name.startswith("_") else field.name
                #print(column_name)
                if args_name:
                    #print(args_name)
                    if len(args_name) == 1:
                        # 只对原表相应对字段名进行修改，args_name 为 str 类型
                        self.df.rename(columns={args_name[0]:column_name}, inplace=True)
                    else:
                        # 进一步合并已经被映射转换过的列
                        self.merge_columns(std_fields, field.value[-1], column_name)
                # 如果原表中无法找到相应的列，用 None 填充新列
                elif self.fill_column:
                    columns = [name for name, obj in self.new_columns.__members__.items() if obj is field]
                    for col in columns:
                ...
                if args_name:
                    ...
                    # 若需要将多列先合并为单列再做传递，可以在模版中先定义合并多列
                    args = [self.df[arg_name] for arg_name in args_name] + list(field.value[field.value[-1]+1:-1])
                    new_cols = field.value[0](*args)()
                    columns = [name for name, obj in self.new_columns.__members__.items() if obj is field]
                    # _ 开头的名称是模板用于临时定义使用，命名时需要去除 _
                    new_cols.columns = [column[1:] if column.startswith("_") else column for column in columns]
