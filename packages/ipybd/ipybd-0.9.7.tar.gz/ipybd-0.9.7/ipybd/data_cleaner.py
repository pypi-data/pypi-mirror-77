...
class BioName:
    ...
    def __build_cache(self, action, querys=None):
        ...
        if cache:
            if querys:
                ...
                names = self.querys
            action_func = { # 注意 stdName 的 col 函数至于元组最后，
                            # 以避免 ipni/powo 的结果被 col 函数解析
            ...
            for org_name, query in names.items():
                try:
                    results[org_name] = self.get_cache_name(cache[org_name], action_func[action])
                except KeyError:
    ...
    def format_name(self, raw_name):
        ...
        if subspec_split == []:
            ...
            elif self.group == "animal":
                simple_name = " ".join([species_split[0], species_split[2], subspec_split[0][2]]) if species_split[1]=="" else " ".join([species_split[0], species_split[1], species_split[2], subspec_split[0][2]])
