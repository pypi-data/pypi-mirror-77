...
BASE_URL = 'https://wqvsv5x50d.execute-api.eu-north-1.amazonaws.com/dev'
...
class Client:
	...
	def _request(self, resource, body):
		raw = requests.post(url=BASE_URL+resource, data=json.dumps(body), headers=self._headers(resource, body)).text
		try:
	...
	def generate_report(self, scriptName, startTime):
		'''
