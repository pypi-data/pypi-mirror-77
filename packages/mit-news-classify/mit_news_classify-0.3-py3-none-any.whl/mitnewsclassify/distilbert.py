...
def initialize( modelfile="", #Haimoshri's model
    ...
    model = MyDistilBertModel(768, 538) #TODO: find Haimoshri's model
...
def getfeatures(txt):
    if (model is None):
        initialize()
    encoded_dict = tokenizer([txt], add_special_tokens=True, pad_to_max_length=True, max_length=1024, return_tensors="pt", return_attention_mask=True)
