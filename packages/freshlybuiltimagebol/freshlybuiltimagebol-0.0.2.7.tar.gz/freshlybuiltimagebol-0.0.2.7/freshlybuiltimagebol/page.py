...
class Page:
    def detect(image):
        ...
        closed_edges =  morphologyEx(image_edges,
                                         MORPH_CLOSE, 
                                         ones((5, 11)))
    ...
    def find_page_contours(edges, img):
        ...
        for cnt in contours:
            ...
            if (len(approx) == 4 and
                     isContourConvex(approx) and
                    max_area <  contourArea(approx) < MAX_COUNTOUR_AREA):
    ...
    def persp_transform(img, s_points):
        ...
        height = max( linalg.norm(s_points[0] - s_points[1]),
                      linalg.norm(s_points[2] - s_points[3]))
        width = max( linalg.norm(s_points[1] - s_points[2]),
