...
def scatter(inputs, target_gpus, chunk_sizes, dim=0):
    ...
    def scatter_map(obj):
        ...
        if isinstance(obj, dict) and len(obj) > 0:
            print(map(scatter_map, obj.items()))
...
class BalancedDataParallel(DataParallel):
    ...
    def scatter(self, inputs, kwargs, device_ids):
        bsz = inputs[0]['input'].size(self.dim)
        num_dev = len(self.device_ids)
        gpu0_bsz = self.gpu0_bsz
        bsz_unit = (bsz - gpu0_bsz) // max(num_dev - 1, 1)
        if gpu0_bsz < bsz_unit:
            chunk_sizes = [gpu0_bsz] + [bsz_unit] * max(num_dev - 1, 1)
            ...
            return scatter_kwargs(inputs, kwargs, device_ids, [gpu0_bsz], dim=self.dim)
