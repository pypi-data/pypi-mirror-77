...
### Метод `call(self, method: str, item_list) -> list`
...
* `item_list` - список параметров вызываемого метода. Может быть списком, а может быть одним словарем, который автоматически оборачивается в список.
