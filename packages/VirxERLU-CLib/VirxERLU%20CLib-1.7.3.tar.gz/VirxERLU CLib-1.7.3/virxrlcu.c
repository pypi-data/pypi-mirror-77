...
static const int max_speed = 2300;
...
struct jump_shot parse_slice_for_jump_shot_with_target(double time_remaining, double best_shot_value, Vector ball_location, Vector car_location, Vector car_forward, int car_boost, Vector left_target, Vector right_target, double cap_)
{
    struct jump_shot r;
    r.found = -1;
    ...
    _Bool forward_flag = forward_time > 0 && (distance / forward_time) < ((car_boost > distance / 100) ? 2275 : 1400);
    _Bool backward_flag = backward_time > 0 && distance < 1500 && (distance * 1.05 / backward_time) < 1200;
    ...
    if (forward_flag || backward_flag)
    {
        ...
        if (!pst_crrctn.swapped)
        {
            ...
            if (in_field(sub(ball_location, multiply(r.best_shot_vector, double_to_vector(best_shot_value))), 1))
            {
                ...
                if ((forward_flag && slope > 0) || (backward_flag && slope > 0.5))
                    r.found = 1;
            }
        }
    }
...
struct jump_shot parse_slice_for_jump_shot(double time_remaining, double best_shot_value, Vector ball_location, Vector car_location, Vector car_forward, int car_boost, double cap_)
{
    struct jump_shot r;
    r.found = -1;
    ...
    _Bool forward_flag = forward_time > 0 && (distance / forward_time) < ((car_boost > distance / 100) ? 2275 : 1400);
    _Bool backward_flag = backward_time > 0 && (distance * 1.05 / backward_time) < 1200;

    r.best_shot_vector = direction;

    if ((forward_flag || backward_flag) && in_field(sub(ball_location, multiply(r.best_shot_vector, double_to_vector(best_shot_value))), 1))
    {
        double slope = find_slope(r.best_shot_vector, car_to_ball);
        if ((forward_flag && slope > 0) || (backward_flag && slope > 0.5))
            r.found = 1;
...
struct jump_shot parse_slice_for_double_jump_with_target(double time_remaining, double best_shot_value, Vector ball_location, Vector car_location, Vector car_forward, int car_boost, Vector left_target, Vector right_target, double cap_)
{
    struct jump_shot r;
    r.found = -1;
    ...
    Vector direction = car_to_ball_norm.vector;
    double distance = car_to_ball_norm.magnitude;
    double forward_time = time_remaining - (angle(flatten(direction), flatten(car_forward)) * 0.318);

    if (forward_time > 0 && (distance / forward_time) < ((car_boost > distance / 100) ? 2275 : 1400))
    {
        struct post_correction pst_crrctn = correct_for_posts(ball_location, left_target, right_target);
        if (!pst_crrctn.swapped)
        {
            ...
            if (in_field(sub(ball_location, multiply(r.best_shot_vector, double_to_vector(best_shot_value))), 1) && find_slope(r.best_shot_vector, car_to_ball) > 0)
                r.found = 1;
        }
    }
...
struct jump_shot parse_slice_for_double_jump(double time_remaining, double best_shot_value, Vector ball_location, Vector car_location, Vector car_forward, int car_boost, double cap_)
{
    struct jump_shot r;
    r.found = -1;
    ...
    if (forward_time > 0 && (distance / forward_time) < ((car_boost > distance / 100) ? 2275 : 1400) && find_slope(r.best_shot_vector, car_to_ball) > 0)
        r.found = 1;
...
_Bool is_viable(double time_remaining, double hitbox_height, double boost_accel, Vector gravity, struct car me, Vector target)
{
...
struct aerial_shot parse_slice_for_aerial_shot_with_target(double time_remaining, double best_shot_value, double boost_accel, Vector gravity, Vector ball_location, struct car me, Vector left_target, Vector right_target, double cap_)
{
    ...
    if (!pst_crrctn.swapped)
    {
        Vector left_vector = normalize(sub(pst_crrctn.left, ball_location)).vector;
        Vector right_vector = normalize(sub(pst_crrctn.right, ball_location)).vector;
        r.ball_intercept = sub(ball_location, multiply(clamp(direction, left_vector, right_vector), double_to_vector(best_shot_value)));

        if (in_field(r.ball_intercept, 1))
        {
            double slope = find_slope(r.ball_intercept, car_to_ball);
            if (slope > 0 && is_viable(time_remaining, ceil(best_shot_value - 92.75) * 2, boost_accel, gravity, me, r.ball_intercept))
                r.found = 1;
        }
    }
...
struct aerial_shot parse_slice_for_aerial_shot(double time_remaining, double best_shot_value, double boost_accel, Vector gravity, Vector ball_location, struct car me, double cap_)
{
    struct aerial_shot r;
    r.found = -1;

    Vector car_to_ball = sub(ball_location, me.location);
    r.ball_intercept = sub(ball_location, multiply(normalize(car_to_ball).vector, double_to_vector(best_shot_value)));
...
{
    int car_boost;
    double time_remaining, best_shot_value, cap_;
    ...
    struct jump_shot data_struct = parse_slice_for_jump_shot_with_target(time_remaining, best_shot_value, ball_location, car_location, car_forward, car_boost, left_target, right_target, cap_);
...
static PyObject *method_parse_slice_for_jump_shot(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "dd(ddd)(ddd)(ddd)id", &time_remaining, &best_shot_value, &ball_location.x, &ball_location.y, &ball_location.z, &car_location.x, &car_location.y, &car_location.z, &car_forward.x, &car_forward.y, &car_forward.z, &car_boost, &cap_))
        return NULL;

    struct jump_shot data_struct = parse_slice_for_jump_shot(time_remaining, best_shot_value, ball_location, car_location, car_forward, car_boost, cap_);
...
static PyObject *method_parse_slice_for_double_jump_with_target(PyObject *self, PyObject *args)
{
    ...
    struct jump_shot data_struct = parse_slice_for_double_jump_with_target(time_remaining, best_shot_value, ball_location, car_location, car_forward, car_boost, left_target, right_target, cap_);
...
static PyObject *method_parse_slice_for_double_jump(PyObject *self, PyObject *args)
{
    ...
    struct jump_shot data_struct = parse_slice_for_double_jump(time_remaining, best_shot_value, ball_location, car_location, car_forward, car_boost, cap_);
...
static PyObject *method_parse_slice_for_aerial_shot_with_target(PyObject *self, PyObject *args)
{
    ...
    struct aerial_shot data_struct = parse_slice_for_aerial_shot_with_target(time_remaining, best_shot_value, boost_accel, gravity, ball_location, me, left_target, right_target, cap_);
...
static PyObject *method_parse_slice_for_aerial_shot(PyObject *self, PyObject *args)
{
    ...
    struct aerial_shot data_struct = parse_slice_for_aerial_shot(time_remaining, best_shot_value, boost_accel, gravity, ball_location, me, cap_);
