...
if [ $CLUSTER = $RUNMON_CLUSTER ]; then
	echo "$($RUNMON_BASE/report.py --run $RUN --base $RUNMON_BASE --cluster $RUNMON_CLUSTER)"
else #Relies on RUNMON_BASE being the same across clusters
	CLUSTER_SSH=$($RUNMON_BASE/get_cluster.py --cluster $CLUSTER)
	echo "$(gsissh -p 2222 $CLUSTER_SSH $RUNMON_BASE/report.py --run $RUN --base $RUNMON_BASE --cluster $CLUSTER)"
fi
