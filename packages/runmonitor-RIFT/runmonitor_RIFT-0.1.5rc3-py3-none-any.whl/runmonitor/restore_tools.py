...
def get_new_value():
        args = get_args()
        index = get_overflow_index()
        if (index != -1):
                print(int(args[index]) + 50)
        else:
                print(50)

def get_args_without_overflow():
        args = get_args()
        new_args = ""
        index = get_overflow_index() - 1
...
returns True/False for above threshold/below threshold respectively
    """
    with open(os.join.path(rd,"marginalize_intrinsic_parameters_BasicIterationWorkflow.dag"),'r') as f:
        lines = f.readlines()
    init = -1
    for i,line in enumerate(lines):
        ...
        if init != -1 and "join.sub" in line:
            ...
            break 
    ilengoal = (end-init)/2
    ileouts = [el for el in os.listdir(os.path.join(rd,"iteration_"+str(itn)+"_il") if "dat" in el]
