...
class LiveMetricRegistry:
    ...
    def observe(self, prediction: PredictionContext):
        ...
        if self._inference_metric:
            ...
            if is_single_value(prediction.output):
                self._inference_metric.observe(prediction.output)
        if hasattr(prediction.features, "__iter__"):
            for i, value in enumerate(prediction.features):
                if i in self._feature_metrics:
                    # Unflattened tensors are unsupported, ignore
                    if is_single_value(value):
                        self._feature_metrics[i].observe(value)
