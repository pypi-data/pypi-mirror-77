...
class PyGraphHandler:
    def _draw_graph_edges(self, surface: pygame.Surface, graph: Graph, coordinate_converter: Callable[[Vector2], Vector2] = None, enabled_color: Color = None, disabled_color: Color = None):
        ...
        for edge in graph.edges():
            ...
            pygame.draw.line(surface, color.value, (start.x, start.y), (end.x, end.y))

    def _draw_graph_nodes(self, surface: pygame.Surface, graph: Graph, coordinate_converter: Callable[[Vector2], Vector2] = None, color: Color = None, radius: int = 1, width: int = 0):
        if not coordinate_converter:
            coordinate_converter = lambda x: x
    ...
    def draw_to_surface(self, surface: pygame.Surface, graph: Graph,
        ...
        self._draw_graph_edges(surface, graph=graph, coordinate_converter=coordinate_converter, enabled_color=enabled_edge_color, disabled_color=disabled_edge_color)
        self._draw_graph_nodes(surface, graph=graph, coordinate_converter=coordinate_converter, color=node_color)
