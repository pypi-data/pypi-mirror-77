...
class Arc(Curve):
    ...
    def draw(self, surface: pygame.Surface, color:Color = None):
        if color is None:
            color = Color.WHITE
...
class Line(Curve):
    ...
    def draw(self, surface: pygame.Surface, color: Color = None):
        if color is None:
            color = Color.WHITE
        pygame.draw.line(surface, color.value, (self.origin.x, self.origin.y), (self.destination.x, self.destination.y))
