...
class PinyinProcessor:
    ...
    def is_chinese(self, name: str) -> bool:
        ...
        :param name: 您可以传整个姓名, 也可以只传姓氏. 姓名只支持 'lastname,
         firstname' 格式的姓名. 当传入姓名时, 判断可能会更加准确. 例如, 如果只传
         入 "jan", 那么在 `self.wade_mode is True` 的情况下, 会判断为 True; 而传
         入 "jan, edward" 和 "jan, ping xin", 则会分别判断为 False 和 True.
        """
        name = name.lower()
        ...
        NOTE: 原先是打算用 self.pinyin_patterns_4_name 的 pinyin 和 wade 的值来
         判断的, 后来发现有些罕见的姓, 虽然符合拼音或韦氏拼音的构造规则, 但由于
         不在 self.pinyin_patterns_4_name 范围内, 所以会被误判为非中国人名. 为避
         免这种情况, 不再使用 self.pinyin_patterns_4_name 提供的 pinyin 和 wade,
         改用 self.pinyin_patterns 的 'pinyin+wade'.
        """
        if self.wade_mode:
