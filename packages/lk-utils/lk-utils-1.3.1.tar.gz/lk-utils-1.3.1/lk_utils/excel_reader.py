...
class ExcelReader(ColIndexLocator):
    ...
    @staticmethod
    def _betterint(v: Any) -> CellValue:
        """
    ...
    def cell_value(self, x, y) -> CellValue:
        v = self.sheet.cell(x, y).value
        return self._betterint(v)
    
    def row_values(self, rowx: int) -> RowValues:
        return [
            self._betterint(x)
            for x in self.sheet.row_values(rowx)
        ]
    
    def row_dict(self, rowx: int) -> Dict[Union[str, int], CellValue]:
        return dict(zip(self.header, self.row_values(rowx)))
    
    def col_values(self, query, offset=0) -> Union[ColValues, None]:
