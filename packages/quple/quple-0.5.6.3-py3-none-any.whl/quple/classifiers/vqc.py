...
class VQC(tf.keras.Sequential):
    ...
    register. Here :math: U(x, θ) :math: is a quantum circuit that depends on the 
    input x and a set of parameters θ which can be seen as weights in an artificial 
    neural network. :math: U(x, θ) :math: can be written as a composition of a data
    encoding circuit :math: S(x) :math: and a variational circuit :math: W(θ) :math:, 
    ...
    predicted label, i.e. :math: y = ϕ(f(x, θ)) :math:. The parameters θ are then 
    trained in an iteractive manner by a classical optimizer to optimize over some
