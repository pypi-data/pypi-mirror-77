        ...
        if explanation_id is not None:
            ...
            if raw is not None:
                ...
                if ExplainType.IS_RAW in explanation_asset.properties:
                    if (explanation_asset.properties.get(ExplainType.IS_RAW, '').lower() == TRUE) != raw:
                        module_logger.debug(error_string)
                        raise ExplanationNotFoundException(error_string)
                else:
                    if (explanation_asset.meta[ExplainType.IS_RAW].lower() == TRUE) != raw:
                        module_logger.debug(error_string)
                        raise ExplanationNotFoundException(error_string)
                ...
                if ExplainType.IS_ENG in explanation_asset.properties:
                    if (explanation_asset.properties.get(ExplainType.IS_ENG, '').lower() == TRUE) != engineered:
                        module_logger.debug(error_string)
                        raise ExplanationNotFoundException(error_string)
                else:
            ...
            if len(explanation_list) == 0:
                ...
                if raw is not None:
                    def exp_filter(asset):
                        result = (asset.meta[ExplainType.IS_RAW].lower() == 'true') == raw
                        result |= (asset.meta[ExplainType.IS_RAW].lower() == TRUE) == raw
                        return result
