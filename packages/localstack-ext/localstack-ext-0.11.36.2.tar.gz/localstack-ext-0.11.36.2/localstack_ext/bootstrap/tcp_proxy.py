...
class AttrDict(oCiPl):
 def __init__(self,*args,**kwargs):
  oCiPu(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and oCiPd(socket,'IPV6_V6ONLY'):
...
class Half(oCiPW):
 def __init__(self,opt,sock,addr,oCiPE):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.oCiPE=oCiPE
  self.name='peer' if self.oCiPE else 'client'
  self.queue=[]
  self.dest=oCiPj
  self.err=oCiPj
  self.ready=oCiPn
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':oCiPn,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':oCiPn,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':oCiPn,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':oCiPn,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
