...
class ApiClient():
    ...
    def list_uploads(self):
        '''Lists all the details associated with the dataset file upload including the current status of the upload (complete or not). Refer to the return table below for a complete description of the method. Use this API method to ensure that your dataset is uploaded properly and all the details associated with it are correct. If everything looks fine then you are good to use the dataset you uploaded in parts for your batch predictins and/or model training.    '''
    ...
    def predict_takeover(self, deployment_token: str, deployment_id: str, query_data: dict):
        '''Returns a probability for each class label associated with the types of fraud or a 'yes' or 'no' type label for the possibility of fraud. Note that the inputs to this method, wherever applicable, will be the column names (e.g. Account_Name) mapped to the column mappings in our system (ACCOUNT_ID).    '''
    ...
    def is_anomaly(self, deployment_token: str, deployment_id: str, query_data: dict = None):
        '''Returns a list of anomaly attributes based on login information for a specified account. Note that the inputs to this method, wherever applicable, will be the column names (e.g. Account_Name) mapped to the column mappings in our system (ACCOUNT_ID).    '''
    ...
    def batch_predict(self, deployment_id: str, input_location: str = None, output_location: str = None, name: str = None, refresh_schedule: str = None, global_prediction_args: dict = None):
        ...
        return self._call_api('batchPredict', 'POST', query_params={'deploymentId': deployment_id}, body={'inputLocation': input_location, 'outputLocation': output_location, 'name': name, 'refreshSchedule': refresh_schedule, 'globalPredictionArgs': global_prediction_args}, parse_type=BatchPrediction)

    def batch_prediction_from_upload(self, deployment_id: str, name: str = None, global_prediction_args: dict = None):
        '''Starts a batch prediction task with the specified deployment ID, file, and batch prediction job name.    '''
        return self._call_api('batchPredictionFromUpload', 'POST', query_params={'deploymentId': deployment_id}, body={'name': name, 'globalPredictionArgs': global_prediction_args}, parse_type=BatchPrediction)
