...
def get_profile(ticker):
    ...
    try:
        ...
        print("...Error #1(get_profile), need to install plugin yfinance first!")
        return None    

    #抓取证券信息，结果为字典
    tp=yf.Ticker(ticker); dic=tp.info
...
def stock_profile(ticker,verbose=False):
    ...
    try:
        ...
        print("*** 错误#1(profile)，搜索证券信息时失败或未找到！若网络不稳可再试")
        return None        
    
    #检查抓取到的数据表
    if (df is None) or (len(df)==0):
        print("*** 错误#2(profile)，未能搜索到证券信息:",ticker)
        return None
