...
class loadprojectini:
    def __init__(self,configini):
        ...
        plotlyInterfaceTitles = ['Sklearn results', 'Time bin analyses', 'Probabilities', 'Severity analysis', 'Entire Machine Classification dataframes']
        toIncludeVar = []
        ...
        self.plotly_file = FileSelect(plotlyInterface, 'File path (h5)', title='Select SimBA/Plotly dataset (h5)')
        self.groups_file = FileSelect(plotlyInterface, 'File path (csv)', title='Select groups file (csv')
    ...
    def open_plotly_interface(self, url):
        ...
        p = subprocess.Popen([sys.executable, r'simba\SimBA_dash_app.py', filePath, groupPath])
        wait_for_internet_connection(url)
        p2 = subprocess.Popen([sys.executable, r'simba\run_dash_tkinter.py', url])
        subprocess_children = [p, p2]
