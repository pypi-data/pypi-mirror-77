...
def train_multimodel(configini):
    ...
    def generateShapLog(trainingSet, target_train, feature_list, classifierName):
        ...
        targetsForShap = targetTrainSet.sample(250, replace=False)
        nontargetsForShap = nonTargetTrain.sample(250, replace=False)
        ...
        for row in range(len(shapTrainingSet)):
            ...
            counter += 1
            print(str(counter) + '/' + str(len(shapTrainingSet)))
        outputDfShap.to_csv(os.path.join(tree_evaluations_out, 'SHAP_values_' + str(classifierName) + '3.csv'))
        outputDfRaw.to_csv(os.path.join(tree_evaluations_out, 'RAW_SHAP_feature_values_' + str(classifierName) + '3.csv'))
        print('All SHAP explainability data saved in project_folder/models/evaluations directory')
    ...
    for i in metaFilesList:
        ...
        if model_to_run == 'RF':
            ...
            if shap_scores_input == 'yes':
                generateShapLog(data_train, target_train, feature_list, classifierName)
