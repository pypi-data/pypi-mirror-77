...
def write_tar(content: List[ArchiveFileBase],
              output_file: str,
              mode="w:gz") -> None:
...
def check_extracted(paths: List[str]):
...
@contextmanager
def extract(archive: io.IOBase, *files):
    with archive as a:
        with tarfile.open(fileobj=a) as tar:
            try:
                ...
                raise UserError(f"In archive {archive.name}: {e.args[0]}")
