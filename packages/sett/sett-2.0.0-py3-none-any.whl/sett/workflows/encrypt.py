...
from ..core.metadata import MetaData, alnum_str, Purpose, verify_transfer
...
def encrypt(files: List[str],
    ...
    with logger.log_task('Input data check'):
        ...
        root_dir = os.path.commonpath(Path(x).absolute().parent.as_posix()
                                      for x in files)
        if not offline and transfer_id is None:
            raise UserError("Transfer ID is missing. Use offline mode if you "
                            "don't require Transfer ID.")
        if output_name:
        ...
        if not offline:
            try:
                project_id = verify_transfer(
                    metadata=MetaData(transfer_id=transfer_id,
                ...
                logger.info("Transfer '%s' is a valid for project '%s'",
