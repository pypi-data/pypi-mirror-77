...
class ProgressInterface:
    ...
    def set_label(self, label: str) -> None:
        """"Method to associate a label to a progress object."""
...
def weights_to_ranges(weights: List[float],
...
def _progress_wrapper_with_sub_progress(progress: ProgressInterface) \
    ...
    """
    def progress_wrapper(item_with_range: Tuple[Iterable, Tuple[float, float]]
                         ) -> Tuple[Iterable, ProgressInterface]:
        """item_with_range: iterable and its completion range
...
def scale_progress(progress: ProgressInterface,
    ...
    class SubProgress(ProgressInterface):
        def set_label(self, label: str):
            return progress.set_label(label)
...
@contextmanager
def subprogress(progress: ProgressInterface,
                step_completion_increase: float,
                step_label: str = '') -> ProgressInterface:
    ...
    :param step_label: label associated with the current step.
...
class FileObjectProgress:
    ...
    def __init__(self, f_obj: io.IOBase, progress):
        self._f = f_obj
        self._update = progress.update
...
def with_progress(f: io.IOBase, progress: Optional[ProgressInterface]) -> io.IOBase:
    if progress is None:
        return f
...
def progress_file_iter(files: List[str],
    ...
    :param progress: ProgressInterface to which the
    :return: file objects with backed-in subprogress.
    """
    file_size = [os.path.getsize(f) for f in files]
    if sum(file_size) == 0:
        yield from (Opener(f, mode) for f in files)
        return
