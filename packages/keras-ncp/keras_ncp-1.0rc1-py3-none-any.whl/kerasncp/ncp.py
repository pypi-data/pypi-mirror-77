...
class RandomNetwork(NetworkArchitecture):
    def __init__(self, units, sparsity_level=0.0, output_dim=None, random_seed=1111):
        super(FullyConnected, self).__init__(units)
        if output_dim is None:
            output_dim = units
        self.set_output_dim(output_dim)
        ...
        number_of_synapses = units * units * (1 - sparsity_level)
        all_synapses = []
        for src in range(self.units):
            for dest in range(self.units):
        ...
        for src, dest in all_synapses:
            polarity = self._rng.choice([-1, 1, 1])
            self.add_synapse(src, dest, polarity)

    def build(self, input_shape):
        super().build(input_shape)
        number_of_sensory_synapses = self.input_dim * self.units * (1 - sparsity_level)
        ...
        for src, dest in all_sensory_synapses:
            polarity = self._rng.choice([-1, 1, 1])
            self.add_sensory_synapse(src, dest, polarity)
                polarity = self._rng.choice([-1, 1, 1])
                self.add_sensory_synapse(src, dest, polarity)
