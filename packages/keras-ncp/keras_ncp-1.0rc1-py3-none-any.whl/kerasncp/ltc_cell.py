...
class LTCCell(tf.keras.layers.Layer):
    ...
    def get_graph(self, include_sensory_neurons=True):
        ...
        for i in range(self.state_size):
            neuron_type = self._architecture.get_type_of_neuron(i)
        ...
        for src in range(self.sensory_size):
            for dest in range(self.state_size):
                if self._architecture.sensory_adjacency_matrix[src, dest] != 0:
                    polarity = (
        ...
        for src in range(self.state_size):
            for dest in range(self.state_size):
                if self._architecture.adjacency_matrix[src, dest] != 0:
    ...
    def draw_graph(
        ...
        for k, v in neuron_colors.items():
            legend_info.append(("{}{} neurons".format(k[0].upper(), k[1:]), v))
        legend_patches = [
            mpatches.Patch(color=color, label=label) for (label, color) in legend_info
        ]
