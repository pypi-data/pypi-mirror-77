...
def segment_single_sample(sample, input_directory, output_directory):
    ...
    reference_round = 2
    initial_round = rounds[reference_round]
    reference_channel_index = initial_round["reference_channel"]
    round_input_subdirectory = sample_input_subdirectory / ("round%d" % (reference_round + 1))
    ...
    print(reference_image_filepath)
...
def segment(reference_image_filepath, merged_composite_filepath, input_directory, output_directory):
    ...
    subprocess.run(["mv", "output/composite_mask.tiff", merged_composite_filepath])
