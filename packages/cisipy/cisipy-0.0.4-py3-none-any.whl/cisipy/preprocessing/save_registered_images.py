...
def find_shift(reference_image, other_image, scale=0.25):
    ...
    shift, error, diffphase = register_translation(rescaled_reference_image, rescaled_other_image)
...
def register_images_all_samples(config):
    """
    """
    ...
    for sample in samples:
        register_single_sample(sample, input_directory, output_directory)
...
def register_images_single_sample(sample, input_directory, output_directory):
    ...
    def build_reference_image_stack(rounds):
        ...
        for round_index, imaging_round in enumerate(rounds):
            ...
            reference_filepaths.append(sample_input_subdirectory / channels[reference_channel_index])
        ...
        cropped_shape = tuple(np.array([image.shape for image in reference_image_stack]).min(axis = 1))
        crop_indices = (slice(0, cropped_shape[0]), slice(0, cropped_shape[1]))

        reference_image_stack = reference_image_stack[crop_indices]

        return reference_image_stack

    # def build_round_stack(imaging_round, sample_input_subdirectory):
    #     """
    #     Helper function to stack all images for a given round.
    #     """
    ...
    #         channel_filepaths.append(sample_input_subdirectory / channel_filename)

    #     round_stack = np.stack([imageio.imread(channel_filepath) for channel_filepath in channel_filepaths], axis = 2)
    #     return round_stack


    rounds = sample["rounds"]
    reference_image_stack = build_reference_image_stack(rounds)

    first_reference, *other_references = reference_image_stack
    required_shifts = [find_shift(first_reference, other_reference) for other_reference in other_references]
    crop_index = get_shift_crop_index(required_shifts)
    ...
    for round_index, imaging_round in enumerate(rounds):
        ...
        for channel in channels:
            channel_filename = channel + ".tif"
            input_path = sample_input_subdirectory / channel_filename
            output_path = sample_output_subdirectory / channel_filename

            channel_image = imageio.imread(input_path)
            registered_image = apply_shift(channel_image, required_shifts[round_index])[crop_index]

            # TODO: Originally, we rescaled the intensity. If we want to keep that, perhaps we can do it here?
            imageio.imwrite(output_path, registered_image)
