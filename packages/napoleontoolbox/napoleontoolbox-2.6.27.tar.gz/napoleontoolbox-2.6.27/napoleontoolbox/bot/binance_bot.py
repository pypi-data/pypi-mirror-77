    ...
    def get_futures_positions_margin_change_history(self, assets, start_time, end_time):
        ...
        for asset in assets:
            if transfer_df is None:
                transfer_df = self.get_futures_positiion_margin_change_history(asset, start_time, end_time)
            else:
                add_transfer_df = self.get_futures_positiion_margin_change_history(asset, start_time, end_time)
                if add_transfer_df is not None:
                    transfer_df = pd.concat([transfer_df, add_transfer_df])
        if transfer_df is not None:
            return transfer_df[['date','asset','amount']]
        return transfer_df

    def get_futures_positiion_margin_change_history(self, asset, start_time, end_time):
