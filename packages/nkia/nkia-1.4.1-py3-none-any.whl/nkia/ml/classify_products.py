...
class classifyProducts(object):
    ...
    def __init__(self):
        model = self.load_model_to_memory('food_model')
        dictionary = self.load_dictionary()
        print(self.inference_from_food_model(model, dictionary, ['geladeira frost free 110v'], ['']))
    ...
    def load_dictionary(self):
        if 'nlp_dictionary' not in os.listdir():
            ...
            gdown.download('https://project-elements-nk.s3.amazonaws.com/dictionary.txt', 'nlp_dictionary/dictionary.txt', quiet=False)        

        return utils.instantiate_spelling_corrector('nlp_dictionary/dictionary.txt')
    ...
    def inference_from_category_model(self, model, sym_spell, product_name=[''], description=['']): 
        ...
        """     
        X = self.preprocessing_fn(sym_spell, product_name, description, './category_model')
    ...
    def inference_from_food_model(self, model, sym_spell, product_name=[''], description=['']):
        ...
        """     
        X = self.preprocessing_fn(sym_spell, product_name, description, './food_model')
    ...
    def preprocessing_fn(self, sym_spell, product_name, description, model_name):       
        model_version = pkg_resources.get_distribution('nkia').version
        tokenizer = tfds.features.text.SubwordTextEncoder.load_from_file(model_name+'/'+model_version+'/token')

        X = utils.get_cleaned_predictor(sym_spell, [description], [product_name])

        tokenized_predictor = [tokenizer.encode(sentence) for sentence in X]

        self.label_encoder = self.get_label_encoder(model_name+'/'+model_version+'/label_encoder.pkl')
