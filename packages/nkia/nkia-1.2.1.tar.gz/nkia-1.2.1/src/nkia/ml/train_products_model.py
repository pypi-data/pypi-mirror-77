...
class trainProductsModel(object):
    ...
    def __init__(self, model_to_train, show_confusion_matrix=False):
        ...
        self.new_model_path, self.last_model_path = self.create_saved_model_folder(self.model_to_train)
        self.train_model()
    ...
    def get_data(self):
        ...
        import pandas as pd
        dataframe = pd.read_excel('/home/alvaro/Área de Trabalho/servicos-biodigital/src/utils/datasets/positive_and_negative_food.xlsx')
    ...
    def get_previsors_attrib(self, dataframe):
        ...
        X = utils.get_cleaned_predictor(utils.instantiate_spelling_corrector(), description, product_name)
    ...
    def create_and_deploy_tarfile(self, output_filename, source_dir):
        output_filename = self.standardize_model_version(output_filename)

        for output in output_filename:
    ...
    def standardize_model_version(self, output_filename):
        another_model_name = 'food_model' if self.model_to_train == 'category_model' else 'category_model'   
        another_model_path = self.new_model_path.replace(self.model_to_train, another_model_name)
        path = '/'.join(another_model_path.split('/')[0:-1])
        os.rename(path +'/'+ os.listdir(path)[0], another_model_path)
        
        output_filename.append(another_model_path)
