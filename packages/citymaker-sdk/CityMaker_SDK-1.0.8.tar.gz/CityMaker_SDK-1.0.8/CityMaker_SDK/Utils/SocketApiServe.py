...
from Utils.Config import serverAddress
import threading
...
async def send_msg(msg):
    ...
    # websocket = await websockets.connect(config.get("serverAddress"))
    async with websockets.connect(serverAddress) as websocket:
...
ws = websocket('ws://192.168.3.216:8181/', protocols=['chat'])
def start():
    try:
        ws.connect()
        ws.run_forever()
