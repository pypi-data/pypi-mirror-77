...
def main():
    ...
    parser.add_argument('--data_path', default="C:/Users/Robbie/boeing_Rsi/data/",type=str)
    ...
    for g in algo_guids:
        ...
        data_source = DataSource(key_id=api_key_id ,secret_key=api_secret_key ,
                    base_url=api_base_url, data_source=data_source, tickers=algo_config[g]["tickers"].split(","), 
                    start_date=start_date ,end_date=end_date
                    )

        algo = AlgoRsiBb(data_path, data_source, g, algo_config[g])
        ...
        for p in algo.positions:
            share_val += algo.positions[p] * algo._get_last_trade(p)
        
        sheet[i][0].value = g
        sheet[i][1].value = algo.wallet
    ...
    while 0 < len(thread_list):
        ...
        for g, thread in thread_list:
            if not thread.is_alive():
                ...
                sheet[i][9].value = share_val + ret[0]
                master_saved = False
