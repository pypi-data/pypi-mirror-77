...
class DataSource:
    ...
    def __init__(self, key_id, secret_key, base_url, mode=backtest, tickers=None, start_date=None, end_date=None):
    ...
    def list_orders(self, ticker):
        try:
            ...
            for o in orders:
                log.info(o)
                if o.side == "sell" and o.symbol == ticker:
                    valid_orders.append(o)
            log.info(valid_orders[0])
            log.info("---------")
            log.info(valid_orders[len(valid_orders)-1])
            # log.info(orders)
            ret = {"cash" : act.cash}
            return ret
    ...
    def submit_order(self, ticker, quantity, side, type, limit_price, time_in_force):
        try:
            ...
            self.api_endpoint.submit_order(
                symbol=ticker,
                qty=quantity,
                side=side,
                type=type,
