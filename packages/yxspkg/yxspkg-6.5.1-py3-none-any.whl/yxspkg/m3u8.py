...
def main_run(uri = None,dirname=None,url=None,suffix='.ts',output=None,delete=False,downloader='requests',multi_requests=2,md5_suffix=True,faststart=False,
    ...
    if uri:
        if Path(uri[0]).is_file():
            cmds = ['skg m3u8 '+i.strip() for i in open(uri[0]).readlines()]
...
if __name__=='__main__':
    os.chdir('/media/yxs/4C68B86268B84C88/DCIM/video/国产')
    sys.argv.append('/media/yxs/4C68B86268B84C88/DCIM/video/国产/skg.txt')
    main()