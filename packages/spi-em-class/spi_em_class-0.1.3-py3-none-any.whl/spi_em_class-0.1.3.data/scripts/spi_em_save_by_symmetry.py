...
def main():
    ...
    parser.add_argument('-t', '--threshold', default=0.3, type=float, help='Selection threshold')
    ...
    for i, data in enumerate(model_data):
        symmetry_scores = symmetry.get_symmetry_scores(data)
        symmetry_scores /= symmetry_scores.sum()
        order_score = symmetry_scores[symmetry_order]
        if order_score >= symmetry_thresh:
            good_class.append(i + 1)
            print('Class {:2d}: {:0.2f} >= {:0.2f}'.format(i + 1, order_score, symmetry_thresh))
        else:
            print('Class {:2d}: {:0.2f} < {:0.2f}'.format(i + 1, order_score, symmetry_thresh))
