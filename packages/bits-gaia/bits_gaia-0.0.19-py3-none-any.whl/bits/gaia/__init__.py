...
class Gaia:
    ...
    def save_bigquery_import(self, dataset, table, entries):
        ...
        blobname = f"{dataset}/{table}_{timestamp}.json"
        blobstring = "\n".join([json.dumps(e) for e in entries])
        blob = storage.Client().bucket(BIGQUERY_IMPORTS_BUCKET).blob(blobname)
    ...
    def save_firestore_import(self, dataset, table, entries):
        ...
        timestamp = datetime.isoformat(datetime.now())
        blobname = f"{dataset}/{table}_{timestamp}.json"
        print(f"Blob Name: {blobname}")
        print(f"Bucket Name: {FIRESTORE_IMPORTS_BUCKET}")
        blobstring = json.dumps(entries, sort_keys=True, default=str)
        blob = storage.Client().bucket(FIRESTORE_IMPORTS_BUCKET).blob(blobname)
        blob.upload_from_string(blobstring, content_type="application/json")
        return blob
