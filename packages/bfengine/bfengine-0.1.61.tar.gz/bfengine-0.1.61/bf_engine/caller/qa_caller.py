...
class QACaller(CallerBase):
    ...
    def upload_json_lq(self, data_id: str, data: dict) -> bool:
        ...
        data = {
            "records": [
                {
                    "sq_id": data_id,
                    "lq_list": list(map(lambda lq: {'lq': lq}, data["lq"] if data["lq"] else []))
                }
            ]
        }
    ...
    def add_corpus(self, question_id, corpus):
        ...
        resp = requests.post(self.upload_faq_json_lq_url, headers=self.header, json=params).json()
        code = int(resp["code"])
        msg = str(resp["message"])
        if code != 0:
            raise BfEngineException(code=code, msg=msg)
    ...
    def update_question(self, data: dict) -> str:
        ...
        if question_id is None:
            raise BfEngineException(code=-1, msg='missing required parameter: id')
        self.delete_question(question_id)
        question_id = self.upload_json_sq(data=data)
        self.upload_json_lq(question_id, data)
        return question_id
    ...
    def delete_answer(self, answer_id):
        ...
        resp = requests.put(self.upload_faq_json_answer_url, headers=self.header, json=params).json()
        code = int(resp["code"])
        msg = str(resp["message"])
        if code != 0:
            raise BfEngineException(code=code, msg=msg)
    ...
    def get_similar_qa_list(self,user_inputs:str,max_topn):
        sq_result = []
        converter = Converter('zh-hans')
        params = {"Text":converter.convert(user_inputs).lower(),'UniqueId':str(random.randint(100,10000)),"IsRelease": False,'Robot':self.app_id}
        print("params: "+str(params))
        ...
        for match_sq in resp['data']:
            sq_result.append(match_sq)
        topn = int(max_topn)
        sq_result = sorted(sq_result, key=lambda x: x["score"], reverse=True)[:topn]
        return sq_result
