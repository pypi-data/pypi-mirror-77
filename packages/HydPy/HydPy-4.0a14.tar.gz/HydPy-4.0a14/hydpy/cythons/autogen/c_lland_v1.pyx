...
cdef class Model:
    ...
    cpdef inline void calc_snratio_v1(self)  nogil:
        cdef int k
        for k in range(self.parameters.control.nhru):
            if self.sequences.fluxes.tkor[k] >= (self.parameters.control.tgr[k]+self.parameters.control.tsp[k]/2.):
                self.sequences.aides.snratio[k] = 0.
            elif self.sequences.fluxes.tkor[k] <= (self.parameters.control.tgr[k]-self.parameters.control.tsp[k]/2.):
                self.sequences.aides.snratio[k] = 1.
            else:
                self.sequences.aides.snratio[k] =                     ((self.parameters.control.tgr[k]+self.parameters.control.tsp[k]/2.)-self.sequences.fluxes.tkor[k])/self.parameters.control.tsp[k]
    ...
    cpdef inline void calc_wnied_v1(self)  nogil:
        ...
        for k in range(self.parameters.control.nhru):
            if self.parameters.control.lnk[k] in (WASSER, FLUSS, SEE):
                self.sequences.fluxes.wnied[k] = 0.
            else:
    ...
    cpdef inline void calc_schmpot_v1(self)  nogil:
        cdef int k
        for k in range(self.parameters.control.nhru):
            self.sequences.fluxes.schmpot[k] = max((self.sequences.fluxes.wgtf[k]+self.sequences.fluxes.wnied[k])/self.parameters.fixed.rschmelz, 0.)
    ...
    cpdef inline void calc_evb_v1(self)  nogil:
        ...
        cdef int k
        for k in range(self.parameters.control.nhru):
            if self.parameters.control.lnk[k] in (VERS, WASSER, FLUSS, SEE) or self.parameters.control.wmax[k] <= 0.:
    ...
    cpdef inline void calc_qkap_v1(self)  nogil:
        ...
        for k in range(self.parameters.control.nhru):
            if ((self.parameters.control.lnk[k] in (VERS, WASSER, FLUSS, SEE))                    or (self.parameters.control.wmax[k] <= 0.)):
                ...
                self.sequences.fluxes.qkap[k] = self.parameters.control.kapmax[k] * (                    1.-(self.sequences.states.bowa[k]-self.parameters.control.kapgrenz[k, 0]) /                    (self.parameters.control.kapgrenz[k, 1]-self.parameters.control.kapgrenz[k, 0]))
            else:
                self.sequences.fluxes.qkap[k] = 0
    cpdef inline void calc_qbb_v1(self)  nogil:
        cdef int k
        for k in range(self.parameters.control.nhru):
            if ((self.parameters.control.lnk[k] in (VERS, WASSER, FLUSS, SEE)) or                    (self.sequences.states.bowa[k] <= self.parameters.control.pwp[k]) or (self.parameters.control.wmax[k] <= 0.)):
                ...
                self.sequences.fluxes.qbb[k] = (                    self.parameters.control.beta[k]*(self.sequences.states.bowa[k]-self.parameters.control.pwp[k]) *                    (1.+(self.parameters.control.fbeta[k]-1.) *                     (self.sequences.states.bowa[k]-self.parameters.control.fk[k])/(self.parameters.control.wmax[k]-self.parameters.control.fk[k]))                )
    ...
    cpdef inline void calc_qdb_v1(self)  nogil:
        ...
        for k in range(self.parameters.control.nhru):
            if self.parameters.control.lnk[k] == WASSER:
            ...
            elif ((self.parameters.control.lnk[k] in (VERS, FLUSS, SEE)) or                  (self.parameters.control.wmax[k] <= 0.)):
                self.sequences.fluxes.qdb[k] = self.sequences.fluxes.wada[k]
            else:
                if self.sequences.states.bowa[k] < self.parameters.control.wmax[k]:
                    d_sfa = (                        (1.-self.sequences.states.bowa[k]/self.parameters.control.wmax[k])**(1./(self.parameters.control.bsf[k]+1.)) -                        (self.sequences.fluxes.wada[k]/((self.parameters.control.bsf[k]+1.)*self.parameters.control.wmax[k]))                    )
                else:
                    d_sfa = 0.
                ...
                if d_sfa > 0.:
                    self.sequences.fluxes.qdb[k] = self.sequences.fluxes.qdb[k] + (d_sfa ** (self.parameters.control.bsf[k] + 1.) * self.parameters.control.wmax[k])
    ...
    cpdef inline void calc_bowa_v1(self)  nogil:
        ...
        for k in range(self.parameters.control.nhru):
            if self.parameters.control.lnk[k] in (VERS, WASSER, FLUSS, SEE):
                ...
                if self.sequences.fluxes.evb[k] > 0.:
    ...
    cpdef inline void calc_qah_v1(self)  nogil:
        ...
        if (not self.parameters.control.negq) and (self.sequences.fluxes.qah < 0.):
            d_area = 0.
            for k in range(self.parameters.control.nhru):
