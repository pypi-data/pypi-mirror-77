...
cdef class Model:
    ...
    cpdef inline void calc_am_um_v1(self)  nogil:
        ...
        for i in range(self.parameters.control.gts):
            ...
            self.sequences.aides.am[i] = (                d_temp*(self.parameters.control.bm+d_temp*self.parameters.control.bnm) +                self.sequences.aides.rhv[i]*(self.parameters.control.bm+2.*d_temp*self.parameters.control.bnm)            )
            self.sequences.aides.um[i] = self.parameters.control.bm+2.*d_temp*self.parameters.derived.bnmf+2.*self.sequences.aides.rhv[i]
    cpdef inline void calc_amdh_umdh_v1(self)  nogil:
        ...
        for i in range(self.parameters.control.gts):
            ...
            self.sequences.aides.amdh[i] = (                self.parameters.control.bnm*d_temp1*d_temp2 +                2.*self.parameters.control.bnm*d_temp2*self.sequences.aides.rhv[i] +                (self.parameters.control.bm+self.parameters.control.bnm*d_temp1)*d_temp2 +                (self.parameters.control.bm+2.*self.parameters.control.bnm*d_temp1)*self.sequences.aides.rhvdh[i]            )
            self.sequences.aides.umdh[i] = 2.*d_temp2*self.parameters.derived.bnmf + 2.*self.sequences.aides.rhvdh[i]
    cpdef inline void calc_alv_arv_ulv_urv_v1(self)  nogil:
        cdef double d_temp
        cdef int i
        for i in range(self.parameters.control.gts):
            d_temp = self.sequences.aides.rhv[i] - self.sequences.aides.rhlvr[i]
            self.sequences.aides.alv[i] = (                d_temp*(self.parameters.control.bv[0]+(d_temp*self.parameters.control.bnv[0]/2.)) +                self.sequences.aides.rhlvr[i]*(self.parameters.control.bv[0]+d_temp*self.parameters.control.bnv[0]))
            ...
            self.sequences.aides.arv[i] = (                d_temp*(self.parameters.control.bv[1]+(d_temp*self.parameters.control.bnv[1]/2.)) +                self.sequences.aides.rhrvr[i]*(self.parameters.control.bv[1]+d_temp*self.parameters.control.bnv[1]))
    ...
    cpdef inline void calc_alvr_arvr_ulvr_urvr_v1(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            self.sequences.aides.alvr[i] = self.sequences.aides.rhlvr[i]**2*self.parameters.control.bnvr[0]/2.
            self.sequences.aides.ulvr[i] = self.sequences.aides.rhlvr[i]*self.parameters.derived.bnvrf[0]
            self.sequences.aides.arvr[i] = self.sequences.aides.rhrvr[i]**2*self.parameters.control.bnvr[1]/2.
    ...
    cpdef inline void calc_qm_v1(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if self.sequences.aides.um[i] > 0.:
                self.sequences.aides.qm[i] = self.parameters.derived.mfm*self.sequences.aides.am[i]**(5./3.)/self.sequences.aides.um[i]**(2./3.)
            else:
                self.sequences.aides.qm[i] = 0.
    cpdef inline void calc_qmdh_v1(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if (self.sequences.aides.am[i] > 0.) and (self.sequences.aides.um[i] > 0.):
                self.sequences.aides.qmdh[i] = self.parameters.derived.mfm * (                    5.*self.sequences.aides.am[i]**(2./3.)*self.sequences.aides.amdh[i]/(3.*self.sequences.aides.um[i]**(2./3.)) -                    2.*self.sequences.aides.am[i]**(5./3.)*self.sequences.aides.umdh[i]/(3.*self.sequences.aides.um[i]**(5./3.)))
            else:
                self.sequences.aides.qmdh[i] = 0.
    cpdef inline void calc_qm_v2(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if self.sequences.aides.amdh[i] > 0.:
    ...
    cpdef inline void calc_qlv_qrv_v1(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if self.sequences.aides.ulv[i] > 0.:
                self.sequences.aides.qlv[i] = self.parameters.derived.mfv[0]*self.sequences.aides.alv[i]**(5./3.)/self.sequences.aides.ulv[i]**(2./3.)
            else:
                self.sequences.aides.qlv[i] = 0.
            if self.sequences.aides.urv[i] > 0:
                self.sequences.aides.qrv[i] = self.parameters.derived.mfv[1]*self.sequences.aides.arv[i]**(5./3.)/self.sequences.aides.urv[i]**(2./3.)
            else:
                self.sequences.aides.qrv[i] = 0.
    cpdef inline void calc_qlvdh_qrvdh_v1(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if (self.sequences.aides.alv[i] > 0.) and (self.sequences.aides.ulv[i] > 0.):
                self.sequences.aides.qlvdh[i] = self.parameters.derived.mfv[0] * (                    5.*self.sequences.aides.alv[i]**(2./3.)*self.sequences.aides.alvdh[i]/(                        3.*self.sequences.aides.ulv[i]**(2./3.)) -                    2.*self.sequences.aides.alv[i]**(5./3.)*self.sequences.aides.ulvdh[i]/(                        3.*self.sequences.aides.ulv[i]**(5./3.)))
            else:
                self.sequences.aides.qlvdh[i] = 0.
            if (self.sequences.aides.arv[i] > 0.) and (self.sequences.aides.urv[i] > 0.):
                self.sequences.aides.qrvdh[i] = self.parameters.derived.mfv[1] * (                    5.*self.sequences.aides.arv[i]**(2./3.)*self.sequences.aides.arvdh[i]/(                        3.*self.sequences.aides.urv[i]**(2./3.)) -                    2.*self.sequences.aides.arv[i]**(5./3.)*self.sequences.aides.urvdh[i]/(                        3.*self.sequences.aides.urv[i]**(5./3.))                )
            else:
                self.sequences.aides.qrvdh[i] = 0.
    cpdef inline void calc_qlv_qrv_v2(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if self.sequences.aides.alvdh[i] > 0.:
                self.sequences.aides.qlv[i] = self.sequences.aides.qlvdh[i]/self.sequences.aides.alvdh[i]*self.sequences.aides.alv[i]
            else:
                self.sequences.aides.qlv[i] = 0.
            if self.sequences.aides.arvdh[i] > 0.:
    ...
    cpdef inline void calc_qlvr_qrvr_v1(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if self.sequences.aides.ulvr[i] > 0.:
                self.sequences.aides.qlvr[i] =                     self.parameters.derived.mfv[0]*self.sequences.aides.alvr[i]**(5./3.)/self.sequences.aides.ulvr[i]**(2./3.)
            else:
                self.sequences.aides.qlvr[i] = 0.
            if self.sequences.aides.urvr[i] > 0.:
                self.sequences.aides.qrvr[i] =                     self.parameters.derived.mfv[1]*self.sequences.aides.arvr[i]**(5./3.)/self.sequences.aides.urvr[i]**(2./3.)
            else:
                self.sequences.aides.qrvr[i] = 0.
    cpdef inline void calc_qlvrdh_qrvrdh_v1(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if (self.sequences.aides.alvr[i] > 0.) and (self.sequences.aides.ulvr[i] > 0.):
                self.sequences.aides.qlvrdh[i] = self.parameters.derived.mfv[0] * (                    5.*self.sequences.aides.alvr[i]**(2./3.)*self.sequences.aides.alvrdh[i]/(                        3.*self.sequences.aides.ulvr[i]**(2./3.)) -                    2.*self.sequences.aides.alvr[i]**(5./3.)*self.sequences.aides.ulvrdh[i]/(                        3.*self.sequences.aides.ulvr[i]**(5./3.)))
            else:
                self.sequences.aides.qlvrdh[i] = 0.
            if (self.sequences.aides.arvr[i] > 0.) and (self.sequences.aides.urvr[i] > 0.):
                self.sequences.aides.qrvrdh[i] = self.parameters.derived.mfv[1] * (                    5.*self.sequences.aides.arvr[i]**(2./3.)*self.sequences.aides.arvrdh[i]/(                        3.*self.sequences.aides.urvr[i]**(2./3.)) -                    2.*self.sequences.aides.arvr[i]**(5./3.)*self.sequences.aides.urvrdh[i]/(                        3.*self.sequences.aides.urvr[i]**(5./3.)))
            else:
                self.sequences.aides.qrvrdh[i] = 0.
    cpdef inline void calc_qlvr_qrvr_v2(self)  nogil:
        cdef int i
        for i in range(self.parameters.control.gts):
            if self.sequences.aides.alvrdh[i] > 0.:
                self.sequences.aides.qlvr[i] = self.sequences.aides.qlvrdh[i]/self.sequences.aides.alvrdh[i]*self.sequences.aides.alvr[i]
            else:
                self.sequences.aides.qlvr[i] = 0.
            if self.sequences.aides.arvrdh[i] > 0.:
    ...
    cpdef inline void calc_wbm_v1(self)  nogil:
        ...
        for i in range(self.parameters.control.gts):
            d_temp1 = self.sequences.aides.rhm[i]-self.sequences.aides.rhv[i]
            d_temp2 = self.sequences.aides.rhmdh[i]-self.sequences.aides.rhvdh[i]
            self.sequences.aides.wbm[i] = (                self.parameters.control.bnm*d_temp1*d_temp2 +                self.parameters.control.bnm*2.*d_temp2*self.sequences.aides.rhv[i] +                (self.parameters.control.bm+self.parameters.control.bnm*d_temp1)*d_temp2 +                (self.parameters.control.bm+self.parameters.control.bnm*2.*d_temp1)*self.sequences.aides.rhvdh[i])
    ...
    cpdef inline void calc_wblv_wbrv_v1(self)  nogil:
        ...
        for i in range(self.parameters.control.gts):
            d_temp1 = self.sequences.aides.rhv[i] - self.sequences.aides.rhlvr[i]
            d_temp2 = self.sequences.aides.rhvdh[i] - self.sequences.aides.rhlvrdh[i]
            self.sequences.aides.wblv[i] = (                self.parameters.control.bnv[0]*d_temp1*d_temp2/2. +                self.parameters.control.bnv[0]*d_temp2*self.sequences.aides.rhlvr[i] +                (self.parameters.control.bnv[0]*d_temp1/2.+self.parameters.control.bv[0])*d_temp2 +                (self.parameters.control.bnv[0]*d_temp1+self.parameters.control.bv[0])*self.sequences.aides.rhlvrdh[i])
            d_temp1 = self.sequences.aides.rhv[i] - self.sequences.aides.rhrvr[i]
            d_temp2 = self.sequences.aides.rhvdh[i] - self.sequences.aides.rhrvrdh[i]
    ...
    cpdef inline void calc_dh_v1(self)  nogil:
        ...
        for i in range(self.parameters.control.gts):
            ...
            self.sequences.fluxes.dh[i] = (d_qz-self.sequences.fluxes.qg[i])/(1000.*self.parameters.control.laen/self.parameters.control.gts*self.sequences.aides.wbg[i])
    ...
    cpdef inline double return_h_v1(self)  nogil:
        return self.pegasush.find_x(            0., 2.*self.parameters.control.hm, -10., 1000., 0., 1e-10, 1000)
