...
cdef class Model:
    ...
    cpdef inline void calc_adjustedwindspeed_v1(self)  nogil:
        ...
        d_d = 2./3.*.12
        d_z0 = .123*.12
        self.sequences.fluxes.adjustedwindspeed = self.sequences.inputs.windspeed * (            log((2.-d_d)/d_z0) /            log((self.parameters.control.measuringheightwindspeed-d_d)/d_z0)        )
    cpdef inline void calc_saturationvapourpressure_v1(self)  nogil:
        self.sequences.fluxes.saturationvapourpressure = .6108*exp(            17.27*self.sequences.inputs.airtemperature/(self.sequences.inputs.airtemperature+237.3)        )
    cpdef inline void calc_saturationvapourpressureslope_v1(self)  nogil:
        self.sequences.fluxes.saturationvapourpressureslope =             4098.*self.sequences.fluxes.saturationvapourpressure/(self.sequences.inputs.airtemperature+237.3)**2
    cpdef inline void calc_actualvapourpressure_v1(self)  nogil:
        self.sequences.fluxes.actualvapourpressure =             self.sequences.fluxes.saturationvapourpressure*self.sequences.inputs.relativehumidity/100.
    cpdef inline void calc_earthsundistance_v1(self)  nogil:
        self.sequences.fluxes.earthsundistance = 1.+0.033*cos(            2*3.141592653589793/366.*(self.parameters.derived.doy[self.idx_sim]+1)        )
    cpdef inline void calc_solardeclination_v1(self)  nogil:
        self.sequences.fluxes.solardeclination = .409*sin(            2*3.141592653589793/366*(self.parameters.derived.doy[self.idx_sim]+1)-1.39        )
    ...
    cpdef inline void calc_solartimeangle_v1(self)  nogil:
        ...
        d_b = 2.*d_pi*(self.parameters.derived.doy[self.idx_sim]-80.)/365.
        d_sc = (            .1645*sin(2.*d_b) -            .1255*cos(d_b) -            .025*sin(d_b)        )
        self.sequences.fluxes.solartimeangle = d_pi/12. * (            (self.parameters.derived.sct[self.idx_sim] +             (self.parameters.control.longitude-self.parameters.derived.utclongitude)/15.+d_sc) - 12.        )
    ...
    cpdef inline void calc_possiblesunshineduration_v1(self)  nogil:
        ...
        cdef double d_pi
        d_pi = 3.141592653589793
        d_hours = self.parameters.derived.seconds/60./60.
        d_days = d_hours/24.
        if d_hours < 24.:
            if self.sequences.fluxes.solartimeangle <= 0.:
                d_thresh = -self.sequences.fluxes.solartimeangle-d_pi*d_days
            else:
                d_thresh = self.sequences.fluxes.solartimeangle-d_pi*d_days
            self.sequences.fluxes.possiblesunshineduration =                 min(max(12./d_pi*(self.sequences.fluxes.sunsethourangle-d_thresh), 0.), d_hours)
        else:
            self.sequences.fluxes.possiblesunshineduration = 24./d_pi*self.sequences.fluxes.sunsethourangle
    cpdef inline void calc_clearskysolarradiation_v1(self)  nogil:
        self.sequences.fluxes.clearskysolarradiation = self.sequences.fluxes.extraterrestrialradiation * (            self.parameters.control.angstromconstant+self.parameters.control.angstromfactor)
    ...
    cpdef inline void calc_globalradiation_v1(self)  nogil:
        if self.sequences.fluxes.possiblesunshineduration > 0.:
            self.sequences.fluxes.globalradiation = self.sequences.fluxes.extraterrestrialradiation * (                self.parameters.control.angstromconstant+self.parameters.control.angstromfactor *                self.sequences.inputs.sunshineduration/self.sequences.fluxes.possiblesunshineduration            )
        else:
            self.sequences.fluxes.globalradiation = 0.
    ...
    cpdef inline void calc_netshortwaveradiation_v1(self)  nogil:
        self.sequences.fluxes.netshortwaveradiation = (1.-.23)*self.sequences.fluxes.globalradiation
    cpdef inline void calc_netlongwaveradiation_v1(self)  nogil:
        ...
        if self.sequences.fluxes.clearskysolarradiation > 0.:
        ...
        self.sequences.fluxes.netlongwaveradiation = (            4.903e-9/24./60./60.*self.parameters.derived.seconds*(self.sequences.inputs.airtemperature+273.16)**4 *            (.34-.14*self.sequences.fluxes.actualvapourpressure**.5) *            (1.35*d_globalradiation/d_clearskysolarradiation-0.35)        )
    ...
    cpdef inline void calc_soilheatflux_v1(self)  nogil:
        if self.parameters.derived.seconds < 60.*60.*24.:
            if self.sequences.fluxes.netradiation >= 0.:
                self.sequences.fluxes.soilheatflux = .1*self.sequences.fluxes.netradiation
            else:
                self.sequences.fluxes.soilheatflux = .5*self.sequences.fluxes.netradiation
        else:
            self.sequences.fluxes.soilheatflux = 0.
