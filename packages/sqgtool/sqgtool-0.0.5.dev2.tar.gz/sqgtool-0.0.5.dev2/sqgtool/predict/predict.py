...
def predict_model_wise(data, filename='DR1_RF_model', verbose=True, return_probabilities=True):
    ...
    if return_probabilities:
        ...
        end_time = time.time()
        if verbose:
            print('Elapsed time to predict probabilities for sources with WISE counterpart considering 12 S-PLUS bands + Morphology + 2 WISE bands:', end_time - init_time)
...
def predict_model(data, filename="DR1_RF_model", verbose=True, return_probabilities=True):
    print(filename)
    ...
    if return_probabilities:
        ...
        end_time = time.time()
        if verbose:
            print('Elapsed time to predict probabilities for all sources considering 12 S-PLUS bands + Morphology:', end_time - init_time)
...
def create_catalog(dr1, verbose=True, return_probabilities=True, save_file=True, filename=result_path+"/TESTE"):
    try:
    ...
    results = predict_model(dr1, return_probabilities=return_probabilities)

    dr1_master = pd.concat([dr1, results], axis=1)
    try:
        results_wise = predict_model_wise(dr1_wise, return_probabilities)
