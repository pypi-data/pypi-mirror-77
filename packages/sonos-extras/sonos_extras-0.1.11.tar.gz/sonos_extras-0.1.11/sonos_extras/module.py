...
class SonosExtras(SoCo):
    def stop_after(self, songs):
        if self.get_current_transport_info()["current_transport_state"] == "PLAYING":
            while songs >= 1:
                ...
                if not WaitPollCurrentTrackTillEnd():
                    return 1
                songs -= 1
            print()
            self.pause()
    ...
    def WaitPollCurrentTrackTillEnd(self, quiet = False):
...
class SonosExtrasCliHelper(SonosExtras):
    ...
    def safe_set_volume(self, requested_volume, forced=False):
        ...
        if requested_volume - current_volume > self.volume_increase_confirmation_threshold and not forced:
            print("Are you sure you want to set volume to " + requested_volume.__str__() + "?")
            user_response = input()
        ...
        while (tracks_to_return > returned_matches and queue.number_returned > 0) :
            for item in queue:
                try:
                    ...
                    # print(item_dict)
                    print(self.track_info_string(item_dict))
                    count = count + 1
                except:
