...
#from graphs import scatter_plot, goscatter_plot, bar_plot, pie_plot, heatmap_plot, histogram_plot, box_plot
from vizdxp import graphs
...
- Python package available for download or pip installs
- Filtering Data before proceeding to visualizations
...
def process_date(df):
    print("in process date")
    for label,content in df.items():
        if is_datetime64_any_dtype(content):
            #st.write("1")
            print(f'Datetime feature {label} and being engineered to multiple columns')
    ...
    if df is not None:
        ...
        if page == 'Dataset stats':
            ...
            if viz_page == 'Plot 1':
                ...
                # multi_yaxis = st.sidebar.checkbox("Multiple yaxis traces")
                if plot1:
                    if charts=='default':
