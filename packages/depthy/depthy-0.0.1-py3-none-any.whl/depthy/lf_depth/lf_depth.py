...
from depthy.lf_depth.structure_tensor import local_structure_tensor
from depthy.misc import Normalizer, primal_dual_algo


def lf_depth(lf_img_arr: np.ndarray = None,
             lf_wid: int = 1,
             primal_opt: bool = True,
             perc_clip: float = 1) -> np.ndarray:
    """
    High-level function for depth map computation from a light-field based on the structure tensor.
