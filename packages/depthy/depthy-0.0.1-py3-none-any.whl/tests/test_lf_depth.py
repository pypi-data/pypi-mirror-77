...
from depthy.lf_depth import lf_depth


class LfDepthTestCase(unittest.TestCase):
    ...
    def load_data_set(self, set_name: str = None):
        ...
        fnames = self.loader.find_archive_fnames(archive_fn, head_str=join(self.set_name, 'input'), tail_str='png')
    ...
    def test_lf_depth_single(self, norm_ref: float = float('Inf')):
        ...
        # compute light-field depth based on epipolar images
        self.disparity = lf_depth(lf_img_arr=self.lf_img_arr, lf_wid=1, perc_clip=1, primal_opt=True)
