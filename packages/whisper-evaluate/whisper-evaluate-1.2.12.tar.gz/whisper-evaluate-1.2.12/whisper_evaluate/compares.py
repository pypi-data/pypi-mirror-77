...
def compare_yidun_whisper_results(result_file: str, target_file: str):
    df = pd.read_csv(result_file)
    df.columns = ["example", "yidun_json", "whisper_json"]
...
def parse_yidun_json(yidun_json: str, standard_json=False):
    if not standard_json:
        x = yidun_json.replace("'", '"')
    x = x.replace("\\x0", "")
    data = json.loads(x)
    _type = id2label[data['labels'][0]['label']] if data['labels'] else '其他'
...
def parse_whisper_json(whisper_json: str, standard_json=False):
    if not standard_json:
        x = whisper_json.replace("'", '"')
    a = json.loads(x)
    results = a['results'][0]
