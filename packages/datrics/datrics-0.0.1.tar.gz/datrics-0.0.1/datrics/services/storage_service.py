...
class StorageService:
    ...
    def get_brick_id(brick_directory):
        try:
            store = StorageService.get_storage()
            if not store['bricks'].get(brick_directory) or not store['bricks'].get(brick_directory, {}).get('brick_id', None):
    ...
    def get_project_id(brick_directory):
        try:
            store = StorageService.get_storage()
            if not store['bricks'].get(brick_directory) or not store['bricks'].get(brick_directory, {}).get('project_id', None):
                raise Exception(f'Brick {brick_directory} does not exists')
