...
import os
import re
from springlabs_python.utils import createDjangoProject, createFlaskProject
from springlabs_python import __version__ as version
...
@click.version_option(version=version, prog_name="Springlabs Manager", message="%(prog)s, v%(version)s")
@click.pass_context
def cli(ctx):
...
@click.option('-d', '--diseno',
    ...
    default="logico",
    show_default=True,
    type=click.Choice(['logico', 'fisico'], case_sensitive=False),
