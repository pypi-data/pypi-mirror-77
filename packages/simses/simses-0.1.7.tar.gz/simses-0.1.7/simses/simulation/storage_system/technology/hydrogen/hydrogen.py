...
class Hydrogen(StorageTechnology):
    ...
    def __init__(self, data_export: DataHandler, fuel_cell: str, fuel_cell_maximal_power: float, electrolyzer: str,
        ...
        self.__management_system: HydrogenManagementSystem = factory.create_hydrogen_management_system(
    ...
    def distribute_and_run(self, time: float, current: float, voltage: float):
        ...
        self.__hydrogen_storage.update_from(time, self.__electrolyzer.state, self.__fuel_cell.state)
    ...
    def get_auxiliaries(self) -> [Auxiliary]:
        return self.__electrolyzer.get_auxiliaries() + self.__fuel_cell.get_auxiliaries()
