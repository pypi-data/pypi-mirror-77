...
class SanyoNMCCyclicDegradationModel(CyclicDegradationModel):
    ...
    def calculate_degradation(self, battery_state: LithiumIonState) -> None:
        ...
        delta_fec: float = self._cycle_detector.get_depth_of_full_cycle() # Delta EFC = DOC
        ...
        beta_cap = 7.348*10**(-3)*(v_average - 3.667)**2 + 7.6*10**(-4) + 4.081*10**(-3)*delta_fec
        virtual_q = (qloss/beta_cap)**2 # virtual charge throughput in Ah
        capacity_loss = beta_cap*(virtual_q + qcell*delta_fec)**0.5 - qloss
    ...
    def calculate_resistance_increase(self, battery_state: LithiumIonState) -> None:
        ...
        beta_res = 2.153*10**(-4)*(v_average - 3.725)**2 - 1.521*10**(-5) + 2.798*10**(-4)*delta_fec
        virtual_q = (rinc_cyclic/beta_res)**2 # virtual charge throughput in Ah
        resistance_increase = beta_res*(virtual_q + delta_fec*qcell)**0.5 - rinc_cyclic
