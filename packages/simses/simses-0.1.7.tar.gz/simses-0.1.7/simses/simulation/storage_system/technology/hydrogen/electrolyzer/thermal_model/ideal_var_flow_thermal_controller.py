...
class IdealVarFlowThermalController(ThermalController):
    ...
    def __init__(self, config: ElectrolyzerConfig, heat_capacity_stack: float):
        super().__init__()
        self.__stack_temp_desire = config.desire_temperature_el  # °C
        ...
        self.__constants = ConstantsHydrogen
    ...
    def calculate(self, stack_temperature, heat_stack, timestep, min_water_flow_stack, current_dens) -> None:
        temp_diff = self.__stack_temp_desire - stack_temperature
        ideal_temp_variation_rate = temp_diff / timestep  # K/s

        if ideal_temp_variation_rate < self.__max_temperature_variation_rate:
            temp_var_rate = ideal_temp_variation_rate
        else:
            temp_var_rate = self.__max_temperature_variation_rate
        ...
        if self.__heat_control_on:
            if heat_water > 0:  # stack needs to be heated
                self.__delta_water_temp_in = self.__delta_water_temperature + temp_var_rate * timestep / 2  # K
                self.__h2o_flow = heat_water / (self.__constants.HEAT_CAPACITY_WATER *
                                               self.__constants.MOLAR_MASS_WATER * self.__delta_water_temp_in)  # mol/s
