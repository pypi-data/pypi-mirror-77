...
class LithiumIonFactory:
    ...
    def create_battery_state_from(self, system_id: int, storage_id: int, cell_type: CellType,
        ...
        if battery_state is None:
            ...
            cell_types_with_start_SOH_smaller_1 = (SonyLFP, PanasonicNCA, MolicelNMC, SanyoNMC)
            if self.__config_battery.start_soh < 1.0:
            ...
            bs.capacity = cell_type.get_capacity() * cell_type.get_nominal_voltage() * self.__config_battery.start_soh
            bs.fulfillment = 1.0
    ...
    def create_cell_type(self, cell_type: str, voltage: float, capacity: float) -> CellType:
        if cell_type == SonyLFP.__name__:
            ...
            return SonyLFP(voltage, capacity, self.__config_battery, self.__config_battery_data)
        elif cell_type == PanasonicNCA.__name__:
            self.__log.debug('Creating cell type as ' + cell_type)
            return PanasonicNCA(voltage, capacity, self.__config_battery, self.__config_battery_data)
        elif cell_type == MolicelNMC.__name__:
            self.__log.debug('Creating cell type as ' + cell_type)
            return MolicelNMC(voltage, capacity, self.__config_battery, self.__config_battery_data)
        elif cell_type == SanyoNMC.__name__:
            self.__log.debug('Creating cell type as ' + cell_type)
            return SanyoNMC(voltage, capacity, self.__config_battery, self.__config_battery_data)
        elif cell_type == DummyCell.__name__:
            self.__log.debug('Creating cell type as ' + cell_type)
            return DummyCell(voltage, capacity, self.__config_battery, self.__config_battery_data)
        elif cell_type == FieldDataNMC.__name__:
            self.__log.debug('Creating cell type as ' + cell_type)
            return FieldDataNMC(voltage, capacity, self.__config_battery, self.__config_battery_data)
        else:
            options: [str] = list()
