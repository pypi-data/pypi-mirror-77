...
class PressureTank(HydrogenStorage):
    ...
    __WALL_DIFFUSION_COEF: float = 4.4 * 10.0 ** (-7) * m.exp(-0.555 / ConstantsHydrogen.BOLTZ_CONST / __TEMPERATURE)  # cm2/s  selfdischarge through wall
    ...
    def __get_max_amount_hydrogen(self) -> float:
        ...
        return self.__capacity / 1000.0 / (ConstantsHydrogen.LHV_H2 * ConstantsHydrogen.MOLAR_MASS_HYDROGEN / 1000.0)
    ...
    def __get_volume(self) -> float:
        ...
        a = 1.0
        b = - (self.__max_amount_hydrogen * ConstantsHydrogen.VAN_D_WAALS_COEF_B + self.__max_amount_hydrogen * ConstantsHydrogen.IDEAL_GAS_CONST \
               * self.__TEMPERATURE / pressure)
        c = ConstantsHydrogen.VAN_D_WAALS_COEF_A * self.__max_amount_hydrogen ** 2 / pressure
        d = - ConstantsHydrogen.VAN_D_WAALS_COEF_B * ConstantsHydrogen.VAN_D_WAALS_COEF_A * self.__max_amount_hydrogen ** 3 / pressure
        coeff = [a, b, c, d]
        n = np.roots(coeff)
    ...
    def __ideal_gas_law(self, act_mass, volume) -> float:
        ...
        return act_mass * ConstantsHydrogen.IDEAL_GAS_CONST * self.__TEMPERATURE / \
               (volume - act_mass * ConstantsHydrogen.VAN_D_WAALS_COEF_B) - ConstantsHydrogen.VAN_D_WAALS_COEF_A * \
