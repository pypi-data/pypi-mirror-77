...
class VarCathodePressureModel(PressureModel):
    ...
    def calculate(self, time, state: ElectrolyzerState):
        ...
        n_h2_prod = state.hydrogen_production * timestep  # mol at stack level
        x_h2o_c = p_h2o_0 / (1 + p_c_0 - p_h2o_0)
        n_o2_prod = state.oxygen_production * timestep
        x_h2o_a = p_h2o_0 / (1 + p_a_0 - p_h2o_0)
        pressure_factor_cathode = ConstantsHydrogen.IDEAL_GAS_CONST * stack_temp / self.VOLUME_GAS_SEPARATOR * (1 + x_h2o_c)
        self.n_h2_out_c = self.__pressure_controller.calculate_n_h2_out(p_c_0, self.pressure_cathode_desire, n_h2_prod, self.max_h2_outflow, timestep, pressure_factor_cathode)
        ...
        self.p_c_1 = p_c_0 + pressure_factor_cathode * (n_h2_prod - self.n_h2_out_c ) * 10 ** (-5)  # bar
        ...
        self.h2_outflow = self.n_h2_out_c / timestep  # mol/s
        self.o2_outflow = self.n_o2_out_a / timestep  # mol/s
