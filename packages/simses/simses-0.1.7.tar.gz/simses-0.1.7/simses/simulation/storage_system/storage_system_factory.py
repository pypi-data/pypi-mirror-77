...
class StorageSystemFactory:
    ...
    def create_hvac_from(self, hvac: str) -> HeatingVentilationAirConditioning:
        ...
        if hvac_type == NoHeatingVentilationAirConditioning.__name__:
            ...
            power: float = float(hvac_configuration[hvac][StorageSystemConfig.HVAC_POWER])
            temperature_set_point: float = float(hvac_configuration[hvac][StorageSystemConfig.HVAC_TEMPERATURE_SETPOINT])
        ...
        for dc_system in dc_systems:
            ...
            if system == name and DcCoupling.__name__ in dc_type:
                if dc_type == NoDcCoupling.__name__:
                    ...
                    res.append(BusChargingProfileDcCoupling(self.__general_config, capacity, file_name))
    ...
    def create_storage_technology(self, technology: str, data_export: DataHandler,
        ...
        if technology == self.__lithium_ion_name:
            ...
            cell = technology_set[StorageSystemConfig.BATTERY_CELL]
            return LithiumIonBattery(cell, voltage, capacity, data_export,
            ...
            nominal_power = float(technology_set[StorageSystemConfig.REDOX_FLOW_POWER])
            pump_type: Pump = technology_set[StorageSystemConfig.REDOX_FLOW_PUMP]
            eta_pump = float(technology_set[StorageSystemConfig.REDOX_FLOW_PUMP_ETA])
            pump_algorithm = technology_set[StorageSystemConfig.REDOX_FLOW_PUMP_ALGORITHM]
            degradation_model = technology_set[StorageSystemConfig.REDOX_FLOW_DEGRADATION]
            return RedoxFlow(stack_type, pump_type, eta_pump, nominal_power, voltage, capacity, ambient_thermal_model,
                             pump_algorithm, degradation_model, data_export, storage_id, system_id,
                             self.__simulation_config)
