...
class AnalysisFactory:
    ...
    __INTRADAY_MARKET: [str] = [FcrIdmOperationStrategy.__name__, IntradayRechargeOperationStrategy.__name__]
    __OPERATIONS_AND_MAINTENANCE: [str] = [UseAllRenewableEnergy.__name__]
    ...
    def __init__(self, path: str, config: ConfigParser):
        ...
        self.__add_electricity_consumption_revenue_stream = self.__economic_analysis_config.add_electricity_consumption_revenue_stream
    ...
    def __create_revenue_streams(self, system_data) -> [RevenueStream]:
        ...
        if energy_management_strategy in self.__FREQUENCY_CONTAINMENT_RESERVE:
            market: FcrMarket = FcrMarket(self.__simulation_config, self.__market_profile_config)
        ...
        if energy_management_strategy in self.__INTRADAY_MARKET:
            market: IntradayMarket = IntradayMarket(self.__simulation_config, self.__market_profile_config)
        ...
        if energy_management_strategy in self.__OPERATIONS_AND_MAINTENANCE:
        ...
        if self.__add_electricity_consumption_revenue_stream:
