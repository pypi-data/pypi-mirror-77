...
class TechnicalEvaluation(Evaluation):
    ...
    def round_trip_efficiency(self) -> float:
        ...
        if charged == 0:
            ...
            self.__log.warn('The storage was only discharged. Thus, only a one-way '
                            'efficiency could be calculated')
        elif discharged ==0:
            efficiency = 100 * difference / charged
            self.__log.warn('The storage was only charged. Thus, only a one-way '
                            'efficiency could be calculated')
        else:
        ...
        if not 100 >= round(efficiency, 10) > 0:
            self.__log.error(Description.Technical.ROUND_TRIP_EFFICIENCY + ' should be between 0 % and 100 %, but is ' +
