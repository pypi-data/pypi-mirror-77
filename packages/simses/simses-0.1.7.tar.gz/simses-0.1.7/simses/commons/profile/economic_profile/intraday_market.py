...
class IntradayMarket(MarketProfile):
    """
    Provides the intraday market prices
    """
    def __init__(self, general_config: GeneralSimulationConfig, config: MarketProfileConfig):
        super().__init__()
        self.__file: FileProfile = FileProfile(general_config, config.intraday_price_file)
        #self.__config: MarketProfileConfig = config
        #self.__file = open(self.__config.intraday_price_file, 'r', newline='')
        ...
        ## self.__header = get_header_from(self.__config.intraday_price_file)
