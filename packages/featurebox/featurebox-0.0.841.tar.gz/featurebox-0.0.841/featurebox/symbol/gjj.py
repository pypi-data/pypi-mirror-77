...
from featurebox.symbol.skflow import SymbolLearning
...
if __name__ == "__main__":
    ...
    # Features
    c = [2, ]  # （输入）
    y_dim = [1, 2, 3, 4, 5, 6, 7]  # (输入，窗口按键方式，或者手动写入，若使用，覆盖掉表格中的量纲)
    ...
    if isinstance(y_dim, (list, tuple)):
        x_dim = Dim(y_dim) if isinstance(y_dim, (list, tuple)) else dless
