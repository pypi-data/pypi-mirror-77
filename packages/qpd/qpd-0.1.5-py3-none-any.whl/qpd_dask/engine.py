...
def run_sql_on_dask(sql: str, **dfs: Any) -> pd.DataFrame:
    return run_sql(QPDDaskEngine(), sql, **dfs)
...
class QPDDaskEngine(QPDEngine):
    ...
    def _group_agg_no_keys(
        self, df: DataFrame, agg_map: Dict[str, Tuple[str, AggFunctionSpec]],
    ) -> DataFrame:
        ndf = self.to_native(df)
        aggs: Dict[str, Any] = {}
        for k, v in agg_map.items():
