...
def test_op_precedence():
    ...
    assert_eq(
        dict(a=a), "SELECT a+1 AS a, b FROM a", [[2, 4], [3, 5]], ["a", "b"],
    )
    assert_eq(
        dict(a=a), "SELECT a+2*3 AS a, b FROM a", [[7, 4], [8, 5]], ["a", "b"],
    )
    assert_eq(
        dict(a=a), "SELECT a+1-2*3/4 AS a, b FROM a", [[0.5, 4], [1.5, 5]], ["a", "b"],
    )
...
def _debug_run(sql, rule, visitor, **kwargs):
    ...
    v = visitor(
        VisitorContext(
            sql=sql, workflow=wf, current=wf.dfs[list(kwargs.keys())[0]], dfs=wf.dfs
        )
    )
