...
def metaval_prototype():
    witness_instrumentor = WitnessInstrumentor("config/cpa-witnesses-instrumentor.yml")

    # In termination only false is verified
    cond_termination = "isinstance(spec, BehaviorSpecification)"
    ua = ProgramVerifier("config/uautomizer.yml")
    ch_termination = ITE(cond_termination, ua, IdentityActor(ua))

    cond_no_overflow = "isinstance(spec, BehaviorSpecification)"
    ch_no_overflow = ITE(
        cond_no_overflow, ProgramVerifier("config/uautomizer.yml"), ch_termination
    )

    cond_mem_safety = "isinstance(spec, BehaviorSpecification)"
    ch_mem_safety = ITE(
        cond_mem_safety, ProgramVerifier("config/symbiotic.yml"), ch_no_overflow
    )

    cond_reach_safety = "isinstance(spec, BehaviorSpecification)"
    ch_reach_safety = ITE(
        cond_reach_safety, ProgramVerifier("config/cpa-seq.yml"), ch_mem_safety
    )
...
def conditional_tester(test_generator):
    pruner = TestGoalPruner("config/test-goal-pruner.yml")
    extractor = TestGoalExtractor("config/test-goal-extractor.yml")
...
def verifier_based_tester(ver):
    ...
    w2test = WitnessToTest("config/cpachecker-witness-to-test.yml")
...
def conditional_tester_verifier_based(ver):
    annotator = TestGoalAnnotator("config/test-goal-annotator.yml")
    extractor = TestGoalExtractor("config/test-goal-extractor.yml")
...
def atva_paper_fig_14(ver):
    ...
    instrumentor = TestCriterionInstrumentor("config/test-criterion-instrumentor.yml")
