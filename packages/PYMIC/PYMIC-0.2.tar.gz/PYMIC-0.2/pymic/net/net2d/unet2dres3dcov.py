...
class UNetBlock(nn.Module):
    def __init__(self,in_channels, out_channels, acti_func, acti_func_param):
        ...
        self.conv1 = ConvolutionLayer(in_channels,  out_channels, 1, 
                dim = 3, padding = 0, acti_func=get_acti_func(acti_func, acti_func_param))
        self.conv2 = ConvolutionLayer(out_channels, out_channels, (1, 3, 3), 
                dim = 3, padding = (0, 1, 1), acti_func=get_acti_func(acti_func, acti_func_param))
        self.conv3 = ConvolutionLayer(out_channels, out_channels, (1, 3, 3), 
                dim = 3, padding = (0, 1, 1), acti_func=get_acti_func(acti_func, acti_func_param))
...
class UNet2DRes3DCov(nn.Module):
    def __init__(self, params):
        super(UNet2DRes3DCov, self).__init__()
        self.params = params
        ...
        if(self.resolution_level == 5):
            ...
            self.up1 = DeconvolutionLayer(self.ft_chns[4], self.ft_chns[3], kernel_size = (1, 2, 2),
                dim = 3, stride = (1, 2, 2), acti_func = get_acti_func(self.acti_func, self.params))
...
if __name__ == "__main__":
    ...
    x  = np.random.rand(5, 1, 32, 96, 96) # N, C, H, W
