...
class CABlock(nn.Module):
    def __init__(self, in_chns):
        ...
        self.conv_s_1 = nn.Conv3d(self.in_chns, half_chns,
                kernel_size = 1, bias = True)
...
class UNet3DCA(nn.Module):
    def __init__(self, params):
        super(UNet3DCA, self).__init__()
        self.params = params
        ...
        if(self.resolution_level == 5):          
            self.ca6 = CABlock(2*self.ft_chns[3])
        self.ca7 = CABlock(2*self.ft_chns[2])
        self.ca8 = CABlock(2*self.ft_chns[1])
        self.ca9 = CABlock(2*self.ft_chns[0])
    ...
    def forward(self, x):
        ...
        if(self.resolution_level == 5):
            ...
            f4cat, att_s4 = self.ca6(f4cat)
            f6    = self.block6(f4cat)

            f6up  = self.up2(f6)
            f3cat = torch.cat((f3, f6up), dim = 1)
            f3cat, att_s3 = self.ca7(f3cat)
        else:
            f4up  = self.up2(f4)
        ...
        f2cat, att_s2 = self.ca8(f2cat)
        f8    = self.block8(f2cat)
        
        f8up  = self.up4(f8)
        f1cat = torch.cat((f1, f8up), dim = 1)
        f1cat, att_s1 = self.ca9(f1cat)
