...
class PolyTcp (threading.Thread):
    ...
    def listen(self):
        try:
            ...
            self.iface.print(self.iface.name + " TCP Listening on port: " + str(self.socket.getsockname()[1]))
        except Exception  as e:
            self.iface.print(str(e))
...
class PolyUdp (threading.Thread):
    def __init__(self, iface, localPort ):
        ...
        self.socket.bind(("0.0.0.0", self.localPort))
        self.iface.print(self.iface.name + "UDP Listening on port: " + str(self.socket.getsockname()[1]))
        self.host = 0
            ...
            if words[0] == 'tcp':
                try:
                    if(words[1].isnumeric()):           #tcp:local_port
                        localPort = int(words[1])   
                        if len(words) == 3:              #tcp:local_port:remote_port
                            remotePort = int(words[2])
                        if len(words) == 4:              #tcp:local_port:remote_ip:remote_port
                            remoteHost = words[2]
                            remotePort = int(words[3])
