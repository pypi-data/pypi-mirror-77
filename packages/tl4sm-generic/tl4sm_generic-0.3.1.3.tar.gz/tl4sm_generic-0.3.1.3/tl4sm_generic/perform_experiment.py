from tl4sm_generic.model_prep import bin_data, evaluate_model_reuse, evaluate_model_tl, evaluate_model
from tl4sm_generic.prepare_data import split_dataset
...
def perform_experiment(model, resFile, file_name, n_test, model_, n_out, verbose, tr_time, med, high):
    ...
    for index, row in df_exp.iterrows():
        ...
        if Path(model_+str(source)+'.h5').is_file():
            ...
            if tl_type == 'reuse':
                ...
                print('Source', source, 'Target', target)
                print('Data Percent', data_percent)
                score, accuracy, trainTime, _, _, _ = evaluate_model(model, train, test, tr_time, n_input, source, exp_num)
        else:
            print('============= Model Training without TL ===============')
            print('Source', source, 'Target', target)
            print('Data Percent', data_percent)
            score, accuracy, trainTime, f1_base, acc_base, tr_time_base = evaluate_model(model, train, test, tr_time, n_input, source, exp_num)
        #append scores
