...
def lsq_eq(A, b, A_eq, b_eq, cond=None):
    """
    ...
    """
    p = 0 if A_eq is None else len(A_eq)
    A, b, A_eq, b_eq = map(np.asarray, (A, b, A_eq, b_eq))

    if p > 0:
        Q, R = linalg.qr(A_eq.T)
        y = linalg.solve_triangular(R[:p, :p], b_eq, trans='T')
        AQ = A @ Q
        z = linalg.lstsq(AQ[:, p:], b - AQ[:, :p] @ y, cond=cond)[0]
        x = Q[:, :p] @ y + Q[:, p:] @ z
    else:
        x = linalg.lstsq(A, b, cond=cond)[0]
...
class lsq_lin:
    ...
    dramatically reduced the number of iterations ain the main loop.
    ...
    def __init__(self, A, b, A_ineq=None, b_ineq=None, A_eq=None, b_eq=None,
                 bounds=None, x=None, verbose=False, ftol=1e-10, cond=None):
        ...
        if x is None or A_ineq is None:
            x = lsq_eq(A, b, A_eq, b_eq, cond=cond)
        ...
        def solve_subproblem(f):
            Ae = A_ineq[~f] if A_eq is None else np.vstack([A_eq, A_ineq[~f]])
            be = b_ineq[~f] if A_eq is None else np.append(b_eq, b_ineq[~f])
            x = lsq_eq(A, b, Ae, be, cond=cond)
            bad = A_ineq[f] @ x > b_ineq[f]
            return x, bad
        ...
        beps = b_ineq - ftol*linalg.norm(A_ineq, axis=1)
        bad = True
        while np.any(bad):  # Initialization loop
            f = A_ineq @ x < beps
            x, bad = solve_subproblem(f)
            if verbose:
                ita += 1
                print(f"a*{ita:6d} {chi2(b - A @ x)/2:15.4e}")

        cost_new, cost_old = chi2(b - A @ x), 1e300
        while np.any(~f) and (cost_old - cost_new > ftol*cost_old):    # Loop A
            lam = np.zeros_like(b_ineq)
            Ae = A_ineq[~f] if A_eq is None else np.vstack([A_eq, A_ineq[~f]])
            lam[~f] = linalg.lstsq(Ae.T, A.T @ (b - A @ x), cond=cond)[0][p:]
            j = np.argmin(lam)
            if lam[j] < 0:
                f[j] = bad = 1
                while np.any(bad):    # Loop B
                    z, bad = solve_subproblem(f)
                    if np.any(bad):
                        alpha = np.min((b_ineq[f][bad] - A_ineq[f][bad] @ x)
                                       /(A_ineq[f][bad] @ (z - x)))
                        x += alpha*(z - x)
                        f = A_ineq @ x < beps
                        if verbose:
                            print(f"b*{ita:6d}** {chi2(b - A @ x)/2:13.4e}   alpha: {alpha:.4f}")
                    else:
                        x = z
                cost_old, cost_new = cost_new, chi2(b - A @ x)
            else:
                if verbose:
            ...
            if verbose:
                ita += 1
                print(f"a {ita:6d} {cost_new/2:15.4e} {(cost_old - cost_new)/2:15.4e}")
...
class lsq_box:
    ...
    """
    def __init__(self, A, b, bounds, x=None, cond=None, verbose=False, ftol=1e-10):
        ...
        while np.any(bad):  # Initialization loop
            x = x.clip(*bounds)
            f = (lb < x) & (x < ub)
            x[f] = linalg.lstsq(A[:, f], b - A[:, ~f] @ x[~f], cond=cond)[0]
            bad = (x < lb) | (ub < x)
            if verbose:
                ita += 1
                print(f"a*{ita:6d} {chi2(b - A @ x)/2:15.4e}")
        ...
        while np.any(~f) and (chi2old - chi2new > ftol*chi2old):    # Loop A
            ...
            if wd[j] > 0:
                ...
                while np.any(bad):    # Loop B
                    x = x.clip(*bounds)
                    z = linalg.lstsq(A[:, f], b - A[:, ~f] @ x[~f], cond=cond)[0]
                    bad = (z < lb[f]) | (ub[f] < z)
                    if np.any(bad):
                        bn = np.select([z < lb[f], ub[f] < z], bounds[:, f])
                        alpha = np.min((bn[bad] - x[f][bad])/(z[bad] - x[f][bad]))
                        x[f] += alpha*(z - x[f])
                        f = (lb < x) & (x < ub)
                        if verbose:
                            print(f"b*{ita:6d}** {chi2(b - A @ x)/2:13.4e}   alpha: {alpha:.4f}")
...
class capfit:
    ...
    def __init__(self, func, p1, abs_step=None, bounds=(-np.inf, np.inf),
        ...
        while(True):
            ...
            if bounds_only:
                h = lsq_box(A, b, bounds=bounds-p1, cond=cond).x
            else:
                b_ineq_p = None if A_ineq is None else b_ineq - A_ineq @ p1
                b_eq_p = None if A_eq is None else b_eq - A_eq @ p1
                h = lsq_lin(A, b, A_ineq, b_ineq_p, A_eq, b_eq_p, bounds=bounds-p1, cond=cond).x
