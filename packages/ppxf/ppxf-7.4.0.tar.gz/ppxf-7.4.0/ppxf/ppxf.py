...
from .capfit import capfit, lsq_lin, lsq_box, cov_err

################################################################################

def lsq_lin_cvxopt(A_lsq, b_lsq, A_ineq, b_ineq, A_eq, b_eq, initvals=None):
    ...
    sol = solvers.qp(P, q, A_ineq, b_ineq, A_eq, b_eq,
...
class ppxf:
    ...
    constr_templ:
        ...
        and ``fraction2`` (of the total weights) respectively (the third
        components will be ``1 - fraction1 - fraction2``), as follows::
    ...
    constr_kinem:
        ...
        component to differs less than 10% from that of the first component
    ...
    linear_method: {'nnls', 'lsq_box', 'lsq_lin', 'cvxopt'} optional
        ...
        on the size of the problem, with 'lsq_box' generally being the fastest
        without linear inequality constraints and 'cvxopt' the fastest with
        linear inequality constraints. The 'nnls' option (the default before v7)
        is generally slower and for this reason is now deprecated.

        The inequality constraints in ``constr_templ`` are only supported
        with ``method='lsq_lin'`` or ``method='cvxopt'``.

        Before v7.0 ``pPXF`` only used the single 'nnls' method.
    mask:
...
################################################################################
    ...
    def set_linear_constraints(self, constr_templ, constr_kinem, method):
        ...
        self.A_ineq_templ = np.pad(-np.identity(pos), [(0, 0), (npoly, 0)])
    ...
    def solve_linear(self, A, b, npoly):
        ...
        if n == 1:  # A is a column vector, not an array
            ...
            if self.linear_method == 'lsq_lin':
                ...
                self.x0 = res
                soluz = np.squeeze(res['x'])
            else:
                if self.A_eq_templ is not None:
                    scale = 1e-4*np.max(self.A_eq_templ)/np.median(A)
