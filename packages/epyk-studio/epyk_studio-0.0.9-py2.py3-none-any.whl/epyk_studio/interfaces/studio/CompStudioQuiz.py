...
class Quiz(object):
  ...
  def theme(self):
    ...
    """
    self.context.rptObj.theme = ThemeRed.Pink()
  ...
  def progress(self, percentage, icon="fas fa-car-side", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    """
    options = options or {}
    icon = self.context.rptObj.ui.icons.awesome(icon, height=("auto", ''))
    progress = self.context.rptObj.ui.div()
    progress.style.css.border = "1px solid %s" % self.context.rptObj.theme.success[1]
    progress.style.css.display = "inline-block"
    progress.style.css.width = "calc(100% - 50px)"
    inner = self.context.rptObj.ui.div("%s%%" % percentage, width=(percentage, '%'))
    inner.style.css.background = self.context.rptObj.theme.success[0]
    inner.style.css.color = self.context.rptObj.theme.success[1]
    inner.style.css.text_align = "center"
    progress.add(inner)
    container = self.context.rptObj.ui.div([progress, icon], width=width, height=height)
  ...
  def position(self, longitude, latitude, flag=None, width=(100, '%'), height=("auto", ''), icon="fas fa-map-marked-alt", options=None, profile=None):
    ...
    container.icon.icon.style.css.color = self.context.rptObj.theme.greys[6]
    container.add(container.icon)
    if flag is None:
      container.button = self.context.rptObj.ui.button("Check Position")
      container.button.style.css.line_height = 20
      container.add(container.button)
    elif flag:
      container.text = self.context.rptObj.ui.text("Well done! Position validated")
      container.text.style.css.line_height = 20
      container.text.style.css.color = self.context.rptObj.theme.success[1]
      container.text.style.css.font_factor(5)
      container.add(container.text)
    else:
      container.text = self.context.rptObj.ui.text("Failed! Position not found")
      container.text.style.css.line_height = 20
      container.text.style.css.color = self.context.rptObj.theme.danger[1]
      container.text.style.css.font_factor(5)
      container.add(container.text)
    return container
  ...
  def picture(self, image, question=None, icon="far fa-image", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    container.icon = self.context.rptObj.ui.icons.awesome(icon, width=(100, '%'), height=("auto", ''))
    container.icon.style.css.background = self.context.rptObj.theme.colors[6]
    ...
    if "timer" in options:
      if options.get("timer", 0) > 0:
        container.image = self.context.rptObj.ui.img(image, align="center", width=('auto', ''), height=(300, 'px'))
    ...
    if question is not None:
      question = self.context.rptObj.ui.text(self.context.rptObj.py.encode_html(question), align="center")
      question.style.css.font_factor(5)
      container.add(question)
    container.add(self.delimiter())
    input = self.context.rptObj.ui.input()
    input.style.css.margin_top = 20
    input.style.css.margin_bottom = 10
    input.style.css.text_align = "left"
    container.add(input)
    button = self.context.rptObj.ui.button("Validate", align="center")
  ...
  def question(self, question, icon="fas fa-question", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    container.icon.icon.style.css.color = "white"
    container.add(container.icon)
    question = self.context.rptObj.ui.text(self.context.rptObj.py.encode_html(question),
                                                  width=(None, 'px'))
    question.style.css.font_factor(5)
    container.add(question)
    container.add(self.delimiter())
  ...
  def charade(self, clues, question=None, icon="fas fa-book", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    container.add(container.icon)
    for clue in clues:
      value = self.context.rptObj.ui.text(self.context.rptObj.py.encode_html(clue), width=(None, 'px'))
      value.style.css.font_factor(5)
      value.style.css.font_family = "Meddon"
      container.add(value)
    if question is not None:
      question = self.context.rptObj.ui.text(self.context.rptObj.py.encode_html(question), width=(None, 'px'))
  ...
  def delimiter(self, size=1):
    ...
    hr.hr.style.css.border_color = self.context.rptObj.theme.colors[5]
  ...
  def missing(self, text, icon="fas fa-ellipsis-h", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    container.add(container.icon)
    text_frg = text.split("*")
    container.add(self.context.rptObj.ui.text(text_frg[0]))
    if text.startswith("*"):
      input = self.context.rptObj.ui.input("", width=(50, 'px'))
      input.style.css.margin = "0 5px"
      container.add(input)
    for text in text_frg[1:-1]:
      container.add(self.context.rptObj.ui.text(text))
      input = self.context.rptObj.ui.input("", width=(50, 'px'))
      input.style.css.margin = "0 5px"
      container.add(input)
    container.add(self.context.rptObj.ui.text(text_frg[-1]))
    if text.endswith("*"):
      container.add(self.context.rptObj.ui.input(""))
    button = self.context.rptObj.ui.button("Validate", align="center")
    container.add(button)
    return container
  ...
  def hangman(self, question, text, proposals=None, icon="fas fa-project-diagram", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    container.result = self.context.rptObj.ui.text("".join(crypt_text), width=(None, 'px'))
  ...
  def choice(self, question, answers, status=None, icon="fas fa-question", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    for answer in answers:
      button = self.context.rptObj.ui.button(answer, width=(100, '%'))
  ...
  def chrono(self, component, time=60, flag=None, icon="fas fa-stopwatch-20", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    """
    container = self.context.rptObj.ui.div(width=width, height=height)
    container.timer = self.context.rptObj.ui.calendars.timer(time / 60, width=(None, '%'))
    ...
    container.add(self.context.rptObj.ui.div([self.context.rptObj.ui.icons.awesome(icon), container.timer]))
  ...
  def estimate(self, text, tooltip=None, width=(None, '%'), height=("auto", ''), icon="fas fa-stopwatch", options=None, profile=None):
    ...
    container.icon = self.context.rptObj.ui.icons.awesome(icon)
    container.icon.icon.style.css.font_factor(10)
    container.icon.icon.style.css.color = self.context.rptObj.theme.success[1]
    container.add(container.icon)
    container.text = self.context.rptObj.ui.text(text, width=(None, 'px'))
  ...
  def ranking(self, number=0, width=(None, '%'), height=("auto", ''), icon="fas fa-flag-checkered", options=None, profile=None):
    ...
    container.icon.icon.style.css.color = self.context.rptObj.theme.success[1]
    container.add(container.icon)
    container.number = self.context.rptObj.ui.texts.number(number, width=(None, 'px'))
    container.number.style.css.line_height = 20
    container.number.style.css.color = self.context.rptObj.theme.success[1]
  ...
  def summary(self, records, width=('auto', '%'), height=(None, "px"), options=None, profile=None):
    ...
    for values in records:
      ...
      if not hasattr(rec[0], 'options'):
        name = self.context.rptObj.ui.text(rec[0], align="left", width=(100, 'px'))
      ...
      move = self.context.rptObj.ui.text(rec[2])
      move.style.css.padding = "5px 10px"
      move.options.managed = False
      percent = self.context.rptObj.ui.text(rec[3])
      percent.style.css.padding = "5px 10px"
      percent.options.managed = False
      ranking = self.context.rptObj.ui.text(rec[4])
      ranking.style.css.padding = "5px 10px"
      ranking.options.managed = False
      html_records.append([name, current, move, percent, ranking])
    table = self.context.rptObj.ui.tables.basic(html_records, rows=[0], cols=[1, 2, 3, 4], width=width,
                                                       height=height, options=options, profile=profile)

    table[0][-1].style.css.border_top = "2px dashed %s" % self.context.rptObj.theme.success[1]
    for row in table:
      row[-1].style.css.background = self.context.rptObj.theme.colors[2]
      row[-1].style.css.border_left = "2px dashed %s" % self.context.rptObj.theme.success[1]
      row[-1].style.css.border_right = "2px dashed %s" % self.context.rptObj.theme.success[1]
