...
class Event(object):
  ...
  def progress(self, percentage, icon="fas fa-plane", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    icon.icon.style.css.color = self.context.rptObj.theme.colors[-1]
    progress = self.context.rptObj.ui.div()
    progress.style.css.border = "1px solid %s" % self.context.rptObj.theme.colors[-1]
    ...
    inner.style.css.background = self.context.rptObj.theme.colors[4]
    inner.style.css.color = self.context.rptObj.theme.greys[0]
  ...
  def search(self, text="", placeholder='search', icon="fas fa-search", width=(100, "%"), height=(None, "px"),
    ...
    container.input.style.css.border = "1px solid %s" % self.context.rptObj.theme.greys[3]
  ...
  def comment(self, avatar, placeholder=None, width=(100, '%'), height=(None, 'px'), options=None, profile=None):
    ...
    container.text = self.context.rptObj.ui.textarea(placeholder=placeholder)
    container.text.style.css.display = "inline-block"
    container.text.style.css.float = "right"
    container.text.style.css.color = self.context.rptObj.theme.greys[4]
  ...
  def dated(self, date, name=None, width=(None, '%'), height=(None, 'px'), align="left", format=None, options=None):
    ...
    container.text = self.context.rptObj.ui.text("Last updated %s, by" % py_date.strftime("%d %B %Y"), width=(None, ""))
    container.text.style.css.display = 'inline-block'
    container.text.style.css.color = self.context.rptObj.theme.greys[5]
    container.add(container.text)
    container.name = self.context.rptObj.ui.link(name)
    container.name.style.css.display = 'inline-block'
    container.name.style.css.color = self.context.rptObj.theme.colors[-1]
  ...
  def flip(self, heads, tails, orientation='v', width=(300, 'px'), height=(200, 'px')):
    ...
    flip_box.style.css.border = "1px solid %s" % self.context.rptObj.theme.colors[3]
    flip_box.style.css.perspective = 1000

    flip_box_inner = self.context.rptObj.ui.div(height=(100, '%'))
    ...
    flip_box_front = self.context.rptObj.ui.div(height=(100, '%'))
    flip_box_front.style.css.position = "absolute"
    flip_box_front.style.css.backface_visibility = "hidden"

    flip_box_back = self.context.rptObj.ui.div(height=(100, '%'))
    ...
    if not hasattr(heads, 'options'):
      flip_box.heads = self.context.rptObj.ui.text(heads, height=(100, '%'))
    ...
    if not hasattr(tails, 'options'):
      flip_box.tails = self.context.rptObj.ui.text(tails, height=(100, '%'))
      ...
      flip_box_back.style.css.background = self.context.rptObj.theme.colors[3]
  ...
  def phone(self, number, icon="fas fa-phone-alt", width=('auto', ''), height=(None, 'px')):
    ...
    container.add(icon)
    link = self.context.rptObj.ui.link(number, "tel:+1-303-499-7111")
    link.style.css.color = self.context.rptObj.theme.colors[5]
    container.add(link)
    return container
...
class Wedding(Event):
  ...
  def picture(self, image=None, label=None, path=None, width=(100, "%"), height=(None, "px"), align="center", htmlCode=None,
    ...
    if height[0] is not None and width[1] == '%':
      width = ("auto", '')
    component = self.context.rptObj.ui.div(align=align, width=width, height=height, options=options, profile=profile)
    component.image = html.HtmlImage.Image(self.context.rptObj, image, path, align, htmlCode, (100, '%'), (100, '%'), profile, options or {})
  ...
  def block(self, text, align="left", width=("80", '%'), height=(None, "px"), options=None, profile=None):
    ...
    component.style.css.border_left = "3px solid %s" % self.context.rptObj.theme.colors[-1]
    if not hasattr(text, 'options'):
      content = self.context.rptObj.py.encode_html(text.strip())
      ...
      component.text = self.context.rptObj.ui.text(content)
  ...
  def paragraph(self, text, align="left", width=("80", '%'), height=(None, "px"), options=None, profile=None):
    ...
    :param profile:
    """
    text = self.context.rptObj.ui.text.paragraph(text, align=align, width=width, height=height, options=options, profile=profile)
    return text
...
class Seminar(Event):
  ...
  def team(self, fill=None, border=None, width=(50, "px"), height=(30, "px")):
    ...
    :param height:
    """
    svg = self.context.rptObj.ui.pictos.path(fill=fill, stroke=border, width=width, height=height, viewbox=(150, 80),
