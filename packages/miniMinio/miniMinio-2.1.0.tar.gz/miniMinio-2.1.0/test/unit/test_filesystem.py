...
def test_minio_get_file(MinioFileSystem):
    MinioFileSystem.get_file("bucket/source.txt", "destination/file.txt")
    MinioFileSystem.__client__.fget_object.assert_called_once_with('bucket',
...
def test_minio_rm(MinioFileSystem):
    ...
    MinioFileSystem.__client__.remove_objects.assert_called_once_with(bucket_name="bucket",
                                                                      objects_iter={"file1.txt", "file2.txt"})
...
@mock.patch('miniMinio.FileSystem.shutil.copy2')
def test_local_get_file(copy, LocalFileSystem):
    LocalFileSystem.get_file('source/file.txt', 'destination/file.txt')
    copy.assert_called_once_with('source/file.txt', 'destination/file.txt')
