...
from uiotedgedriverlinksdk import (_deviceInfos, _driverInfo,
                                   _gateway_device_sn, _gateway_product_sn)
...
class Config(object):
    ...
    def getGatewayProductSN(self):
        return _gateway_product_sn

    def getGatewayDeviceSN(self):
        return _gateway_device_sn


def getConfig():
    config = {}
    if _driverInfo is not None:
        config['deviceList'] = _deviceInfos

    if _deviceInfos is not None:
        config['config'] = _driverInfo

    config['productSN'] = _gateway_product_sn
    config['deviceSN'] = _gateway_device_sn
