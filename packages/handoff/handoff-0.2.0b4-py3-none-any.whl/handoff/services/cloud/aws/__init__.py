...
def create_metric_filter(filter_name, filter_pattern, metric_name,
                         metric_value, default_value):
    """
    aws logs put-metric-filter --log-group-name tiny-etl-zendesk-jira
    --filter-name tiny-etl-zendesk-jira-record-count
    --filter-pattern '{ $.type = "counter" && $.metric = "record_count"
                        && $.value = "*" }'
    --metric-transformations 'metricName=zendesk-jira-record-count,
        metricNamespace=tiny-etl,metricValue=$.value,defaultValue=0'
    """
    state = get_state()
    log_group_name = state.get(RESOURCE_GROUP)
    metric_namespace = state.get(RESOURCE_GROUP)

    response = logs.put_metric_filter(log_group_name, filter_name,
                                      filter_pattern, metric_namespace,
