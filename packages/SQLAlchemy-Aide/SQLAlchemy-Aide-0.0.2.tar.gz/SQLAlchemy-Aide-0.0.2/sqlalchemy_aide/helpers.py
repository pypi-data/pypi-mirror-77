...
def hidden_fields(element):
    return set([col.key for col in getattr(element, '__hidden__', set())])
...
def related(model):
    return [i.entity.entity for i in model.__mapper__.relationships]
...
def iserialize(data, fields=None, exc: Optional[set] = None, rels=None, root=None, exclude=None, functions=None,
               **kwargs):
    """
    ...
    exclude.update(map(lambda col: col.name, getattr(data, 'hidden', set())))
    exclude.update(map(lambda col: col.name, getattr(data, '__hidden__', set())))

    if not data:
        return []

    if rels is True:
        rels = relationships(data, True)

    if not fields:
        fields = set(columns(data, strformat=True))
...
def serialize(model, data, fields=None, exc: Optional[set] = None, rels=None, root=None, exclude=None, functions=None,
    ...
    exclude.update(map(lambda col: col.name, getattr(model, '__hidden__', set())))
