...
class Parser(object):
    ...
    def _parse_text(self, aspirate_result_cleaned_text: str) -> dict:
        return pipe(
            aspirate_result_cleaned_text, self._entry_divide, self._form_dict
        )
    ...
    def _extract_entry(self, entry_text: str) -> Optional[Entry]:
        ...
        entry = None
        if Parser._is_legit_entry(sudo_entry):
            entry = self._form_entry(sudo_entry)
        if Parser._deserve_warning(entry, sudo_entry):
...
_remove_signature = partial(
    re.compile(r"REPORTING PHYSICIAN.*|D[rR][.]? .*").sub, ""
)
_replace_add = methodcaller("replace", "&", "AND")
_remove_reporting_system = partial(re.compile(r"\(REPORTING.*\)").sub, "")
_special_field = partial(
    re.compile(r"PERIPHERAL (?!BLOOD[\s.])").sub, "\nPERIPHERAL "
)
