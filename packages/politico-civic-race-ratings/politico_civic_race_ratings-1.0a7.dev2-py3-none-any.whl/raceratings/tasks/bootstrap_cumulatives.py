    ...
    for runoff in list(runoffs_for_body):
        if runoff.election_mode != ElectionEvent.UPCOMING_MODE:
            ...
            for race_key, race_data in groupby(
                ...
                runoff_configs[race_key]["state_results"].extend(
                    all_race_data
                )
...
def apply_manual_overrides(race_config):
    ...
    for override in in_house_overrides:
        if override['winner'] == True or override['winner'].lower() == 'true':
            result_to_override = [
                _
...
# def bake_detailed_cumulative_rollups():
#     formatted_state_results = []
#     for _ in asen['data']['stateResults']:
#
#         politician_id_number = f"polnum-{_['polnum']}"
#
#         try:
#             candidate_election = CandidateElection.objects.get(
#                 ap_candidate_number=politician_id_number
#             )
#
#             candidate_metadata = dict(
#                 first_name=candidate_election.candidate.person.first_name,
#                 middle_name=candidate_election.candidate.person.middle_name,
#                 last_name=candidate_election.candidate.person.last_name,
#                 suffix=candidate_election.candidate.person.suffix,
#                 ap_candidate_number=candidate_election.candidate.ap_candidate_id,
#                 ballot_order=candidate_election.ballot_order,
#                 incumbent=candidate_election.candidate.incumbent,
#                 party=candidate_election.candidate.party.ap_code,
#                 uid=candidate_election.candidate.uid,
#                 uncontested=candidate_election.uncontested,
#             )
#         except CandidateElection.DoesNotExist:
#             candidate_metadata = {}
#
#         formatted_state_results.append(dict(
#             metadata=candidate_metadata,
#             race_type_slug=_['race_type_slug'],
#             office_slug=_['office_slug'],
#             polnum=_['polnum'],
#             precinctsreporting=int(_['precinctsreporting']),
#             precinctstotal=int(_['precinctstotal']),
#             precinctsreportingpct=float(_['precinctsreportingpct']),
#             runoff=_['runoff'].lower() is 'false',
#             votecount=int(_['votecount']),
#             votepct=float(_['votepct']),
#             winner=_['winner'].lower() is 'false',
#         ))
