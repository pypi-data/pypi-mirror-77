...
from mlflow.utils.qubole_context_utils_factory import QuboleContextUtilsFactory
from mlflow.utils.qubole_utils import is_running_in_qubole
...
def get_tracking_uri():
    ...
    qubole_context_utils = QuboleContextUtilsFactory.get_utils()

    if _tracking_uri is not None:
        return _tracking_uri
    elif env.get_env(_TRACKING_URI_ENV_VAR) is not None:
        return env.get_env(_TRACKING_URI_ENV_VAR)
    elif is_running_in_qubole() \
            and qubole_context_utils is not None \
            and qubole_context_utils.is_qubole_tracking_server_available():
        return qubole_context_utils.get_tracking_uri()
    else:
...
def _get_qubole_rest_store(store_uri, **_):
    return RestStore(
        lambda: (
            QuboleContextUtilsFactory
            .get_utils()
            .get_qubole_host_creds(store_uri)
        )
    )
...
_tracking_store_registry.register('qubole', _get_qubole_rest_store)
