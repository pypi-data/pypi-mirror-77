...
class _HadoopFileSystem:
    ...
    def maybe_copy_from_local_file(cls, src, dst):
        ...
        cls._get_logger().info("Copied SparkML model to %s", dst)
        return dst
        ...
        try:
            ...
            if cls._try_file_exists(dfs_path):
                cls._get_logger().info(
                    "File '%s' is already on DFS, copy is not necessary.", src_uri
                )
                return src_uri
        except Exception:  # pylint: disable=broad-except
            cls._get_logger().info(
                "URI '%s' does not point to the current DFS.",
                src_uri
            )
        cls._get_logger().info(
            "File '%s' not found on DFS. Will attempt to upload the file.",
            src_uri
        )
...
def load_model(model_uri, dfs_tmpdir=None):
    ...
    """
    _logger = logging.getLogger("load_model")
