...
from mlflow.protos.service_pb2 import CreateExperiment, MlflowService, GetExperiment, \
    ...
    DeleteTag, SetExperimentTag, GetExperimentByName, ListDetailedExperiments, LogModel
...
from mlflow.utils.qubole_metrics_events import (
    SERVER_CREATE_EXPERIMENT,
    SERVER_CREATE_RUN,
    SERVER_LIST_EXPERIMENTS_DETAILED,
    SERVER_LOG_MODEL
)
...
def _create_experiment():
    ...
    QuboleMetricsUtils.send_event(
        SERVER_CREATE_EXPERIMENT,
        {
            "request_message": message_to_json(request_message),
            "response_message": message_to_json(response_message)
        })
...
@catch_mlflow_exception
def _list_detailed_experiments():
    start_time = time.time()

    request_message = _get_request_message(ListDetailedExperiments())

    filter_string = request_message.filter
    max_experiments = request_message.max_experiments
    max_runs_per_experiment = request_message.max_runs_per_experiment
    run_view_type = ViewType.ACTIVE_ONLY
    if request_message.HasField('run_view_type'):
        run_view_type = ViewType.from_proto(request_message.run_view_type)
    order_by = ["attributes.start_time DESC"]
    if len(request_message.order_by):
        order_by = request_message.order_by

    experiment_entities = _get_tracking_store().list_experiments_with_runs(
            filter_string,
            order_by,
            max_experiments,
    ...
    response_message = ListDetailedExperiments.Response()
    response_message \
        .experiments \
        .extend([e.to_proto(include_runs=True) for e in experiment_entities])
    response = Response(mimetype='application/json')
    response.set_data(message_to_json(response_message))
...
HANDLERS = {
    ...
    ListDetailedExperiments: _list_detailed_experiments,
