...
def http_request(host_creds, endpoint, retries=3, retry_interval=3,
    ...
    if host_creds.x_auth_token:
        headers['X-AUTH-TOKEN'] = host_creds.x_auth_token

    if host_creds.xsrf_token:
        headers['X-XSRFToken'] = host_creds.xsrf_token

    if host_creds.cookie:
        headers['Cookie'] = host_creds.cookie
...
class MlflowHostCreds(object):
    ...
    def __init__(self, host, username=None, password=None, token=None,
                 ignore_tls_verification=False, x_auth_token=None,
                 xsrf_token=None, cookie=None):
        if not host:
