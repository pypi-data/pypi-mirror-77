...
class SqlAlchemyStore(AbstractStore):
    ...
    def list_experiments_with_runs(
            self,
            filter_string,
            order_by,
            max_experiments,
        ...
        with self.ManagedSessionMaker() as session:
            ...
            for j in sorting_joins:
                runs_query = runs_query.outerjoin(j)

            queried_runs = runs_query.distinct() \
            ...
            experiment_query = session \
                .query(SqlExperiment) \
                .filter(SqlExperiment.lifecycle_stage.in_(stages)) \
                .join(queried_runs.subquery())

            experiments = []
            for idx, exp in enumerate(experiment_query):
                if idx == max_experiments:
                    return experiments
                else:
                    exp_runs = queried_runs \
                        .filter(SqlRun.experiment_id == exp.experiment_id) \
                        .limit(max_runs_per_experiment)

                    filtered_runs = [run.to_mlflow_entity() for run in exp_runs]
                    experiments.append(exp.to_mlflow_entity_with_filtered_runs(filtered_runs))
            return experiments
    ...
    def delete_experiment(self, experiment_id):
        if str(experiment_id) == SqlAlchemyStore.DEFAULT_EXPERIMENT_ID:
            raise MlflowException(
                'Default experiment cannot be deleted',
