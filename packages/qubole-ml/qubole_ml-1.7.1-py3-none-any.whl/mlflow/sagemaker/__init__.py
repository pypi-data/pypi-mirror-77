...
from mlflow.utils.boto_utils import get_client, get_session
...
def deploy(app_name, model_uri, execution_role_arn=None, bucket=None,
    ...
    sage_client = get_client('sagemaker', region_name=region_name)
    s3_client = get_client('s3', region_name=region_name)
...
def _get_default_image_url(region_name):
    ...
    ecr_client = get_client("ecr", region_name=region_name)
...
def _upload_s3(local_model_path, bucket, prefix, region_name, s3_client):
    ...
    """
    sess = get_session(region_name=region_name)
    with TempDir() as tmp:
