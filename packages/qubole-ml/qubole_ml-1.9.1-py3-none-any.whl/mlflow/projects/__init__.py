...
def _run(uri, experiment_id, entry_point, version, parameters,
    ...
    if backend_name == "databricks":
        ...
        (tracking
            .MlflowClient()
            .set_tag(
                active_run.info.run_id, MLFLOW_PROJECT_BACKEND,
                "qubole"))
...
def run(uri, entry_point="main", version=None, parameters=None,
    ...
    if backend == "databricks":
        ...
        mlflow.projects.qubole.before_run_validations(backend_config_dict)
...
def _get_docker_tracking_cmd_and_envs(tracking_uri):
    ...
    # Disabled due to security vulnerability ZEP-4715
    ...
    #     # We set these via environment variables so that only the current profile is exposed,
    #     # rather than all profiles in ~/.databrickscfg; maybe better would be to mount the
    #     # necessary part of ~/.databrickscfg into the container
