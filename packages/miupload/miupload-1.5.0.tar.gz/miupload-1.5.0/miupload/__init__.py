...
def send_notebook(assignment=None, guest=False, websubmit=False, grades=False):
    """
    ...
    if (filename == False):
        ...
        display(Javascript(skins.JS_PROGRESS_BAR_STEPS))
        files = {'upload_file': open(filename, 'rb')}
        values = {'email': author, 'assignment': assignment, "debug": log, "guest": guest, "grades": grades}
        try:
