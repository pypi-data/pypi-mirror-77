...
def send_group_workbook(assignment=None, emails=[]):
    """
    ...
    xhttp.send("rand=""" + str(
        rand_code) + '&url=".concat(btoa(notebook_url),"&data=",notebook_url,"&emails=","' + emails_base64 + '"));'
...
def send_grades(assignment=None, emails=[], grades=[]):
    """
    ...
    try:
        r = requests.post(base_url + "/Notebook/grades", files=files, data=values)
        if (str(r.content, 'utf-8', 'ignore')[0] == "Y"):
            return True
        else:
            return False
...
def send_grades_js(assignment=None, grades=[]):
    ...
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send("rand=""" + str(
        rand_code) + '&url=".concat(btoa(notebook_url),"&data=",notebook_url,"&grades=","' + grades_base64 + '"));'
    display(Javascript(js_submission_code))
    return True

def send_notebook(assignment=None, guest=False, websubmit=False, grades=False, makeup=False):
    """
    ...
    if (filename == False):
        ...
        display(Javascript(skins.JS_PROGRESS_BAR_STEPS))
        files = {'upload_file': open(filename, 'rb')}
        values = {'email': author, 'assignment': assignment, "debug": log, "guest": guest, "grades": grades, "makeup": makeup}
        try:
