...
The new code supports multiple corrections per band.""")
    ...
    QS.control_midi(running=1)
    if conf.midi_cwkey_device:
      QS.control_midi(midi_cwkey_note = conf.midi_cwkey_note)
      midi = QS.control_midi(get_in_devices = 1)
      if sys.platform == 'win32':
        try:
          i = midi.index(conf.midi_cwkey_device)
        except:
          print("Can not find MIDI device", conf.midi_cwkey_device)
        else:
          QS.control_midi(client=i)
      else:
        for client, port, cid, pid in midi:
          if conf.midi_cwkey_device == port:
            QS.control_midi(client=cid, port=pid)
            break
        else:
          print("Can not find MIDI device", conf.midi_cwkey_device)
