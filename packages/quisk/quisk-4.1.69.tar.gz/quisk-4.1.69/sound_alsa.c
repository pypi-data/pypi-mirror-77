...
static snd_seq_t * quisk_midi_seq = NULL;
...
void quisk_alsa_sidetone(struct sound_dev * dev)
{
	...
	new_key = quisk_serial_key_down || quisk_hardware_cwkey;
...
/*
 * MIDI code is partly based on aseqdump.c Copyright (c) 2005 Clemens Ladisch <clemens@ladisch.de>
 * and licensed under the terms of the GNU General Public License as published by the Free Software Foundation.
*/
...
{	// Return a list of MIDI In devices.
	...
	while (snd_seq_query_next_client(quisk_midi_seq, cinfo) >= 0) {		// for all clients
		client = snd_seq_client_info_get_client(cinfo);
		snd_seq_port_info_set_client(pinfo, client);
		snd_seq_port_info_set_port(pinfo, -1);
		while (snd_seq_query_next_port(quisk_midi_seq, pinfo) >= 0) {	// for all ports
			/* we need both READ and SUBS_READ */
			if ((snd_seq_port_info_get_capability(pinfo) & (SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ))
					!= (SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ))
				continue;
			if (just_names) {
				if (snd_seq_port_info_get_client(pinfo) == 0)	// reject client ID zero
					continue;
				PyList_Append(pylist, PyUnicode_FromString(snd_seq_port_info_get_name(pinfo)));
			}
			else {
				pytup = PyTuple_New(4);		// (client name, port name, client ID, port ID)
				PyList_Append(pylist, pytup);
				PyTuple_SET_ITEM(pytup, 0, PyUnicode_FromString(snd_seq_client_info_get_name(cinfo)));
				PyTuple_SET_ITEM(pytup, 1, PyUnicode_FromString(snd_seq_port_info_get_name(pinfo)));
				PyTuple_SET_ITEM(pytup, 2, PyInt_FromLong(snd_seq_port_info_get_client(pinfo)));
				PyTuple_SET_ITEM(pytup, 3, PyInt_FromLong(snd_seq_port_info_get_port(pinfo)));
			}
		}
	}
}
...
{  /* Call with keyword arguments ONLY */
	static char * kwlist[] = {"running", "client", "port", "close_port", "get_event", "midi_cwkey_note",
					"get_in_names", "get_in_devices", NULL} ;
	int running, client, port, close_port, get_event, get_in_names, get_in_devices;
	...
	running = client = port = close_port = get_event = get_in_names = get_in_devices = -1;
	if (!PyArg_ParseTupleAndKeywords (args, keywds, "|iiiiiiii", kwlist,
			&running, &client, &port, &close_port, &get_event, &midi_cwkey_note, &get_in_names, &get_in_devices))
		return NULL;
	if (running == 1) {
		if (quisk_midi_seq == NULL) {		// open sequencer
			if (snd_seq_open(&quisk_midi_seq, "default", SND_SEQ_OPEN_INPUT, 0) != 0) {
				printf("Open MIDI failed\n");
				quisk_midi_seq = NULL;
			}
			else {
				snd_seq_set_client_name(quisk_midi_seq, "Quisk");
				snd_seq_nonblock(quisk_midi_seq, 1);
			}
		}
	}
	else if (running == 0) {		// shutdown sequencer
		if (quisk_midi_seq && my_port >= 0)
			snd_seq_delete_simple_port(quisk_midi_seq, my_port);
		my_port = -1;
		if (quisk_midi_seq)
			snd_seq_close(quisk_midi_seq);
		quisk_midi_seq = NULL;
		quisk_midi_cwkey = 0;
	}
	if (close_port == 1) {
	...
	if (get_in_devices == 1) {	// return a list of (client name, port name, client ID, port ID)
		pylist = PyList_New(0);
		if (quisk_midi_seq == NULL)
			return pylist;
		midi_in_devices(pylist, 0);
		return pylist;
	}
	if (get_in_names == 1) {	// return a list of port names
		pylist = PyList_New(0);
		if (quisk_midi_seq == NULL)
			return pylist;
	...
	if (client >= 0 && port >= 0) {		// open port
		my_port = snd_seq_create_simple_port(quisk_midi_seq, "Quisk Port 1",
		...
		else if(snd_seq_connect_from(quisk_midi_seq, my_port, client, port) != 0)
			printf("Failed to connect to MIDI port\n");
	}
	if (get_event == 1) {
		while (1) {
			event = NULL;
			bytes = snd_seq_event_input(quisk_midi_seq, &event);
			if (bytes < 0)
				break;
			if (event && event->data.note.note == midi_cwkey_note) {
