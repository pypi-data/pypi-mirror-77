...
def mge_vcirc_example():
    ...
    dens, sigma = 10**lg_dens, 10**lg_sigma
    qobs = np.sqrt((qintr*np.sin(np.radians(inc)))**2 + np.cos(np.radians(inc))**2)  # Eq.(35) Cappellari (2020)
    ...
    vtrue = np.sqrt(G*rad**2/(3 + rad**2)**1.5 + G*mbh/rad)
