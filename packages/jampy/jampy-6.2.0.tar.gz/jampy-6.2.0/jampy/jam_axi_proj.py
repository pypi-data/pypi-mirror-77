...
def psf_conv(x, y, inc_deg,
             ...
             pixSize, pixAng, step, nrad, nang, nlos, epsrel, interp):
    """
...
class jam_axi_proj:
    ...
    def __init__(self, surf_lum, sigma_lum, qobs_lum, surf_pot, sigma_pot, qobs_pot,
        ...
        model, psfConvolution, vel, vel2 = psf_conv(
            ...
            pixSize_pc, pixang, step_pc, nrad, nang, nlos, epsrel, interp)
...
##############################################################################
    ...
    def plot(self, **kwargs):
        ...
        plt.clf()
        plt.subplot(121)
        plot_velfield(self.xbin, self.ybin, data1, vmin=vmin, vmax=vmax, flux=self.flux_obs, **kwargs)
        plt.title(f"Input V$_{{{self.moment}}}$ moment")

        plt.subplot(122)
        plot_velfield(self.xbin, self.ybin, self.model, vmin=vmin, vmax=vmax, flux=self.flux, **kwargs)
