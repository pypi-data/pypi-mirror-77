...
def setupFolderStructure(projectName=''):
    """
    ...
    path2project = pt.Path(CWD, 'projects', projectName)
    path2userConfig = pt.Path(path2project, 'user.yaml')

    path2actions.parent.mkdir(parents=True, exist_ok=True)
    path2project.mkdir(parents=True, exist_ok=True)
    ...
    projectStartScript = path2project / 'run.py'
