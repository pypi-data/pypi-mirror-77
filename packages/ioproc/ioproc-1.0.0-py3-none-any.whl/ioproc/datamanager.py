...
class DataManager(dict):
    ...
    def entersAction(self, actionName):
        """
    ...
    def report(self):
        ...
        :return:
        """
        ret = f'During execution of action {self._actionName} the following data sets were accessed:\n'
    ...
    def toCache(self, tag):
        ...
        in .hdf5 format to disc with filename Cache_[tag], so that the work can be resumed at a later point.
        :param tag: user specified filename extension
        """
        with pd.HDFStore('./Cache_{}.h5f'.format(tag)) as cache:
            otherData = pd.Series()
