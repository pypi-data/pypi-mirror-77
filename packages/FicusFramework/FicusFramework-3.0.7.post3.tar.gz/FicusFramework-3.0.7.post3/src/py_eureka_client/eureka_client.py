    ...
    def __heartbeat(self):
        while True:
            try:
                ...
                _logger.error(f'【eureka-client】PID:{os.getpid()}, 获取apps异常:{str(e)}')
    ...
    def __fetch_delta(self):
        def do_fetch(url):
            ...
            if self.__delta is not None \
                ...
                _logger.info(f'【eureka-client】PID:{os.getpid()}, 没有变化delta')
                return
            _logger.info(f'【eureka-client】PID:{os.getpid()}, 合并delta，sefl:{self.__delta.versionsDelta}, remote：{delta.versionsDelta}')
