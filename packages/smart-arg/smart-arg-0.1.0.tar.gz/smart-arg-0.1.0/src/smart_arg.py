...
The following is a simple usage example::
    ...
    # by using
    parsed_arg: ArgClass = ArgClass.__from_argv__(sys.argv[1:])  # with factory method, need the manual type hint `:ArgClass` to help IDE
...
class SmartArgError(Exception): pass  # noqa: E701 # TODO Extend to better represent different types of errors.
...
class _namedtuple:  # TODO expand lambdas to static methods or use a better holder representation
    """Helper function holder for NamedTuple support"""
    @staticmethod
    def new_instance(arg_class, kwargs):
        """Replacement for NamedTuple.__new__: call original __new__ -> __post_init__ -> post_validation"""
        new_instance = arg_class.__original_new__(arg_class, **kwargs)
        post_init = getattr(arg_class, '__post_init__', lambda me: me)
        fake_namedtuple = KwargsType(**new_instance._asdict())
        post_init(fake_namedtuple)  # make the faked NamedTuple mutable only in post_init
    ...
    @staticmethod
    def is_supported(t):
        """:return this class if supported or None"""
...
class _dataclasses:
    @staticmethod
    def patch(cls):
        def throw_if_frozen(self, fun, *args, **kwargs):
            if getattr(self, '__frozen__', False):
                raise SmartArgError(f"{self.__class__} instance is frozen.")
            else:
                object.__setattr__(self, *args, **kwargs)

        def smart_arg_post_init(self, *args, **kwargs):
            original_post_init = getattr(self, '__original_post_init__', _black_hole)
            original_post_init(*args, **kwargs)
            object.__setattr__(self, '__frozen__', True)
            self.__class__.__arg_suite__.post_validation(self)
        o_p_i = getattr(cls, '__post_init__', smart_arg_post_init)
        if o_p_i.__name__ != smart_arg_post_init.__name__:
            cls.__original_post_init__ = o_p_i
        cls.__post_init__ = smart_arg_post_init
        if not cls.__dataclass_params__.frozen:
            cls.__setattr__ = lambda self, *args, **kwargs: throw_if_frozen(self, '__setattr__', *args, **kwargs)
            cls.__delattr__ = lambda self, *args, **kwargs: throw_if_frozen(self, '__delattr__', *args, **kwargs)
