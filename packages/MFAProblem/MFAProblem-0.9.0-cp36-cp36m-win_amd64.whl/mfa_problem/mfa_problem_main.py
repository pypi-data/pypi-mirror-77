...
def optimisation(
    ...
    if main_problem:
        ...
        post_process = post_process.nonzero()[0]
        mask_is_not_measured = np.setdiff1d(
        ...
        B = AConstraintEq[:, mask_is_not_measured]
        ...
        AConstraintEqReordered = scipy.sparse.hstack((B, A, li, ui), format='csr')

        AConstraintIneq = AConstraintIneq.tocsc()
        B = AConstraintIneq[:, mask_is_not_measured]
        A = AConstraintIneq[:, mask_is_measured]
        li = AConstraintIneq[:, full_ter_size]
        ui = AConstraintIneq[:, full_ter_size+1]
        AIneqReordered = scipy.sparse.hstack((B, A, li, ui), format='csr')
        ...
        AEqReorderedRef, determinable_col2row, non_redundant, reordered_vars_type, rank_unmeasured = \
            mfa_problem_solver.classify_with_matrix_reduction(
                AConstraintEqReordered, sum(mask_is_measured)
            )
        ...
        solved_vector, vars_type, intervals = mfa_problem_solver.resolve_mfa_problem(
            rank_unmeasured,
            AEqReorderedRef, AIneqReordered,
            nb_measured, ter_vectors_reordered,
            determinable_col2row, reordered_vars_type, post_process,
            full_ter_size,
            full_ter_vectors,
            AConstraintEq,
            AConstraintIneq,
