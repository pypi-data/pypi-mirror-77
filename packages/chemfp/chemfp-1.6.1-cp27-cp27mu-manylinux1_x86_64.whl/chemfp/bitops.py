...
from _chemfp import (hex_isvalid, hex_popcount, hex_intersect_popcount,
                     hex_tanimoto, hex_tversky, hex_contains, hex_contains_bit,
                     hex_encode, hex_encode_as_bytes, hex_decode,
                     hex_from_bitlist, hex_to_bitlist,
                     hex_intersect, hex_union, hex_difference)
                    

from _chemfp import (byte_popcount, byte_intersect_popcount,
                     byte_tanimoto, byte_tversky, byte_contains, byte_contains_bit,
                     byte_hex_tanimoto, byte_hex_tversky,
                     byte_from_bitlist, byte_to_bitlist,
...
__all__ = ["byte_popcount", "byte_intersect_popcount",
           ...
           "byte_hex_tanimoto", "byte_hex_tversky",
           "hex_encode", "hex_encode_as_bytes", "hex_decode"
           # Don't export the method and alignment-related functions
               ]
...
def get_option(option):
    return _chemfp.get_option(option)
...
def print_report(out=sys.stdout):
    ...
    print >>out, "Number of available threads:", _chemfp.get_num_threads()
...
def use_environment_variables(environ=None):
    ...
    for alignment in get_alignments():
        name = "CHEMFP_" + alignment.upper().replace("-", "_")
        known.add(name)
        try:
            value = environ[name]
        except KeyError:
    ...
    for option in get_options():
        name = "CHEMFP_" + option.upper().replace("-", "_")
        known.add(name)
        try:
            value = environ[name]
        except KeyError:
    ...
    known.add("CHEMFP_REPORT")
    report = environ.get("CHEMFP_REPORT", "0") == "1"
    ...
    known.add("CHEMFP_PRINT_CONFIG")
    if (environ.get("CHEMFP_PRINT_CONFIG", "0") == "1" or report):
        print_report(sys.stderr)


    # Don't warn about variables that chemfp 3.x understands
    known.add("CHEMFP_LICENSE")
    known.add("CHEMFP_LICENSE_MANAGER")
    for k in environ:
        if not k.startswith("CHEMFP_"):
            continue
        if k not in known:
