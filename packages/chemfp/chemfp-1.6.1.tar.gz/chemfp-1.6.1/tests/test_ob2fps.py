...
class TestFingerprintTypes(unittest2.TestCase):
    ...
    def test_rdmaccs(self):
        ...
        self.assertEquals(headers["#type"], "RDMACCS-OpenBabel/2")
    ...
    def test_rdmaccs1_key44(self):
        ...
        self.assertEqual(header[b"#type"], b"RDMACCS-OpenBabel/1")
    ...
    def test_rdmaccs2_key44(self):
        ...
        self.assertEqual(header[b"#type"], b"RDMACCS-OpenBabel/2")
        hex_fp, id = output[0].split(b"\t")
    ...
    def test_both_rdmaccs_flags_gives_error(self):
        ...
        self.assertIn("argument --rdmaccs/--rdmaccs/2: not allowed with argument --rdmaccs/1", result)
        result = runner.run_exit("--rdmaccs --rdmaccs/1")
        self.assertIn("argument --rdmaccs/1: not allowed with argument --rdmaccs/--rdmaccs/2", result)
...
@unittest2.skipIf(skip_openbabel, "OpenBabel not installed")
...
@unittest2.skipUnless(skip_openbabel, "Open Babel installed - can't check for missing Open Babel")
class TestMissingOpenBabelModule(unittest2.TestCase):
    def test_ob2fps(self):
        from chemfp import commandline
        try:
            commandline.run_ob2fps()
        except SystemExit as err:
            result = str(err)
            self.assertIn(result, [
                "Cannot run ob2fps: It appears that Open Babel is not installed: No module named openbabel",
                "Cannot run ob2fps: It appears that Open Babel is not installed: No module named 'openbabel'",
                ])
