...
class TestArenaFormatIdAndScoresAsBytes(TestCase, CreateSearchResults):
    def test_empty(self):
        results = self._create(1, [])
        self.assertEqual(results[0].format_ids_and_scores_as_bytes(), b"")
        
    def test_one_value_no_ids(self):
        results = self._create(1, [(0, 12, 0.123)])
        with self.assertRaisesRegexp(ValueError, "Cannot format ids and scores because the result has no ids and no ids were passed in"):
            results[0].format_ids_and_scores_as_bytes()
        
    def test_one_value_one_id(self):
        results = self._create(1, [(0, 12, 0.123)])
        self.assertEqual(results[0].format_ids_and_scores_as_bytes([u"spam"]), b"spam\t0.1230")
        
    def test_one_value_two_ids(self):
        results = self._create(1, [(0, 12, 0.123)])
        with self.assertRaisesRegexp(ValueError, r"Mismatch between the number of ids \(2\) and the number of scores \(1\)"):
            results[0].format_ids_and_scores_as_bytes([u"spam", u"eggs"])
    
    def test_two_values_two_ids(self):
        results = self._create(1, [(0, 12, 0.123), (0, 3, 0.87654)])
        self.assertEqual(results[0].format_ids_and_scores_as_bytes([u"spam", u"eggs"]),
                         b"spam\t0.1230\teggs\t0.8765")

    # Ensure that the row index is being passed to the underlying call
        
    def test_one_value_no_ids_second_row(self):
        results = self._create(2, [(1, 12, 0.123)])
        self.assertEqual(results[0].format_ids_and_scores_as_bytes(), b"")
        with self.assertRaisesRegexp(ValueError, "Cannot format ids and scores because the result has no ids and no ids were passed in"):
            results[1].format_ids_and_scores_as_bytes()
        
    def test_one_value_one_id_second_row(self):
        results = self._create(2, [(1, 12, 0.123)])
        self.assertEqual(results[1].format_ids_and_scores_as_bytes([u"spam"]), b"spam\t0.1230")
        
    def test_one_value_two_ids_second_row(self):
        results = self._create(2, [(1, 12, 0.123)])
        with self.assertRaisesRegexp(ValueError, r"Mismatch between the number of ids \(2\) and the number of scores \(1\)"):
            results[1].format_ids_and_scores_as_bytes([u"spam", u"eggs"])
    
    def test_two_values_two_ids_second_row(self):
        results = self._create(2, [(1, 12, 0.123), (1, 3, 0.87654)])
        self.assertEqual(results[1].format_ids_and_scores_as_bytes([u"spam", u"eggs"]),
                         b"spam\t0.1230\teggs\t0.8765")
        
    # precision tests
    def test_precision_one_element(self):
        result = self._create(1, [(0, 981, 0.2)])[0]
        self.assertEqual(result.format_ids_and_scores_as_bytes([u"blah"], 1),
                        b"blah\t0.2")
        self.assertEqual(result.format_ids_and_scores_as_bytes(precision=1, ids=[u"blah"]),
                        b"blah\t0.2")
        
    def test_precision_out_of_range(self):
        result = self._create(1, [(0, 981, 0.2)])[0]
        for precision in (-1, 0, 11, 12):
            with self.assertRaisesRegexp(ValueError, "precision must be between 1 and 10, inclusive"):
                result.format_ids_and_scores_as_bytes([u"xyz"], precision)
        

    def test_precision_range_one_element(self):
        # Try different precisions for different values
        formatters = [(i, ("biblan\t%." + str(i) + "f").encode("ascii")) for i in range(1, 11)]
        for test_case in range(1000):
            value = test_case / 1000.0
            result = self._create(1, [(0, 123, value)])[0]
            
            for precision, formatter in formatters:
                self.assertEqual(result.format_ids_and_scores_as_bytes([u"biblan"], precision=precision),
                                 formatter % (value,), (precision, value))

    def test_precision_range_two_elements(self):
        # Try different precisions for different values
        formatters = [(i, ("ABC\t%." + str(i) + "f\tDEF\t%." + str(i) + "f" ).encode("ascii")) for i in range(1, 11)]
        for test_case in range(1000):
            value1 = test_case / 1000.0
            value2 = test_case / 1001.0
            result = self._create(1, [(0, 123, value1), (0, 456, value2)])[0]
            
            for precision, formatter in formatters:
                self.assertEqual(result.format_ids_and_scores_as_bytes([u"ABC", u"DEF"], precision=precision),
                                 formatter % (value1, value2))

    def test_format_resize(self):
        # The code optimistically assumes 0 <= score <= 1.0 but will resize to handle scores
        # outside of that range.
        result = self._create(1, [(0, 981, 123.456), (0, 765, 987.654), (0, 444, -12345.678)])[0]
        self.assertEqual(result.format_ids_and_scores_as_bytes([u"AAA", u"BBBB", u"CCCCC"]),
                         b"AAA\t123.4560\tBBBB\t987.6540\tCCCCC\t-12345.6780")
        self.assertEqual(result.format_ids_and_scores_as_bytes([u"AAA", u"BBBB", u"CCCCC"], 1),
                         b"AAA\t123.5\tBBBB\t987.7\tCCCCC\t-12345.7")
        self.assertEqual(result.format_ids_and_scores_as_bytes([u"AAA", u"BBBB", u"CCCCC"], 2),
                         b"AAA\t123.46\tBBBB\t987.65\tCCCCC\t-12345.68")
        self.assertEqual(result.format_ids_and_scores_as_bytes([u"AAA", u"BBBB", u"CCCCC"], 3),
                         b"AAA\t123.456\tBBBB\t987.654\tCCCCC\t-12345.678")
        self.assertEqual(result.format_ids_and_scores_as_bytes([u"AAA", u"BBBB", u"CCCCC"], 10),
                         b"AAA\t123.4560000000\tBBBB\t987.6540000000\tCCCCC\t-12345.6780000000")

    def test_format_nan_and_inf(self):
        result = self._create(1, [(0, 981, nan), (0, 765, inf), (0, 444, -inf)])[0]
        # -inf requires 4 bytes while the default for precision=1 only expects 3
        s = result.format_ids_and_scores_as_bytes([u"AAA", u"BBBB", u"CCCCC"], 1)
        terms = s.split(b"\t")
        # let Python figure out the system-specific string representation for nan/inf/-inf
        self.assertTrue(math.isnan(float(terms[1])), terms[1])
        self.assertEqual(float(terms[3]), inf)
        self.assertEqual(float(terms[5]), -inf)

    def test_ids_not_a_list(self):
        result = self._create(1, [(0, 1, 0.1)])[0]
        with self.assertRaisesRegexp(TypeError, "ids must be a list"):
            result.format_ids_and_scores_as_bytes(ids="asdf")
        with self.assertRaisesRegexp(TypeError, "ids must be a list"):
            # XXX should I allow tuples?
            result.format_ids_and_scores_as_bytes(ids=("asdf",))
        
    def test_non_unicode_id(self):
        result = self._create(1, [(0, 1, 0.1)])[0]
        with self.assertRaisesRegexp(TypeError, "id 0 must be a byte string or Unicode string, not int"):
            result.format_ids_and_scores_as_bytes([33])
        
        result = self._create(2, [(1, 1, 0.1), (1, 8, 0.25)])[1]
        with self.assertRaisesRegexp(TypeError, "id 1 must be a byte string or Unicode string, not int"):
            result.format_ids_and_scores_as_bytes([u"blah", 33])

    def test_using_arena_ids_from_python(self):
        arena = chemfp.load_fingerprints([(u"first", b"AA"), (u"second", b"AB")],
                                         metadata = chemfp.Metadata(num_bytes=2))
        result = arena.threshold_tanimoto_search_fp(b"AE", threshold=0.0)
        self.assertEqual(result.format_ids_and_scores_as_bytes(),
                         b"first\t0.8000\tsecond\t0.5000")
        self.assertEqual(result.format_ids_and_scores_as_bytes(precision=2),
                         b"first\t0.80\tsecond\t0.50")
        
    def test_using_arena_ids_from_fps(self):
        arena = chemfp.load_fingerprints(SIMPLE_FPS)
        result = arena.knearest_tanimoto_search_fp(b"ABCD", threshold=0.0, k=1000)
        self.assertEqual(result.format_ids_and_scores_as_bytes(),
                         b"Deaf Beef\t0.1724\t"
                         b"DEADdead\t0.1481\t"
                         b"several\t0.1429\t"
...
query_fingerprints = [("one", b"\x01\x34\x8f\x51"),
                      ("two", b"\x08\x37\x7f\x53")]
target_fingerprints = [("a", b"\x01\x79\x8f\x61"),
                       ("b", b"\x08\x7a\x7f\x63"),
                       ("c", b"\xf0\xa0\xe0\xb8")]
query_arena = chemfp.load_fingerprints(query_fingerprints, chemfp.Metadata(num_bytes=4))
target_arena = chemfp.load_fingerprints(target_fingerprints, chemfp.Metadata(num_bytes=4))
                    
class TestArenaSearch(TestCase):
    def test_knearest_shape(self):
        results = search.knearest_tanimoto_search_arena(query_arena, target_arena)
        self.assertEqual(results.shape, (2, 3))

    def test_knearest_shape_empty(self):
        results = search.knearest_tanimoto_search_arena(query_arena, target_arena, threshold=1.0)
        self.assertEqual(results.shape, (2, 3))

    def test_threshold_shape(self):
        results = search.threshold_tanimoto_search_arena(query_arena, target_arena)
        self.assertEqual(results.shape, (2, 3))

    def test_threshold_shape_empty(self):
        results = search.threshold_tanimoto_search_arena(query_arena, target_arena, threshold=1.0)
        self.assertEqual(results.shape, (2, 3))

@unittest2.skipUnless(has_scipy, "SciPy not available")
class TestArenaCSR(TestCase):
    def test_threshold_default(self):
        results = search.knearest_tanimoto_search_arena(query_arena, target_arena, threshold=0.2)
        csr = results.to_csr()
        self.assertEqual(csr.shape, (2, 3))
        self.assertEqual(csr.dtype, "float64")
        self.assertEqual(csr[0,0], 0.0)
        self.assertEqual(csr[0,1], 0.625)
        self.assertEqual(csr[0,2], 0.38095238095238093)
        self.assertEqual(csr[1,0], 0.0)
        self.assertEqual(csr[1,1], 0.40909090909090912)
        self.assertEqual(csr[1,2], 0.7)
        self.assertTrue((csr.toarray() == np.array(
            [[0.0, 0.625, 0.38095238095238093],
             [0.0, 0.40909090909090912, 0.7]])).all())

    def test_knearest_k1_default(self):
        results = search.knearest_tanimoto_search_arena(query_arena, target_arena, k=1, threshold=0.0)
        csr = results.to_csr()
        self.assertEqual(csr.shape, (2, 3))
        self.assertEqual(csr.dtype, "float64")
        self.assertEqual(csr[0,0], 0.0)
        self.assertEqual(csr[0,1], 0.625)
        self.assertEqual(csr[0,2], 0.0)
        self.assertEqual(csr[1,0], 0.0)
        self.assertEqual(csr[1,1], 0.0)
        self.assertEqual(csr[1,2], 0.7)
        self.assertTrue((csr.toarray() == np.array(
            [[0.0, 0.625, 0.0],
             [0.0, 0.0, 0.7]])).all())
        
    def test_knearest_k2(self):
        results = search.knearest_tanimoto_search_arena(query_arena, target_arena, k=2, threshold=0.0)
        csr = results.to_csr()
        self.assertEqual(csr.shape, (2, 3))
        self.assertEqual(csr.dtype, "float64")
        self.assertEqual(csr[0,0], 0.0)
        ...
        self.assertEqual(csr[0,2], 0.38095238095238093)
        self.assertEqual(csr[1,0], 0.0)
        self.assertEqual(csr[1,1], 0.4090909090909091)
        self.assertEqual(csr[1,2], 0.7)
        self.assertTrue((csr.toarray() == np.array(
            [[0.0, 0.625, 0.38095238095238093],
             [0.0, 0.4090909090909091, 0.7]])).all())
        
    def test_knearest_dtype_float32(self):
        results = search.knearest_tanimoto_search_arena(query_arena, target_arena, k=2, threshold=0.0)
        csr = results.to_csr(dtype="float32")
        self.assertEqual(csr.shape, (2, 3))
        self.assertEqual(csr.dtype, "float32")
        self.assertEqual(csr[0,0], 0.0)
        self.assertEqual(csr[0,1], 0.625)
        self.assertAlmostEqual(csr[0,2], 0.38095238095238093)
        self.assertEqual(csr[1,0], 0.0)
        self.assertAlmostEqual(csr[1,1], 0.4090909090909091)
        self.assertAlmostEqual(csr[1,2], 0.7)
