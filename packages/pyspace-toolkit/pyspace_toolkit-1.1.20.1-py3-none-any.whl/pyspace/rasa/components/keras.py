...
class KerasBiLSTM(IntentClassifier):
    ...
    def generate_data(self, intent_examples, train_predict='predict'):
        ...
        if train_predict == 'train':
            ...
            # TODO 1 # self.maxlen = max([len(x) for x in texts])
            self.kerasindexer = KerasTokenizer()
    ...
    def read_embedding_matrix(word_index, ):
        ...
        ft_path = f'{pyres_path}/cc.tr.300.bin'
    ...
    @staticmethod
    def create_model(embedding_matrix, max_len, out_len, ):
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        X, y = generate_data(self, intent_examples, train_predict='train')

        embedding_matrix = self.read_embedding_matrix(self.kerasindexer.word_index)
        self.model = self.create_model(embedding_matrix, self.max_len, len(self.le.classes_))

        # callback = EarlyStopping(monitor='val_loss', min_delta=0, patience=5, verbose=1, mode='min', restore_best_weights=True)
        # reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=2, min_lr=0.00001)
        keras_history = model.fit(x=X, y=y, epochs=20, batch_size=16, 
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        X, _ = generate_data(self, [message], train_predict='train')
    ...
    def persist(self, file_name: Text, model_dir: Text) -> Optional[Dict[Text, Any]]:
        ...
        self.clf.save( open(os.path.join(model_dir, classifier_file_name) ) )
