...
class EntityManager(Component):
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        for e in entities_updated:
            if temp == []:
                ...
                if temp[-1]['entity'] in ['account'] and temp[-1]['extractor'] in ['RegexEntityExtractor'] and e['entity'] in ['account'] and e['extractor'] in ['RegexEntityExtractor']:
                    ...
                    if len(temp) == 1:
                        merged_entities_updated.append(temp[0])
                    else:
                        if all([ temp[i+1]['start'] - temp[i]['end'] <= 1 for i in range(len(temp)-1)]):
                            temp_value = message.text[temp[0]['start']:temp[-1]['end']]
                            if all( [xNormalizer.tr_normalize(tt['value']) in xNormalizer.tr_normalize(temp_value) for tt in temp]):
                                ...
                                print(temp)
                                merged_entities_updated.extend(temp)
        ...
        ############################################################################
        ############################################################################
        ## SORT WITH PRIORITY AND REMOVE IF NOT IN PRIORITY
        ## TODO KEEP AT LAST IF NOT IN PRIORITY
...
class MCMEntityMapper(Component):
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        def generate_entity(etype, evalue_list):
            ...
            if etype in ['history_tr_DATE_SVP_', 'txnlist_tr_DATE_SVP_']:
                ...
                for evalue in evalue_list:
                    ...
                    if temp in ['en fazla', 'en cok']:
                        output.append({'value': 'MAX'})
                    elif temp in ['en az', 'en dusuk']:
        ...
        if intent not in ner_intents or intent == 'inform':
            ...
            ## MCM EXPECTED ENTITIES
            ############################
            val = ['_CARD_TYPE_TR_', '_AMOUNT_TR_']
            ...
            ## OUTPUT VARIABLE
            ############################
            output = {'_CARD_TYPE_TR_':[], '_AMOUNT_TR_':[] }
            ...
            ## MCM EXPECTED ENTITIES
            ############################
            val = ['_TYPE_', '_AMOUNT_TR_']

            ############################
            ## OUTPUT CONTROL BOOLEANS
            ############################
            checks ={'_TYPE_':False, '_AMOUNT_TR_':False}

            
            ############################
            ## OUTPUT VARIABLE
            ############################
            output = {'_TYPE_':[], '_AMOUNT_TR_':[] }
            ...
            for e in entities:
                if e['extractor'] ==  'DucklingHTTPExtractor':
                    ...
                    elif e['entity'] in ['B-BUY-X']:
                        output['_BUY_TR_'].append(e['value'])
                        
                    elif e['entity'] in ['B-SELL-X', 'B-EXCHANGE-X']: 
                        # TODO B-EXCHANGE-X can also belong to B-BUY-X.
            ...
            ## MCM EXPECTED ENTITIES
            ############################
            val = ['_CURRENCY_', '_AMOUNT_', '_DATE_MONTH_', '_DATE_YEAR_','_DATE_DAY_']
            ...
            ## OUTPUT VARIABLE
            ############################
            output = {'_CURRENCY_':[], '_AMOUNT_':[], '_DATE_MONTH_':[], '_DATE_YEAR_':[],'_DATE_DAY_':[] }
            ...
            for e in entities:
                if e['extractor'] ==  'DucklingHTTPExtractor':
                    ...
                    if e['entity'] == 'amount-of-money':
                        output['_CURRENCY_'].append(e['additional_info']['unit'])
                        output['_AMOUNT_'].append(e['additional_info']['value'])
            ...
            ## MCM EXPECTED ENTITIES
            ############################
            val = ['_TYPE_', '_AMOUNT_', '_DATE_MONTH_', '_DATE_YEAR_','_DATE_DAY_']
            ...
            ## OUTPUT VARIABLE
            ############################
            output = {'_TYPE_':[], '_AMOUNT_':[], '_DATE_MONTH_':[], '_DATE_YEAR_':[],'_DATE_DAY_':[] }
            ...
            for e in entities:
                if e['extractor'] ==  'DucklingHTTPExtractor':
                    ...
                    if e['entity'] == 'amount-of-money':
                        output['_TYPE_'].append(e['additional_info']['unit'])
                        output['_AMOUNT_'].append(e['additional_info']['value'])
                        checks['_AMOUNT_'] = True

                elif e['extractor'] in  ['DIETClassifierExtended', 'DIETClassifier']:
                    if e['entity'] in ['B-AMOUNT-OF-MONEY']:
                        output['_AMOUNT_'].append(e['value']) # TODO # CURRENCY
                ...
                if e['extractor'] ==  'DucklingHTTPExtractor':
                    ...
                    if e['entity'] == 'amount-of-money':
                        output['_MONEY_'].append({
                            ...
                            'value': e['value'], 
                            'currency': e['additional_info']['unit'], 
                            'svp_tokens': e['text']})
            ...
            ## OUTPUT VARIABLE
            ############################
            output = {'_JOINED_TR_':[], '_TARGET_TR_':[] }
