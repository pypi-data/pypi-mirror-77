    ...
    def train(
        ...
        **kwargs: Any,
        ...
        if validation:
            ...
            callback = EarlyStopping(monitor='val_loss', min_delta=0, patience=10, verbose=1, mode='min', restore_best_weights=True)
            reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.6, patience=4, min_lr=0.00001)

            keras_history = self.clf.fit(x=X_train, y=y_train, epochs=20, batch_size=32, 
                                    shuffle=True, verbose=2, validation_data=[X_val,y_val],
                                    callbacks=[callback, reduce_lr],
                                )
            print(val_acc = keras_history.history["val_acc"][-1])
        else:
            keras_history = self.clf.fit(x=X, y=y, epochs=20, batch_size=32, 
