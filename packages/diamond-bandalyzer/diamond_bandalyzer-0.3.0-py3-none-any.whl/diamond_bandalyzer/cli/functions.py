...
def init_folder(directory):
    ...
    for option, value in zip(['settings_file', 'local_dir'], ["settings.ini", str(directory.absolute())]):
        try:
            inline_comments = " ; " + config_parser.get(diamond_bandalyzer.settingsobject.SettingsObject._settings_heading_, option).split(";")[1]
        except IndexError:
            inline_comments = ''
        config_parser.set(diamond_bandalyzer.settingsobject.SettingsObject._settings_heading_, option,
...
def build_defaults(ctx, param, value):
    ...
    diamond_bandalyzer.settingsobject.__create_default_ini__(default_settings_dict)
