...
import bandalyzer_app.differentiationStencil as differentiationStencil
...
class PoissonNRSolver(Solver, PoissonEquations):
    ...
    default_settings = {'derivative_order': 4, 'omega_nr': 0.5, 'extra_phantom_pts' : 10}
    ...
    def __init__(self, z_mesh=None, init=None, **kwargs):
        ...
        self.numex = self.settings['extra_phantom_pts']
    ...
    def __step__(self):
        ...
        rhs = self.generate_rhs()  # RHS of the equation
    ...
    def generate_rhs(self):
        ...
        rhs[int(self.order + self.numex):-int(self.order + self.numex)] += self.constRhotoV * self.rho_from_v(
            self.s_mesh)  # Add scaled charge density
